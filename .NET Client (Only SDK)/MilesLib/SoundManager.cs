...
public class CSoundManager : CSingleton<CSoundManager>
{
	...
	public void SetDirection(float fxDir, float fyDir, float fzDir, float fxUp, float fyUp, float fzUp)
	{
	...
	public void SetSoundScale(float fScale)
	{
		m_fSoundScale = fScale;
	}

	public void SetAmbienceSoundScale(float fScale)
	{
	...
	public void SetSoundVolumeRatio(float fRatio)
	{
...
//	void SetMusicVolumeRatio(float fRatio);
	public void SetSoundVolumeGrade(int iGrade)
	{
...
//	void SetMusicVolumeGrade(int iGrade);
	...
	public void PlaySound2D(string c_szFileName)
	{
	...
	public void PlaySound3D(float fx, float fy, float fz, string c_szFileName, int iPlayCount = 1)
	{
		if (0.0f == GetSoundVolume())
		{
			return;
		...
		pInstance.SetPosition((fx - m_fxPosition) / m_fSoundScale, (fy - m_fyPosition) / m_fSoundScale, (fz - m_fzPosition) / m_fSoundScale);

		pInstance.SetVolume(GetSoundVolume());
		pInstance.Play(iPlayCount);
	}

	public void StopSound3D(int iIndex)
	{
		ISoundInstance pInstance = ms_SoundManager3D.GetInstance(iIndex);
	...
	public int PlayAmbienceSound3D(float fx, float fy, float fz, string c_szFileName, int iPlayCount = 1)
	{
		if (0.0f == GetSoundVolume())
		{
			return -1;
		...
		pInstance.SetPosition((fx - m_fxPosition) / m_fAmbienceSoundScale, (fy - m_fyPosition) / m_fAmbienceSoundScale, (fz - m_fzPosition) / m_fAmbienceSoundScale);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
	private float PlayCharacterSound3D_s_fLimitDistance = 5000 * 5000;

	public void PlayCharacterSound3D(float fx, float fy, float fz, string c_szFileName, bool bCheckFrequency = false)
	{
		if (0.0f == GetSoundVolume())
		{
			return;
...
//			static float s_fLimitDistance = 5000 *5000;
			...
			if (fdx + fdy > PlayCharacterSound3D_s_fLimitDistance)
			{
				return;
			}

			SortedDictionary<string, float>.Enumerator itor = m_PlaySoundHistoryMap.find(c_szFileName);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			m_PlaySoundHistoryMap.insert(SortedDictionary<string, float>.value_type(c_szFileName, CTimer.Instance().GetCurrentSecond()));
		}
	...
	public void SetSoundVolume3D(int iIndex, float fVolume)
	{
		ISoundInstance pInstance = ms_SoundManager3D.GetInstance(iIndex);
	...
	{
		for (int LaniatusDefVariables = 0; LaniatusDefVariables < CSoundManager3D.INSTANCE_MAX_COUNT; ++i)
		{
			StopSound3D(i);
		}
	}

	public void PlayMusic(string c_szFileName)
	{
		PlayMusic(0, c_szFileName, GetMusicVolume(), 0.0f);
	}

	public void FadeInMusic(string c_szFileName, float fVolumeSpeed = 0.016f)
	{
		uint dwIndex;
	...
	public void FadeOutMusic(string c_szFileName, float fVolumeSpeed = 0.016f)
	{
		uint dwIndex;
		if (!GetMusicIndex(c_szFileName, dwIndex))
		{
	...
	public void FadeLimitOutMusic(string c_szFileName, float fLimitVolume, float fVolumeSpeed = 0.016f)
	{
		uint dwIndex;
		if (!GetMusicIndex(c_szFileName, dwIndex))
		{
	...
	public void UpdateSoundData(uint dwcurTime, List<SSoundData> c_pSoundDataVector)
	{
		Debug.Assert(!"CSoundManager::UpdateSoundData");
	}

	public void UpdateSoundData(float fx, float fy, float fz, uint dwcurTime, List<SSoundData> c_pSoundDataVector)
	{
		Debug.Assert(!"CSoundManager::UpdateSoundData");
	}

	public void UpdateSoundInstance(float fx, float fy, float fz, uint dwcurFrame, List<SSoundInstance> c_pSoundInstanceVector, bool bCheckFrequency = false)
	{
	...
	public void UpdateSoundInstance(uint dwcurFrame, List<SSoundInstance> c_pSoundInstanceVector)
	{
		for (uint LaniatusDefVariables = 0; LaniatusDefVariables < c_pSoundInstanceVector.Count; ++i)
		{
			SSoundInstance c_rSoundInstance = c_pSoundInstanceVector[LaniatusDefVariables];
	...
	protected class SMusicInstance
	{
		public uint dwMusicFileNameCRC;
		public EMusicState MusicState = new EMusicState();
		public float fVolume;
		public float fLimitVolume;
		public float fVolumeSpeed;
	}

	protected void PlayMusic(uint dwIndex, string c_szFileName, float fVolume, float fVolumeSpeed)
	{
		if (dwIndex >= CSoundManagerStream.MUSIC_INSTANCE_MAX_NUM)
		{
			return;
		...
		rMusicInstance.dwMusicFileNameCRC = GetCaseCRC32(strFileName, strFileName.Length);
	}

	protected void StopMusic(uint dwIndex)
	{
		if (dwIndex >= CSoundManagerStream.MUSIC_INSTANCE_MAX_NUM)
		{
			return;
	...
	protected bool GetMusicIndex(string c_szFileName, ref uint pdwIndex)
	{
		string strFileName = "";
		StringPath(c_szFileName, strFileName);
		uint dwCRC = GetCaseCRC32(strFileName, strFileName.Length);
	...
	protected float __ConvertGradeVolumeToApplyVolume(int nGradeVolume)
	{
		return __ConvertRatioVolumeToApplyVolume(nGradeVolume / 5.0f);
	}

	protected float __ConvertRatioVolumeToApplyVolume(float fRatioVolume)
	{
		...
		return (float)Math.Pow(10.0f, (-1.0f + fRatioVolume));
	}

	protected void __SetMusicVolume(float fVolume)
	{
		if (m_isSoundDisable)
		{
	...
	protected bool GetSoundInstance2D(string c_szSoundFileName, ISoundInstance[] ppInstance)
	{
		ppInstance[0] = ms_SoundManager2D.GetInstance(c_szSoundFileName);

		if (ppInstance[0] == null)
		{
			return false;
		}
	...
	protected bool GetSoundInstance3D(string c_szFileName, ISoundInstance[] ppInstance)
	{
		int iIndex = ms_SoundManager3D.SetInstance(c_szFileName);
		...
		ppInstance[0] = ms_SoundManager3D.GetInstance(iIndex);
	...
	protected bool m_isSoundDisable;

	protected float m_fxPosition;
	protected float m_fyPosition;
	protected float m_fzPosition;

	protected float m_fSoundScale;
	protected float m_fAmbienceSoundScale;
	protected float m_fSoundVolume;
	protected float m_fMusicVolume;

	protected float m_fBackupMusicVolume;
	protected float m_fBackupSoundVolume;

	protected SMusicInstance[] m_MusicInstances = Arrays.InitializeWithDefaultInstances<SMusicInstance>(CSoundManagerStream.MUSIC_INSTANCE_MAX_NUM);
	protected SortedDictionary<string, float> m_PlaySoundHistoryMap = new SortedDictionary<string, float>();

	protected static CSoundManager2D ms_SoundManager2D = new CSoundManager2D();
	protected static CSoundManager3D ms_SoundManager3D = new CSoundManager3D();
	protected static CSoundManagerStream ms_SoundManagerStream = new CSoundManagerStream();
}
