...
{
		public const int FLAG_DATA_SIZE = 1;
		public const int SOUND_FILE_MAX_NUM = 5;
		...
		public CSoundData()
		{
			...
			this.m_iRefCount = 0;
			this.m_dwPlayTime = 0;
			this.m_dwAccessTime = ELTimer_GetMSec();
			this.m_size = 0;
			this.m_data = null;
			this.m_flag = 0;
		}
		...
		protected bool ReadFromDisk()
		{
			Debug.Assert(m_assigned == true);
			...
			AILSOUNDINFO info = new AILSOUNDINFO();
			...
			switch (type)
			{
				...
				default:
					Debug.Assert(!"Unknown File Type");
					AIL_mem_free_lock(s);
					return false;
			}
		...
		protected string m_filename = new string(new char[128]);
		protected int m_iRefCount;
		protected uint m_dwAccessTime;
		...
		private static U32 AILCALLBACK open_callback(string filename, U32 file_handle)
		{
		...
		private static void AILCALLBACK close_callback(U32 file_handle)
		{
		...
		private static S32 AILCALLBACK seek_callback(U32 file_handle, S32 offset, U32 type)
		{
			if (!isSlotIndex(file_handle))
			{
				return 0;
		...
		private static U32 AILCALLBACK read_callback(U32 file_handle, object buffer, U32 bytes)
		{
			if (!isSlotIndex(file_handle))
			{
				return 0;
			...
			uint dwRealSize = Math.Min(ms_SoundFile[file_handle].Size(), bytes);
			ms_SoundFile[file_handle].Read(buffer, dwRealSize);
			return dwRealSize;
		}

		private static bool isSlotIndex(uint dwIndex)
		{
		...
		private static bool[] ms_isSoundFile = new bool[SOUND_FILE_MAX_NUM];
		private static CMappedFile[] ms_SoundFile = Arrays.InitializeWithDefaultInstances<CMappedFile>(SOUND_FILE_MAX_NUM);
}
