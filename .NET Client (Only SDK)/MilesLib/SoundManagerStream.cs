...
public class CSoundManagerStream : CSoundBase
{
		public const int MUSIC_INSTANCE_MAX_NUM = 3;

		public CSoundManagerStream()
		{
		}
		...
		public bool SetInstance(uint dwIndex, string filename)
		{
		...
		public CSoundInstanceStream GetInstance(uint dwIndex)
		{
			if (!CheckInstanceIndex(dwIndex))
			{
				return null;
		...
		protected bool CheckInstanceIndex(uint dwIndex)
		{
			if (dwIndex >= (uint)MUSIC_INSTANCE_MAX_NUM)
			{
				return false;
			}
		...
		protected CSoundInstanceStream[] m_Instances = Arrays.InitializeWithDefaultInstances<CSoundInstanceStream>(MUSIC_INSTANCE_MAX_NUM);
}