...
public class CSoundManager3D : CSoundBase
{
		public const int INSTANCE_MAX_COUNT = 32;
		public const int MAX_PROVIDERS = 32;

		public CSoundManager3D()
		{
			m_bInit = false;
		}
		...
		public bool Initialize()
		{
			...
			Debug.Assert(ms_pProviderDefault != null);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private uint SetInstance_k = 0;

		public int SetInstance(string c_pszFileName)
		{
			uint dwFileCRC = GetFileCRC(c_pszFileName);
			SortedDictionary<uint, CSoundData>.Enumerator itor = ms_dataMap.find(dwFileCRC);
...
//			static uint k = 0;
			...
			uint start = (uint)(SetInstance_k++);
			uint end = start + INSTANCE_MAX_COUNT;
		...
		public ISoundInstance GetInstance(uint dwIndex)
		{
			if (dwIndex >= INSTANCE_MAX_COUNT)
			{
				Debug.Assert(dwIndex < INSTANCE_MAX_COUNT);
				return null;
			}
			return m_Instances[dwIndex];
		}

		public void SetListenerDirection(float fxDir, float fyDir, float fzDir, float fxUp, float fyUp, float fzUp)
		{
			if (null == m_pListener)
			{
				return;
		...
		public void SetListenerPosition(float fX, float fY, float fZ)
		{
			if (null == m_pListener)
			{
				return;
		...
		public void SetListenerVelocity(float fDistanceX, float fDistanceY, float fDistanceZ, float fNagnitude)
		{
			if (null == m_pListener)
			{
				return;
		...
		protected bool IsValidInstanceIndex(int iIndex)
		{
		...
		protected bool[] m_bLockingFlag = new bool[INSTANCE_MAX_COUNT];
		protected CSoundInstance3D[] m_Instances = Arrays.InitializeWithDefaultInstances<CSoundInstance3D>(INSTANCE_MAX_COUNT);
		protected H3DPOBJECT m_pListener = new H3DPOBJECT();
