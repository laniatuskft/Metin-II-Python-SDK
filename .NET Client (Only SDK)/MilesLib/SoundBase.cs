...
{
	public string name;
	public HPROVIDER hProvider = new HPROVIDER();
}


public class CSoundBase
{
		public CSoundBase()
		{
		}
		...
		public void Destroy()
		{
			...
			if (!ms_dataMap.empty())
			{
				SortedDictionary<uint, CSoundData>.Enumerator i;
				for (i = ms_dataMap.begin(); i.MoveNext();)
				{
					CSoundData pSoundData = i.Current.Value;
					pSoundData = null;
				}
		...
		public CSoundData AddFile(uint dwFileCRC, string filename)
		{
			CSoundData pSoundData = new CSoundData();
			pSoundData.Assign(filename);
			ms_dataMap.insert(SortedDictionary<uint, CSoundData>.value_type(dwFileCRC, pSoundData));
			return pSoundData;
		}
		...
		protected static int ms_iRefCount = 0;
		protected static HDIGDRIVER ms_DIGDriver = null;
		protected static SProvider ms_pProviderDefault = null;
		protected static List<SProvider> ms_ProviderVector = new List<SProvider>();
		protected static SortedDictionary<uint, CSoundData> ms_dataMap = new SortedDictionary<uint, CSoundData>();
		protected static bool ms_bInitialized = false;
}