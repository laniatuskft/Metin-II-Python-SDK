...
public class CVertexShader : CGraphicBase
{
		...
		public bool CreateFromDiskFile(string c_szFileName, uint c_pdwVertexDecl)
		{
			Destroy();

			@interface @interface ID3DXBuffer * lpd3dxShaderBuffer;
			@interface @interface ID3DXBuffer * lpd3dxErrorBuffer;

			if (FAILED(D3DXAssembleShaderFromFile(c_szFileName, 0, null, lpd3dxShaderBuffer, lpd3dxErrorBuffer)))
			{
				return false;
			}

			CDirect3DXBuffer shaderBuffer = new CDirect3DXBuffer(lpd3dxShaderBuffer);
			CDirect3DXBuffer errorBuffer = new CDirect3DXBuffer(lpd3dxErrorBuffer);

			if (FAILED(ms_lpd3dDevice.CreateVertexShader(c_pdwVertexDecl, (uint)shaderBuffer.GetPointer(), m_handle, 0)))
			{
				return false;
			}
		...
		public void Set()
		{
			(CStateManager.Instance()).SetVertexShader(m_handle);
		}
