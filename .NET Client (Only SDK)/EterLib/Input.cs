...
public class CInputDevice
{
		...
		public int CreateDevice(IntPtr hWnd)
		{
			...
			if (FAILED(hr = DirectInput8Create(GetModuleHandle(null), DIRECTINPUT_VERSION, IID_IDirectInput8, (object) & ms_lpDI, null)))
			{
				return hr;
			}
		...
		protected static LPDIRECTINPUT8 ms_lpDI = null;
}

public class CInputKeyboard : CInputDevice
{
		...
		public virtual void Dispose()
		{
...

			{
				if (ms_lpKeyboard)
				{
					(ms_lpKeyboard).Release();
					(ms_lpKeyboard) = null;
				}
			};
		}

		public bool InitializeKeyboard(IntPtr hWnd)
		{
			NANOBEGIN if (ms_lpKeyboard) return true = new NANOBEGIN();
			...
			uint dwCoopFlags = (uint)(DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public bool IsPressed(int iIndex)
		{
			return ms_bPressedKey[iIndex];
		}

		public void KeyDown(int iIndex)
		{
			ms_bPressedKey[iIndex] = true;
			OnKeyDown(iIndex);
		}

		public void KeyUp(int iIndex)
		{
			ms_bPressedKey[iIndex] = false;
			OnKeyUp(iIndex);
		}

		protected abstract void OnKeyDown(int iIndex);
		protected abstract void OnKeyUp(int iIndex);

		protected static LPDIRECTINPUTDEVICE8 ms_lpKeyboard = null;
		protected static bool[] ms_bPressedKey = new bool[256];
		protected static string ms_diks = new string(new char[256]);
}
