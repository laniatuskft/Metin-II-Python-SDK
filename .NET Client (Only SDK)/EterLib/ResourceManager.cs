...
public class CResourceManager : CSingleton<CResourceManager>
{
		...
		public void LoadStaticCache(string c_szFileName)
		{
			...
			uint dwCacheKey = GetCRC32(c_szFileName, strlen(c_szFileName));
		...
		public void DestroyDeletingList()
		{
		...
		public void Destroy()
		{
			Debug.Assert(m_ResourceDeletingMap.empty() && "CResourceManager::Destroy - YOU MUST CALL DestroyDeletingList");
...
//		void EndThreadLoading();
		...
		public CResource InsertResourcePointer(uint dwFileCRC, CResource pResource)
		{
			...
			if (m_pResMap.end() != itor)
			{
				...
				Debug.Assert(!"CResource::InsertResourcePointer: Resource already resistered");
				pResource = null;
				return itor.second;
			}
		...
		public CResource FindResourcePointer(uint dwFileCRC)
		{
			TResourcePointerMap.iterator itor = m_pResMap.find(dwFileCRC);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string GetResourcePointer_s_szFileExt = new string(new char[8 + 1]);

		public CResource GetResourcePointer(string c_szFileName)
		{
			if (!c_szFileName != '\0' || !c_szFileName[0] != '\0')
			{
			...
			string c_pszFile;
			uint dwFileCRC = __GetFileCRC(c_szFileName, c_pszFile);
...
//Original Metin2 CPlus Line: const char * pcFileExt = strrchr(c_pszFile, '.');
			char pcFileExt = strrchr(c_pszFile, '.');
...
//				static char s_szFileExt[8 + 1];
				strncpy(newFuncDelegate_s_szFileExt, pcFileExt + 1, 8);

				TResourceNewFunctionPointerMap.iterator f = m_pResNewFuncMap.find(newFuncDelegate_s_szFileExt);
		...
		private delegate CResource newFuncDelegate(string UnnamedParameter);

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string GetTypeResourcePointer_s_szFileExt = new string(new char[8 + 1]);

		public CResource GetTypeResourcePointer(string c_szFileName, int iType = -1)
		{
			if (!c_szFileName != '\0' || !c_szFileName[0] != '\0')
			{
				Debug.Assert(c_szFileName != null && c_szFileName[0] != '\0');
				return null;
			}
...
//					static char s_szFileExt[8 + 1];
		...
		public bool isResourcePointerData(uint dwFileCRC)
		{
			TResourcePointerMap.iterator itor = m_pResMap.find(dwFileCRC);
		...
		private delegate CResource pNewFuncDelegate(string c_szFileName);

		public void RegisterResourceNewFunctionPointer(string c_szFileExt, pNewFuncDelegate pNewFunc)
		{
			m_pResNewFuncMap[c_szFileExt] = pNewFunc;
		}

		public void RegisterResourceNewFunctionByTypePointer(int iType, pNewFuncDelegate pNewFunc)
		{
			Debug.Assert(iType >= 0);
			m_pResNewFuncByTypeMap[iType] = pNewFunc;
		}

		public void DumpFileListToTextFile(string c_szFileName)
		{
			List<SDumpData> dumpVector = new List<SDumpData>();

			for (TResourcePointerMap.iterator LaniatusDefVariables = m_pResMap.begin(); LaniatusDefVariables != m_pResMap.end(); ++i)
			{
				CResource pResource = i.second;
				SDumpData data = new SDumpData();
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char * ext = strrchr(data.filename, '.');
				char ext = strrchr(data.filename, '.');
			...
			if (fp != null)
			{
//# Laniatus Games Studio Inc. | TODO TASK: The 'Compare' parameter of std::sort produces a boolean value, while the .NET Comparison parameter produces a tri-state result:
//Original Metin2 CPlus Line: std::sort(dumpVector.begin(), dumpVector.end(), DumpKBCompare);
				dumpVector.Sort(DumpKBCompare);

				FDumpPrint DumpPrint = new FDumpPrint();
				DumpPrint.m_fp = fp;
				DumpPrint.m_totalKB = 0;

				dumpVector.ForEach(DumpPrint);
				fprintf(fp, "total: %.2fmb", DumpPrint.m_totalKB / 1024.0f);

				FDumpCostPrint DumpCostPrint = new FDumpCostPrint();
				DumpCostPrint.m_fp = fp;

//# Laniatus Games Studio Inc. | TODO TASK: The 'Compare' parameter of std::sort produces a boolean value, while the .NET Comparison parameter produces a tri-state result:
//Original Metin2 CPlus Line: std::sort(dumpVector.begin(), dumpVector.end(), DumpCostCompare);
				dumpVector.Sort(DumpCostCompare);
				dumpVector.ForEach(DumpCostPrint);
				fprintf(fp, "total: %.2fmb", DumpPrint.m_totalKB / 1024.0f);
		...
		public bool IsFileExist(string c_szFileName)
		{
		...
		public void Update()
		{
			uint CurrentTime = ELTimer_GetMSec();
			CResource pResource;
			int Count = 0;
		...
		public void ProcessBackgroundLoading()
		{
			...
			while (ms_loadingThread.Fetch(pData))
			{
				...
				((string) pData.pvBuf) = null;
				pData = null;
			}

			int lCurrentTime = ELTimer_GetMSec();
...
//Original Metin2 CPlus Line: const int & rCreatingTime = itorRef->first;
				int rCreatingTime = itorRef.first;
		...
		public void PushBackgroundLoadingSet(SortedSet<string> LoadingSet)
		{
			SortedSet<string>.Enumerator itor = LoadingSet.GetEnumerator();

			while (itor.MoveNext())
			{
				uint dwFileCRC = __GetFileCRC(itor);
				...
				m_RequestMap.insert(TResourceRequestMap.value_type(dwFileCRC, itor));
			}
		}

		protected void __DestroyDeletingResourceMap()
		{
			Tracenf("CResourceManager::__DestroyDeletingResourceMap %d", m_ResourceDeletingMap.size());
			for (TResourceDeletingMap.iterator LaniatusDefVariables = m_ResourceDeletingMap.begin(); LaniatusDefVariables != m_ResourceDeletingMap.end(); ++i)
			{
				(i.first).Clear();
			}
		...
		protected void __DestroyResourceMap()
		{
			Tracenf("CResourceManager::__DestroyResourceMap %d", m_pResMap.size());

			TResourcePointerMap.iterator LaniatusDefVariables = new TResourcePointerMap.iterator();
			for (i = m_pResMap.begin(); LaniatusDefVariables != m_pResMap.end(); ++i)
			{
				CResource pResource = i.second;
				pResource.Clear();
			}
		...
		protected void __DestroyCacheMap()
		{
			TResourcePointerMap.iterator LaniatusDefVariables = new TResourcePointerMap.iterator();
			for (i = m_pCacheMap.begin(); LaniatusDefVariables != m_pCacheMap.end(); ++i)
			{
				CResource pResource = i.second;
				pResource.Release();
			}
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string __GetFileCRC_s_szFullPathFileName = new string(new char[MAX_PATH]);

		protected uint __GetFileCRC(string c_szFileName, string[] c_ppszLowerFileName = null)
		{
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static char s_szFullPathFileName[MAX_PATH];
			string src = c_szFileName;
			string dst = __GetFileCRC_s_szFullPathFileName;
			int len = 0;
			...
			while (src[len] != '\0')
			{
				if (src[len] == '/')
				{
					dst = StringFunctions.ChangeCharacter(dst, len, '\\');
				}
				else
				{
					dst = StringFunctions.ChangeCharacter(dst, len, (char) korean_tolower(src[len]));
				}

				++len;
			}

			dst = StringFunctions.ChangeCharacter(dst, len, '\0');

			if (c_ppszLowerFileName)
			{
				c_ppszLowerFileName[0] = __GetFileCRC_s_szFullPathFileName[0];
			}

			return (GetCRC32(__GetFileCRC_s_szFullPathFileName, len));
		}

		protected delegate SortedDictionary<string, CResource * (string UnnamedParameter)> TResourceNewFunctionPointerMap;
		protected delegate SortedDictionary<int, CResource * (string UnnamedParameter)> TResourceNewFunctionByTypePointerMap;

		protected SortedDictionary<uint, CResource > m_pCacheMap = new SortedDictionary<uint, CResource >();
		protected SortedDictionary<uint, CResource > m_pResMap = new SortedDictionary<uint, CResource >();
		protected TResourceNewFunctionPointerMap m_pResNewFuncMap = new TResourceNewFunctionPointerMap();
		protected TResourceNewFunctionByTypePointerMap m_pResNewFuncByTypeMap = new TResourceNewFunctionByTypePointerMap();
		protected SortedDictionary<CResource , uint> m_ResourceDeletingMap = new SortedDictionary<CResource , uint>();
		protected SortedDictionary<uint, string> m_RequestMap = new SortedDictionary<uint, string>();
		protected SortedDictionary<uint, string> m_WaitingMap = new SortedDictionary<uint, string>();
		protected SortedDictionary<int, CResource> m_pResRefDecreaseWaitingMap = new SortedDictionary<int, CResource>();

		protected static CFileLoaderThread ms_loadingThread = new CFileLoaderThread();
}

public class SDumpData
{
...
public class FDumpPrint
{
	public FILE m_fp;
	public static float m_totalKB;

	public void functorMethod(SDumpData data)
	{
		m_totalKB += data.KB;
