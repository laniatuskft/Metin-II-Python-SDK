...
public class CGraphicFontTexture : CGraphicTexture
{

	public class SCharacterInfomation
	{
		public short index;
		public short width;
		public short height;
	...
	public void Destroy()
	{
		...
		if (m_fontMap.size() != 0)
		{
			TFontMap.iterator LaniatusDefVariables = m_fontMap.begin();

			while (i != m_fontMap.end())
			{
				DeleteObject((IntPtr)i.second);
				++i;
			}
	...
	public bool Create(string c_szFontName, int fontSize, bool bItalic)
	{
		Destroy();
		...
		m_hFontOld = (IntPtr)SelectObject(hDC, m_hFont);
	...
	public bool CheckTextureIndex(uint dwTexture)
	{
	...
	public void SelectTexture(uint dwTexture)
	{
		Debug.Assert(CheckTextureIndex(dwTexture));
		m_lpd3dTexture = m_pFontTextureVector[dwTexture].GetD3DTexture();
	}

	public CGraphicImageTexture GetTexture(uint dwTexture)
	{
		Debug.Assert(CheckTextureIndex(dwTexture));
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		...
		if (!pFontTexture.Lock(pitch, (object) & pwDst))
		{
			return false;
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		uint * pdwSrc = (uint)m_dib.GetPointer();

		for (int y = 0; y < height; ++y, pwDst += (ushort)pitch, pdwSrc += (uint)width)
		{
	...
	public SCharacterInfomation GetCharacterInfomation(ushort codePage, char keyValue)
	{
		TCharacterKey code = new TCharacterKey(codePage, keyValue);
...
//	SCharacterInfomation UpdateCharacterInfomation(System.Tuple<ushort,char> code);
...
//Original Metin2 CPlus Line: bool IsEmpty() const
	...
	protected IntPtr GetFont(ushort codePage)
	{
		IntPtr hFont = null;
		TFontMap.iterator LaniatusDefVariables = m_fontMap.find(codePage);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
	...
	protected CGraphicDib m_dib = new CGraphicDib();

	protected IntPtr m_hFontOld;
	protected IntPtr m_hFont;

	protected List<CGraphicImageTexture> m_pFontTextureVector = new List<CGraphicImageTexture>();

	protected SortedDictionary<Tuple<ushort,char>, SCharacterInfomation> m_charInfoMap = new SortedDictionary<Tuple<ushort,char>, SCharacterInfomation>();

	protected SortedDictionary<ushort, IntPtr> m_fontMap = new SortedDictionary<ushort, IntPtr>();
	...
	protected string m_fontName = new string(new char[LF_FACESIZE]);
	protected int m_fontSize;
	protected bool m_bItalic;
}
