...
public class CNetworkDatagram
{
		public CNetworkDatagram()
		{
			__Initialize();
		}
		...
		public bool Create(uint uPort)
		{
			Debug.Assert(INVALID_SOCKET == m_sock);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public int PeekRecvFrom(uint uBufLen, object pvBuf, SOCKADDR_IN pkSockAddrIn)
		{
			int nSockAddrInLen = sizeof(SOCKADDR_IN);
			return recvfrom(m_sock, (string)pvBuf, uBufLen, MSG_PEEK, (PSOCKADDR)pkSockAddrIn, nSockAddrInLen);
		}

		public int RecvFrom(uint uBufLen, object pvBuf, SOCKADDR_IN pkSockAddrIn)
		{
			int nSockAddrInLen = sizeof(SOCKADDR_IN);
			return recvfrom(m_sock, (string)pvBuf, uBufLen, 0, (PSOCKADDR)pkSockAddrIn, nSockAddrInLen);
		}

		public int SendTo(uint uBufLen, object c_pvBuf, in SOCKADDR_IN c_rkSockAddrIn)
		{
			return sendto(m_sock, (string)c_pvBuf, uBufLen, 0, (PSOCKADDR) c_rkSockAddrIn, sizeof(SOCKADDR_IN));
		}
		...
		private fd_set m_fdsRecv = new fd_set();
		private fd_set m_fdsSend = new fd_set();
}