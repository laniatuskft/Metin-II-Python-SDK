...
public class CGraphicText : CResource
{

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static TType Type_s_type = StringToType("CGraphicText");

		public static CGraphicText.TType Type()
		{
...
//			static TType s_type = StringToType("CGraphicText");
		...
		public CGraphicText(string c_szFileName) : base(c_szFileName)
		{
		}
		...
		public CGraphicFontTexture GetFontTexturePointer()
		{
			return m_fontTexture;
		}

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string OnLoad_strName = new string(new char[32]);
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string OnLoad_num = new string(new char[8]);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * p = strrchr(GetFileName(), ':');

			if (p != '\0')
			{
				strncpy(OnLoad_strName, GetFileName(), MIN(31, p - GetFileName()));
				++p;
...
//				static char num[8];
			...
			else
			{
				...
				if (!p != '\0')
				{
					Debug.Assert(!"CGraphicText::OnLoadFromFile there is no extension (ie: .fnt)");
					OnLoad_strName[0] = '\0';
				}
				else
				{
			...
			if (!m_fontTexture.Create(OnLoad_strName, size, bItalic))
			{
				return false;
			}
...
//Original Metin2 CPlus Line: bool OnIsEmpty() const
		...
		protected CGraphicFontTexture m_fontTexture = new CGraphicFontTexture();
}
