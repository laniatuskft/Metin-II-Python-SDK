...
{
		public class SGroupNode
		{
			public static uint GenNameKey(string c_szGroupName, uint uGroupNameLen)
			{
				return GetCRC32(c_szGroupName, uGroupNameLen);
			}

			public void SetGroupName(in string c_rstGroupName)
			{
			...
			public bool IsGroupNameKey(uint dwGroupNameKey)
			{
			...
			public CTokenVector GetTokenVector(in string c_rstGroupName)
			{
				uint dwGroupNameKey = GenNameKey(c_rstGroupName, c_rstGroupName.Length);

				SortedDictionary<uint, CTokenVector>.Enumerator f = m_kMap_dwKey_kVct_stToken.find(dwGroupNameKey);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			public bool IsExistTokenVector(in string c_rstGroupName)
			{
				uint dwGroupNameKey = GenNameKey(c_rstGroupName, c_rstGroupName.Length);
			...
			public void InsertTokenVector(in string c_rstGroupName, in CTokenVector c_rkVct_stToken)
			{
				uint dwGroupNameKey = GenNameKey(c_rstGroupName, c_rstGroupName.Length);

				m_kMap_dwKey_kVct_stToken.insert(SortedDictionary<uint, CTokenVector>.value_type(dwGroupNameKey, c_rkVct_stToken));
			}

			public uint m_dwGroupNameKey;
			public string m_strGroupName = "";

			public SortedDictionary<uint, CTokenVector> m_kMap_dwKey_kVct_stToken = new SortedDictionary<uint, CTokenVector>();

			public SGroupNode pParentNode;
			public List<SGroupNode> ChildNodeVector = new List<SGroupNode>();

			public static CTextFileLoader.SGroupNode New()
			{
				return ms_kPool.Alloc();
			}

			public static void Delete(SGroupNode pkNode)
			{
			...
			public static CDynamicPool<SGroupNode> ms_kPool = new CDynamicPool<SGroupNode>();
		}


		public class CGotoChild
		{
			public CGotoChild(CTextFileLoader pOwner, string c_szKey)
			{
				this.m_pOwner = pOwner;
				m_pOwner.SetChildNode(c_szKey);
			}
			public CGotoChild(CTextFileLoader pOwner, uint dwIndex)
			{
				this.m_pOwner = pOwner;
			...
			public CTextFileLoader m_pOwner;
		}

		public static void DestroySystem()
		{
			{
				SortedDictionary<uint, CTextFileLoader>.Enumerator i;
				for (i = ms_kMap_dwNameKey_pkTextFileLoader.begin(); i.MoveNext();)
				{
		...
		public static CTextFileLoader Cache(string c_szFileName)
		{
			uint dwNameKey = GetCRC32(c_szFileName, strlen(c_szFileName));
			SortedDictionary<uint, CTextFileLoader>.Enumerator f = ms_kMap_dwNameKey_pkTextFileLoader.find(dwNameKey);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			ms_kMap_dwNameKey_pkTextFileLoader.insert(SortedDictionary<uint, CTextFileLoader>.value_type(dwNameKey, pkNewTextFileLoader));
			return pkNewTextFileLoader;
		}

		public CTextFileLoader()
		{
			SetTop();
		...
		public virtual void Dispose()
		{
			...
			if (m_acBufData)
			{
				Arrays.DeleteArray(m_acBufData);
			}
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		public uint GetChildNodeCount()
		{
			if (!m_pcurNode)
			{
				Debug.Assert(!"Node to access has not set!");
				return 0;
			}
		...
		public bool SetChildNode(string c_szKey)
		{
			if (!m_pcurNode)
			{
				Debug.Assert(!"Node to access has not set!");
			...
			uint dwKey = SGroupNode.GenNameKey(c_szKey, strlen(c_szKey));
		...
		public bool SetChildNode(in string c_rstrKeyHead, uint dwIndex)
		{
			string szKey = new string(new char[32 + 1]);
			_snprintf(szKey, sizeof(char), "%s%02u", c_rstrKeyHead, dwIndex);

			return SetChildNode(szKey);
		}

		public bool SetChildNode(uint dwIndex)
		{
			if (!m_pcurNode)
			{
				Debug.Assert(!"Node to access has not set!");
			...
			{
				Debug.Assert(!"Node index to set is too large to access!");
				return false;
			}
		...
		public bool SetParentNode()
		{
			...
			{
				Debug.Assert(!"Current group node is already top!");
				return false;
			}
		...
		public bool GetCurrentNodeName(string pstrName)
		{
			if (!m_pcurNode)
			{
				return false;
		...
		public bool IsToken(in string c_rstrKey)
		{
			if (!m_pcurNode)
			{
				Debug.Assert(!"Node to access has not set!");
		...
		public bool GetTokenVector(in string c_rstrKey, CTokenVector[] ppTokenVector)
		{
			if (!m_pcurNode)
			{
				Debug.Assert(!"Node to access has not set!");
...
//Original Metin2 CPlus Line: *ppTokenVector = pkRetTokenVector;
			ppTokenVector[0].CopyFrom(pkRetTokenVector);

			return true;
		}

		public bool GetTokenBoolean(in string c_rstrKey, ref bool pData)
		{
		...
		public bool GetTokenByte(in string c_rstrKey, ref byte pData)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenWord(in string c_rstrKey, ref ushort pData)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
			...
			pData = (ushort)atoi(pTokenVector.at(0).c_str());

			return true;
		}

		public bool GetTokenInteger(in string c_rstrKey, ref int pData)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenDoubleWord(in string c_rstrKey, ref uint pData)
		{
			return GetTokenInteger(c_rstrKey, (int)(pData));
		}

		public bool GetTokenFloat(in string c_rstrKey, ref float pData)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenVector2(in string c_rstrKey, D3DXVECTOR2 pVector2)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenVector3(in string c_rstrKey, _D3DVECTOR pVector3)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenVector4(in string c_rstrKey, D3DXVECTOR4 pVector4)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenPosition(in string c_rstrKey, _D3DVECTOR pVector)
		{
			return GetTokenVector3(c_rstrKey, pVector);
		}

		public bool GetTokenQuaternion(in string c_rstrKey, D3DXQUATERNION pQ)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenDirection(in string c_rstrKey, D3DVECTOR pVector)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenColor(in string c_rstrKey, D3DXCOLOR pColor)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenColor(in string c_rstrKey, D3DCOLORVALUE pColor)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		public bool GetTokenString(in string c_rstrKey, string pString)
		{
			CTokenVector pTokenVector;
			if (!GetTokenVector(c_rstrKey, pTokenVector))
			{
		...
		protected void __DestroyGroupNodeVector()
		{
			List<SGroupNode>.Enumerator i;
			for (i = m_kVct_pkNode.begin(); i.MoveNext();)
			{
				SGroupNode.Deletei.Current;
			}
			m_kVct_pkNode.clear();
		}

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool LoadGroup(SGroupNode pGroupNode);

		protected string m_strFileName = "";

		protected string m_acBufData;
		protected uint m_dwBufSize;
		protected uint m_dwBufCapacity;

		protected uint m_dwcurLineIndex;

		protected CMemoryTextFileLoader m_textFileLoader = new CMemoryTextFileLoader();

		protected SGroupNode m_GlobalNode = new SGroupNode();
		protected SGroupNode m_pcurNode;

		protected List<SGroupNode> m_kVct_pkNode = new List<SGroupNode>();

		protected static SortedDictionary<uint, CTextFileLoader> ms_kMap_dwNameKey_pkTextFileLoader = new SortedDictionary<uint, CTextFileLoader>();
		protected static bool ms_isCacheMode = false;
}
