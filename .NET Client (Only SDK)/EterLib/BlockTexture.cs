...
public class CBlockTexture : CGraphicBase
{
		public CBlockTexture()
		{
			m_pDIB = null;
			m_lpd3dTexture = null;
		}
		...
		public bool Create(CGraphicDib pDIB, in RECT c_rRect, uint dwWidth, uint dwHeight)
		{
		...
		public void SetClipRect(in RECT c_rRect)
		{
			m_bClipEnable = true;
			m_clipRect = c_rRect;
		}

		public void Render(int ix, int iy)
		{
			...
			if (CGraphicBase.SetPDTStream(vertices, 4))
			{
				...
				(CStateManager.Instance()).SetTexture(0, m_lpd3dTexture);
				(CStateManager.Instance()).SetTexture(1, null);
				(CStateManager.Instance()).SetVertexShader(D3DFVF_XYZ | D3DFVF_TEX1 | D3DFVF_DIFFUSE);
				(CStateManager.Instance()).DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 4, 0, 2);
			}
		}

		public void InvalidateRect(in RECT c_rsrcRect)
		{
			RECT dstRect = m_rect;
			if (c_rsrcRect.right < dstRect.left || c_rsrcRect.left > dstRect.right || c_rsrcRect.bottom < dstRect.top || c_rsrcRect.top > dstRect.bottom)
			{
				Tracef("InvalidateRect() - Strange rect");
				return;
			}

			RECT clipRect = new RECT(Math.Max(c_rsrcRect.left - dstRect.left, 0), Math.Max(c_rsrcRect.top - dstRect.top, 0), Math.Min(c_rsrcRect.right - dstRect.left, dstRect.right - dstRect.left), Math.Min(c_rsrcRect.bottom - dstRect.top, dstRect.bottom - dstRect.top));
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			uint * pdwDst = (uint)lockedRect.pBits;
			uint dwDstWidth = (uint)(lockedRect.Pitch >> 2);
			uint dwSrcWidth = m_pDIB.GetWidth();
			for (int LaniatusDefVariables = 0; LaniatusDefVariables < iclipHeight; ++i)
			{
		...
		protected CGraphicDib m_pDIB;
		protected RECT m_rect = new RECT();
		protected RECT m_clipRect = new RECT();
		protected bool m_bClipEnable;
		protected uint m_dwWidth;
		protected uint m_dwHeight;
		protected LPDIRECT3DTEXTURE8 m_lpd3dTexture = new LPDIRECT3DTEXTURE8();
}
