...
public class CGraphicDib
{
		...
		public bool Create(IntPtr hDC, int width, int height)
		{
			Destroy();
			...
			if (!m_hDC)
			{
				Debug.Assert(!"CGraphicDib::Create CreateCompatibleDC Error");
				return false;
			}
			...
			if (!m_hBmp)
			{
				Debug.Assert(!"CGraphicDib::Create CreateDIBSection Error");
				return false;
			}

			SelectObject(m_hDC, m_hBmp);

			global::SetTextColor(m_hDC, RGB(255, 255, 255));
		...
		{
			global::SetBkMode(m_hDC, iBkMode);
		}

		public void TextOut(int ix, int iy, string c_szText)
		{
			global::SetBkColor(m_hDC, 0);
			global::TextOut(m_hDC, ix, iy, c_szText, strlen(c_szText));
		}

		public void Put(IntPtr hDC, int x, int y)
		{
			SetDIBitsToDevice(hDC, x, y, m_width, m_height, 0, 0, 0, m_height, m_pvBuf, m_bmi, DIB_RGB_COLORS);
		}
