...
public class RangeTester <T> : SpherePackCallback
{
	public T[] f;
	public float dist;
	public RangeTester(T fn, float distance = -1F)
	{
		this.f = fn;
		this.dist = distance;
	}
...
//# Laniatus Games Studio Inc. | TODO TASK: C# has no concept of 'private' inheritance:
//Original Metin2 CPlus Line: class CCullingManager : public CSingleton<CCullingManager>, public SpherePackCallback, private CScreen
//# Laniatus Games Studio Inc. | TODO TASK: Multiple inheritance is not available in C#:
public class CCullingManager : CSingleton<CCullingManager>, SpherePackCallback, CScreen
{

	public CCullingManager()
	{
		m_Factory = new SpherePackFactory(10000, 6400, 1600, 400);
	}
...
public int showingcount = 0;
	...
	public void RangeTestCallback(in Vector3d p, float distance, SpherePack sphere, ViewState state)
	{
#if SPHERELIB_STRICT
			if (sphere.IS_SPHERE)
			{
	...
	public void FindRange(in Vector3d p, float radius)
	{
		m_list.clear();
		m_Factory.RangeTest(p, radius, this);
	}

	public void FindRay(in Vector3d p1, in Vector3d dir)
	{
		m_RayFarDistance = -1;
		m_list.clear();
	...
	public void FindRayDistance(in Vector3d p1, in Vector3d dir, float distance)
	{
	...
	public void RangeTest(in Vector3d p, float radius, SpherePackCallback callback)
	{
		m_Factory.RangeTest(p, radius, callback);
	}

	public void PointTest2d(in Vector3d p, SpherePackCallback callback)
	{
...
//# Laniatus Games Studio Inc. | WARN: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//Original Metin2 CPlus Line: template <class T>
	public void ForInRange2d<T>(in Vector3d p, T pFunc)
	{
		RangeTester<T> r = new RangeTester<T>(pFunc);
		m_Factory.PointTest2d(p, r);
	}

//# Laniatus Games Studio Inc. | WARN: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//Original Metin2 CPlus Line: template <class T>
	public void ForInRange<T>(in Vector3d p, float radius, T pFunc)
	{
		RangeTester<T> r = new RangeTester<T>(pFunc);
...
//# Laniatus Games Studio Inc. | WARN: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//Original Metin2 CPlus Line: template <class T>
	public void ForInRay<T>(in Vector3d p1, in Vector3d dir, T pFunc)
	{
		RangeTester<T> r = new RangeTester<T>(pFunc);
...
//# Laniatus Games Studio Inc. | WARN: The original C++ template specifier was replaced with a C# generic specifier, which may not produce the same behavior:
//Original Metin2 CPlus Line: template <class T>
	public void ForInRayDistance<T>(in Vector3d p, in Vector3d dir, float distance, T pFunc)
	{
		RangeTester<T> r = new RangeTester<T>(pFunc, distance);
		m_Factory.RayTrace(p, dir, r);
	}

	public SpherePack Register(CGraphicObjectInstance obj)
	{
		Debug.Assert(obj);
#if COUNT_SHOWING_SPHERE
		Tracef("CR : %p  ",obj);
...
//	void Unregister(SpherePack h);

	public List<CGraphicObjectInstance >.Enumerator begin()
	{
		return m_list.begin();
	}
	public List<CGraphicObjectInstance >.Enumerator end()
	{
		return m_list.end();
	}

	protected List<CGraphicObjectInstance > m_list = new List<CGraphicObjectInstance >();

	protected float m_RayFarDistance;

	protected SpherePackFactory m_Factory;
}
