...
namespace script
{
	public class SArgumet
	{
		public SArgumet(in string c_stName, in string c_stValue)
		{
			strName = c_stName;
			strValue = c_stValue;
		}
		public SArgumet(in SArgumet c_arg)
		{
...
//Original Metin2 CPlus Line: void operator =(const SArgumet& c_arg)
		public void CopyFrom(in SArgumet c_arg)
		{
			strName = c_arg.strName;
			strValue = c_arg.strValue;
		}
	...
	{
		public string name = "";
		public TArgList argList = new TArgList();

		public SCmd()
		{
		}
		public SCmd(in SCmd c_cmd)
		{
			name = c_cmd.name;
			argList = c_cmd.argList;
		}
//# Laniatus Games Studio Inc. |: This 'CopyFrom' method was converted from the original copy assignment operator:
//Original Metin2 CPlus Line: void operator =(const SCmd& c_cmd)
		public void CopyFrom(in SCmd c_cmd)
		{
			name = c_cmd.name;
			argList = c_cmd.argList;
		}
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
			private string Create_stLetter = "";

			public bool Create(in string stSource)
			{
				m_cmdList.clear();

				if (string.IsNullOrEmpty(stSource))
				{
					return false;
				}

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char *str_base = stSource.c_str();
				char str_base = stSource;
				if (!str_base != '\0' || !str_base != '\0')
				{
				...
				uint codePage = GetDefaultCodePage();

				string box_data = new string(new char[1024 + 1]);
...
//				static string stLetter;

				while (str_pos < str_len)
				{
					TCmd cmd = new TCmd();

					string word = str_base + str_pos;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* word_next = CharNextExA(codePage, word, 0);
					char word_next = CharNextExA(codePage, word, 0);
					...
					if (word_len > 1)
					{
						...
						{
							Create_stLetter.assign(word, word_next);

							cmd.name.assign("LETTER");
							cmd.argList.push_back(TArg("value", Create_stLetter));
					...
					else if (word_len == 1)
					{
						...
						if ('[' == cur)
						{
							...
							string box_begin = str_base + str_pos;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* box_end = LocaleString_FindChar(box_begin, str_len - str_pos, ']');
							char box_end = LocaleString_FindChar(box_begin, str_len - str_pos, ']');
							if (!box_end != '\0')
							{
								TraceError(" !! PARSING ERROR - Syntax Error : %s\n", box_begin);
								return false;
							}
							str_pos += box_end - box_begin.Substring(1);


							int data_len = 0;
							{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* data_begin = LocaleString_Skip(codePage, box_begin);
								char data_begin = LocaleString_Skip(codePage, box_begin);
								string data_end = box_end;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
								...
								box_data = StringFunctions.ChangeCharacter(box_data, data_len, '\0');
							...
							{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* space = LocaleString_FindChar(box_data, data_len, ' ');
								char space = LocaleString_FindChar(box_data, data_len, ' ');
								if (space != '\0')
								{
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* space_next = CharNextExA(codePage, space, 0);
									char space_next = CharNextExA(codePage, space, 0);
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* arg = LocaleString_Skip(codePage, space_next);
									char arg = LocaleString_Skip(codePage, space_next);
			...
			public bool GetCmd(ref TCmd cmd)
			{
				if (m_cmdList.empty())
				{
					return false;
			...
			public bool ReadCmd(ref TCmd cmd)
			{
				if (m_cmdList.empty())
				{
					return false;
			...
			private void SetError(string c_pszError)
			{
				m_stError.assign(c_pszError);
			}

			private bool GetArg(string c_arg_base, int arg_len, TArgList argList)
			{
				string szName = new string(new char[32 + 1]);
				string szValue = new string(new char[64 + 1]);
				...
				while (pos < arg_len)
				{
					string cur = c_arg_base.Substring(pos);
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* next = CharNextExA(codePage, cur, 0);
					char next = CharNextExA(codePage, cur, 0);
					iCharLen = next - cur;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
							memcpy(szValue.Substring(iValueLen), cur, iCharLen);
							iValueLen += iCharLen;
							szValue = StringFunctions.ChangeCharacter(szValue, iValueLen, '\0');
						}
						else
						{
							if (iNameLen >= 32)
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
							memcpy(szName.Substring(iNameLen), cur, iCharLen);
							iNameLen += iCharLen;
							szName = StringFunctions.ChangeCharacter(szName, iNameLen, '\0');
						}
					}
					else if (iCharLen == 1)
					{
						...
						else if (!isValue && iNameLen == 0 && isspace((byte) c))
						{
						}
						else if (c == '\r' || c == '\n')
						{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			private LinkedList<TCmd> m_cmdList = new LinkedList<TCmd>();
	}
}