...
public class CDecal
{
	...
	public const int MAX_DECAL_VERTICES = 256;

	public CDecal()
	{
		this.m_cfDecalEpsilon = 0.25f;
		Clear();
	}
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
	...
	public abstract void Make(_D3DVECTOR v3Center, _D3DVECTOR v3Normal, _D3DVECTOR v3Tangent, float fWidth, float fHeight, float fDepth);
	public virtual void Render()
	{
		...
		for (uint dwi = 0; dwi < m_TriangleFanStructVector.size(); ++dwi)
		{
			(CStateManager.Instance()).DrawIndexedPrimitiveUP(D3DPT_TRIANGLEFAN, m_TriangleFanStructVector[dwi].m_wMinIndex, m_TriangleFanStructVector[dwi].m_dwVertexCount, m_TriangleFanStructVector[dwi].m_dwPrimitiveCount, m_Indices + m_TriangleFanStructVector[dwi].m_wMinIndex, D3DFMT_INDEX16, m_Vertices, sizeof(SPDTVertex));
		}
	}

	protected _D3DVECTOR m_v3Center = new _D3DVECTOR();
	protected _D3DVECTOR m_v3Normal = new _D3DVECTOR();

	protected D3DXPLANE m_v4LeftPlane = new D3DXPLANE();
	protected D3DXPLANE m_v4RightPlane = new D3DXPLANE();
	protected D3DXPLANE m_v4BottomPlane = new D3DXPLANE();
	protected D3DXPLANE m_v4TopPlane = new D3DXPLANE();
	protected D3DXPLANE m_v4FrontPlane = new D3DXPLANE();
	protected D3DXPLANE m_v4BackPlane = new D3DXPLANE();

	protected uint m_dwVertexCount;
	protected uint m_dwPrimitiveCount;

	protected class TTRIANGLEFANSTRUCT
	{
		public ushort m_wMinIndex;
		public uint m_dwVertexCount;
		public uint m_dwPrimitiveCount;
		public uint m_dwVBOffset;
	}

	protected List<TTRIANGLEFANSTRUCT> m_TriangleFanStructVector = new List<TTRIANGLEFANSTRUCT>();

	protected SPDTVertex[] m_Vertices = Arrays.InitializeWithDefaultInstances<SPDTVertex>(MAX_DECAL_VERTICES);
	protected ushort[] m_Indices = new ushort[MAX_DECAL_VERTICES];

	protected readonly float m_cfDecalEpsilon;

	protected bool AddPolygon(uint dwAddCount, _D3DVECTOR[] c_pv3Vertex, _D3DVECTOR c_pv3Normal)
	{
		...
		TTRIANGLEFANSTRUCT aTriangleFanStruct = new TTRIANGLEFANSTRUCT();
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		ushort * wIndex = m_Indices + dwCount;

		m_dwPrimitiveCount += dwAddCount - 2;

		for (uint dwVertexNum = 0; dwVertexNum < dwAddCount; ++dwVertexNum)
		{
			*wIndex++ = (ushort) dwCount;
	...
	protected void ClipMesh(uint dwPrimitiveCount, _D3DVECTOR[] c_pv3Vertex, _D3DVECTOR[] c_pv3Normal)
	{
		_D3DVECTOR[] v3NewVertex = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(9);
		_D3DVECTOR[] v3NewNormal = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(9);

		for (uint dwi = 0; dwi < dwPrimitiveCount; ++dwi)
		{
			_D3DVECTOR v3_1 = c_pv3Vertex[3 * dwi];
			_D3DVECTOR v3_2 = c_pv3Vertex[3 * dwi + 1];
			_D3DVECTOR v3_3 = c_pv3Vertex[3 * dwi + 2];
...
//Original Metin2 CPlus Line: v3NewVertex[0] = v3_1;
				v3NewVertex[0].CopyFrom(v3_1);
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: v3NewVertex[1] = v3_2;
				v3NewVertex[1].CopyFrom(v3_2);
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: v3NewVertex[2] = v3_3;
				v3NewVertex[2].CopyFrom(v3_3);

//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: v3NewNormal[0] = c_pv3Normal[3 * dwi];
				v3NewNormal[0].CopyFrom(c_pv3Normal[3 * dwi]);
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: v3NewNormal[1] = c_pv3Normal[3 * dwi + 1];
				v3NewNormal[1].CopyFrom(c_pv3Normal[3 * dwi + 1]);
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: v3NewNormal[2] = c_pv3Normal[3 * dwi + 2];
				v3NewNormal[2].CopyFrom(c_pv3Normal[3 * dwi + 2]);

				uint dwCount = ClipPolygon(3, v3NewVertex, v3NewNormal, v3NewVertex, v3NewNormal);
...
//Original Metin2 CPlus Line: uint ClipPolygon(uint dwVertexCount, const struct _D3DVECTOR *c_pv3Vertex, const struct _D3DVECTOR *c_pv3Normal, struct _D3DVECTOR *c_pv3NewVertex, struct _D3DVECTOR *c_pv3NewNormal) const
	protected uint ClipPolygon(uint dwVertexCount, _D3DVECTOR c_pv3Vertex, _D3DVECTOR c_pv3Normal, _D3DVECTOR c_pv3NewVertex, _D3DVECTOR c_pv3NewNormal)
	{
		_D3DVECTOR[] v3TempVertex = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(9);
		_D3DVECTOR[] v3TempNormal = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(9);

		uint dwCount = ClipPolygonAgainstPlane(m_v4LeftPlane, dwVertexCount, c_pv3Vertex, c_pv3Normal, v3TempVertex, v3TempNormal);
		if (dwCount != 0)
		{
	...
	protected static uint ClipPolygonAgainstPlane(in D3DXPLANE c_rv4Plane, uint dwVertexCount, _D3DVECTOR[] c_pv3Vertex, _D3DVECTOR[] c_pv3Normal, _D3DVECTOR[] c_pv3NewVertex, _D3DVECTOR[] c_pv3NewNormal)
	{
		bool[] bNegative = new bool[10];

		uint dwNegativeCount = 0;
		for (uint dwi = 0; dwi < dwVertexCount; ++dwi)
		{
			bool bNeg = (D3DXPlaneDotCoord(c_rv4Plane, c_pv3Vertex[dwi]) < 0.0F);
			bNegative[dwi] = bNeg;
			dwNegativeCount += bNeg ? 1 : 0;
		}
		...
		uint dwCount = 0;
		for (uint dwCurIndex = 0; dwCurIndex < dwVertexCount; ++dwCurIndex)
		{
			uint dwPrevIndex = (dwCurIndex != 0) ? dwCurIndex - 1 : dwVertexCount - 1;

			if (bNegative[dwCurIndex])
			{
				if (!bNegative[dwPrevIndex])
				{
					_D3DVECTOR v3_1 = c_pv3Vertex[dwPrevIndex];
					_D3DVECTOR v3_2 = c_pv3Vertex[dwCurIndex];
...
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewVertex[dwCount] = v3_1 * (1.0f - ft) + v3_2 * ft;
					 c_pv3NewVertex[dwCount].CopyFrom(v3_1 * (1.0f - ft) + v3_2 * ft);
					_D3DVECTOR v3_n1 = c_pv3Normal[dwPrevIndex];
					_D3DVECTOR v3_n2 = c_pv3Normal[dwCurIndex];
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewNormal[dwCount] = v3_n1 * (1.0f - ft) + v3_n2 * ft;
					 c_pv3NewNormal[dwCount].CopyFrom(v3_n1 * (1.0f - ft) + v3_n2 * ft);
					++dwCount;
				}
			}
			else
			{
				if (bNegative[dwPrevIndex])
				{
					_D3DVECTOR v3_1 = c_pv3Vertex[dwCurIndex];
					_D3DVECTOR v3_2 = c_pv3Vertex[dwPrevIndex];
					float ft = D3DXPlaneDotCoord(c_rv4Plane, v3_1) / (c_rv4Plane.a * (v3_1.x - v3_2.x) + c_rv4Plane.b * (v3_1.y - v3_2.y) + c_rv4Plane.c * (v3_1.z - v3_2.z));
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewVertex[dwCount] = v3_1 * (1.0f - ft) + v3_2 * ft;
					 c_pv3NewVertex[dwCount].CopyFrom(v3_1 * (1.0f - ft) + v3_2 * ft);
					_D3DVECTOR v3_n1 = c_pv3Normal[dwCurIndex];
					_D3DVECTOR v3_n2 = c_pv3Normal[dwPrevIndex];
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewNormal[dwCount] = v3_n1 * (1.0f - ft) + v3_n2 * ft;
					 c_pv3NewNormal[dwCount].CopyFrom(v3_n1 * (1.0f - ft) + v3_n2 * ft);
					++dwCount;
...
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewVertex[dwCount] = c_pv3Vertex[dwCurIndex];
				 c_pv3NewVertex[dwCount].CopyFrom(c_pv3Vertex[dwCurIndex]);
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: c_pv3NewNormal[dwCount] = c_pv3Normal[dwCurIndex];
				 c_pv3NewNormal[dwCount].CopyFrom(c_pv3Normal[dwCurIndex]);
				++dwCount;
			}
		}
