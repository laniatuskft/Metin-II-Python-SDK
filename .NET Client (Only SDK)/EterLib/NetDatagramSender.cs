...
public class CNetDatagramSender
{
		public CNetDatagramSender()
		{
			m_isSocket = false;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public bool SetSocket(string c_szIP, ushort wPortIndex)
		{
			return SetSocket(inet_addr(c_szIP), wPortIndex);
		}

		public bool SetSocket(uint dwAddress, ushort wPortIndex)
		{
			m_isSocket = true;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public bool Send(object pBuffer, int iSize)
		{
			Debug.Assert(isSocket());

			int iSendingLength = sendto(m_Socket, (string)pBuffer, iSize, 0, (PSOCKADDR) & m_SockAddr, sizeof(SOCKADDR_IN));
			if (iSendingLength < 0)
			{
		...
		protected bool m_isSocket;

		protected ushort m_dwAddress;
		protected ushort m_wPortIndex;
