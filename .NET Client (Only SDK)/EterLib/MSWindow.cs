...
public class CMSWindow
{
		...
		public bool Create(string c_szName, int brush = BLACK_BRUSH, uint cs = 0, uint ws = WS_OVERLAPPEDWINDOW, IntPtr hIcon = null, int iCursorResource = 32512)
		{
			Destroy();

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* c_szClassName = RegisterWindowClass(cs, brush, MSWindowProcedure, hIcon, iCursorResource);
			char c_szClassName = RegisterWindowClass(cs, brush, MSWindowProcedure, hIcon, iCursorResource);

			m_hWnd = CreateWindow(c_szClassName, c_szName, ws, 0, 0, 0, 0, null, null, ms_hInstance, null);
			...
			{
				string myAppID = new string(new char[128]);
//# Laniatus Games Studio Inc. | WARN: This 'sizeof' ratio was replaced with a direct reference to the array length:
//Original Metin2 CPlus Line: swprintf(myAppID, sizeof(myAppID) / sizeof(myAppID[0]), "MyMetin2AppID%u", GetCurrentProcessId());
				swprintf(myAppID, myAppID.Length, "MyMetin2AppID%u", GetCurrentProcessId());

				SetCurrentProcessExplicitAppUserModelIDptr SetCurrentProcessExplicitAppUserModelIDo = SetCurrentProcessExplicitAppUserModelID;
				if (SetCurrentProcessExplicitAppUserModelIDo != null)
				{
					int hr = SetCurrentProcessExplicitAppUserModelIDo(myAppID);
					if (!(((int)(hr)) >= 0))
					{
						return false ? 1 : 0;
					}
			...
			SetWindowLong(m_hWnd, GWL_USERDATA, (uint) this);

			return true ? 1 : 0;
		}

		private delegate int SetCurrentProcessExplicitAppUserModelIDptr(in _SAL2_Source_(_In_, (), (p1)[__P_impl(__d_ = 0)](__notnull_impl_notref) _Pre_valid_impl_(p1)[__P_impl(__d_ = 0)](__readaccess_impl_notref)) char * AppID);
		...
		public void SetVisibleMode(bool isVisible)
		{
		...
		public void AdjustSize(int width, int height)
		{
			...
			AdjustWindowRectEx(m_rect, ((uint)GetWindowLong(m_hWnd, GWL_STYLE)), GetMenu(m_hWnd) != null, ((uint)GetWindowLong(m_hWnd, GWL_EXSTYLE)));

			MoveWindow(m_hWnd, 0, 0, m_rect.right - m_rect.left, m_rect.bottom - m_rect.top, false);
		}
		...
		public void GetMousePosition(POINT ppt)
		{
			GetCursorPos(ppt);
			ScreenToClient(m_hWnd, ppt);
		}

		public void GetClientRect(RECT prc)
		{
			global::GetClientRect(m_hWnd, prc);
		}

		public void GetWindowRect(RECT prc)
		{
			global::GetWindowRect(m_hWnd, prc);
		}
		...
		public virtual int WindowProcedure(IntPtr hWnd, uint uiMsg, IntPtr wParam, IntPtr lParam)
		{
			switch (uiMsg)
			{
				case WM_SIZE:
		...
		public virtual void OnSize(IntPtr wParam, IntPtr lParam)
		{
		...
		protected string RegisterWindowClass(uint style, int brush, WNDPROC pfnWndProc, IntPtr hIcon = null, int iCursorResource = 32512)
		{
			string szClassName = new string(new char[1024]);
			sprintf(szClassName, "eter - s%x:b%x:p:%x", style, brush, (uint) pfnWndProc);

			TWindowClassSet.iterator f = ms_stWCSet.find((string) szClassName);
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* c_szStaticClassName = stl_static_string(szClassName).c_str();
			char c_szStaticClassName = stl_static_string(szClassName).c_str();
			...
			wc.hbrBackground = (IntPtr) GetStockObject(brush);
		...
		protected static SortedSet<char, stl_sz_less> ms_stWCSet = new SortedSet<char, stl_sz_less>();
		protected static IntPtr ms_hInstance = null;

	[System.Runtime.InteropServices.DllImport("shell32.dll")]
	internal static extern int SetCurrentProcessExplicitAppUserModelID(in _SAL2_Source_(_In_, (), (p1)[__P_impl(__d_ = 0)](__notnull_impl_notref) _Pre_valid_impl_(p1)[__P_impl(__d_ = 0)](__readaccess_impl_notref)) char * AppID);
}