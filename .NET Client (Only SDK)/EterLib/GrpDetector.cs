...
public delegate bool PFNCONFIRMDEVICE(D3DCAPS8 rkD3DCaps, uint uBehavior, D3DFORMAT eD3DFmt);

public class D3D_SModeInfo
{
	public uint m_uScrWidth;
	public uint m_uScrHeight;
	public uint m_uScrDepthBit;
	 public uint m_dwD3DBehavior;
	public D3DFORMAT m_eD3DFmtPixel = new D3DFORMAT();
	public D3DFORMAT m_eD3DFmtDepthStencil = new D3DFORMAT();

	public void GetString(string pstEnumList)
	{
		...
		string[] szVP = {"UNKNOWN", "HWVP", "MXVP", "SWVP"};

		string szText = new string(new char[1024 + 1]);
		_snprintf(szText, sizeof(char), "%dx%dx%d %s\r\n", m_uScrWidth, m_uScrHeight, uScrDepthBits, szVP[iVP]);
		pstEnumList.append(szText);
	}
}

public class D3D_CAdapterDisplayModeList
{
		public D3D_CAdapterDisplayModeList()
		{
		}
		public void Dispose()
		{
		}
		public void Build(IDirect3D8 rkD3D, D3DFORMAT eD3DFmtDefault, uint iD3DAdapterInfo)
		{
...
//Original Metin2 CPlus Line: aeD3DFmt[uD3DFmtNum++]=eD3DFmtDefault;
			aeD3DFmt[uD3DFmtNum++].CopyFrom(eD3DFmtDefault);
			...
			{
				D3DDISPLAYMODE kD3DDMCur = new D3DDISPLAYMODE();
		...
		public uint GetDisplayModeNum()
		{
			return m_uD3DDMNum;
		}

		public uint GetPixelFormatNum()
		{
			return m_uD3DFmtNum;
		}

		public D3DDISPLAYMODE GetDisplayModer(uint iD3DDM)
		{
			Debug.Assert(iD3DDM < m_uD3DDMNum);
			return m_akD3DDM[iD3DDM];
		}

		public D3DFORMAT GetPixelFormatr(uint iD3DFmt)
		{
			Debug.Assert(iD3DFmt < m_uD3DFmtNum);
			return m_aeD3DFmt[iD3DFmt];
		}


		protected const int D3DDISPLAYMODE_MAX = 100;
		protected const int D3DFORMAT_MAX = 20;
		protected const int FILTEROUT_LOWRESOLUTION_WIDTH = 640;
		protected const int FILTEROUT_LOWRESOLUTION_HEIGHT = 480;

		protected D3DDISPLAYMODE[] m_akD3DDM = Arrays.InitializeWithDefaultInstances<D3DDISPLAYMODE>(D3DDISPLAYMODE_MAX);
		protected D3DFORMAT[] m_aeD3DFmt = Arrays.InitializeWithDefaultInstances<D3DFORMAT>(D3DFORMAT_MAX);

		protected uint m_uD3DDMNum;
		protected uint m_uD3DFmtNum;

}

public class D3D_CDeviceInfo
{
		public D3D_CDeviceInfo()
		{
		}
		public void Dispose()
		{
...
//		bool Build(IDirect3D8 rkD3D, uint iAdapter, uint iDevType, D3D_CAdapterDisplayModeList rkD3DADMList, PFNCONFIRMDEVICE pfnConfirmDevice);
		public bool Find(uint uScrWidth, uint uScrHeight, uint uScrDepthBits, bool isWindowed, ref uint piD3DModeInfo)
		{
			if (isWindowed)
			{
				if (!m_isWindowed)
		...
		{
			string szText = new string(new char[1024 + 1]);
			_snprintf(szText, sizeof(char), "%s\r\n========================================\r\n", m_szDevDesc);
			pstEnumList.append(szText);

			for (uint iD3D_SModeInfo = 0; iD3D_SModeInfo < m_uD3DModeInfoNum; ++iD3D_SModeInfo)
			{
				_snprintf(szText, sizeof(char), "%d. ", iD3D_SModeInfo);
				pstEnumList.append(szText);
		...
		public bool FindDepthStencilFormat(IDirect3D8 rkD3D, uint iD3DAdapterInfo, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT pDepthStencilFormat)
		{
...
//		D3D_SModeInfo GetD3DModeInfor(uint iD3DModeInfo);
		public D3D_SModeInfo GetD3DModeInfop(uint iD3D_SModeInfo)
		{
		...
		protected const int D3DMODEINFO_NUM = 150;


		protected readonly string m_szDevDesc;

		protected D3DDEVTYPE m_eD3DDevType = new D3DDEVTYPE();
		protected D3DCAPS8 m_kD3DCaps = new D3DCAPS8();
		protected bool m_canDoWindowed;

		protected uint m_iCurD3DModeInfo;
		protected uint m_uD3DModeInfoNum;
		protected D3D_SModeInfo[] m_akD3DModeInfo = Arrays.InitializeWithDefaultInstances<D3D_SModeInfo>(D3DMODEINFO_NUM);


		protected bool m_isWindowed;

		protected D3DMULTISAMPLE_TYPE m_eD3DMSTWindowed = new D3DMULTISAMPLE_TYPE();
		protected D3DMULTISAMPLE_TYPE m_eD3DMSTFullscreen = new D3DMULTISAMPLE_TYPE();


		protected const string[] msc_aszD3DDevDesc = {"HAL", "REF"};
		protected const D3DDEVTYPE[] msc_aeD3DDevType = Arrays.PadReferenceTypeArrayWithDefaultInstances(D3DDEVICETYPE_NUM, new D3DDEVTYPE[] {D3DDEVTYPE_HAL, D3DDEVTYPE_REF});
}

public class D3D_CAdapterInfo
{
		public D3D_CAdapterInfo()
		{
		}
		public void Dispose()
		{
		}
		public bool Find(uint uScrWidth, uint uScrHeight, uint uScrDepthBits, bool isWindowed, ref uint piD3DModeInfo, ref uint piD3DDevInfo)
		{
		...
		public bool Build(IDirect3D8 rkD3D, uint iD3DAdapterInfo, PFNCONFIRMDEVICE pfnConfirmDevice)
		{
			...
			D3D_CAdapterDisplayModeList kD3DADMList = new D3D_CAdapterDisplayModeList();
		...
		public void GetString(string pstEnumList)
		{
			...
			{
				string szText = new string(new char[1024 + 1]);
				_snprintf(szText, sizeof(char), "Device %d\r\n", iDevInfo);
				pstEnumList.append(szText);
		...
		public D3DADAPTER_IDENTIFIER8 GetIdentifier()
		{
			return m_kD3DAdapterIdentifier;
		}

		public D3DDISPLAYMODE GetDesktopD3DDisplayModer()
		{
			return m_kD3DDMDesktop;
		}

		public D3DDISPLAYMODE GetDesktopD3DDisplayModep()
		{
			return m_kD3DDMDesktop;
		}

		public D3D_CDeviceInfo GetD3DDeviceInfop(uint iD3DDevInfo)
		{
		...
		protected const int D3DDEVICEINFO_NUM = 5;


		protected D3DADAPTER_IDENTIFIER8 m_kD3DAdapterIdentifier = new D3DADAPTER_IDENTIFIER8();
		protected D3DDISPLAYMODE m_kD3DDMDesktop = new D3DDISPLAYMODE();

		protected uint m_iCurD3DDevInfo;
		protected uint m_uD3DDevInfoNum;
		protected D3D_CDeviceInfo[] m_akD3DDevInfo = Arrays.InitializeWithDefaultInstances<D3D_CDeviceInfo>(D3DDEVICEINFO_NUM);

}

public class D3D_CDisplayModeAutoDetector
{
		public D3D_CDisplayModeAutoDetector()
		{
		...
		public bool Find(uint uScrWidth, uint uScrHeight, uint uScrDepthBits, bool isWindowed, ref uint piD3DModeInfo, ref uint piD3DDevInfo, ref uint piD3DAdapterInfo)
		{
		...
		public bool Build(IDirect3D8 rkD3D, PFNCONFIRMDEVICE pfnConfirmDevice)
		{
			m_uD3DAdapterInfoCount = 0;

			uint uTotalAdapterCount = rkD3D.GetAdapterCount();
			uTotalAdapterCount = Math.Min(uTotalAdapterCount, D3DADAPTERINFO_NUM);
		...
		public D3D_CAdapterInfo GetD3DAdapterInfop(uint iD3DAdapterInfo)
		{
		...
		public D3D_SModeInfo GetD3DModeInfop(uint iD3DAdapterInfo, uint iD3DDevInfo, uint iD3D_SModeInfo)
		{
		...
		public void GetString(string pstEnumList)
		{
			...
			{
				string szText = new string(new char[1024 + 1]);
				_snprintf(szText, sizeof(char), "Adapter %d\r\n", iD3DAdapterInfo);
				pstEnumList.append(szText);
		...
		protected const int D3DADAPTERINFO_NUM = 10;

		protected D3D_CAdapterInfo[] m_akD3DAdapterInfo = Arrays.InitializeWithDefaultInstances<D3D_CAdapterInfo>(D3DADAPTERINFO_NUM);
		protected uint m_uD3DAdapterInfoCount;
}

public class FIsEqualD3DDisplayMode
{
	public FIsEqualD3DDisplayMode(D3DDISPLAYMODE pkD3DDMChk)
	{
		m_pkD3DDMChk = pkD3DDMChk;
	}
	public bool functorMethod(D3DDISPLAYMODE rkD3DDMTest)
	{
	...
	public D3DDISPLAYMODE m_pkD3DDMChk;
}
private delegate bool pfnConfirmDeviceDelegate(D3DCAPS8 rkD3DCaps, uint uBehavior, D3DFORMAT eD3DFmt);
