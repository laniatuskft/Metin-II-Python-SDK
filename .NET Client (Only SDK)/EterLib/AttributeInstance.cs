...
public class CAttributeInstance
{
		public CAttributeInstance()
		{
		}
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const char * GetDataFileName() const
		public string GetDataFileName()
		{
			return m_roAttributeData.GetFileName();
		}

		public void SetObjectPointer(CAttributeData pAttributeData)
		{
			Clear();
			m_roAttributeData.SetPointer(pAttributeData);
		}

		public void RefreshObject(in D3DXMATRIX c_rmatGlobal)
		{
			Debug.Assert(!m_roAttributeData.IsNull());

			m_matGlobal = c_rmatGlobal;

			m_fHeightRadius = m_roAttributeData.GetMaximizeRadius();

			uint dwHeightDataCount = m_roAttributeData.GetHeightDataCount();
			m_v3HeightDataVector.clear();
			m_v3HeightDataVector.resize(dwHeightDataCount);
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < dwHeightDataCount; ++i)
			{
				SHeightData c_pHeightData;
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: CAttributeData * GetObjectPointer() const
		public CAttributeData GetObjectPointer()
		{
...
//Original Metin2 CPlus Line: x = v+t *dir;
					x.CopyFrom(v + t * dir);
		...
		public bool IsInHeight(float fx, float fy)
		{
		...
		public bool GetHeight(float fx, float fy, ref float pfHeight)
		{
			if (IsEmpty())
			{
				return false;
			...
			for (uint j = 0; j < m_v3HeightDataVector[i].size(); j += 3)
			{
				...
				if (IsInTriangle2D(c_rv3Vertex0.x, c_rv3Vertex0.y, c_rv3Vertex1.x, c_rv3Vertex1.y, c_rv3Vertex2.x, c_rv3Vertex2.y, fx, fy))
				{
					...
					D3DXVECTOR3 v3Cross = new D3DXVECTOR3();
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool IsHeightData() const;
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool IsHeightData();

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void SetGlobalMatrix(in D3DXMATRIX c_rmatGlobal);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void SetGlobalPosition(in D3DXVECTOR3 c_rv3Position);

		protected float m_fCollisionRadius;
		protected float m_fHeightRadius;
		protected D3DXMATRIX m_matGlobal = new D3DXMATRIX();
		protected List< List<D3DXVECTOR3>> m_v3HeightDataVector = new List< List<D3DXVECTOR3>>();
		protected CRef<CAttributeData> m_roAttributeData = new CRef<CAttributeData>();
		...
		public static CAttributeInstance New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CAttributeInstance pkInst)
		{
			ms_kPool.Free(pkInst);
		}

		public static CDynamicPool<CAttributeInstance> ms_kPool = new CDynamicPool<CAttributeInstance>();
}