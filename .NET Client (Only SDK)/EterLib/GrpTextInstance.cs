...
using Service.MSP.LaniaControlServer;
...
// Laniatus Games Studio Inc. |: First class, CGraphic
public class CGraphicTextInstance
{
		...
		public static void Hyperlink_UpdateMousePos(int x, int y)
		{
			gs_mx = x;
			gs_my = y;
			gs_hyperlinkText = "";
		...
		public static int Hyperlink_GetText(ref string buf, int len)
		{
		...
		public void Destroy()
		{
			...
			{
				foreach (SEmoji itor in m_emojiVector)
				{
					SEmoji rEmo = itor;
					if (rEmo.pInstance)
					{
		...
		public void Update()
		{
			...
			if (m_roText.IsNull())
			{
				Tracef("CGraphicTextInstance::Update - ??????????????????????? \n");
				return;
			}
			...
			SCharacterInfomation pSpaceInfo = pFontTexture.GetCharacterInfomation(dataCodePage, ' ');

			int spaceHeight = pSpaceInfo != null ? pSpaceInfo.height : 12;
...
//Original Metin2 CPlus Line: >>>>>>>>> const char* begin = m_stText.c_str();
			//CONST CHAR// removed
			char begin = m_stText.c_str();
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char* end = begin + m_stText.length();
			char end = begin + m_stText.length();

			int wTextMax = (end - begin) * 2;
			char[] wText = (string)_alloca(sizeof(char) * wTextMax);

			uint dwColor = m_dwTextColor;

			while (begin < end)
			{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: const char * token = FindToken(begin, end);
				char token = FindToken(begin, end);

				int wTextLen = Ymir_MultiByteToWideChar(dataCodePage, 0, begin, token - begin, wText, wTextMax);

				if (m_isSecret)
				{
					for (int LaniatusDefVariables = 0; LaniatusDefVariables < wTextLen; ++i)
					{
				...
				else
				{
					if (defCodePage == CP_ARABIC)
					{
						...
						char[] wArabicText = (string)_alloca(sizeof(char) * wTextLen);
						...
						int len;
						int hyperlinkStep = 0;
						SHyperlink kHyperlink = new SHyperlink();
						string hyperlinkBuffer = "";
						int no_hyperlink = 0;
						...
						for (i = wArabicTextLen - 1 ; LaniatusDefVariables >= 0; --i)
						{
							...
							if (isEnglish)
							{
								...
								{
									for (int e = LaniatusDefVariables + 1; e <= nEnglishBase;)
									{
										...
										else
										{
											...
											{
												dwColor = htoi(hyperlinkBuffer, 8);
											}
					...
					else
					{
						...
						for (int LaniatusDefVariables = 0; LaniatusDefVariables < wTextLen;)
						{
							...
							else if (ret == TEXT_TAG_EMOJI_END)
							{
								...
								string retBuf = new string(new char[1024]);
								int retLen = Ymir_WideCharToMultiByte(GetDefaultCodePage(), 0, emojiBuffer, emojiBuffer.Length, retBuf, sizeof(char) - 1, null, null);
								retBuf = StringFunctions.ChangeCharacter(retBuf, retLen, '\0');

								string szPath = new string(new char[255]);
								snprintf(szPath, sizeof(char), "icon/%s.tga", retBuf);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
									...
									for (int LaniatusDefVariables = 0; LaniatusDefVariables < pImage.GetWidth() / (pSpaceInfo.width - 1); ++i)
									{
		...
		public void Render(RECT pClipRect = null)
		{
			if (!m_isUpdate)
			{
				return;
			...
			SortedDictionary<CGraphicImageTexture, List<SPDTVertexRaw>> verticesMap = new SortedDictionary<CGraphicImageTexture, List<SPDTVertexRaw>>();

			{
				...
				if (m_isOutline)
				{
					...
					List<SCharacterInfomation>.Enumerator i;
					for (i = m_pCharInfoVector.begin(); i.MoveNext();)
					{
						pCurCharInfo = i.Current;
						...
						var tex = pFontTexture.GetTexture(pCurCharInfo.index);
						var batchVertices = verticesMap[tex];
				...
				for (int LaniatusDefVariables = 0; LaniatusDefVariables < m_pCharInfoVector.size(); ++i)
				{
					...
					fFontWidth = (float)pCurCharInfo.width;
					fFontHeight = (float)pCurCharInfo.height;
					fFontMaxHeight = Math.Max(fFontHeight, pCurCharInfo.height);
					fFontAdvance = (float)pCurCharInfo.advance;
			...
			{
				(CStateManager.Instance()).SetTexture(0, p.first.GetD3DTexture());

				for (var f = p.second.begin(), l = p.second.end(); f != l;)
				{
					var batchCount = Math.Min<uint>(LARGE_PDT_VERTEX_BUFFER_SIZE, l - f);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
					...
					{
						(CStateManager.Instance()).DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, batchCount - 2);
					}
			...
			if (m_isCursor)
			{
				...
				if (ulbegin < ulend)
				{
					...
					(CStateManager.Instance()).DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, 4, 2, c_FillRectIndices, D3DFMT_INDEX16, vertices, sizeof(SPDTVertex));
				}
			}
			...
			if (m_hyperlinkVector.size() != 0)
			{
				...
				{
					List<SHyperlink>.Enumerator it = m_hyperlinkVector.begin();

					while (it.MoveNext())
					{
						SHyperlink link = it.Current++;
...
//Original Metin2 CPlus Line: SEmoji& rEmo = itor;
					SEmoji rEmo = itor;
					if (rEmo.pInstance)
					{
		...
		public void SetOutLineColor(float r, float g, float b, float a = 1.0f)
		{
			m_dwOutLineColor = D3DXCOLOR(r, g, b, a);
		}

		public void SetHorizonalAlign(int hAlign)
		{
			m_hAlign = hAlign;
		}
		...
		public void SetTextPointer(CGraphicText pText)
		{
			m_roText = pText;
		}

		public void SetValueString(in string c_stValue)
		{
		...
		public void SetValue(string c_szText, size_t len = -1)
		{
		...
		public void SetPosition(float fx, float fy, float fz = 0.0f)
		{
			m_v3Position.x = fx;
			m_v3Position.y = fy;
			m_v3Position.z = fz;
		...
		public void SetFeather(bool Value)
		{
			if (Value)
			{
		...
		public void GetTextSize(ref int pRetWidth, ref int pRetHeight)
		{
			pRetWidth = m_textWidth;
			pRetHeight = m_textHeight;
		}

		public string GetValueStringReference()
		{
			return m_stText;
		}

		public ushort GetTextLineCount()
		{
			SCharacterInfomation pCurCharInfo;
			List<SCharacterInfomation>.Enumerator itor;

			float fx = 0.0f;
			ushort wLineCount = 1;
			for (itor = m_pCharInfoVector.begin(); itor.MoveNext();)
			{
				pCurCharInfo = itor.Current;

				float fFontWidth = pCurCharInfo.width;
				float fFontAdvance = pCurCharInfo.advance;
		...
		public int PixelPositionToCharacterPosition(int iPixelPosition)
		{
			int icurPosition = 0;
		...
		protected int __DrawCharacter(CGraphicFontTexture pFontTexture, ushort codePage, char text, uint dwColor)
		{
			SCharacterInfomation pInsCharInfo = pFontTexture.GetCharacterInfomation(codePage, text);

			if (pInsCharInfo != null)
			{
				m_dwColorInfoVector.push_back(dwColor);
				m_pCharInfoVector.push_back(pInsCharInfo);

				m_textWidth += pInsCharInfo.advance;
				m_textHeight = Math.Max(pInsCharInfo.height, m_textHeight);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'x', so pointers on this parameter are left unchanged:
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'y', so pointers on this parameter are left unchanged:
		protected void __GetTextPos(uint index, float * x, float * y)
		{
			index = Math.Min(index, m_pCharInfoVector.size());

			float sx = 0F;
			float sy = 0F;
			float fFontMaxHeight = 0F;
...
//		int __GetTextTag(string src, int maxLen, ref int tagLen, string extraInfo);

		protected class SHyperlink
		{
			public short sx;
			public short ex;
			public string text = "";
		...
		protected uint m_dwTextColor;
		protected uint m_dwOutLineColor;

		protected ushort m_textWidth;
		protected ushort m_textHeight;

		protected byte m_hAlign;
		protected byte m_vAlign;

		protected ushort m_iMax;
		protected float m_fLimitWidth;

		protected bool m_isCursor;
		protected bool m_isSecret;
		protected bool m_isMultiLine;

		protected bool m_isOutline;
		protected float m_fFontFeather;

		protected string m_stText = "";
		protected _D3DVECTOR m_v3Position = new _D3DVECTOR();

		private bool m_isUpdate;
		private bool m_isUpdateFontTexture;

		private CGraphicText.TRef m_roText = new CGraphicText.TRef();
		private List<SCharacterInfomation> m_pCharInfoVector = new List<SCharacterInfomation>();
		private List<uint> m_dwColorInfoVector = new List<uint>();
		private List<SHyperlink> m_hyperlinkVector = new List<SHyperlink>();
		private List<SEmoji> m_emojiVector = new List<SEmoji>();
		...
		public static CGraphicTextInstance New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CGraphicTextInstance pkInst)
		{
			pkInst.Destroy();
			ms_kPool.Free(pkInst);
		}

		public static CDynamicPool<CGraphicTextInstance> ms_kPool = new CDynamicPool<CGraphicTextInstance>();
}