...
public class CGraphicSubImage : CGraphicImage
{

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static TType Type_s_type = StringToType("CGraphicSubImage");

		public static CGraphicSubImage.TType Type()
		{
...
//			static TType s_type = StringToType("CGraphicSubImage");
		...
		public static const string m_SearchPath = "t:/Ymir Work/UI/";

		public CGraphicSubImage(string c_szFileName) : base(c_szFileName)
		{
		}
		...
		public bool SetImageFileName(string c_szFileName)
		{
		...
		public void SetRectReference(in RECT c_rRect)
		{
			m_rect = c_rRect;
		}

		public static void SetSearchPath(string c_szFileName)
		{
		...
		protected bool OnLoad(int iSize, object c_pvBuf)
		{
			...
			SortedDictionary<string, string> stTokenMap = new SortedDictionary<string, string>();
			...
			string c_rstTitle = stTokenMap["title"];
			string c_rstVersion = stTokenMap["version"];
			string c_rstImage = stTokenMap["image"];
			string c_rstLeft = stTokenMap["left"];
			string c_rstTop = stTokenMap["top"];
			string c_rstRight = stTokenMap["right"];
			string c_rstBottom = stTokenMap["bottom"];
			...
			if ("2.0" == c_rstVersion)
			{
				string c_rstSubFileName = GetFileNameString();
				int nPos = c_rstSubFileName.LastIndexOfAny((Convert.ToString('\\')).ToCharArray(), -1);
				if (nPos >= 0)
				{
					nPos++;
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(szFileName, c_rstSubFileName, nPos);
					szFileName = StringFunctions.ChangeCharacter(szFileName, nPos, '\0');
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(szFileName.Substring(nPos), c_rstImage, c_rstImage.Length);
					szFileName = StringFunctions.ChangeCharacter(szFileName, nPos + c_rstImage.Length, '\0');
				}
				else
				{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(szFileName, c_rstImage, c_rstImage.Length);
				}
			}
			else
			{
				_snprintf(szFileName, sizeof(char), "%s%s", m_SearchPath, c_rstImage);
			}

			SetImageFileName(szFileName);

			SetRectPosition(atoi(c_rstLeft), atoi(c_rstTop), atoi(c_rstRight), atoi(c_rstBottom));
...
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected CGraphicImage.TRef m_roImage = new CGraphicImage.TRef();
}
