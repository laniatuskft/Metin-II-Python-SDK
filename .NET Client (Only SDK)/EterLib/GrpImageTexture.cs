...
public class CGraphicImageTexture : CGraphicTexture
{
		public CGraphicImageTexture()
		{
			Initialize();
		}
		...
		public bool Create(uint width, uint height, D3DFORMAT d3dFmt, uint dwFilter = (uint)(3 << 0))
		{
		...
		{
			Debug.Assert(ms_lpd3dDevice != null);
			Debug.Assert(m_lpd3dTexture == null);
		...
		public void CreateFromTexturePointer(CGraphicTexture c_pSrcTexture)
		{
			if (m_lpd3dTexture)
			{
		...
		public bool CreateFromDiskFile(string c_szFileName, D3DFORMAT d3dFmt, uint dwFilter = (uint)(3 << 0))
		{
			Destroy();
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private CDXTCImage CreateFromMemoryFile_image = new CDXTCImage();

		public bool CreateFromMemoryFile(uint bufSize, object c_pvBuf, D3DFORMAT d3dFmt, uint dwFilter = (uint)(3 << 0))
		{
			Debug.Assert(ms_lpd3dDevice != null);
			Debug.Assert(m_lpd3dTexture == null);
...
//			static CDXTCImage image;
			...
			if (CreateFromMemoryFile_image.LoadHeaderFromMemory((byte)c_pvBuf, bufSize))
			{
				return (CreateDDSTexture(CreateFromMemoryFile_image, (byte) c_pvBuf));
			}
			else
			{
				_D3DXIMAGE_INFO imageInfo = new _D3DXIMAGE_INFO();
				if (FAILED(D3DXCreateTextureFromFileInMemoryEx(ms_lpd3dDevice, c_pvBuf, bufSize, uint.MaxValue, uint.MaxValue, uint.MaxValue, 0, d3dFmt, D3DPOOL_MANAGED, dwFilter, dwFilter, 0xffff00ff, imageInfo, null, m_lpd3dTexture)))
				{
...
//				extern bool GRAPHICS_CAPS_HALF_SIZE_IMAGE;
				...
				{
				if (uTexBias != 0 || format != imageInfo.Format)
				{
					IDirect3DTexture8 pkTexSrc = m_lpd3dTexture;
					IDirect3DTexture8 pkTexDst;


					if (SUCCEEDED(D3DXCreateTexture(ms_lpd3dDevice, imageInfo.Width >> (int)uTexBias, imageInfo.Height >> (int)uTexBias, imageInfo.MipLevels, 0, format, D3DPOOL_MANAGED, pkTexDst)))
					{
						m_lpd3dTexture = pkTexDst;

						for (int LaniatusDefVariables = 0; LaniatusDefVariables < imageInfo.MipLevels; ++i)
						{
							...
							if (SUCCEEDED(pkTexSrc.GetSurfaceLevel(i, ppsSrc)))
							{
								...
								{
									D3DXLoadSurfaceFromSurface(ppsDst, null, null, ppsSrc, null, null, (3 << 0), 0);
									ppsDst.Release();
								}
								ppsSrc.Release();
							}
		...
		public bool CreateDDSTexture(CDXTCImage image, byte c_pbBuf)
		{
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < mipmapCount; ++i)
			{
...
//				extern bool GRAPHICS_CAPS_HALF_SIZE_IMAGE;
				...
				for (int LaniatusDefVariables = 0; LaniatusDefVariables < mipmapCount; ++i)
				{
					...
					if (SUCCEEDED(pkTexSrc.GetSurfaceLevel(i, ppsSrc)))
					{
						if (SUCCEEDED(pkTexDst.GetSurfaceLevel(i, ppsDst)))
						{
							D3DXLoadSurfaceFromSurface(ppsDst, null, null, ppsSrc, null, null, (1 << 0), 0);
							ppsDst.Release();
						}
						ppsSrc.Release();
					}
		...
		public bool Lock(ref int pRetPitch, object[] ppRetPixels, int level = 0)
		{
			D3DLOCKED_RECT lockedRect = new D3DLOCKED_RECT();
			...
			ppRetPixels[0] = (object)lockedRect.pBits;
			return true;
		}

		public void Unlock(int level = 0)
		{
			Debug.Assert(m_lpd3dTexture != null);
		...
		protected D3DFORMAT m_d3dFmt = new D3DFORMAT();
		protected uint m_dwFilter;
