...
public class SHeightData
{
	public string szName = new string(new char[32 + 1]);
	public List<D3DXVECTOR3> v3VertexVector = new List<D3DXVECTOR3>();
}


public class CAttributeData : CResource
{

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static TType Type_s_type = StringToType("CAttributeData");

		public static CAttributeData.TType Type()
		{
...
//			static TType s_type = StringToType("CAttributeData");
		...
		public CAttributeData(string c_szFileName) : base(c_szFileName)
		{
...
//Original Metin2 CPlus Line: const System.Collections.Generic.List<CStaticCollisionData> & GetCollisionDataVector() const
		public List<CStaticCollisionData> GetCollisionDataVector()
		{
			return m_StaticCollisionDataVector;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const System.Collections.Generic.List<SHeightData> & GetHeightDataVector() const
		public List<SHeightData> GetHeightDataVector()
		{
			return m_HeightDataVector;
		}

		public size_t AddCollisionData(in CStaticCollisionData data)
		{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: uint GetHeightDataCount() const
		public uint GetHeightDataCount()
		{
			return (uint)m_HeightDataVector.size();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool GetHeightDataPointer(uint dwIndex, const SHeightData ** c_ppHeightData) const
		public bool GetHeightDataPointer(uint dwIndex, SHeightData[] c_ppHeightData)
		{
			...
			c_ppHeightData[0] = &m_HeightDataVector[dwIndex];

			return true;
		}

		public float GetMaximizeRadius()
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			...
			string szHeader = new string(new char[c_iAttributeDataFileHeaderLength + 1]);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			uint dwCollisionDataCount;
			uint dwHeightDataCount;
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(dwCollisionDataCount, c_pbBuf, sizeof(uint));
			c_pbBuf += sizeof(uint);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(dwHeightDataCount, c_pbBuf, sizeof(uint));
			c_pbBuf += sizeof(uint);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			for (uint j = 0; j < dwHeightDataCount; ++j)
			{
				SHeightData rHeightData = m_HeightDataVector[j];
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(dwPrimitiveCount, c_pbBuf, sizeof(uint));
				c_pbBuf += sizeof(uint);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				...
				for (uint k = 0; k < rHeightData.v3VertexVector.size(); ++k)
				{
					m_fMaximizeRadius = fMAX(m_fMaximizeRadius, Math.Abs(rHeightData.v3VertexVector[k].x) + 50.0f);
					m_fMaximizeRadius = fMAX(m_fMaximizeRadius, Math.Abs(rHeightData.v3VertexVector[k].y) + 50.0f);
					m_fMaximizeRadius = fMAX(m_fMaximizeRadius, Math.Abs(rHeightData.v3VertexVector[k].z) + 50.0f);
				}
			}
...
//Original Metin2 CPlus Line: bool OnIsEmpty() const
		...
		protected float m_fMaximizeRadius;
		protected List<CStaticCollisionData> m_StaticCollisionDataVector = new List<CStaticCollisionData>();
		protected List<SHeightData> m_HeightDataVector = new List<SHeightData>();
}