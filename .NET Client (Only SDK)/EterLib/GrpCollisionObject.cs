...
public class CGraphicCollisionObject : CGraphicBase
{
		public CGraphicCollisionObject()
		{
		}
		...
		protected bool IntersectTriangle(in _D3DVECTOR c_orig, in _D3DVECTOR c_dir, in _D3DVECTOR c_v0, in _D3DVECTOR c_v1, in _D3DVECTOR c_v2, ref float pu, ref float pv, ref float pt)
		{
			_D3DVECTOR edge1 = c_v1 - c_v0;
			_D3DVECTOR edge2 = c_v2 - c_v0;
			_D3DVECTOR pvec = new _D3DVECTOR();
			D3DXVec3Cross(pvec, c_dir, edge2);

			float det = D3DXVec3Dot(edge1, pvec);
			_D3DVECTOR tvec = new _D3DVECTOR();
...
//Original Metin2 CPlus Line: tvec = c_orig - c_v0;
				tvec.CopyFrom(c_orig - c_v0);
			}
			else
			{
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: tvec = c_v0 - c_orig;
				tvec.CopyFrom(c_v0 - c_orig);
				det = -det;
			}
			...
			_D3DVECTOR qvec = new _D3DVECTOR();
			...
			_D3DVECTOR spot = edge1 * u + edge2 * v;
			spot += c_v0;
		...
		protected bool IntersectBoundBox(_D3DMATRIX c_pmatWorld, in SBoundBox c_rboundBox, ref float pu, ref float pv, ref float pt)
		{
		...
		protected bool IntersectCube(_D3DMATRIX c_pmatWorld, float sx, float sy, float sz, float ex, float ey, float ez, _D3DVECTOR RayOriginal, _D3DVECTOR RayDirection, ref float pu, ref float pv, ref float pt)
		{
			...
			return IntersectIndexedMesh(c_pmatWorld, posVertices, sizeof(_D3DVECTOR), 8, c_awFillCubeIndices, 36, RayOriginal, RayDirection, pu, pv, pt);
		}

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private IntersectIndexedMesh__D3DVECTOR[] s_v3PositionArray = Arrays.InitializeWithDefaultInstances<IntersectIndexedMesh__D3DVECTOR>(c_iLimitVertexCount);
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private uint IntersectIndexedMesh_s_dwPositionCount;

		protected bool IntersectIndexedMesh(_D3DMATRIX c_pmatWorld, object vertices, int step, int vtxCount, object indices, int idxCount, _D3DVECTOR RayOriginal, _D3DVECTOR RayDirection, ref float pu, ref float pv, ref float pt)
		{
...
//			static struct _D3DVECTOR s_v3PositionArray[c_iLimitVertexCount];
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static uint s_dwPositionCount;
			...
			IntersectIndexedMesh_s_dwPositionCount = 0;
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * pcurVtx = (string)vertices;

			while ((vtxCount--) != 0)
			{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: float* pos = (float*)pcurVtx;
				float pos = (float)pcurVtx;

				D3DXVec3TransformCoord(s_v3PositionArray[IntersectIndexedMesh_s_dwPositionCount++], (IntersectIndexedMesh__D3DVECTOR)pos, c_pmatWorld);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		...
		protected bool IntersectMesh(_D3DMATRIX c_pmatWorld, object vertices, uint dwStep, uint dwvtxCount, _D3DVECTOR RayOriginal, _D3DVECTOR RayDirection, ref float pu, ref float pv, ref float pt)
		{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * pcurVtx = (string) vertices;

			_D3DVECTOR[] v3Vertex = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(3);

			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < dwvtxCount; LaniatusDefVariables += 3)
			{
				D3DXVec3TransformCoord(v3Vertex[0], (_D3DVECTOR)pcurVtx, c_pmatWorld);
				pcurVtx += dwStep;

				D3DXVec3TransformCoord(v3Vertex[1], (_D3DVECTOR)pcurVtx, c_pmatWorld);
				pcurVtx += dwStep;

				D3DXVec3TransformCoord(v3Vertex[2], (_D3DVECTOR)pcurVtx, c_pmatWorld);
				pcurVtx += dwStep;
		...
		protected bool IntersectSphere(in _D3DVECTOR c_rv3Position, float fRadius, in _D3DVECTOR c_rv3RayOriginal, in _D3DVECTOR c_rv3RayDirection)
		{
			_D3DVECTOR v3RayOriginal = c_rv3RayOriginal - c_rv3Position;
		...
		protected bool IntersectCylinder(in _D3DVECTOR c_rv3Position, float fRadius, float fHeight, in _D3DVECTOR c_rv3RayOriginal, in _D3DVECTOR c_rv3RayDirection)
		{
			_D3DVECTOR v3RayOriginal = c_rv3RayOriginal - c_rv3Position;
		...
		protected bool IntersectSphere(in _D3DVECTOR c_rv3Position, float fRadius)
		{
			return IntersectSphere(c_rv3Position, fRadius, ms_vtPickRayOrig, ms_vtPickRayDir);
		}

		protected bool IntersectCylinder(in _D3DVECTOR c_rv3Position, float fRadius, float fHeight)
		{
			return IntersectCylinder(c_rv3Position, fRadius, fHeight, ms_vtPickRayOrig, ms_vtPickRayDir);
		}
}