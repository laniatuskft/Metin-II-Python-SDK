...
public class CDibBar
{
		...
		public bool Create(IntPtr hdc, uint dwWidth, uint dwHeight)
		{
		...
		{
			RECT rect = new RECT(0, 0, m_dwWidth, m_dwHeight);

			List<CBlockTexture >.Enumerator itor = m_kVec_pkBlockTexture.begin();
			while (itor.MoveNext())
			{
				CBlockTexture pTexture = itor.Current;
...
//Original Metin2 CPlus Line: uint * pdwBuf = (uint *)m_dib.GetPointer();
			uint pdwBuf = (uint)m_dib.GetPointer();
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		protected uint __NearTextureSize(uint dwSize)
		{
		...
		protected void __DivideTextureSize(uint dwSize, uint dwMax, ref uint pdwxStep, ref uint pdwxCount, ref uint pdwxRest)
		{
			if (dwSize < dwMax)
			{
				pdwxStep = dwMax;
		...
		protected CBlockTexture __BuildTextureBlock(uint dwxPos, uint dwyPos, uint dwImageWidth, uint dwImageHeight, uint dwTextureWidth, uint dwTextureHeight)
		{
			...
			RECT posRect = new RECT(dwxPos, dwyPos, dwxPos + dwImageWidth, dwyPos + dwImageHeight);
		...
		protected void __BuildTextureBlockList(uint dwWidth, uint dwHeight, uint dwMax = 256)
		{
			uint dwxStep;
			uint dwyStep;
			uint dwxCount;
			...
			uint dwxTexRest = __NearTextureSize(dwxRest);
			uint dwyTexRest = __NearTextureSize(dwyRest);
		...
		protected List<CBlockTexture > m_kVec_pkBlockTexture = new List<CBlockTexture >();
