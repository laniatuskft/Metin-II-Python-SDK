...
public class CNetworkStream
{
		public CNetworkStream()
		{
			...
			m_kVec_bSequenceTable.resize(DefineConstants.SEQUENCE_TABLE_SIZE);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_kVec_bSequenceTable[0], s_bSequenceTable, sizeof(byte) * DefineConstants.SEQUENCE_TABLE_SIZE);
		}
		...
		public virtual void Dispose()
		{
			...
			if (m_sendTEABuf)
			{
				Arrays.DeleteArray(m_sendTEABuf);
				m_sendTEABuf = null;
			}

			if (m_recvTEABuf)
			{
				Arrays.DeleteArray(m_recvTEABuf);
				m_recvTEABuf = null;
			}

			if (m_recvBuf)
			{
				Arrays.DeleteArray(m_recvBuf);
				m_recvBuf = null;
			}

			if (m_sendBuf)
			{
				Arrays.DeleteArray(m_sendBuf);
				m_sendBuf = null;
			}
		}

		public void SetRecvBufferSize(int recvBufSize)
		{
			if (m_recvBuf)
			{
		...
		public void SetSendBufferSize(int sendBufSize)
		{
			if (m_sendBuf)
			{
...
#if ! _IMPROVED_PACKET_ENCRYPTION_
#if ! _IMPROVED_PACKET_ENCRYPTION_
		public void SetSecurityMode(bool isSecurityMode, string c_szTeaKey)
		{
			m_isSecurityMode = isSecurityMode;
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_szEncryptKey, c_szTeaKey, DefineConstants.TEA_KEY_LENGTH);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_szDecryptKey, c_szTeaKey, DefineConstants.TEA_KEY_LENGTH);
		}
#endif

		public void SetSecurityMode(bool isSecurityMode, string c_szTeaEncryptKey, string c_szTeaDecryptKey)
		{
			m_isSecurityMode = isSecurityMode;
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_szEncryptKey, c_szTeaEncryptKey, DefineConstants.TEA_KEY_LENGTH);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_szDecryptKey, c_szTeaDecryptKey, DefineConstants.TEA_KEY_LENGTH);
		}

#endif
		public bool IsSecurityMode()
		{
#if _IMPROVED_PACKET_ENCRYPTION_
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public void Process()
		{
			...
			fd_set fdsRecv = new fd_set();
			fd_set fdsSend = new fd_set();
		...
		public bool Connect(in CNetworkAddress c_rkNetAddr, int limitSec = 3)
		{
			Clear();
...
//# Laniatus Games Studio Inc. | TODO TASK: There is no equivalent to 'reinterpret_cast' in C#:
		...
		public bool Connect(string c_szAddr, int port, int limitSec = 3)
		{
			CNetworkAddress kNetAddr = new CNetworkAddress();
			kNetAddr.Set(c_szAddr, port);

			return Connect(kNetAddr);
		}

		public bool Connect(uint dwAddr, int port, int limitSec = 3)
		{
			string szAddr = new string(new char[256]);
			{
				byte[] ip = new byte[4];
				ip[0] = (byte)(dwAddr & 0xff);
				dwAddr >>= 8;
				ip[1] = (byte)(dwAddr & 0xff);
				dwAddr >>= 8;
				ip[2] = (byte)(dwAddr & 0xff);
				dwAddr >>= 8;
				ip[3] = (byte)(dwAddr & 0xff);
				dwAddr >>= 8;
		...
		public bool Peek(int size, ref string pDestBuf)
		{
			if (GetRecvBufferSize() < size)
			{
				return false;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		public bool Recv(int size, ref string pDestBuf)
		{
		...
		public bool Send(int size, string pSrcBuf)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		public bool Peek(int len, object pDestBuf)
		{
			return Peek(len, (string)pDestBuf);
		}

		public bool Recv(int len, object pDestBuf)
		{
			return Recv(len, (string)pDestBuf);
		}

		public bool Send(int len, object pSrcBuf)
		{
			return Send(len, (string)pSrcBuf);
		}

		public bool SendFlush(int len, object pSrcBuf)
		{
			if (!Send(len, pSrcBuf))
			{
				return false;
		...
		public void SetPacketSequenceMode(bool isOn)
		{
			m_bUseSequence = isOn;
		}
		...
		public bool SendSequence()
		{
			...
			if (m_iSequence == DefineConstants.SEQUENCE_TABLE_SIZE)
			{
				m_iSequence = 0;
			}
		...
		protected bool __SendInternalBuffer()
		{
#if _IMPROVED_PACKET_ENCRYPTION_
			...
			if (IsSecurityMode())
			{
				...
				m_sendTEABufInputPos += tea_encrypt((uint)(m_sendTEABuf + m_sendTEABufInputPos), (uint)(m_sendBuf + m_sendBufOutputPos), (uint) m_szEncryptKey, encodeSize);
				...
				if (m_sendTEABufInputPos > 0)
				{
					...
					{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memmove' has no equivalent in C#:
		...
		protected bool __RecvInternalBuffer()
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memmove' has no equivalent in C#:
			...
			if (IsSecurityMode())
			{
				int restSize = Math.Min(m_recvTEABufSize - m_recvTEABufInputPos, m_recvBufSize - m_recvBufInputPos);
				...
				if (restSize > 0)
				{
					...
					if (decodeSize >= 8)
					{
						...
						int decodeDstSize = tea_decrypt((uint)(m_recvBuf + m_recvBufInputPos), (uint) m_recvTEABuf, (uint) m_szDecryptKey, decodeSize);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memmove' has no equivalent in C#:
		...
		protected void __PopSendBuffer()
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memmove' has no equivalent in C#:
		...
		protected int __GetSendBufferSize()
		{
...
#if _IMPROVED_PACKET_ENCRYPTION_
#if _IMPROVED_PACKET_ENCRYPTION_
		protected size_t Prepare(object buffer, size_t length)
		{
			return m_cipher.Prepare(buffer, length);
		}

		protected bool Activate(size_t agreed_length, object buffer, size_t length)
		{
			return m_cipher.Activate(true, agreed_length, buffer, length);
		}

		protected void ActivateCipher()
		{
		...
		private time_t m_connectLimitTime = new time_t();

		private string m_recvTEABuf;
		private int m_recvTEABufInputPos;
		private int m_recvTEABufSize;

		private string m_recvBuf;
		private int m_recvBufSize;
		private int m_recvBufInputPos;
		private int m_recvBufOutputPos;

		private string m_sendBuf;
		private int m_sendBufSize;
		private int m_sendBufInputPos;
		private int m_sendBufOutputPos;

		private string m_sendTEABuf;
		private int m_sendTEABufSize;
		private int m_sendTEABufInputPos;

		private bool m_isOnline;

#if _IMPROVED_PACKET_ENCRYPTION_
		private Cipher m_cipher = new Cipher();
#else
		private bool m_isSecurityMode;
		private string m_szEncryptKey = new string(new char[DefineConstants.TEA_KEY_LENGTH]);
		private string m_szDecryptKey = new string(new char[DefineConstants.TEA_KEY_LENGTH]);
#endif

		private SOCKET m_sock = new SOCKET();

		private CNetworkAddress m_addr = new CNetworkAddress();

		private uint m_iSequence;
		private bool m_bUseSequence;
		private List<byte> m_kVec_bSequenceTable = new List<byte>();
}
