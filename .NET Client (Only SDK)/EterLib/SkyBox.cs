...
public class SColor
{
	public SColor(float _r = 0.0f, float _g = 0.0f, float _b = 0.0f, float _a = 0.0f)
	{
		this.r = _r;
		this.g = _g;
		this.b = _b;
...
public class TGradientColor
{
	public SColor m_FirstColor = new SColor();
	public SColor m_SecondColor = new SColor();
}


public class CSkyObjectQuad
{
	public CSkyObjectQuad()
	{
		m_Indices[0] = 0;
		m_Indices[1] = 2;
		m_Indices[2] = 1;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
	...
	public void Clear(in byte c_rucNumVertex, in float c_rfRed, in float c_rfGreen, in float c_rfBlue, in float c_rfAlpha)
	{
		if (c_rucNumVertex > 3)
		{
			return;
	...
	public void SetSrcColor(in byte c_rucNumVertex, in float c_rfRed, in float c_rfGreen, in float c_rfBlue, in float c_rfAlpha)
	{
		if (c_rucNumVertex > 3)
		{
			return;
	...
	public void SetTransition(in byte c_rucNumVertex, in float c_rfRed, in float c_rfGreen, in float c_rfBlue, in float c_rfAlpha, uint dwDuration)
	{
		if (c_rucNumVertex > 3)
		{
			return;
	...
	public void SetVertex(in byte c_rucNumVertex, in SPDTVertex c_rPDTVertex)
	{
		if (c_rucNumVertex > 3)
		{
			return;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		memcpy(m_Vertex[m_Indices[c_rucNumVertex]], c_rPDTVertex, sizeof(SPDTVertex));
	}
	...
	public void Render()
	{
		...
		{
			(CStateManager.Instance()).DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2);
		}
	}

	private SPDTVertex[] m_Vertex = Arrays.InitializeWithDefaultInstances<SPDTVertex>(4);
	private ushort[] m_Indices = new ushort[4];
	private CColorTransitionHelper[] m_Helper = Arrays.InitializeWithDefaultInstances<CColorTransitionHelper>(4);
}

public class CSkyObject : CScreen
{
	public const int SKY_RENDER_MODE_DEFAULT = 0;
	public const int SKY_RENDER_MODE_DIFFUSE = 1;
	public const int SKY_RENDER_MODE_TEXTURE = 2;
	public const int SKY_RENDER_MODE_MODULATE = 3;
	public const int SKY_RENDER_MODE_MODULATE2X = 4;
	public const int SKY_RENDER_MODE_MODULATE4X = 5;

	public CSkyObject()
	{
		this.m_v3Position = new <type missing>(0.0f, 0.0f, 0.0f);
		this.m_fScaleX = 1.0f;
		this.m_fScaleY = 1.0f;
		this.m_fScaleZ = 1.0f;
	...
	public void SetRenderMode(byte ucRenderMode)
	{
	...
	protected CGraphicImageInstance GenerateTexture(string szfilename)
	{
		...
		pImageInstance.SetImagePointer((CGraphicImage)pResource);
		return (pImageInstance);
	}

	protected void DeleteTexture(CGraphicImageInstance pImageInstance)
	{
	...
	protected class TSkyObjectFace
	{
		public void StartTransition()
		{
			for (byte uci = 0; uci < m_SkyObjectQuadVector.size(); ++uci)
			{
		...
		{
			bool bResult = false;
			for (uint dwi = 0; dwi < m_SkyObjectQuadVector.size(); ++dwi)
			{
		...
		public string m_strfacename = "";
		public string m_strFaceTextureFileName = "";
		public TSkyObjectQuadVector m_SkyObjectQuadVector = new TSkyObjectQuadVector();
	}


	protected TSkyObjectFace m_FaceCloud = new TSkyObjectFace();
	protected _D3DMATRIX m_matWorldCloud = new _D3DMATRIX();
	protected _D3DMATRIX m_matTranslationCloud = new _D3DMATRIX();
	protected _D3DMATRIX m_matTextureCloud = new _D3DMATRIX();
	protected _D3DVECTOR m_v3PositionCloud = new _D3DVECTOR();
	protected float m_fCloudScaleX;
	protected float m_fCloudScaleY;
	protected float m_fCloudHeight;
	protected float m_fCloudTextureScaleX;
	protected float m_fCloudTextureScaleY;
	protected float m_fCloudScrollSpeedU;
	protected float m_fCloudScrollSpeedV;
	protected float m_fCloudPositionU;
	protected float m_fCloudPositionV;

	protected uint m_dwlastTime;

	protected SortedDictionary<string, CGraphicImageInstance> m_GraphicImageInstanceMap = new SortedDictionary<string, CGraphicImageInstance>();

	protected _D3DMATRIX m_matWorld = new _D3DMATRIX();
	protected _D3DMATRIX m_matTranslation = new _D3DMATRIX();
	...
	protected byte m_ucRenderMode;

	protected string m_strCurTime = "";
	protected bool m_bTransitionStarted;
	protected bool m_bSkyMatrixUpdated;

	protected CGraphicImageInstance m_CloudAlphaImageInstance = new CGraphicImageInstance();
}

public class CSkyBox : CSkyObject
{
	public CSkyBox()
	{
	...
	public void Render()
	{
		(CStateManager.Instance()).SaveRenderState(D3DRS_ZENABLE, true);
		...
		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_COLOROP, D3DTOP_SELECTARG2);
		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_COLORARG2, D3DTA_DIFFUSE);
		...
		(CStateManager.Instance()).SetTransform(D3DTS_WORLD, m_matWorld);
	...
	public void RenderCloud()
	{
		...
		if (pCloudGraphicImageInstance == null)
		{
			return;
		}
		...
		(CStateManager.Instance()).SaveRenderState(D3DRS_SRCBLEND, D3DBLEND_ONE);
		(CStateManager.Instance()).SaveRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCCOLOR);
		...
		uint dwCurTime = CTimer.Instance().GetCurrentMillisecond();
		...
		(CStateManager.Instance()).SaveTransform(D3DTS_TEXTURE0, m_matTextureCloud);

		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATEINVALPHA_ADDCOLOR);
		...
		_D3DMATRIX matProjCloud = new _D3DMATRIX();
		D3DXMatrixPerspectiveFovRH(matProjCloud, ((float) 3.141592654f) * 0.25f, 1.33333f, 50.0f, 999999.0f);
		(CStateManager.Instance()).SetTransform(D3DTS_WORLD, m_matWorldCloud);
		(CStateManager.Instance()).SaveTransform(D3DTS_PROJECTION, matProjCloud);
		(CStateManager.Instance()).SetTexture(0, pCloudGraphicImageInstance.GetTexturePointer().GetD3DTexture());
		m_FaceCloud.Render();
	...
	public void SetSkyBoxScale(in _D3DVECTOR c_rv3Scale)
	{
	...
	public void SetGradientLevel(byte byUpper, byte byLower)
	{
		m_ucVirticalGradientLevelUpper = byUpper;
		m_ucVirticalGradientLevelLower = byLower;
	}

	public void SetFaceTexture(string c_szFileName, int iFaceIndex)
	{
	...
	public void SetCloudTexture(string c_szFileName)
	{
		TGraphicImageInstanceMap.iterator itor = m_GraphicImageInstanceMap.find(c_szFileName);
		if (m_GraphicImageInstanceMap.end() != itor)
		{
	...
	public void SetCloudScale(in D3DXVECTOR2 c_rv2CloudScale)
	{
	...
	public void SetCloudHeight(float fHeight)
	{
		m_fCloudHeight = fHeight;
	}

	public void SetCloudTextureScale(in D3DXVECTOR2 c_rv2CloudTextureScale)
	{
	...
	public void SetCloudScrollSpeed(in D3DXVECTOR2 c_rv2CloudScrollSpeed)
	{
		m_fCloudScrollSpeedU = c_rv2CloudScrollSpeed.x;
		m_fCloudScrollSpeedV = c_rv2CloudScrollSpeed.y;
	}

	public void SetCloudColor(in TGradientColor c_rColor, in TGradientColor c_rNextColor, in uint dwTransitionTime)
	{
		TSkyObjectFace aFaceCloud = m_FaceCloud;
		for (uint dwk = 0; dwk < aFaceCloud.m_SkyObjectQuadVector.size(); ++dwk)
		{
			CSkyObjectQuad aSkyObjectQuad = aFaceCloud.m_SkyObjectQuadVector[dwk];

			aSkyObjectQuad.SetSrcColor(0, c_rColor.m_FirstColor.r, c_rColor.m_FirstColor.g, c_rColor.m_FirstColor.b, c_rColor.m_FirstColor.a);
			aSkyObjectQuad.SetTransition(0, c_rNextColor.m_FirstColor.r, c_rNextColor.m_FirstColor.g, c_rNextColor.m_FirstColor.b, c_rNextColor.m_FirstColor.a, dwTransitionTime);
			aSkyObjectQuad.SetSrcColor(1, c_rColor.m_FirstColor.r, c_rColor.m_FirstColor.g, c_rColor.m_FirstColor.b, c_rColor.m_FirstColor.a);
			aSkyObjectQuad.SetTransition(1, c_rNextColor.m_FirstColor.r, c_rNextColor.m_FirstColor.g, c_rNextColor.m_FirstColor.b, c_rNextColor.m_FirstColor.a, dwTransitionTime);
			aSkyObjectQuad.SetSrcColor(2, c_rColor.m_FirstColor.r, c_rColor.m_FirstColor.g, c_rColor.m_FirstColor.b, c_rColor.m_FirstColor.a);
			aSkyObjectQuad.SetTransition(2, c_rNextColor.m_FirstColor.r, c_rNextColor.m_FirstColor.g, c_rNextColor.m_FirstColor.b, c_rNextColor.m_FirstColor.a, dwTransitionTime);
			aSkyObjectQuad.SetSrcColor(3, c_rColor.m_FirstColor.r, c_rColor.m_FirstColor.g, c_rColor.m_FirstColor.b, c_rColor.m_FirstColor.a);
			aSkyObjectQuad.SetTransition(3, c_rNextColor.m_FirstColor.r, c_rNextColor.m_FirstColor.g, c_rNextColor.m_FirstColor.b, c_rNextColor.m_FirstColor.a, dwTransitionTime);
		}
	}

	public void Refresh()
	{
		_D3DVECTOR[] v3QuadPoints = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(4);
		...
		if (m_ucRenderMode == CSkyObject.SKY_RENDER_MODE_DEFAULT || m_ucRenderMode == CSkyObject.SKY_RENDER_MODE_DIFFUSE)
		{
			...
			D3DXVECTOR2[] v2QuadPoints = Arrays.InitializeWithDefaultInstances<D3DXVECTOR2>(2);
	...
	public void SetSkyColor(in List<TGradientColor> c_rColorVector, in List<TGradientColor> c_rNextColorVector, int lTransitionTime)
	{
		uint ulVectorGradientColornum = 0;
		uint uck;
		for (byte ucj = 0; ucj < 4; ++ucj)
		{
			...
			for (uck = 0; uck < aFace.m_SkyObjectQuadVector.size(); ++uck)
			{
				...
				aSkyObjectQuad.SetSrcColor(0, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
				aSkyObjectQuad.SetTransition(0, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
				aSkyObjectQuad.SetSrcColor(1, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
				aSkyObjectQuad.SetTransition(1, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
				aSkyObjectQuad.SetSrcColor(2, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
				aSkyObjectQuad.SetTransition(2, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
				aSkyObjectQuad.SetSrcColor(3, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
				aSkyObjectQuad.SetTransition(3, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
		...
		for (uck = 0; uck < aFaceTop.m_SkyObjectQuadVector.size(); ++uck)
		{
			...
			aSkyObjectQuad.SetSrcColor(0, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
			aSkyObjectQuad.SetTransition(0, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(1, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
			aSkyObjectQuad.SetTransition(1, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(2, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
			aSkyObjectQuad.SetTransition(2, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(3, c_rColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rColorVector[ulVectorGradientColornum].m_FirstColor.a);
			aSkyObjectQuad.SetTransition(3, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.r, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.g, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.b, c_rNextColorVector[ulVectorGradientColornum].m_FirstColor.a, lTransitionTime);
		}
		TSkyObjectFace aFaceBottom = m_Faces[5];
		ulVectorGradientColornum = (uint)(c_rColorVector.Count - 1);
		for (uck = 0; uck < aFaceBottom.m_SkyObjectQuadVector.size(); ++uck)
		{
			...
			aSkyObjectQuad.SetSrcColor(0, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
			aSkyObjectQuad.SetTransition(0, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(1, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
			aSkyObjectQuad.SetTransition(1, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(2, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
			aSkyObjectQuad.SetTransition(2, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
			aSkyObjectQuad.SetSrcColor(3, c_rColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rColorVector[ulVectorGradientColornum].m_SecondColor.a);
			aSkyObjectQuad.SetTransition(3, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.r, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.g, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.b, c_rNextColorVector[ulVectorGradientColornum].m_SecondColor.a, lTransitionTime);
		}
	}
	...
	protected void SetSkyObjectQuadVertical(TSkyObjectQuadVector pSkyObjectQuadVector, D3DXVECTOR2[] c_pv2QuadPoints)
	{
		SPDTVertex aPDTVertex = new SPDTVertex();
	...
	protected void SetSkyObjectQuadHorizon(TSkyObjectQuadVector pSkyObjectQuadVector, _D3DVECTOR[] c_pv3QuadPoints)
	{
		pSkyObjectQuadVector.clear();
	...
	protected byte m_ucVirticalGradientLevelUpper;
	protected byte m_ucVirticalGradientLevelLower;
	protected TSkyObjectFace[] m_Faces = Arrays.InitializeWithDefaultInstances<TSkyObjectFace>(6);
}
