...
//		void SetCurrentTime();
		...
		protected static float ms_fCurTime = 0.0f;
}

//# Laniatus Games Studio Inc. | TODO TASK: Multiple inheritance is not available in C#:
public class CLight : CGraphicBase, CLightBase
{
		public CLight()
		{
			Initialize();
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public void SetParameter(uint id, in D3DLIGHT8 c_rLight)
		{
			m_LightID = id;
			m_d3dLight = c_rLight;
		}
...
//Original Metin2 CPlus Line: float GetDistance() const
		public float GetDistance()
		{
			return m_fDistance;
		}

		public uint GetLightID()
		{
			return m_LightID;
		}
		...
		public void SetDeviceLight(bool bActive)
		{
		...
		public void SetDiffuseColor(float fr, float fg, float fb, float fa = 1.0f)
		{
			if (m_d3dLight.Diffuse.r == fr && m_d3dLight.Diffuse.g == fg && m_d3dLight.Diffuse.b == fb && m_d3dLight.Diffuse.a == fa)
			{
				return;
			}
		...
		public void SetAmbientColor(float fr, float fg, float fb, float fa = 1.0f)
		{
			if (m_d3dLight.Ambient.r == fr && m_d3dLight.Ambient.g == fg && m_d3dLight.Ambient.b == fb && m_d3dLight.Ambient.a == fa)
			{
				return;
			}
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const D3DVECTOR & GetPosition() const
		public D3DVECTOR GetPosition()
		{
			return m_d3dLight.Position;
		}

		public void BlendDiffuseColor(in D3DXCOLOR c_rColor, float fBlendTime, float fDelayTime = 0.0f)
		{
			D3DXCOLOR Color = new D3DXCOLOR(m_d3dLight.Diffuse);
			m_DiffuseColorTransitor.SetTransition(Color, c_rColor, ms_fCurTime + fDelayTime, fBlendTime);
		}

		public void BlendAmbientColor(in D3DXCOLOR c_rColor, float fBlendTime, float fDelayTime = 0.0f)
		{
			D3DXCOLOR Color = new D3DXCOLOR(m_d3dLight.Ambient);
			m_AmbientColorTransitor.SetTransition(Color, c_rColor, ms_fCurTime + fDelayTime, fBlendTime);
		}

		public void BlendRange(float fRange, float fBlendTime, float fDelayTime = 0.0f)
		{
			m_RangeTransitor.SetTransition(m_d3dLight.Range, fRange, ms_fCurTime + fDelayTime, fBlendTime);
		}

		private uint m_LightID;

		private D3DLIGHT8 m_d3dLight = new D3DLIGHT8();
		private bool m_isEdited;
		private float m_fDistance;

		private CTransitor<D3DXCOLOR> m_DiffuseColorTransitor = new CTransitor<D3DXCOLOR>();
		private CTransitor<D3DXCOLOR> m_AmbientColorTransitor = new CTransitor<D3DXCOLOR>();
		private CTransitor<float> m_RangeTransitor = new CTransitor<float>();
}

//# Laniatus Games Studio Inc. | TODO TASK: Multiple inheritance is not available in C#:
public class CLightManager : CGraphicBase, CLightBase, CSingleton<CLightManager>
{
		public const int LIGHT_LIMIT_DEFAULT = 3;
		...
		public void FlushLight()
		{
			...
			for (; itor != m_LightMap.end(); ++itor)
			{
				...
				_D3DVECTOR v3LightPos = new _D3DVECTOR(pLight.GetPosition());
				_D3DVECTOR v3Distance = new _D3DVECTOR(v3LightPos - m_v3CenterPosition);
			...
			for (uint k = 0; k < Math.Min(m_dwLimitLightCount, m_LightSortVector.size()); ++k)
			{
		...
		public void RegisterLight(ELightType LightType, ref uint poutLightID, D3DLIGHT8 LightData)
		{
		...
		public CLight GetLight(uint LightID)
		{
			TLightMap.iterator itor = m_LightMap.find(LightID);

			if (m_LightMap.end() == itor)
			{
				Debug.Assert(!"CLightManager::SetLightData - Failed to find light ID!");
				return null;
			}
		...
		public void DeleteLight(uint LightID)
		{
			TLightMap.iterator itor = m_LightMap.find(LightID);

			if (m_LightMap.end() == itor)
			{
				Debug.Assert(!"CLightManager::DeleteLight - Failed to find light ID!");
				return;
			}
		...
		public void SetCenterPosition(in _D3DVECTOR c_rv3Position)
		{
			m_v3CenterPosition = c_rv3Position;
		}

		public void SetLimitLightCount(uint dwLightCount)
		{
			m_dwLimitLightCount = dwLightCount;
		}

		public void SetSkipIndex(uint dwSkipIndex)
		{
			m_dwSkipIndex = dwSkipIndex;
		}

		protected LinkedList<uint> m_NonUsingLightIDDeque = new LinkedList<uint>();

		protected SortedDictionary<uint, CLight > m_LightMap = new SortedDictionary<uint, CLight >();
		protected List<CLight > m_LightSortVector = new List<CLight >();

		protected _D3DVECTOR m_v3CenterPosition = new _D3DVECTOR();
		protected uint m_dwLimitLightCount;
		protected uint m_dwSkipIndex;

		protected uint NewLightID()
		{
			if (!m_NonUsingLightIDDeque.empty())
			{
				uint id = m_NonUsingLightIDDeque.back();
				m_NonUsingLightIDDeque.pop_back();
				return (id);
			}

			return (uint)(m_dwSkipIndex + m_LightMap.size());
		}

		protected void ReleaseLightID(uint LightID)
		{
			m_NonUsingLightIDDeque.push_back(LightID);
		}

		protected CDynamicPool<CLight> m_LightPool = new CDynamicPool<CLight>();
}
...
{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool operator ()(const CLight * l, const CLight * r) const
	public bool functorMethod(CLight l, CLight r)
	{
