...
public class CScreen : CGraphicCollisionObject
{
	public CScreen()
	{
	}
	...
	{
		Debug.Assert(ms_lpd3dDevice != null);
		ms_lpd3dDevice.Clear(0, null, D3DCLEAR_ZBUFFER, ms_clearColor, ms_clearDepth, ms_clearStencil);
	}

	public void Clear()
	{
		Debug.Assert(ms_lpd3dDevice != null);
		ms_lpd3dDevice.Clear(0, null, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, ms_clearColor, ms_clearDepth, ms_clearStencil);
	}
	...
	public bool Begin()
	{
		...
		if (!(CStateManager.Instance()).BeginScene())
		{
	...
	public void End()
	{
		(CStateManager.Instance()).EndScene();
	}

	public void Show(IntPtr hWnd = null)
	{
		Debug.Assert(ms_lpd3dDevice != null);

		if (g_isBrowserMode)
		{
			RECT rcTop = new RECT(0, 0, ms_d3dPresentParameter.BackBufferWidth, g_rcBrowser.top);
			RECT rcBottom = new RECT(0, g_rcBrowser.bottom, ms_d3dPresentParameter.BackBufferWidth, ms_d3dPresentParameter.BackBufferHeight);
			RECT rcLeft = new RECT(0, g_rcBrowser.top, g_rcBrowser.left, g_rcBrowser.bottom);
			RECT rcRight = new RECT(g_rcBrowser.right, g_rcBrowser.top, ms_d3dPresentParameter.BackBufferWidth, g_rcBrowser.bottom);
		...
		{
			int hr = ms_lpd3dDevice.Present(null, null, hWnd, null);
	...
	public void Show(RECT pSrcRect)
	{
		Debug.Assert(ms_lpd3dDevice != null);
		ms_lpd3dDevice.Present(pSrcRect, null, null, null);
	}

	public void Show(RECT pSrcRect, IntPtr hWnd)
	{
		Debug.Assert(ms_lpd3dDevice != null);
		ms_lpd3dDevice.Present(pSrcRect, null, hWnd, null);
	}

	public void RenderLine2d(float sx, float sy, float ex, float ey, float z = 0.0f)
	{
		RenderLine3d(sx, sy, z, ex, ey, z);
	}

	public void RenderBox2d(float sx, float sy, float ex, float ey, float z = 0.0f)
	{
		RenderBox3d(sx, sy, z, ex, ey, z);
	}

	public void RenderBar2d(float sx, float sy, float ex, float ey, float z = 0.0f)
	{
		RenderBar3d(sx, sy, z, ex, ey, z);
	}

	public void RenderGradationBar2d(float sx, float sy, float ex, float ey, uint dwStartColor, uint dwEndColor, float ez = 0.0f)
	{
		RenderGradationBar3d(sx, sy, ez, ex, ey, ez, dwStartColor, dwEndColor);
	}

	public void RenderCircle2d(float fx, float fy, float fz, float fRadius, int iStep = 50)
	{
		int count;
		float theta;
		float delta;
		...
		float y;
		float z;
		List<_D3DVECTOR> pts = new List<_D3DVECTOR>();

		pts.Clear();
		pts.Resize(iStep);

		theta = 0.0F;
		delta = 2 * ((float) 3.141592654f) / (float)iStep;
	...
	public void RenderCircle3d(float fx, float fy, float fz, float fRadius, int iStep = 50)
	{
		int count;
		float theta;
		float delta;
		...
		_D3DMATRIX c_rmatInvView = CCameraManager.Instance().GetCurrentCamera().GetBillboardMatrix();
	...
	public void RenderLine3d(float sx, float sy, float sz, float ex, float ey, float ez)
	{
		...
		SPDTVertexRaw[] vertices =
		{
			new SPDTVertexRaw(sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, ey, ez, ms_diffuseColor, 0.0f, 0.0f)
		};
		...
		if (SetPDTStream(vertices, 2))
		{
			...
			(CStateManager.Instance()).DrawPrimitive(D3DPT_LINELIST, 0, 1);
		}
	}

	public void RenderBox3d(float sx, float sy, float sz, float ex, float ey, float ez)
	{
		if (GRAPHICS_CAPS_CAN_NOT_DRAW_LINE)
		{
			return;
		...
		{
			new SPDTVertexRaw(sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, ey, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, ey, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, ey, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex + 1.0f, ey, ez, ms_diffuseColor, 0.0f, 0.0f)
		};
		...
		if (SetPDTStream(vertices, 8))
		{
			...
			(CStateManager.Instance()).SetTexture(1, null);
			(CStateManager.Instance()).SetVertexShader(D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1);
			(CStateManager.Instance()).DrawPrimitive(D3DPT_LINELIST, 0, 4);
		}
	}

	public void RenderBar3d(float sx, float sy, float sz, float ex, float ey, float ez)
	{
		Debug.Assert(ms_lpd3dDevice != null);
	...
	public void RenderBar3d(_D3DVECTOR[] c_pv3Positions)
	{
		Debug.Assert(ms_lpd3dDevice != null);

		SPDTVertexRaw[] vertices =
		{
			new SPDTVertexRaw(c_pv3Positions[0].x, c_pv3Positions[0].y, c_pv3Positions[0].z, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(c_pv3Positions[2].x, c_pv3Positions[2].y, c_pv3Positions[2].z, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(c_pv3Positions[1].x, c_pv3Positions[1].y, c_pv3Positions[1].z, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(c_pv3Positions[3].x, c_pv3Positions[3].y, c_pv3Positions[3].z, ms_diffuseColor, 0.0f, 0.0f)
		};
	...
	public void RenderGradationBar3d(float sx, float sy, float sz, float ex, float ey, float ez, uint dwStartColor, uint dwEndColor)
	{
		Debug.Assert(ms_lpd3dDevice != null);
		if (sx == ex)
		{
		...
		SPDTVertexRaw[] vertices =
		{
			new SPDTVertexRaw(sx, sy, sz, dwStartColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, ey, ez, dwEndColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, sy, sz, dwStartColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, ey, ez, dwEndColor, 0.0f, 0.0f)
		};
	...
	public void RenderLineCube(float sx, float sy, float sz, float ex, float ey, float ez)
	{
		SPDTVertexRaw[] vertices =
		{
			new SPDTVertexRaw(sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, ey, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, ey, sz, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, sy, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, sy, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(sx, ey, ez, ms_diffuseColor, 0.0f, 0.0f),
			new SPDTVertexRaw(ex, ey, ez, ms_diffuseColor, 0.0f, 0.0f)
		};
		...
		if (SetPDTStream(vertices, 8))
		{
			...
			(CStateManager.Instance()).SetTexture(1, null);
			(CStateManager.Instance()).SetVertexShader(D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1);
			(CStateManager.Instance()).SetTransform(D3DTS_WORLD, ms_lpd3dMatStack.GetTop());
			SetDefaultIndexBuffer(DEFAULT_IB_LINE_CUBE);

			(CStateManager.Instance()).DrawIndexedPrimitive(D3DPT_LINELIST, 0, 8, 0, 4 * 3);
		}
	}

	public void RenderCube(float sx, float sy, float sz, float ex, float ey, float ez)
	{
		SPDTVertexRaw[] vertices =
		{
			new SPDTVertexRaw(sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f),
		...
		if (SetPDTStream(vertices, 8))
		{
			...
			(CStateManager.Instance()).DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 8, 0, 4 * 3);
		}
	}

	public void RenderCube(float sx, float sy, float sz, float ex, float ey, float ez, _D3DMATRIX matRotation)
	{
		_D3DVECTOR v3Center = D3DXVECTOR3((sx + ex) * 0.5f, (sy + ey) * 0.5f, (sz + ez) * 0.5f);
...
//Original Metin2 CPlus Line: v3Vertex[LaniatusDefVariables] = v3Vertex[LaniatusDefVariables] + v3Center;
	...
	public void RenderTextureBox(float sx, float sy, float ex, float ey, float z = 0.0f, float su = 0.0f, float sv = 0.0f, float eu = 1.0f, float ev = 1.0f)
	{
		Debug.Assert(ms_lpd3dDevice != null);

		SPDTVertex[] vertices = Arrays.InitializeWithDefaultInstances<SPDTVertex>(4);
		...
		if (SetPDTStream(vertices, 4))
		{
			(CStateManager.Instance()).DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 4, 0, 2);
		}
	}

	public void RenderBillboard(_D3DVECTOR[] Position, D3DXCOLOR Color)
	{
		Debug.Assert(ms_lpd3dDevice != null);
	...
	public void DrawMinorGrid(float xMin, float yMin, float xMax, float yMax, float xminorStep, float yminorStep, float zPos = 0F)
	{
		float x;
		float y;
	...
	public void DrawGrid(float xMin, float yMin, float xMax, float yMax, float xmajorStep, float ymajorStep, float xminorStep, float yminorStep, float zPos = 0F)
	{
		xMin *= xminorStep;
		xMax *= xminorStep;
		yMin *= yminorStep;
	...
	public void RenderD3DXMesh(ID3DXMesh lpMesh, _D3DMATRIX c_pmatWorld, float fx, float fy, float fz, float fRadius, D3DFILLMODE d3dFillMode)
	{
		_D3DMATRIX matTranslation = new _D3DMATRIX();
		_D3DMATRIX matScaling = new _D3DMATRIX();
...
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: matWorld = matScaling * matTranslation;
		matWorld.CopyFrom(matScaling * matTranslation);
		...
		CD3DXMeshRenderingOption SetRenderingOption = new CD3DXMeshRenderingOption(d3dFillMode, matWorld);
		LPDIRECT3DINDEXBUFFER8 lpIndexBuffer = new LPDIRECT3DINDEXBUFFER8();
		LPDIRECT3DVERTEXBUFFER8 lpVertexBuffer = new LPDIRECT3DVERTEXBUFFER8();
		lpMesh.GetIndexBuffer(lpIndexBuffer);
		lpMesh.GetVertexBuffer(lpVertexBuffer);
		(CStateManager.Instance()).SetVertexShader(lpMesh.GetFVF());
		(CStateManager.Instance()).SetIndices(lpIndexBuffer, 0);
		(CStateManager.Instance()).SetStreamSource(0, lpVertexBuffer, 24);
		(CStateManager.Instance()).DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, lpMesh.GetNumVertices(), 0, lpMesh.GetNumFaces());
	}

	public void RenderSphere(_D3DMATRIX c_pmatWorld, float fx, float fy, float fz, float fRadius, D3DFILLMODE d3dFillMode = D3DFILL_SOLID)
	{
		RenderD3DXMesh(ms_lpSphereMesh, c_pmatWorld, fx, fy, fz, fRadius, d3dFillMode);
	}

	public void RenderCylinder(_D3DMATRIX c_pmatWorld, float fx, float fy, float fz, float fRadius, float fLength, D3DFILLMODE d3dFillMode = D3DFILL_SOLID)
	{
	...
	{
		(CStateManager.Instance()).SetTexture(0, null);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_DIFFUSE);
	...
	public void SetDiffuseOperation()
	{
		(CStateManager.Instance()).SetTexture(0, null);
	...
	public void SetBlendOperation()
	{
		(CStateManager.Instance()).SetTexture(0, null);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_CURRENT);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_CURRENT);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
		(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_COLOROP, D3DTOP_DISABLE);
		(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_ALPHAOP, D3DTOP_DISABLE);
	}

	public void SetOneColorOperation(D3DXCOLOR rColor)
	{
		(CStateManager.Instance()).SetTexture(0, null);
		(CStateManager.Instance()).SetTexture(1, null);

		(CStateManager.Instance()).SetRenderState(D3DRS_TEXTUREFACTOR, rColor);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
	}

	public void SetAddColorOperation(D3DXCOLOR rColor)
	{
		(CStateManager.Instance()).SetRenderState(D3DRS_TEXTUREFACTOR, rColor);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TFACTOR);
		(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_ADD);
	}
	...
	public void SetClearStencil(uint stencil)
	{
		ms_clearStencil = stencil;
	}

	public void SetCursorPosition(int x, int y, int hres, int vres)
	{
		_D3DVECTOR v = new _D3DVECTOR();
		v.x = -(((2.0f * x) / hres) - 1) / ms_matProj._11;
		v.y = (((2.0f * y) / vres) - 1) / ms_matProj._22;
		v.z = 1.0f;

		_D3DMATRIX matViewInverse = ms_matInverseView;
	...
	public bool GetCursorPosition(ref float px, ref float py, ref float pz)
	{
	...
	public bool GetCursorXYPosition(ref float px, ref float py)
	{
		_D3DVECTOR v3Eye = CCameraManager.Instance().GetCurrentCamera().GetEye();

		_D3DVECTOR[] posVertices = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(4);
		...
		ushort[] sc_awFillRectIndices = {0, 2, 1, 2, 3, 1};

		float u;
		float v;
		float t;
		for (int LaniatusDefVariables = 0; LaniatusDefVariables < 2; ++i)
		{
	...
	public bool GetCursorZPosition(ref float pz)
	{
		_D3DVECTOR v3Eye = CCameraManager.Instance().GetCurrentCamera().GetEye();
	...
	public void GetPickingPosition(float t, ref float x, ref float y, ref float z)
	{
	...
	public void ProjectPosition(float x, float y, float z, ref float pfX, ref float pfY)
	{
		_D3DVECTOR Input = new _D3DVECTOR(x, y, z);
		_D3DVECTOR Output = new _D3DVECTOR();
	...
	public void ProjectPosition(float x, float y, float z, ref float pfX, ref float pfY, ref float pfZ)
	{
		_D3DVECTOR Input = new _D3DVECTOR(x, y, z);
		_D3DVECTOR Output = new _D3DVECTOR();
		D3DXVec3Project(Output, Input, ms_Viewport, ms_matProj, ms_matView, ms_matWorld);
	...
	public void UnprojectPosition(float x, float y, float z, ref float pfX, ref float pfY, ref float pfZ)
	{
		_D3DVECTOR Input = new _D3DVECTOR(x, y, z);
		_D3DVECTOR Output = new _D3DVECTOR();
	...
	public bool IsLostDevice()
	{
		...
		int hrTestCooperativeLevel = rkD3DDev.TestCooperativeLevel();
	...
	public bool RestoreDevice()
	{
		...
		if (FAILED(hrTestCooperativeLevel))
		{
			...
			if (D3DERR_DEVICENOTRESET == hrTestCooperativeLevel)
			{
				...
				int hrReset = rkD3DDev.Reset(rkD3DPP);
				...
				(CStateManager.Instance()).SetDefaultState();
			}
		}
	...
	{
		_D3DVECTOR c_rv3Eye = CCameraManager.Instance().GetCurrentCamera().GetEye();
		_D3DVECTOR c_rv3View = CCameraManager.Instance().GetCurrentCamera().GetView();
		 ms_frustum.BuildViewFrustum2(ms_matView * ms_matProj, ms_fNearY, ms_fFarY, ms_fFieldOfView, ms_fAspect, c_rv3Eye, c_rv3View);
	}
	...
	protected static uint ms_diffuseColor = 0xffffffff;
	protected static uint ms_clearColor = 0;
	protected static uint ms_clearStencil = 0;
	protected static float ms_clearDepth = 1.0f;

	protected static Frustum ms_frustum = new Frustum();
}

public class CD3DXMeshRenderingOption : CScreen
{
	public uint m_dwVS;

	public CD3DXMeshRenderingOption(D3DFILLMODE d3dFillMode, in _D3DMATRIX c_rmatWorld)
	{
		ms_lpd3dDevice.GetVertexShader(m_dwVS);

		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
		...
		(CStateManager.Instance()).SetRenderState(D3DRS_FILLMODE, d3dFillMode);
		(CStateManager.Instance()).SaveTransform(D3DTS_WORLD, c_rmatWorld);
	...
	public virtual void Dispose()
	{
		...
		(CStateManager.Instance()).RestoreTransform(D3DTS_WORLD);
		...
		(CStateManager.Instance()).SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
	}
}