...
public class CGraphicDevice : CGraphicBase
{
	...
	public enum ECreateReturnValues
	{
		CREATE_OK = (1 << 0),
	...
	public CGraphicDevice()
	{
		this.m_uBackBufferCount = 0;
		__Initialize();
	}
	...
	public void InitBackBufferCount(uint uBackBufferCount)
	{
	...
	public int Create(IntPtr hWnd, int iHres, int iVres, bool Windowed = true, int bit = 32, int iReflashRate = 0)
	{
		int iRet = CREATE_OK;
		...
		ms_hWnd = hWnd;
		ms_hDC = GetDC(hWnd);
		ms_lpd3d = Direct3DCreate8(DefineConstants.D3D_SDK_VERSION);
	...
	RETRY:
		...
		if (ms_lpd3d.CheckDeviceFormat(ms_iD3DAdapterInfo, D3DDEVTYPE_HAL, ms_d3dPresentParameter.BackBufferFormat, 0, D3DRTYPE_TEXTURE, D3DFMT_DXT1) == D3DERR_NOTAVAILABLE)
		{
			ms_bSupportDXT = false;
		}

		if (ms_lpd3d.CheckDeviceFormat(ms_iD3DAdapterInfo, D3DDEVTYPE_HAL, ms_d3dPresentParameter.BackBufferFormat, 0, D3DRTYPE_TEXTURE, D3DFMT_DXT3) == D3DERR_NOTAVAILABLE)
		{
			ms_bSupportDXT = false;
		}

		if (ms_lpd3d.CheckDeviceFormat(ms_iD3DAdapterInfo, D3DDEVTYPE_HAL, ms_d3dPresentParameter.BackBufferFormat, 0, D3DRTYPE_TEXTURE, D3DFMT_DXT5) == D3DERR_NOTAVAILABLE)
		{
			ms_bSupportDXT = false;
		}
		...
		ms_lpd3dDevice.Clear(0, null, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0xff000000, 1.0f, 0);
		...
		uint dwTexMemSize = GetAvailableTextureMemory();
		...
		if ((ms_d3dCaps.TextureAddressCaps & D3DPTADDRESSCAPS_BORDER) != 0)
		{
	...
	public CGraphicDevice.EDeviceState GetDeviceState()
	{
		if (!ms_lpd3dDevice)
		{
	...
	public void EnableWebBrowserMode(in RECT c_rcWebPage)
	{
		if (!ms_lpd3dDevice)
		{
			return;
	...
	public void DisableWebBrowserMode()
	{
		if (!ms_lpd3dDevice)
		{
			return;
	...
	public void MoveWebBrowserRect(in RECT c_rcWebPage)
	{
		g_rcBrowser = c_rcWebPage;
	}

	public bool ResizeBackBuffer(uint uWidth, uint uHeight)
	{
		if (!ms_lpd3dDevice)
		{
			return false;
	...
	public void RegisterWarningString(uint uiMsg, string c_szString)
	{
	...
	protected bool __IsInDriverBlackList(D3D_CAdapterInfo rkD3DAdapterInfo)
	{
		D3DADAPTER_IDENTIFIER8 rkD3DAdapterIdentifier = rkD3DAdapterInfo.GetIdentifier();

		string szSrcDriver = new string(new char[256]);
		strncpy(szSrcDriver, rkD3DAdapterIdentifier.Driver, sizeof(char) - 1);
		uint dwSrcHighVersion = (uint)(rkD3DAdapterIdentifier.DriverVersion.QuadPart >> 32);
		uint dwSrcLowVersion = (uint)(rkD3DAdapterIdentifier.DriverVersion.QuadPart & 0xffffffff);
		...
		if (fp != null)
		{
			uint dwChkHighVersion;
			uint dwChkLowVersion;

			string szChkDriver = new string(new char[256]);

			string szLine = new string(new char[256]);
			while (fgets(szLine, sizeof(char) - 1, fp))
			{
				...
				szLine = StringFunctions.ChangeCharacter(szLine, 0, '\0');
			}
			fclose(fp);
		}
	...
	protected void __WarningMessage(IntPtr hWnd, uint uiMsg)
	{
	...
	protected void __InitializeDefaultIndexBufferList()
	{
	...
	protected void __DestroyDefaultIndexBufferList()
	{
		for (uint LaniatusDefVariables = 0; LaniatusDefVariables < DEFAULT_IB_NUM; ++i)
		{
	...
	protected bool __CreateDefaultIndexBufferList()
	{
		ushort[] c_awLineIndices = {0, 1};
		ushort[] c_awLineTriIndices = {0, 1, 0, 2, 1, 2};
		ushort[] c_awLineRectIndices = {0, 1, 0, 2, 1, 3, 2, 3};
		ushort[] c_awLineCubeIndices = {0, 1, 0, 2, 1, 3, 2, 3, 0, 4, 1, 5, 2, 6, 3, 7, 4, 5, 4, 6, 5, 7, 6, 7};
		ushort[] c_awFillTriIndices = {0, 1, 2};
		ushort[] c_awFillRectIndices = {0, 2, 1, 2, 3, 1};
		ushort[] c_awFillCubeIndices = {0, 1, 2, 1, 3, 2, 2, 0, 6, 0, 4, 6, 0, 1, 4, 1, 5, 4, 1, 3, 5, 3, 7, 5, 3, 2, 7, 2, 6, 7, 4, 5, 6, 5, 7, 6};
	...
	protected bool __CreateDefaultIndexBuffer(uint eDefIB, uint uIdxCount, ushort c_awIndices)
	{
		Debug.Assert(ms_alpd3dDefIB[eDefIB] == null);

		if (FAILED(ms_lpd3dDevice.CreateIndexBuffer(sizeof(ushort) * uIdxCount, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, ms_alpd3dDefIB[eDefIB])))
		{
			return false;
		}

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: ushort* dstIndices;
		ushort dstIndices;
		if (FAILED(ms_alpd3dDefIB[eDefIB].Lock(0, 0, (byte) & dstIndices, 0)))
		{
			return false;
		}

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		memcpy(dstIndices, c_awIndices, sizeof(ushort) * uIdxCount);
	...
	protected void __InitializePDTVertexBufferList()
	{
		m_smallPdtVertexBuffer = null;
		m_largePdtVertexBuffer = null;
	}

	protected void __DestroyPDTVertexBufferList()
	{
	...
	protected bool __CreatePDTVertexBufferList()
	{
		var res = ms_lpd3dDevice.CreateVertexBuffer(sizeof(SPDTVertex) * SMALL_PDT_VERTEX_BUFFER_SIZE, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, SPDTVertex.kFVF, D3DPOOL_DEFAULT, m_smallPdtVertexBuffer);
		...
		res = ms_lpd3dDevice.CreateVertexBuffer(sizeof(SPDTVertex) * LARGE_PDT_VERTEX_BUFFER_SIZE, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, SPDTVertex.kFVF, D3DPOOL_DEFAULT, m_largePdtVertexBuffer);
	...
	protected uint CreatePTStreamVertexShader()
	{
		Debug.Assert(ms_lpd3dDevice != null);

		uint[] declVector = {D3DVSD_STREAM(0), D3DVSD_REG(0, D3DVSDT_FLOAT3), D3DVSD_STREAM(1), D3DVSD_REG(7, D3DVSDT_FLOAT2), D3DVSD_END()};
	...
	protected uint CreatePNTStreamVertexShader()
	{
		Debug.Assert(ms_lpd3dDevice != null);

		uint[] declVector = {D3DVSD_STREAM(0), D3DVSD_REG(0, D3DVSDT_FLOAT3), D3DVSD_REG(3, D3DVSDT_FLOAT3), D3DVSD_REG(7, D3DVSDT_FLOAT2), D3DVSD_END()};
	...
	protected uint CreatePNT2StreamVertexShader()
	{
		Debug.Assert(ms_lpd3dDevice != null);

		uint[] declVector = {D3DVSD_STREAM(0), D3DVSD_REG(0, D3DVSDT_FLOAT3), D3DVSD_REG(3, D3DVSDT_FLOAT3), D3DVSD_REG(7, D3DVSDT_FLOAT2), D3DVSD_REG(D3DVSDE_TEXCOORD1, D3DVSDT_FLOAT2), D3DVSD_END()};
	...
	protected uint CreateDoublePNTStreamVertexShader()
	{
		Debug.Assert(ms_lpd3dDevice != null);

		uint[] declVector = {D3DVSD_STREAM(0), D3DVSD_REG(0, D3DVSDT_FLOAT3), D3DVSD_REG(3, D3DVSDT_FLOAT3), D3DVSD_REG(7, D3DVSDT_FLOAT2), D3DVSD_STREAM(1), D3DVSD_REG(D3DVSDE_POSITION2, D3DVSDT_FLOAT3), D3DVSD_REG(D3DVSDE_NORMAL2, D3DVSDT_FLOAT3), D3DVSD_REG(D3DVSDE_TEXCOORD1, D3DVSDT_FLOAT2), D3DVSD_END()};
	...
	protected uint m_uBackBufferCount;
	protected SortedDictionary<uint, string> m_kMap_strWarningMessage = new SortedDictionary<uint, string>();
	protected CStateManager m_pStateManager;
}