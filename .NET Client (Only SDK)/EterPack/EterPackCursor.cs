...
public class CEterPackCursor
{
		public CEterPackCursor(CEterPack pack)
		{
			this.m_pPack = pack;
			this.m_pData = null;
			this.m_ReadPoint = 0;
		...
		{
			Debug.Assert(m_pPack != null);

			string tmpFilename = new string(new char[MAX_PATH + 1]);
		...
		{
			m_ReadPoint = Math.Max(0, Math.Min(Size(), offset));
		}

		public bool Read(object data, int size)
		{
			if (m_file.IsNull())
			{
				return false;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(data, (string) m_pData + m_ReadPoint, size);
			m_ReadPoint += size;
			return true;
		}
		...
		private CEterPack m_pPack;
		private CMappedFile m_file = new CMappedFile();
		private LPCVOID m_pData = new LPCVOID();
		private int m_ReadPoint;
}
