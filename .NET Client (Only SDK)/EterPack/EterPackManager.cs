...
public class CEterPackManager : CSingleton<CEterPackManager>
{
		public class SCache
		{
			public byte[] m_abBufData;
		...
		public CEterPackManager()
		{
			this.m_bTryRelativePath = false;
			this.m_iSearchMode = SEARCH_FILE_FIRST;
			this.m_isCacheMode = false;
		...
		public virtual void Dispose()
		{
			...
			TEterPackMap.iterator LaniatusDefVariables = m_PackMap.begin();
		...
		public void SetRelativePathMode()
		{
		...
		public void LoadStaticCache(string c_szFileName)
		{
			...
			uint dwFileNameHash = GetCRC32(strFileName, strFileName.Length);

			Dictionary<uint, SCache>.Enumerator f = m_kMap_dwNameKey_kCache.find(dwFileNameHash);
			if (m_kMap_dwNameKey_kCache.end() != f)
			{
				return;
			}

			CMappedFile kMapFile = new CMappedFile();
			object c_pvData;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			m_kMap_dwNameKey_kCache.insert(Dictionary<uint, SCache>.value_type(dwFileNameHash, kNewCache));
		}

		public void SetSearchMode(bool bPackFirst)
		{
		...
		public bool Get(CMappedFile rMappedFile, string c_szFileName, LPCVOID pData)
		{
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string GetFromPack_strFileName = "";

		public bool GetFromPack(CMappedFile rMappedFile, string c_szFileName, LPCVOID pData)
		{
			FinderLock @lock = new FinderLock(m_csFinder);
...
//			static string strFileName;
			...
			if (0 == ConvertFileName(c_szFileName, GetFromPack_strFileName))
			{
				return m_RootPack.Get(rMappedFile, GetFromPack_strFileName.c_str(), pData);
			}
			else
			{
				uint dwFileNameHash = GetCRC32(GetFromPack_strFileName.c_str(), GetFromPack_strFileName.length());
				...
				CEterFileDict.Item pkFileItem = m_FileDict.GetItem(dwFileNameHash, GetFromPack_strFileName.c_str());

				if (pkFileItem != null)
				{
					if (pkFileItem.pkPack)
					{
						bool r = pkFileItem.pkPack.Get2(rMappedFile, GetFromPack_strFileName.c_str(), pkFileItem.pkInfo, pData);
						return r;
					}
				}
			}
#if DEBUG
			TraceError("CANNOT_FIND_PACK_FILE [%s]", GetFromPack_strFileName.c_str());
#endif

			return false;
		}

		public bool GetFromFile(CMappedFile rMappedFile, string c_szFileName, LPCVOID pData)
		{
			return rMappedFile.Create(c_szFileName, pData, 0, 0) ? true : false;
		}

		public bool isExist(string c_szFileName)
		{
			return isExistInPack(c_szFileName);
		}

		public bool isExistInPack(string c_szFileName)
		{
			string strFileName = "";

			if (0 == ConvertFileName(c_szFileName, strFileName))
			{
				return m_RootPack.IsExist(strFileName);
			}
			else
			{
				uint dwFileNameHash = GetCRC32(strFileName, strFileName.Length);
				CEterFileDict.Item pkFileItem = m_FileDict.GetItem(dwFileNameHash, strFileName);

				if (pkFileItem != null)
				{
					if (pkFileItem.pkPack)
					{
						return pkFileItem.pkPack.IsExist(strFileName);
					}
				}
			}
			return false;
		...
		public bool RegisterPack(string c_szName, string c_szDirectory, Nullable<byte> c_pbIV = null)
		{
			...
			if (c_szDirectory != '\0' && c_szDirectory[0] != '*')
			{
		...
		public void RegisterRootPack(string c_szName)
		{
		...
		public bool RegisterPackWhenPackMaking(string c_szName, string c_szDirectory, CEterPack pPack)
		{
...
//		bool DecryptPackIV(uint key);

		public string GetRootPackFileName()
		{
			return m_RootPack.GetDBName();
		}

		public void ArrangeMemoryMappedPack()
		{
		}

		protected int ConvertFileName(string c_szFileName, ref string rstrFileName)
		{
		...
		protected bool CompareName(string c_szDirectoryName, uint iLength, string c_szFileName)
		{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * c_pszSrc = c_szDirectoryName;
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * c_pszCmp = c_szFileName;
		...
		protected CEterPack FindPack(string c_szPathName)
		{
			string strFileName = "";
			...
			else
			{
				...
				{
					string c_rstrName = itor.first;
					CEterPack pEterPack = itor.second;

					if (CompareName(c_rstrName, c_rstrName.Length, strFileName))
					{
						return pEterPack;
					}
				}
		...
		protected CEterPackManager.SCache __FindCache(uint dwFileNameHash)
		{
			Dictionary<uint, SCache>.Enumerator f = m_kMap_dwNameKey_kCache.find(dwFileNameHash);
			if (m_kMap_dwNameKey_kCache.end() == f)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		protected void __ClearCacheMap()
		{
			Dictionary<uint, SCache>.Enumerator i;

			for (i = m_kMap_dwNameKey_kCache.begin(); i.MoveNext();)
			{
				i.Current.Value.m_abBufData = null;
			}

			m_kMap_dwNameKey_kCache.clear();
		}

		protected bool m_bTryRelativePath;
		protected bool m_isCacheMode;
		protected int m_iSearchMode;

		protected CEterFileDict m_FileDict = new CEterFileDict();
		protected CEterPack m_RootPack = new CEterPack();
		protected LinkedList<CEterPack> m_PackList = new LinkedList<CEterPack>();
		protected Dictionary<string, CEterPack, stringhash> m_PackMap = new Dictionary<string, CEterPack, stringhash>();
		protected Dictionary<string, CEterPack, stringhash> m_DirPackMap = new Dictionary<string, CEterPack, stringhash>();

		protected Dictionary<uint, SCache> m_kMap_dwNameKey_kCache = new Dictionary<uint, SCache>();

		protected CRITICAL_SECTION m_csFinder = new CRITICAL_SECTION();
}
...
public class FinderLock
{
	public FinderLock(CRITICAL_SECTION cs)
	{
		this.p_cs = cs;
