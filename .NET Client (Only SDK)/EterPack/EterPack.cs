...
//#pragma pack(push, 4)
public class SEterPackIndex
{
	public int id;
	public string filename = new string(new char[FILENAME_MAX_LEN + 1]);
	public uint filename_crc;
	public int real_data_size;
	public int data_size;
	public uint data_crc;
	public int data_position;
	public char compressed_type;
}
//# Laniatus Games Studio Inc. | TODO TASK: There is no equivalent to most C++ 'pragma' directives in C#:
//#pragma pack(pop)
...
public class CEterFileDict
{
	public class Item
	{
		...
		public CEterPack pkPack;
		public SEterPackIndex pkInfo;
	}

	public const int BUCKET_SIZE = 16;


	public void InsertItem(CEterPack pkPack, SEterPackIndex pkInfo)
	{
		Item item = new Item();
	...
	public void UpdateItem(CEterPack pkPack, SEterPackIndex pkInfo)
	{
		Item item = new Item();
	...
	public CEterFileDict.Item GetItem(uint dwFileNameHash, string c_pszFileName)
	{
		Tuple<TDict.iterator, TDict.iterator> iter_pair = m_dict.equal_range(dwFileNameHash);

		TDict.iterator iter = iter_pair.Item1;

		while (iter != iter_pair.Item2)
		{
...
//Original Metin2 CPlus Line: const boost::unordered_multimap<uint, Item>& GetDict() const
	public boost.unordered_multimap<uint, Item> GetDict()
	{
		return m_dict;
	}

	private boost.unordered_multimap<uint, Item> m_dict = new boost.unordered_multimap<uint, Item>();
}


public class CEterPack
{
		public CEterPack()
		{
			this.m_indexCount = 0;
			this.m_indexData = null;
			this.m_FragmentSize = 0;
			this.m_bEncrypted = false;
			this.m_bReadOnly = false;
			this.m_bDecrypedIV = false;
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public bool Create(CEterFileDict rkFileDict, string dbname, string pathName, bool bReadOnly = true, Nullable<byte> iv = null)
		{
			if (iv != null)
			{
				m_bDecrypedIV = false;
		...
		public bool Get(CMappedFile out_file, string filename, LPCVOID data)
		{
			SEterPackIndex index = FindIndex(filename);
			...
			{
				uint dwCrc32 = GetCRC32((string)(data), index.data_size);
			...
			if (COMPRESSED_TYPE_COMPRESS == index.compressed_type)
			{
				...
				if (!CLZO.Instance().Decompress(zObj, (byte) data))
				{
			...
			else if (COMPRESSED_TYPE_SECURITY == index.compressed_type)
			{
				...
				if (!CLZO.Instance().Decompress(zObj, (byte) data, s_adwEterPackSecurityKey))
				{
		...
		public bool Get2(CMappedFile out_file, string filename, SEterPackIndex index, LPCVOID data)
		{
			if (index == null)
			{
				return false;
		...
		public bool Put(string filename, string sourceFilename, byte packType, in string strRelateMapName)
		{
			CMappedFile mapFile = new CMappedFile();
			LPCVOID data = new LPCVOID();
...
//Original Metin2 CPlus Line: byte* pMappedData = (byte*)data;
		...
		public bool Put(string filename, LPCVOID data, int len, byte packType)
		{
			if (m_bEncrypted)
			{
			...
			CFileBase fileIndex = new CFileBase();
			...
			CFileBase fileData = new CFileBase();
			...
			uint data_crc;
			data_crc = GetCRC32((string) data, len);
		...
		{
			SEterPackIndex pIndex = FindIndex(filename);
		...
		public bool Extract()
		{
			CMappedFile dataMapFile = new CMappedFile();
			LPCVOID data = new LPCVOID();
			...
			for (Dictionary<uint, SEterPackIndex>.Enumerator LaniatusDefVariables = m_DataPositionMap.begin(); i.MoveNext();)
			{
				SEterPackIndex index = i.Current.Value;
				CFileBase writeFile = new CFileBase();
				...
				if (COMPRESSED_TYPE_COMPRESS == index.compressed_type)
				{
					if (!CLZO.Instance().Decompress(zObj, (byte) data + index.data_position))
					{
				...
				{
					if (!CLZO.Instance().Decompress(zObj, (byte)data + index.data_position, s_adwEterPackSecurityKey))
					{
						printf("cannot decompress");
						return false;
					}
				...
				{
					writeFile.Write((string) data + index.data_position, index.data_size);
				}
		...
		public Dictionary<uint, SEterPackIndex > GetIndexMap()
		{
			return m_DataPositionMap;
		}

		public bool EncryptIndexFile()
		{
			CMappedFile file = new CMappedFile();
			LPCVOID pvData = new LPCVOID();
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		public bool DecryptIndexFile()
		{
			...
			file.Write(eterpack.c_IndexCC, sizeof(uint));
			file.Write(eterpack.c_Version, sizeof(uint));
			file.Write(m_indexCount, sizeof(int));
			file.Write(m_indexData, sizeof(SEterPackIndex) * m_indexCount);
		...
		public uint DeleteUnreferencedData()
		{
			Dictionary<uint, SEterPackIndex >.Enumerator LaniatusDefVariables = m_DataPositionMap.begin();
			uint dwCount = 0;
		...
		public bool GetNames(List<string> retNames)
		{
			CMappedFile dataMapFile = new CMappedFile();
			LPCVOID data = new LPCVOID();
			...
			for (Dictionary<uint, SEterPackIndex>.Enumerator LaniatusDefVariables = m_DataPositionMap.begin(); i.MoveNext();)
			{
				...
				retNames.Add(index.filename);
			}
			return true;
		}

		private bool __BuildIndex(CEterFileDict rkFileDict, bool bOverwrite = false)
		{
			CMappedFile file = new CMappedFile();
			LPCVOID pvData = new LPCVOID();
			CLZObject zObj = new CLZObject();
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			...
			else if (fourcc == CLZObject.ms_dwFourCC)
			{
				...
				if (!CLZO.Instance().Decompress(zObj, (byte)pvData, s_adwEterPackKey))
				{
					return false;
				}
			...
			if (uiFileSize < eterpack.c_HeaderSize + sizeof(SEterPackIndex) * m_indexCount)
			{
				TraceError("Pack index file size error! %s, indexCount %d", m_dbName, m_indexCount);
				return false;
			}

			m_indexData = Arrays.InitializeWithDefaultInstances<SEterPackIndex>(m_indexCount);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_indexData, pbData, sizeof(SEterPackIndex) * m_indexCount);

//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			SEterPackIndex * index = m_indexData;

			for (int LaniatusDefVariables = 0; LaniatusDefVariables < m_indexCount; ++i, ++index)
			{
				...
				else
				{
					...
					m_DataPositionMap.insert(Dictionary<uint, SEterPackIndex>.value_type(index.filename_crc, index));
		...
		private bool CreateIndexFile()
		{
			...
			fwrite(eterpack.c_IndexCC, sizeof(uint), 1, fp);
			fwrite(eterpack.c_Version, sizeof(uint), 1, fp);
			fwrite(m_indexCount, sizeof(int), 1, fp);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string FindIndex_tmpFilename = new string(new char[MAX_PATH + 1]);

		private SEterPackIndex FindIndex(string filename)
		{
...
//			static char tmpFilename[MAX_PATH + 1];
			strncpy(FindIndex_tmpFilename, filename, MAX_PATH);
			inlineConvertPackFilename(FindIndex_tmpFilename);

			uint filename_crc = GetCRC32(FindIndex_tmpFilename, strlen(FindIndex_tmpFilename));
			Dictionary<uint, SEterPackIndex >.Enumerator LaniatusDefVariables = m_DataPositionMap.find(filename_crc);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		private int GetNewIndexPosition(CFileBase file)
		{
			int pos = (file.Size() - eterpack.c_HeaderSize) / sizeof(SEterPackIndex);
			++m_indexCount;
			return (pos);
		}

		private SEterPackIndex NewIndex(CFileBase file, string filename, int size)
		{
			SEterPackIndex index = null;
			int block_size = size + (DATA_BLOCK_SIZE - (size % DATA_BLOCK_SIZE));
			int blockidx = GetFreeBlockIndex(block_size);

			for (LinkedList<SEterPackIndex>.Enumerator LaniatusDefVariables = m_FreeIndexList[blockidx].begin(); i.MoveNext();)
			{
				 if (i.Current.real_data_size >= size)
				 {
					 index = i.Current;
					 m_FreeIndexList[blockidx].erase(i);

					 Debug.Assert(index.filename_crc == 0);
					 break;
				 }
			}
		...
		private void WriteIndex(CFileBase file, SEterPackIndex index)
		{
			file.Seek(sizeof(uint) + sizeof(uint));
			file.Write(m_indexCount, sizeof(int));
			file.Seek(eterpack.c_HeaderSize + (index.id * sizeof(SEterPackIndex)));

			if (!file.Write(index, sizeof(SEterPackIndex)))
			{
				Debug.Assert(!"WriteIndex: fwrite failed");
				return;
			}
		}

		private int GetFreeBlockIndex(int size)
		{
			return Math.Min(FREE_INDEX_MAX_SIZE, size / FREE_INDEX_BLOCK_SIZE);
		}

		private void PushFreeIndex(SEterPackIndex index)
		{
			if (index.filename_crc != 0)
			{
				Dictionary<uint, SEterPackIndex >.Enumerator LaniatusDefVariables = m_DataPositionMap.find(index.filename_crc);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		private int GetNewDataPosition(CFileBase file)
		{
			return file.Size();
		}

		private bool ReadData(CFileBase file, SEterPackIndex index, object data, int maxsize)
		{
		...
		private bool WriteData(CFileBase file, SEterPackIndex index, LPCVOID data)
		{
			file.Seek(index.data_position);

			if (!file.Write(data, index.data_size))
			{
				Debug.Assert(!"WriteData: fwrite data failed");
				return false;
			}
		...
		private bool WriteNewData(CFileBase file, SEterPackIndex index, LPCVOID data)
		{
			file.Seek(index.data_position);
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: char * empty_buf = (char *) calloc(empty_size, sizeof(char));
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'calloc' has no equivalent in C#:
			char empty_buf = (string) calloc(empty_size, sizeof(char));

			if (!file.Write(empty_buf, empty_size))
			{
				Debug.Assert(!"WriteData: fwrite empty data failed");
				return false;
			}

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'free' has no equivalent in C#:
			free(empty_buf);
			return true;
		}

		private bool Delete(SEterPackIndex pIndex)
		{
			CFileBase fileIndex = new CFileBase();
		...
		protected CMappedFile m_file = new CMappedFile();

		protected string m_file_data;
		protected uint m_file_size;

		protected int m_indexCount;
		protected bool m_bEncrypted;

		protected string m_dbName = new string(new char[DBNAME_MAX_LEN + 1]);
		protected string m_indexFileName = new string(new char[MAX_PATH + 1]);
		protected SEterPackIndex m_indexData;
		protected int m_FragmentSize;
		protected bool m_bReadOnly;
		protected bool m_bDecrypedIV;

		protected Dictionary<uint, uint> m_map_indexRefCount = new Dictionary<uint, uint>();
		protected Dictionary<uint, SEterPackIndex > m_DataPositionMap = new Dictionary<uint, SEterPackIndex >();
		protected LinkedList<SEterPackIndex >[] m_FreeIndexList = Arrays.InitializeWithDefaultInstances<LinkedList<SEterPackIndex >>(FREE_INDEX_MAX_SIZE + 1);

		protected string m_stDataFileName = "";
		protected string m_stPathName = "";
}

public class CMakePackLog
{
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static CMakePackLog GetSingleton_s_kMakePackLog = new CMakePackLog();

		public static CMakePackLog GetSingleton()
		{
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static CMakePackLog s_kMakePackLog;
			return GetSingleton_s_kMakePackLog;
		}
		...
		public void Writef(string c_szFormat, params object[] LegacyParamArray)
		{
			if (!__IsLogMode())
			{
				return;
...
//			va_start(args, c_szFormat);
			...
			string szBuf = new string(new char[1024]);
			int nBufLen = _vsnprintf(szBuf, sizeof(char), c_szFormat, args);
			szBuf = StringFunctions.ChangeCharacter(szBuf, nBufLen++, '\0');
			__Write(szBuf, nBufLen);
		}

		public void Writenf(string c_szFormat, params object[] LegacyParamArray)
		{
			if (!__IsLogMode())
			{
				return;
...
//			va_start(args, c_szFormat);
			...
			string szBuf = new string(new char[1024 + 1]);
			int nBufLen = _vsnprintf(szBuf, sizeof(char) - 1, c_szFormat, args);
			if (nBufLen > 0)
			{
				szBuf = StringFunctions.ChangeCharacter(szBuf, nBufLen++, '\n');
				szBuf = StringFunctions.ChangeCharacter(szBuf, nBufLen++, '\0');
			}
			__Write(szBuf, nBufLen);
		}

		public void Write(string c_szBuf)
		{
			if (!__IsLogMode())
			{
				return;
		...
		public void WriteErrorf(string c_szFormat, params object[] LegacyParamArray)
		{
			if (!__IsLogMode())
			{
				return;
...
//			va_start(args, c_szFormat);
		...
		public void WriteErrornf(string c_szFormat, params object[] LegacyParamArray)
		{
			if (!__IsLogMode())
			{
				return;
...
//			va_start(args, c_szFormat);
		...
		public void WriteError(string c_szBuf)
		{
			if (!__IsLogMode())
			{
				return;
		...
		{
			std::wifstream iFile = new std::wifstream(m_stErrorFileName.c_str());
			std::istream_iterator<string, char> iit = new std::istream_iterator<string, char>(iFile);
			std::istream_iterator<string, char> eos = new std::istream_iterator<string, char>();

			List<string> vText = new List<string>();

			std::copy(iit, eos, std::back_inserter(vText));

			std::ostream_iterator<string, char, std::char_traits <char>> oit = new std::ostream_iterator<string, char, std::char_traits <char>>(cout);

			vText.Sort();
			std::copy(vText.GetEnumerator(), vText.end(), oit);
		}

		private void __Write(string c_szBuf, int nBufLen)
		{
			if (!__IsLogMode())
			{
				return;
		...
		private void __WriteError(string c_szBuf, int nBufLen)
		{
			if (!__IsLogMode())
			{
				return;
		...
		private FILE m_fp;
		private FILE m_fp_err;

		private string m_stFileName = "";
		private string m_stErrorFileName = "";
}