...
public class CTerrainImpl
{


		public const int XSIZE = DefineConstants.TERRAIN_SIZE;
		public const int YSIZE = DefineConstants.TERRAIN_SIZE;
		public const int HEIGHTMAP_XSIZE = XSIZE+1;
		public const int HEIGHTMAP_YSIZE = YSIZE+1;
		public const int HEIGHTMAP_RAW_XSIZE = XSIZE+3;
		public const int HEIGHTMAP_RAW_YSIZE = YSIZE+3;
		public const int ATTRMAP_XSIZE = XSIZE * 2;
		public const int ATTRMAP_YSIZE = YSIZE * 2;
		public const int ATTRMAP_ZCOUNT = 8;
		public const int TILEMAP_XSIZE = XSIZE * 2;
		public const int TILEMAP_YSIZE = YSIZE * 2;
		public const int TILEMAP_RAW_XSIZE = XSIZE * 2 + 2;
		public const int TILEMAP_RAW_YSIZE = YSIZE * 2 + 2;
		public const int WATERMAP_XSIZE = XSIZE;
		public const int WATERMAP_YSIZE = YSIZE;
		public const int NORMALMAP_XSIZE = XSIZE+1;
		public const int NORMALMAP_YSIZE = YSIZE+1;
		public const int SHADOWMAP_XSIZE = XSIZE * 2;
		public const int SHADOWMAP_YSIZE = YSIZE * 2;
		public const int SPLATALPHA_RAW_XSIZE = XSIZE * 2 + 2;
		public const int SPLATALPHA_RAW_YSIZE = YSIZE * 2 + 2;
		public const int SPLATALPHA_ZCOUNT = 7;
		public const int HEIGHT_TILE_XRATIO = TILEMAP_XSIZE / XSIZE;
		public const int HEIGHT_TILE_YRATIO = TILEMAP_XSIZE / XSIZE;
		public const int PATCH_XSIZE = DefineConstants.TERRAIN_PATCHSIZE;
		public const int PATCH_YSIZE = DefineConstants.TERRAIN_PATCHSIZE;
		public const int PATCH_TILE_XSIZE = PATCH_XSIZE * HEIGHT_TILE_XRATIO;
		public const int PATCH_TILE_YSIZE = PATCH_YSIZE * HEIGHT_TILE_YRATIO;
		public const int PATCH_XCOUNT = DefineConstants.TERRAIN_SIZE / DefineConstants.TERRAIN_PATCHSIZE;
		public const int PATCH_YCOUNT = DefineConstants.TERRAIN_SIZE / DefineConstants.TERRAIN_PATCHSIZE;
		public const int CELLSCALE = 200;
		public const int CELLSCALE_IN_METER = CELLSCALE / 100;
		public const int HALF_CELLSCALE = CELLSCALE / 2;
		public const int HALF_CELLSCALE_IN_METER = HALF_CELLSCALE / 100;
		public const int TERRAIN_XSIZE = XSIZE * CELLSCALE;
		public const int TERRAIN_YSIZE = YSIZE * CELLSCALE;

		public const int MAX_ATTRIBUTE_NUM = 8;
		public const int MAX_WATER_NUM = 255;

		public int ATTRIBUTE_BLOCK = (1 << 0);
		public int ATTRIBUTE_WATER = (1 << 1);
		public int ATTRIBUTE_BANPK = (1 << 2);

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static CTextureSet SetTextureSet_s_EmptyTextureSet = new CTextureSet();

		public static void SetTextureSet(CTextureSet pTextureSet)
		{
...
//			static CTextureSet s_EmptyTextureSet;
			...
			if (pTextureSet == null)
			{
				ms_pTextureSet = &SetTextureSet_s_EmptyTextureSet;
			}
			else
			{
		...
		public static CTextureSet GetTextureSet()
		{
			if (!ms_pTextureSet)
			{
				SetTextureSet(null);
		...
		protected static CTextureSet ms_pTextureSet = null;
		...
		public TTerrainSplatPatch GetTerrainSplatPatch()
		{
		...
		public STerrainTexture GetTexture(in int c_rlTextureNum)
		{
			return ms_pTextureSet.GetTexture(c_rlTextureNum);
		}

		public bool LoadWaterMap(string c_szFileName)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public class SWaterMapHeader
		{
			public ushort m_wMagic;
			public ushort m_wWidth;
			public ushort m_wHeight;
			public byte m_byLayerCount;
		}
		public bool LoadWaterMapFile(string c_szFileName)
		{
			CMappedFile kMappedFile = new CMappedFile();
			LPCVOID lpcvFileData = new LPCVOID();
			...
			uint dwFileSize = kMappedFile.Size();
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte* abFileData = (byte*)lpcvFileData;
		...
		//#pragma pack(pop)
				...
				SWaterMapHeader kWaterMapHeader = new SWaterMapHeader();
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(kWaterMapHeader, abFileData, sizeof(SWaterMapHeader));

				const ushort c_wWaterMapMagic = 5426;
				...
				uint dwFileRestSize = dwFileSize - sizeof(SWaterMapHeader);
				uint dwFileNeedSize = sizeof(m_abyWaterMap) + sizeof(int) * m_byNumWater;
				uint dwFileNeedSize2 = sizeof(m_abyWaterMap) + sizeof(ushort) * m_byNumWater;
				if (dwFileRestSize == dwFileNeedSize2)
				{
					ushort[] wWaterHeight = new ushort[MAX_WATER_NUM + 1];

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte * abSrcWaterData = abFileData + sizeof(kWaterMapHeader);
					byte abSrcWaterData = abFileData + sizeof(SWaterMapHeader);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(m_abyWaterMap, abSrcWaterData, sizeof(m_abyWaterMap));

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte * abSrcWaterHeight = abSrcWaterData + sizeof(m_abyWaterMap);
					...
					{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
						memcpy(wWaterHeight, abSrcWaterHeight, sizeof(ushort) * m_byNumWater);
...
//Original Metin2 CPlus Line: byte * abSrcWaterHeight = abSrcWaterData + sizeof(m_abyWaterMap);
				...
				{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(m_lWaterHeight, abSrcWaterHeight, sizeof(int) * m_byNumWater);
				}
			}
		...
		public LPDIRECT3DTEXTURE8 GetShadowTexture()
		{
		...
		public uint GetShadowMapColor(float fx, float fy)
		{
			...
			if (fx < 0F || fy < 0F || fx >= fMapSize || fy >= fMapSize)
			{
				return 0xFFFFFFFF;
			}
			...
			ushort w = *(m_awShadowMap + (iy * SHADOWMAP_XSIZE) + ix);
			...
			return (uint)(0xff << 24) | (g << 16) | (g << 8) | r;
		}
...
//		void LoadTextures();
		protected bool LoadHeightMap(string c_szFileName)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(m_awRawHeightMap, lpcvFileData, sizeof(ushort) * HEIGHTMAP_RAW_XSIZE * HEIGHTMAP_RAW_YSIZE);

			return true;
		}

		protected bool RAW_LoadTileMap(string c_szFileName)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		protected class SAttrMapHeader
		{
			public ushort m_wMagic;
			public ushort m_wWidth;
			public ushort m_wHeight;
		}
		protected bool LoadAttrMap(string c_szFileName)
		{
			uint dwStart = ELTimer_GetMSec();
...
//Original Metin2 CPlus Line: byte * abFileData = (byte *) lpcvFileData;
		...
		//#pragma pack(pop)
				...
				SAttrMapHeader kAttrMapHeader = new SAttrMapHeader();
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(kAttrMapHeader, abFileData, sizeof(SAttrMapHeader));

				const ushort c_wAttrMapMagic = 2634;
				...
				uint dwFileRestSize = dwFileSize - sizeof(SAttrMapHeader);
				uint dwFileNeedSize = sizeof(m_abyAttrMap);
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte* abSrcAttrData= abFileData+sizeof(kAttrMapHeader);
				byte abSrcAttrData = abFileData + sizeof(SAttrMapHeader);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		protected ushort GetHeightMapValue(short sx, short sy)
		{
			return m_awRawHeightMap[(sy + 1) * HEIGHTMAP_RAW_XSIZE + sx + 1];
		}

		protected LPDIRECT3DTEXTURE8[] m_lpAlphaTexture = Arrays.InitializeWithDefaultInstances<LPDIRECT3DTEXTURE8>(DefineConstants.MAXTERRAINTEXTURES);
		protected ushort[] m_awRawHeightMap = new ushort[HEIGHTMAP_RAW_YSIZE * HEIGHTMAP_RAW_XSIZE];

		protected byte[] m_abyTileMap = new byte[TILEMAP_RAW_YSIZE * TILEMAP_RAW_XSIZE];
		protected byte[] m_abyAttrMap = new byte[ATTRMAP_YSIZE * ATTRMAP_XSIZE];
		protected byte[] m_abyWaterMap = new byte[WATERMAP_YSIZE * WATERMAP_XSIZE];
		protected string m_acNormalMap = new string(new char[NORMALMAP_YSIZE * NORMALMAP_XSIZE * 3]);
		protected TGA_HEADER m_HeightMapHeader = new TGA_HEADER();
		protected ushort m_wTileMapVersion;
		protected int m_lViewRadius;
		protected float m_fHeightScale;
		protected TTerrainSplatPatch m_TerrainSplatPatch = new TTerrainSplatPatch();
		protected byte m_byNumWater;
		protected int[] m_lWaterHeight = new int[MAX_WATER_NUM + 1];
		protected LPDIRECT3DTEXTURE8 m_lpShadowTexture = new LPDIRECT3DTEXTURE8();
		protected ushort[] m_awShadowMap = new ushort[SHADOWMAP_YSIZE * SHADOWMAP_XSIZE];

		protected int m_lSplatTilesX;
		protected int m_lSplatTilesY;
}
