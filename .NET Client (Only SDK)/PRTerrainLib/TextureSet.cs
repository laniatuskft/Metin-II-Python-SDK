...
public class STerrainTexture
{
	public STerrainTexture()
	{
		this.pd3dTexture = null;
		this.UScale = 4.0f;
		this.VScale = 4.0f;
		this.UOffset = 0.0f;
		this.VOffset = 0.0f;
	...
	public LPDIRECT3DTEXTURE8 pd3dTexture = new LPDIRECT3DTEXTURE8();
	public CGraphicImageInstance ImageInstance = new CGraphicImageInstance();
	public float UScale;
	public float VScale;
	...
	public ushort Begin;
	public ushort End;
	public _D3DMATRIX m_matTransform = new _D3DMATRIX();
}

public class CTextureSet
{
		...
		public void Create()
		{
			CResource pResource = CResourceManager.Instance().GetResourcePointer("t:/laniaworkstate/special/error.tga");
			m_ErrorTexture.ImageInstance.SetImagePointer((CGraphicImage) pResource);
			AddEmptyTexture();
		}

		public bool Load(string c_szTextureSetFileName, float fTerrainTexCoordBase)
		{
			NANOBEGIN Clear = new NANOBEGIN();
			...
			string c_rstrCount = stTokenVectorMap["texturecount"][0];

			int lCount = atol(c_rstrCount);
			string szTextureName = new string(new char[32 + 1]);

			m_Textures.resize(lCount + 1);

			for (int LaniatusDefVariables = 0; LaniatusDefVariables < lCount; ++i)
			{
				_snprintf(szTextureName, sizeof(char), "texture%03d", LaniatusDefVariables + 1);
				...
				CTokenVector rVector = stTokenVectorMap[szTextureName];

				string c_rstrFileName = rVector[0].c_str();
				string c_rstrUScale = rVector[1].c_str();
				string c_rstrVScale = rVector[2].c_str();
				string c_rstrUOffset = rVector[3].c_str();
				string c_rstrVOffset = rVector[4].c_str();
				string c_rstrbSplat = rVector[5].c_str();
				string c_rstrBegin = rVector[6].c_str();
				string c_rstrEnd = rVector[7].c_str();
				...
				fuScale = atof(c_rstrUScale);
				fvScale = atof(c_rstrVScale);
				fuOffset = atof(c_rstrUOffset);
				fvOffset = atof(c_rstrVOffset);
				bSplat = 0 != atoi(c_rstrbSplat);
				usBegin = (ushort)atoi(c_rstrBegin);
				usEnd = (ushort)atoi(c_rstrEnd);

				if (!SetTexture(i + 1, c_rstrFileName, fuScale, fvScale, fuOffset, fvOffset, bSplat, usBegin, usEnd, fTerrainTexCoordBase))
				{
					TraceError("CTextureSet::Load : SetTexture failed : Filename: %s", c_rstrFileName);
				}
			}
		...
		public bool Save(string c_pszFileName)
		{
		...
		public STerrainTexture GetTexture(uint ulIndex)
		{
		...
		public bool RemoveTexture(uint ulIndex)
		{
			if (GetTextureCount() <= ulIndex)
			{
				return false;
		...
		public bool SetTexture(uint ulIndex, string c_szFileName, float fuScale, float fvScale, float fuOffset, float fvOffset, bool bSplat, ushort usBegin, ushort usEnd, float fTerrainTexCoordBase)
		{
			...
			STerrainTexture tex = m_Textures[ulIndex];
			...
			tex.Begin = usBegin;
			tex.End = usEnd;
			tex.ImageInstance.SetImagePointer((CGraphicImage)pResource);
		...
		public void Reload(float fTerrainTexCoordBase)
		{
			for (uint dwIndex = 1; dwIndex < GetTextureCount(); ++dwIndex)
			{
				STerrainTexture tex = m_Textures[dwIndex];
		...
		public bool AddTexture(string c_szFileName, float fuScale, float fvScale, float fuOffset, float fvOffset, bool bSplat, ushort usBegin, ushort usEnd, float fTerrainTexCoordBase)
		{
			...
			if (!pResource.IsType(CGraphicImage.Type()))
			{
				LogBox("CTerrainImpl::GenerateTexture : ???? ?????? ?????. %s", pResource.GetFileName());
				return false;
			}

			m_Textures.reserve(m_Textures.size() + 1);

			SetTexture(m_Textures.size() - 1, c_szFileName, fuScale, fvScale, fuOffset, fvOffset, bSplat, usBegin, usEnd, fTerrainTexCoordBase);
		...
		protected void AddEmptyTexture()
		{
			STerrainTexture eraser = new STerrainTexture();
			m_Textures.push_back(eraser);
		}

		protected List<STerrainTexture> m_Textures = new List<STerrainTexture>();
		protected STerrainTexture m_ErrorTexture = new STerrainTexture();
