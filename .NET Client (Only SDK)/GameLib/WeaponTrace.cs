...
public class CWeaponTrace
{
		...
		public static void Delete(CWeaponTrace pkWTDel)
		{
			Debug.Assert(pkWTDel != null && "CWeaponTrace::Delete");

			pkWTDel.Clear();
			ms_kPool.Free(pkWTDel);
		}

		public static CWeaponTrace New()
		{
		...
		public void SetTexture(string c_szFileName)
		{
		...
		public bool SetWeaponInstance(CGraphicThingInstance pInstance, uint dwModelIndex, string c_szBoneName)
		{
			pInstance.Update();
			...
			_D3DVECTOR v3Bone = new _D3DVECTOR(pmat._41,pmat._42,pmat._43);

			m_fLength = sqrtf(fMAX(D3DXVec3LengthSq((v3Bone - v3Min)), D3DXVec3LengthSq((v3Bone - v3Max))));
		...
		public void SetLifeTime(float fLifeTime)
		{
			m_fLifeTime = fLifeTime;
		}

		public void SetSamplingTime(float fSamplingTime)
		{
			m_fSamplingTime = fSamplingTime;
		}

		public void Update(float fReachScale)
		{
			...
			{
				TTimePointList.iterator it = new TTimePointList.iterator();
			...
			if (m_isPlaying && m_fz >= 0.0001f)
			{
				...
				if (m_pInstance.GetCompositeBoneMatrix(m_dwModelInstanceIndex, m_iBoneIndex, pMatrix))
				{
					...
					_D3DMATRIX mat = new _D3DMATRIX(pMatrix);
					...
					D3DXMatrixRotationZ(matRotation, ((m_fRotation) * (((float) 3.141592654f) / 180.0f)));
...
//Original Metin2 CPlus Line: matPoint = mat * matRotation;
					matPoint.CopyFrom(mat * matRotation);
					m_ShortTimePointList.push_front(TTimePoint(0.0f, D3DXVECTOR3(m_fx + matPoint._41, m_fy + matPoint._42, m_fz + matPoint._43)));

//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: matPoint = matTranslation * matPoint;
					matPoint.CopyFrom(matTranslation * matPoint);

					m_LongTimePointList.push_front(TTimePoint(0.0f, D3DXVECTOR3(m_fx + matPoint._41, m_fy + matPoint._42, m_fz + matPoint._43)));
				}
			}
		...
		public void Render()
		{
			...
			(CStateManager.Instance()).SaveTransform(D3DTS_WORLD, matWorld);
			(CStateManager.Instance()).SaveVertexShader(D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1);
			...
			(CStateManager.Instance()).SaveRenderState(D3DRS_ALPHAREF, 0x00000011);
			...
			(CStateManager.Instance()).SaveRenderState(D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
			...
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TEXTURE);
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, (m_bUseTexture)?D3DTOP_SELECTARG2:D3DTOP_SELECTARG1);
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TEXTURE);
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAOP, (m_bUseTexture)?D3DTOP_SELECTARG2:D3DTOP_SELECTARG1);
			...
			(CStateManager.Instance()).SetTexture(0, lpTexture);
			(CStateManager.Instance()).SetTexture(1, null);
			(CStateManager.Instance()).DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, (int)(m_PDTVertexVector.size() - 2), m_PDTVertexVector[0], sizeof(SPDTVertex));

			(CStateManager.Instance()).SetRenderState(D3DRS_LIGHTING, true);

			(CStateManager.Instance()).RestoreRenderState(D3DRS_ZENABLE);
			(CStateManager.Instance()).RestoreRenderState(D3DRS_ZFUNC);
		...
		protected bool BuildVertex()
		{
			const int max_size = 300;
			float[] h = new float[max_size];
			float[] stk = new float[max_size];
			int sp = 0;
			_D3DVECTOR[] r = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(max_size);
			...
			List<SPDTVertex> m_ShortVertexVector = new List<SPDTVertex>();
			List<SPDTVertex> m_LongVertexVector = new List<SPDTVertex>();

			float length = Math.Min(m_fLifeTime, m_LongTimePointList.back().first);

			int n = m_LongTimePointList.size() - 1;
			Debug.Assert(n < max_size-1);

			for (int loop = 0; loop <= 1; ++loop)
			{
				TTimePointList Input = (loop) != 0 ? m_LongTimePointList : m_ShortTimePointList;
				List<SPDTVertex> Output = (loop) != 0 ? m_LongVertexVector : m_ShortVertexVector;
				TTimePointList.iterator it = new TTimePointList.iterator();
				int i;
				...
				int @base = 0;
				_D3DVECTOR a = new _D3DVECTOR();
				_D3DVECTOR b = new _D3DVECTOR();
				_D3DVECTOR c = new _D3DVECTOR();
				_D3DVECTOR d = new _D3DVECTOR();
				_D3DVECTOR v3Tmp = Input[@base+1].second - Input[@base].second;
				float timebase = 0F;
				float timenext = h[@base];
				float dt = m_fSamplingTime;
				a = Input[@base].second;
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: b = r[base];
				b.CopyFrom(r[@base]);
...
//Original Metin2 CPlus Line: b = r[base];
					...
					SPDTVertex v = new SPDTVertex();
					float ttt = Math.Min(Math.Max((t + Input[0].first) / m_fLifeTime,0.0f),1.0f);
					v.diffuse = D3DXCOLOR(0.3f,0.8f,1.0f, (loop) != 0?Math.Min(Math.Max((1.0f - ttt) * (1.0f - ttt) / 2.5 - 0.1f,0.0f),1.0f):0.0f);
					v.position = a + cc * (b + cc * (c + cc * d));
					v.texCoord.x = t / m_fLifeTime;
					v.texCoord.y = loop != 0 ? 0 : 1;
					Output.Add(v);
					if (loop != 0)
					{
					}
			...
			List<SPDTVertex>.Enumerator lit;
			List<SPDTVertex>.Enumerator sit;
			for (lit = m_LongVertexVector.GetEnumerator(), sit = m_ShortVertexVector.GetEnumerator(); lit.MoveNext(); ++lit,++sit)
			{
					m_PDTVertexVector.push_backlit.Current;
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		protected float m_fLastUpdate;

		protected LinkedList<Tuple<float, _D3DVECTOR>> m_ShortTimePointList = new LinkedList<Tuple<float, _D3DVECTOR>>();
		protected LinkedList<Tuple<float, _D3DVECTOR>> m_LongTimePointList = new LinkedList<Tuple<float, _D3DVECTOR>>();

		protected List<SPDTVertex> m_PDTVertexVector = new List<SPDTVertex>();

		protected float m_fLifeTime;
		protected float m_fSamplingTime;

		protected CGraphicThingInstance m_pInstance;
		protected uint m_dwModelInstanceIndex;

		protected CGraphicImageInstance m_ImageInstance = new CGraphicImageInstance();
		...
		protected static CDynamicPool<CWeaponTrace> ms_kPool = new CDynamicPool<CWeaponTrace>();
}