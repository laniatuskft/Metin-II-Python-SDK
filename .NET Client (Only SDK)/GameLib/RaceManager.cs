...
public class CRaceManager : CSingleton<CRaceManager>
{
		...
		public void RegisterRaceName(uint dwRaceIndex, string c_szName)
		{
			m_kMap_dwRaceKey_stRaceName.insert(SortedDictionary<uint, string>.value_type(dwRaceIndex, c_szName));
		}

#if ENABLE_LOAD_PLAYERSETTING
		public void RegisterRaceSrcName(in string c_szName, in string c_szSrcName)
#else
		public void RegisterRaceSrcName(string c_szName, string c_szSrcName)
#endif
		{
			m_kMap_stRaceName_stSrcName.insert(SortedDictionary<string, string>.value_type(c_szName, c_szSrcName));
		}

#else
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RegisterRaceName(uint dwRaceIndex, string c_szName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RegisterRaceSrcName(string c_szName, string c_szSrcName);
#endif
		public void SetPathName(string c_szPathName)
		{
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string GetFullPathFileName_s_stFileName = "";

		public string GetFullPathFileName(string c_szFileName)
		{
...
//			static string s_stFileName;

			if (c_szFileName[0] != '.')
			{
				GetFullPathFileName_s_stFileName = m_strPathName;
				GetFullPathFileName_s_stFileName += c_szFileName;
			}
			else
			{
				GetFullPathFileName_s_stFileName = c_szFileName;
			...
			return GetFullPathFileName_s_stFileName.c_str();
		}

		public void CreateRace(uint dwRaceIndex)
		{
		...
		public void SelectRace(uint dwRaceIndex)
		{
			TRaceDataIterator itor = m_RaceDataMap.find(dwRaceIndex);
			if (m_RaceDataMap.end() == itor)
			{
				Debug.Assert(!"Failed to select race data!");
				return;
			}
		...
		public CRaceData GetSelectedRaceDataPointer()
		{
			return m_pSelectedRaceData;
		}

		public bool GetRaceDataPointer(uint dwRaceIndex, CRaceData[] ppRaceData)
		{
			TRaceDataIterator itor = m_RaceDataMap.find(dwRaceIndex);
...
//Original Metin2 CPlus Line: *ppRaceData = pRaceData;
					ppRaceData[0].CopyFrom(pRaceData);
					return true;
				}
			...
			ppRaceData[0] = itor.second;
			return true;
		}


		protected CRaceData __LoadRaceData(uint dwRaceIndex)
		{
			SortedDictionary<uint, string>.Enumerator fRaceName = m_kMap_dwRaceKey_stRaceName.find(dwRaceIndex);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			string c_rstRaceName = fRaceName.second;

			if (string.IsNullOrEmpty(c_rstRaceName))
			{
				return null;
			}
			...
			if (c_rstRaceName[0] == '#')
			{
				string pathName = c_rstRaceName + 1;
				string shapeFileName = new string(new char[256]);
				string motionListFileName = new string(new char[256]);
				_snprintf(shapeFileName, sizeof(char), "%sshape.msm", pathName);
				_snprintf(motionListFileName, sizeof(char), "%smotlist.txt", pathName);
			...
			List<string> vec_stFullPathName = new List<string>();
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
				...
				if (!pRaceData.LoadRaceData(stMSMFileName))
				{
					if (i != vec_stFullPathName.Count - 1)
					{
						TraceError("CRaceManager::RegisterRacePath : RACE[%u] LOAD MSMFILE[%s] ERROR. Will Find Another Path.", dwRaceIndex, stMSMFileName);
						continue;
					}

					TraceError("CRaceManager::RegisterRacePath : RACE[%u] LOAD MSMFILE[%s] ERROR", dwRaceIndex, stMSMFileName);
					CRaceData.Delete(pRaceData);
					return null;
				}

				string stMotionListFileName = stFullPathName;
				stMotionListFileName += pRaceData.GetMotionListFileName();

				__LoadRaceMotionList(pRaceData, stFullPathName, stMotionListFileName);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private SortedDictionary<string, uint> __LoadRaceMotionList_s_kMap_stType_dwIndex = new SortedDictionary<string, uint>();
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private bool __LoadRaceMotionList_s_isInit = false;
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string __LoadRaceMotionList_stSpawnMotionFileName = "";
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string __LoadRaceMotionList_stMotionFileName = "";

		protected bool __LoadRaceMotionList(CRaceData rkRaceData, string pathName, string motionListFileName)
		{
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static System.Collections.Generic.SortedDictionary<string, uint> s_kMap_stType_dwIndex;
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static bool s_isInit=false;

			if (!__LoadRaceMotionList_s_isInit)
			{
				__LoadRaceMotionList_s_isInit = true;

				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPAWN", CRaceMotionData.NAME_SPAWN));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WAIT", CRaceMotionData.NAME_WAIT));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WAIT1", CRaceMotionData.NAME_WAIT));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WAIT2", CRaceMotionData.NAME_WAIT));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WALK", CRaceMotionData.NAME_WALK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WALK1", CRaceMotionData.NAME_WALK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("WALK2", CRaceMotionData.NAME_WALK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("RUN", CRaceMotionData.NAME_RUN));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("RUN1", CRaceMotionData.NAME_RUN));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("RUN2", CRaceMotionData.NAME_RUN));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("STOP", CRaceMotionData.NAME_STOP));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("DEAD", CRaceMotionData.NAME_DEAD));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("COMBO_ATTACK", CRaceMotionData.NAME_COMBO_ATTACK_1));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("COMBO_ATTACK1", CRaceMotionData.NAME_COMBO_ATTACK_2));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("COMBO_ATTACK2", CRaceMotionData.NAME_COMBO_ATTACK_3));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("NORMAL_ATTACK", CRaceMotionData.NAME_NORMAL_ATTACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("NORMAL_ATTACK1", CRaceMotionData.NAME_NORMAL_ATTACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("NORMAL_ATTACK2", CRaceMotionData.NAME_NORMAL_ATTACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DAMAGE", CRaceMotionData.NAME_DAMAGE));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DAMAGE1", CRaceMotionData.NAME_DAMAGE));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DAMAGE2", CRaceMotionData.NAME_DAMAGE));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DAMAGE3", CRaceMotionData.NAME_DAMAGE));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DEAD", CRaceMotionData.NAME_DEAD));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DEAD1", CRaceMotionData.NAME_DEAD));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_DEAD2", CRaceMotionData.NAME_DEAD));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_KNOCKDOWN", CRaceMotionData.NAME_DAMAGE_FLYING));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_KNOCKDOWN1", CRaceMotionData.NAME_DAMAGE_FLYING));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_STANDUP", CRaceMotionData.NAME_STAND_UP));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("FRONT_STANDUP1", CRaceMotionData.NAME_STAND_UP));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_DAMAGE", CRaceMotionData.NAME_DAMAGE_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_DAMAGE1", CRaceMotionData.NAME_DAMAGE_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_DEAD", CRaceMotionData.NAME_DEAD_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_DEAD1", CRaceMotionData.NAME_DEAD_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_DEAD2", CRaceMotionData.NAME_DEAD_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_KNOCKDOWN", CRaceMotionData.NAME_DAMAGE_FLYING_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_KNOCKDOWN1", CRaceMotionData.NAME_DAMAGE_FLYING_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_STANDUP", CRaceMotionData.NAME_STAND_UP_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("BACK_STANDUP1", CRaceMotionData.NAME_STAND_UP_BACK));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL", CRaceMotionData.NAME_SPECIAL_1));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL1", CRaceMotionData.NAME_SPECIAL_2));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL2", CRaceMotionData.NAME_SPECIAL_3));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL3", CRaceMotionData.NAME_SPECIAL_4));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL4", CRaceMotionData.NAME_SPECIAL_5));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SPECIAL5", CRaceMotionData.NAME_SPECIAL_6));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SKILL1", CRaceMotionData.NAME_SKILL + 121));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SKILL2", CRaceMotionData.NAME_SKILL + 122));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SKILL3", CRaceMotionData.NAME_SKILL + 123));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SKILL4", CRaceMotionData.NAME_SKILL + 124));
				__LoadRaceMotionList_s_kMap_stType_dwIndex.insert(SortedDictionary<string, uint>.value_type("SKILL5", CRaceMotionData.NAME_SKILL + 125));
			}

			object pvData;
			CMappedFile kMappedFile = new CMappedFile();
			...
			CMemoryTextFileLoader kTextFileLoader = new CMemoryTextFileLoader();
			kTextFileLoader.Bind(kMappedFile.Size(), pvData);

			rkRaceData.RegisterMotionMode(CRaceMotionData.MODE_GENERAL);

			string szMode = new string(new char[256]);
			string szType = new string(new char[256]);
			string szFile = new string(new char[256]);
			int nPercent = 0;

			bool isSpawn = false;

//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static string stSpawnMotionFileName;
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//			static string stMotionFileName;

			__LoadRaceMotionList_stSpawnMotionFileName = "";
			__LoadRaceMotionList_stMotionFileName = "";

			uint uLineCount = kTextFileLoader.GetLineCount();
			for (uint uLineIndex = 0; uLineIndex < uLineCount; ++uLineIndex)
			{
				uint motionType = CRaceMotionData.NAME_NONE;

				string c_rstLine = kTextFileLoader.GetLineString(uLineIndex);
				sscanf(c_rstLine, "%s %s %s %d", szMode, szType, szFile, nPercent);

				SortedDictionary<string, uint>.Enumerator fTypeIndex = __LoadRaceMotionList_s_kMap_stType_dwIndex.find(szType);

				if (__LoadRaceMotionList_s_kMap_stType_dwIndex.end() == fTypeIndex)
				{
					...
					{
						for (size_t LaniatusDefVariables = 1; LaniatusDefVariables <= c_cutLengthLimit; ++i)
						{
							string typeName = new string(szType).substr(0, strlen(szType) - i);
							fTypeIndex = __LoadRaceMotionList_s_kMap_stType_dwIndex.find(typeName);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
				...
				__LoadRaceMotionList_stMotionFileName = pathName;
				__LoadRaceMotionList_stMotionFileName += szFile;

				rkRaceData.RegisterMotionData(CRaceMotionData.MODE_GENERAL, motionType, __LoadRaceMotionList_stMotionFileName.c_str(), nPercent);
				...
				switch (motionType)
				{
					...
					case CRaceMotionData.NAME_DAMAGE:
						__LoadRaceMotionList_stSpawnMotionFileName = __LoadRaceMotionList_stMotionFileName;
						break;
				}
			}

			if (!isSpawn && __LoadRaceMotionList_stSpawnMotionFileName != "")
			{
				rkRaceData.RegisterMotionData(CRaceMotionData.MODE_GENERAL, CRaceMotionData.NAME_SPAWN, __LoadRaceMotionList_stSpawnMotionFileName.c_str(), nPercent);
			}
		...
		protected void __DestroyRaceDataMap()
		{
			TRaceDataMap.iterator LaniatusDefVariables = new TRaceDataMap.iterator();
			for (i = m_RaceDataMap.begin(); LaniatusDefVariables != m_RaceDataMap.end(); ++i)
			{
		...
		protected SortedDictionary<uint, CRaceData > m_RaceDataMap = new SortedDictionary<uint, CRaceData >();

		protected SortedDictionary<string, string> m_kMap_stRaceName_stSrcName = new SortedDictionary<string, string>();
		protected SortedDictionary<uint, string> m_kMap_dwRaceKey_stRaceName = new SortedDictionary<uint, string>();

		private string m_strPathName = "";
		private CRaceData m_pSelectedRaceData;
