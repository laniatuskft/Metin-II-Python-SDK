...
public class TEMP_CAreaLoaderThread
{
	...
	public TEMP_CAreaLoaderThread()
	{
		this.m_bShutdowned = false;
		this.m_pArg = null;
		this.m_hThread = null;
		this.m_uThreadID = 0;
	...
	public void Request(CTerrain pTerrain)
	{
	...
	public bool Fetch(CTerrain[] ppTerrain)
	{
		...
		ppTerrain[0] = m_pTerrainCompleteDeque.front();
	...
	public void Request(CArea pArea)
	{
	...
	public bool Fetch(CArea[] ppArea)
	{
		...
		ppArea[0] = m_pAreaCompleteDeque.front();
...
//Original Metin2 CPlus Line: object* Arg() const
	...
	protected uint Setup()
	{
		m_hSemaphore = CreateSemaphore(null, 0, 65535, null);
		if (!m_hSemaphore)
		{
			return 0;
		}
	...
	protected uint Execute(object pvArg)
	{
	...
	protected void ProcessTerrain()
	{
		...
		uint dwID = (uint)(wCoordX) * 1000 + (uint)(wCoordY);

		string c_rStrMapName = pTerrain.GetOwner().GetName();
		string filename = new string(new char[256]);
		sprintf(filename, "%s\\%06u\\AreaProperty.txt", c_rStrMapName, dwID);
		...
		_snprintf(szRawHeightFieldname, sizeof(char), "%s\\%06u\\height.raw", c_rStrMapName, dwID);
		_snprintf(szSplatName, sizeof(char), "%s\\%06u\\tile.raw", c_rStrMapName, dwID);
		_snprintf(szAttrMapName, sizeof(char), "%s\\%06u\\attr.atr", c_rStrMapName, dwID);
		_snprintf(szWaterMapName, sizeof(char), "%s\\%06u\\water.wtr", c_rStrMapName, dwID);
		_snprintf(szShadowTexName, sizeof(char), "%s\\%06u\\shadowmap.dds", c_rStrMapName, dwID);
		_snprintf(szShadowMapName, sizeof(char), "%s\\%06u\\shadowmap.raw", c_rStrMapName, dwID);
		_snprintf(szMiniMapTexName, sizeof(char), "%s\\%06u\\minimap.dds", c_rStrMapName, dwID);
	...
	protected void ProcessArea()
	{
		...
		uint dwID = (uint)(wAreaCoordX) * 1000 + (uint)(wAreaCoordY);

		string c_rStrMapName = pArea.GetOwner().GetName();

		string szAreaPathName = new string(new char[64 + 1]);
		_snprintf(szAreaPathName, sizeof(char), "%s\\%06u\\", c_rStrMapName, dwID);
	...
	private LinkedList<CTerrain > m_pTerrainRequestDeque = new LinkedList<CTerrain >();
	private Mutex m_TerrainRequestMutex = new Mutex();

	private LinkedList<CTerrain > m_pTerrainCompleteDeque = new LinkedList<CTerrain >();
	private Mutex m_TerrainCompleteMutex = new Mutex();

	private LinkedList<CArea > m_pAreaRequestDeque = new LinkedList<CArea >();
	private Mutex m_AreaRequestMutex = new Mutex();

	private LinkedList<CArea > m_pAreaCompleteDeque = new LinkedList<CArea >();
	private Mutex m_AreaCompleteMutex = new Mutex();

	private IntPtr m_hSemaphore;
	private int m_iRestSemCount;
	private bool m_bShutdowned;
