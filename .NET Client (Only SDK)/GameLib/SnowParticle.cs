...
public class SParticleVertex
{
	public D3DXVECTOR3 v3Pos = new D3DXVECTOR3();
	public float u;
	public float v;
}

public class BlurVertex
{
	public D3DXVECTOR3 pos = new D3DXVECTOR3();
	public float rhw;
	public uint color;
	public float tu;
	public float tv;

	public const uint FVF;

	public BlurVertex(D3DXVECTOR3 p, float w, uint c, float u, float v)
	{
		this.pos = p;
		this.rhw = w;
		this.color = c;
...
public class CSnowParticle
{
		public CSnowParticle()
		{
		}
		...
		public static CSnowParticle New()
		{
		...
		public static void Delete(CSnowParticle pSnowParticle)
		{
		...
		public void Init(in D3DXVECTOR3 c_rv3Pos)
		{
			float fRot = frandom(0.0f, 36000.0f) / 100.0f;
			float fDistance = frandom(0.0f, c_fSnowDistance) / 10.0f;

			m_v3Position.x = c_rv3Pos.x + fDistance * Math.Sin((double)D3DXToRadian(fRot));
			m_v3Position.y = c_rv3Pos.y + fDistance * Math.Cos((double)D3DXToRadian(fRot));
		...
		public void SetCameraVertex(in D3DXVECTOR3 rv3Up, in D3DXVECTOR3 rv3Cross)
		{
		...
		public void Update(float fElapsedTime, in D3DXVECTOR3 c_rv3Pos)
		{
			m_v3Position += m_v3Velocity * fElapsedTime;

			m_v3Position.x += m_v3Cross.x * Math.Sin(m_fcurRadian) / 10.0f;
			m_v3Position.y += m_v3Cross.y * Math.Sin(m_fcurRadian) / 10.0f;
			...
			else if (Math.Abs(m_v3Position.x - c_rv3Pos.x) > c_fSnowDistance)
			{
				m_bActivate = false;
			}
			else if (Math.Abs(m_v3Position.y - c_rv3Pos.y) > c_fSnowDistance)
			{
				m_bActivate = false;
			}
		}

		public void GetVerticies(SParticleVertex rv3Vertex1, SParticleVertex rv3Vertex2, SParticleVertex rv3Vertex3, SParticleVertex rv3Vertex4)
		{
		...
		protected bool m_bChangedSize;
		protected float m_fHalfWidth;
		protected float m_fHalfHeight;

		protected D3DXVECTOR3 m_v3Velocity = new D3DXVECTOR3();
		protected D3DXVECTOR3 m_v3Position = new D3DXVECTOR3();

		protected D3DXVECTOR3 m_v3Up = new D3DXVECTOR3();
		protected D3DXVECTOR3 m_v3Cross = new D3DXVECTOR3();

		protected float m_fPeriod;
		protected float m_fcurRadian;
		protected float m_fAmplitude;

		public static List<CSnowParticle> ms_kVct_SnowParticlePool = new List<CSnowParticle>();
}