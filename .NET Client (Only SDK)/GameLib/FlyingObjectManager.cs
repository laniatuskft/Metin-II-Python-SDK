...
public class CFlyingManager : CSingleton<CFlyingManager>
{
	public enum EIndexFlyType
	{
	...
	public bool RegisterFlyingData(string c_szFilename)
	{
		string s = "";
		StringPath(c_szFilename,s);
		uint dwRetCRC = GetCaseCRC32(s,s.Length);
		...
		m_kMap_pkFlyData.insert(Tuple.Create(dwRetCRC,pFlyingData));
		return true;
	}

	public bool RegisterFlyingData(string c_szFilename, ref uint r_dwRetCRC)
	{
		string s = "";
		StringPath(c_szFilename,s);
		r_dwRetCRC = GetCaseCRC32(s,s.Length);
		...
		m_kMap_pkFlyData.insert(Tuple.Create(r_dwRetCRC,pFlyingData));
		return true;
	}

	public CFlyingInstance CreateFlyingInstanceFlyTarget(in uint dwID, in _D3DVECTOR v3StartPosition, in CFlyTarget cr_FlyTarget, bool canAttack)
	{
	...
	public CFlyingInstance CreateIndexedFlyingInstanceFlyTarget(in uint dwIndex, in _D3DVECTOR v3StartPosition, in CFlyTarget cr_FlyTarget)
	{
	...
	public void Update()
	{
		TFlyingInstanceList.iterator LaniatusDefVariables = m_kLst_pkFlyInst.begin();
	...
	public void SetMapManagerPtr(CMapManager pMapManager)
	{
		m_pMapManager = pMapManager;
	}
	public CMapManager GetMapManagerPtr()
	{
		return m_pMapManager;
	}

	public bool RegisterIndexedFlyData(uint dwIndex, byte byType, string c_szFileName)
	{
		uint dwCRC;
		...
		TIndexFlyData IndexFlyData = new TIndexFlyData();
		IndexFlyData.byType = byType;
		IndexFlyData.dwCRC = dwCRC;
		m_kMap_dwIndexFlyData.insert(Tuple.Create(dwIndex, IndexFlyData));

		return true;
	}

	public void CreateIndexedFly(uint dwIndex, CActorInstance pStartActor, CActorInstance pEndActor)
	{
		if (m_kMap_dwIndexFlyData.end() == m_kMap_dwIndexFlyData.find(dwIndex))
		{
		...
		TPixelPosition posStart = new TPixelPosition();
		...
		switch (rIndexFlyData.byType)
		{
			...
			case INDEX_FLY_TYPE_FIRE_CRACKER:
			{
				...
				float fxRand = fDistance * cosf(((fRot) * (((float) 3.141592654f) / 180.0f)));
				float fyRand = fDistance * sinf(((fRot) * (((float) 3.141592654f) / 180.0f)));
				float fzRand = frandom(1000.0f, 2500.0f);

				CreateFlyingInstanceFlyTarget(rIndexFlyData.dwCRC, D3DXVECTOR3(posStart.x, posStart.y, posStart.z + 200), D3DXVECTOR3(posStart.x + fxRand, posStart.y + fyRand, posStart.z + fzRand), false);
				break;
			}
	...
	private void __DestroyFlyingInstanceList()
	{
		TFlyingInstanceList.iterator LaniatusDefVariables = new TFlyingInstanceList.iterator();
		for (i = m_kLst_pkFlyInst.begin(); LaniatusDefVariables != m_kLst_pkFlyInst.end(); ++i)
		{
			CFlyingInstance pkFlyInst = *i;
			CFlyingInstance.Delete(pkFlyInst);
		}
	...
	private void __DestroyFlyingDataMap()
	{
		TFlyingDataMap.iterator LaniatusDefVariables = new TFlyingDataMap.iterator();
		for (i = m_kMap_pkFlyData.begin(); LaniatusDefVariables != m_kMap_pkFlyData.end(); ++i)
		{
	...
	private class SIndexFlyData
	{
		public byte byType;
		public uint dwCRC;
	}

	private SortedDictionary<uint, CFlyingData > m_kMap_pkFlyData = new SortedDictionary<uint, CFlyingData >();
	private LinkedList<CFlyingInstance > m_kLst_pkFlyInst = new LinkedList<CFlyingInstance >();
	private SortedDictionary<uint, SIndexFlyData> m_kMap_dwIndexFlyData = new SortedDictionary<uint, SIndexFlyData>();

	private CMapManager m_pMapManager;
