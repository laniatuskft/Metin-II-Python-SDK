...
public class CItemManager : CSingleton<CItemManager>
{
		public enum EItemDescCol
		{
			ITEMDESC_COL_VNUM,
			ITEMDESC_COL_NAME,
			ITEMDESC_COL_DESC,
		...
		public void Destroy()
		{
			TItemMap.iterator LaniatusDefVariables = new TItemMap.iterator();
			for (i = m_ItemMap.begin(); LaniatusDefVariables != m_ItemMap.end(); ++i)
			{
		...
		public bool SelectItemData(uint dwIndex)
		{
		...
		public CItemData GetSelectedItemDataPointer()
		{
			return m_pSelectedItemData;
		}

		public bool GetItemDataPointer(uint dwItemID, CItemData[] ppItemData)
		{
			if (0 == dwItemID)
			{
				return false;
...
//Original Metin2 CPlus Line: *ppItemData = p;
						ppItemData[0].CopyFrom(p);
						return true;
					}
				}
			...
			ppItemData[0] = f.second;

			return true;
		}

		public bool LoadItemDesc(string c_szFileName)
		{
			object pvData;
			...
			List<string> kTokenVector = new List<string>();
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < kTextFileLoader.GetLineCount(); ++i)
			{
				...
				while (kTokenVector.Count < ITEMDESC_COL_NUM)
				{
					kTokenVector.Add("");
				}

				uint dwVnum = atoi(kTokenVector[ITEMDESC_COL_VNUM]);
				string c_rstDesc = kTokenVector[ITEMDESC_COL_DESC];
				string c_rstSumm = kTokenVector[ITEMDESC_COL_SUMM];
		...
		public bool LoadItemList(string c_szFileName)
		{
			CMappedFile File = new CMappedFile();
			LPCVOID pData = new LPCVOID();
			...
			List<string> TokenVector = new List<string>();
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < textFileLoader.GetLineCount(); ++i)
			{
				...
				if (!(TokenVector.Count == 3 || TokenVector.Count == 4))
				{
					TraceError(" CItemManager::LoadItemList(%s) - StrangeLine in %d\n", c_szFileName, i);
					continue;
				}

				string c_rstrID = TokenVector[0];
				string c_rstrIcon = TokenVector[2];

				uint dwItemVNum = atoi(c_rstrID);
...
//				extern bool USE_VIETNAM_CONVERT_WEAPON_VNUM;
				...
				{
//# Laniatus Games Studio Inc. |: 'extern' variable declarations are not required in C#:
//					extern uint Vietnam_ConvertWeaponVnum(uint vnum);
					uint dwMildItemVnum = Vietnam_ConvertWeaponVnum(dwItemVNum);
					if (dwMildItemVnum == dwItemVNum)
					{
						if (4 == TokenVector.Count)
						{
							string c_rstrModelFileName = TokenVector[3];
					...
					{
						uint dwMildBaseVnum = dwMildItemVnum / 10 * 10;
						string szMildIconPath = new string(new char[MAX_PATH]);
						sprintf(szMildIconPath, "icon/item/%.5d.tga", dwMildBaseVnum);
						if (4 == TokenVector.Count)
						{
							string szMildModelPath = new string(new char[MAX_PATH]);
							sprintf(szMildModelPath, "t:/laniaworkstate/item/weapon/%.5d.gr2", dwMildBaseVnum);
		...
		public bool LoadItemTable(string c_szFileName)
		{
			CMappedFile file = new CMappedFile();
			...
			file.Read(dwFourCC, sizeof(uint));

			if (dwFourCC == ((uint)(byte)('M') | ((uint)(byte)('I') << 8) | ((uint)(byte)('P') << 16) | ((uint)(byte)('X') << 24)))
			{
				file.Read(dwVersion, sizeof(uint));
				file.Read(dwStride, sizeof(uint));
				...
				{
					TraceError("CPythonItem::LoadItemTable: invalid item_proto[%s] STRIDE[%d] != sizeof(SItemTable)", c_szFileName, dwStride, sizeof(SItemTable));
					return false;
				}
			}
			else if (dwFourCC != ((uint)(byte)('M') | ((uint)(byte)('I') << 8) | ((uint)(byte)('P') << 16) | ((uint)(byte)('T') << 24)))
			{
				TraceError("CPythonItem::LoadItemTable: invalid item proto type %s", c_szFileName);
				return false;
			}

			file.Read(dwElements, sizeof(uint));
			file.Read(dwDataSize, sizeof(uint));
			...
			string szName = new string(new char[64 + 1]);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			...
			SortedDictionary<uint,uint> itemNameMap = new SortedDictionary<uint,uint>();

			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < dwElements; ++i, ++table)
			{
				CItemData pItemData;
				uint dwVnum = table.dwVnum;

				TItemMap.iterator f = m_ItemMap.find(dwVnum);
				if (m_ItemMap.end() == f)
				{
					_snprintf(szName, sizeof(char), "icon/item/%05d.tga", dwVnum);

					if (CResourceManager.Instance().IsFileExist(szName) == false)
					{
						SortedDictionary<uint, uint>.Enumerator itVnum = itemNameMap.find(GetHashCode(table.szName));
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
						...
						{
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
							_snprintf(szName, sizeof(char), "icon/item/%05d.tga", itVnum.second);
						}
						else
						{
							_snprintf(szName, sizeof(char), "icon/item/%05d.tga", dwVnum - dwVnum % 10);
						}
						...
						if (CResourceManager.Instance().IsFileExist(szName) == false)
						{
							...
							#endif
							const uint EmptyBowl = 27995;
							_snprintf(szName, sizeof(char), "icon/item/%05d.tga", EmptyBowl);
						}
					}
				...
				if (!itemNameMap.ContainsKey(GetHashCode(table.szName)))
				{
					itemNameMap.insert(SortedDictionary<uint,uint>.value_type(GetHashCode(table.szName),table.dwVnum));
				}
...
#if ENABLE_SHINING_SYSTEM
		public bool LoadShiningTable(string szShiningTable)
		{
			CMappedFile File = new CMappedFile();
			LPCVOID pData = new LPCVOID();
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < textFileLoader.GetLineCount(); ++i)
			{
				...
				if (TokenVector.Count > (1 + CItemData.ITEM_SHINING_MAX_COUNT))
				{
					TraceError("CItemManager::LoadShiningTable(%s) - LoadShiningTable in %d\n - RowSize: %d MaxRowSize: %d", szShiningTable, i, TokenVector.Count, CItemData.ITEM_SHINING_MAX_COUNT);
				}
				string c_rstrID = TokenVector[0];
				int pos = c_rstrID.IndexOf("~");
				if (-1 == pos)
				{
					uint dwItemVNum = atoi(c_rstrID);
					CItemData pItemData = MakeItemData(dwItemVNum);
					if (pItemData != null)
					{
						for (byte LaniatusDefVariables = 0; LaniatusDefVariables < CItemData.ITEM_SHINING_MAX_COUNT; LaniatusDefVariables++)
						{
							...
							{
								string c_rstrEffectPath = TokenVector[i + 1];
								pItemData.SetItemShiningTableData(i, c_rstrEffectPath);
							}
							else
							{
				...
				{
					string s_start_vnum = c_rstrID.Substring(0, pos);
					string s_end_vnum = c_rstrID.Substring(pos + 1);
					int start_vnum = atoi(s_start_vnum);
					int end_vnum = atoi(s_end_vnum);
					uint vnum = (uint)start_vnum;
					while (vnum <= end_vnum)
					{
...
#endif
		public bool LoadItemScale(string szItemScale)
		{
			CMappedFile File = new CMappedFile();
			LPCVOID pData = new LPCVOID();
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < textFileLoader.GetLineCount(); ++i)
			{
				...
				if (!(TokenVector.Count == 6 || TokenVector.Count == 7))
				{
					TraceError(" CItemManager::LoadItemScale(%s) - Error on line %d\n", szItemScale, i);
					continue;
				}

				string c_rstrID = TokenVector[ITEMSCALE_COL_VNUM];
				string c_rstrJob = TokenVector[ITEMSCALE_COL_JOB];
				string c_rstrSex = TokenVector[ITEMSCALE_COL_SEX];
				string c_rstrScaleX = TokenVector[ITEMSCALE_COL_SCALE_X];
				string c_rstrScaleY = TokenVector[ITEMSCALE_COL_SCALE_Y];
				string c_rstrScaleZ = TokenVector[ITEMSCALE_COL_SCALE_Z];

				uint dwItemVnum = atoi(c_rstrID);
				byte bJob = 0;
				if (!strcmp(c_rstrJob, "JOB_WARRIOR"))
				{
					bJob = NRaceData.JOB_WARRIOR;
				}
				if (!strcmp(c_rstrJob, "JOB_ASSASSIN"))
				{
					bJob = NRaceData.JOB_ASSASSIN;
				}
				if (!strcmp(c_rstrJob, "JOB_SURA"))
				{
					bJob = NRaceData.JOB_SURA;
				}
				if (!strcmp(c_rstrJob, "JOB_SHAMAN"))
				{
					bJob = NRaceData.JOB_SHAMAN;
				}
#if ENABLE_WOLFMAN
				if (!strcmp(c_rstrJob, "JOB_WOLFMAN"))
				{
					bJob = NRaceData.JOB_WOLFMAN;
				}
#endif
				byte bySex = c_rstrSex[0] == 'M' ? 1 : 0;

				float fScaleX = atof(c_rstrScaleX) * 0.01f;
				float fScaleY = atof(c_rstrScaleY) * 0.01f;
				float fScaleZ = atof(c_rstrScaleZ) * 0.01f;
				float fParticleScale = 1.0f;
				if (TokenVector.Count == 7)
				{
					string c_rstrParticleScale = TokenVector[ITEMSCALE_COL_PARTICLE_SCALE];
					fParticleScale = atof(c_rstrParticleScale);
				}
				...
				for (byte LaniatusDefVariables = 0; LaniatusDefVariables < bGradeMax; ++i)
				{
		...
		public CItemData MakeItemData(uint dwIndex)
		{
			TItemMap.iterator f = m_ItemMap.find(dwIndex);
		...
		protected SortedDictionary<uint, CItemData> m_ItemMap = new SortedDictionary<uint, CItemData>();
		protected List<CItemData> m_vec_ItemRange = new List<CItemData>();
		protected CItemData m_pSelectedItemData;
}