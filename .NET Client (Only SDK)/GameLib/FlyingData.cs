...
public class CFlyingData
{

	public const int FLY_ATTACH_NONE = 0;
	public const int FLY_ATTACH_EFFECT = 1;
	public const int FLY_ATTACH_OBJECT = 2;

	public const int FLY_ATTACH_TYPE_NONE = 0;
	public const int FLY_ATTACH_TYPE_LINE = 1;
	public const int FLY_ATTACH_TYPE_MULTI_LINE = 2;
	public const int FLY_ATTACH_TYPE_SINE = 3;
	public const int FLY_ATTACH_TYPE_EXP = 4;

	public class TFlyingAttachData
	{
		public int iType;
		public int iFlyType;
		public string strFilename = "";

		public bool bHasTail;
		public uint dwTailColor;
		public float fTailLength;
		public float fTailSize;
		public bool bRectShape;
...
//	friend class CFlyTabPage;
	...
	public bool LoadScriptFile(string c_szFilename)
	{
		NANOBEGIN Destroy = new NANOBEGIN();
		...
		{
			m_bSpreading = temp != 0?true:false;
		}
		...
		{
			m_bMaintainParallel = temp != 0?true:false;
		}
		...
		{
			m_bHitOnBackground = temp != 0?true:false;
		}
		else
		{
		...
		{
			m_bHitOnAnotherMonster = temp != 0?true:false;
		}
		else
		{
		...
		for (i = 0;i < TextFileLoader.GetChildNodeCount();i++)
		{
			...
			{
				TFlyingAttachData fad = new TFlyingAttachData();
				...
				if (temp != 0)
				{
					...
					fad.bRectShape = temp != 0?true:false;
				}
				else
				{
					fad.bHasTail = false;
	...
	public bool SaveScriptFile(string c_szFilename)
	{
		...
		string strGlobalPathName = "";
		StringPath(GetOnlyPathName(c_szFilename), strGlobalPathName);

		string strLocalFileName = "";
		if (GetLocalFileName(strGlobalPathName, m_strBombEffectName.c_str(), strLocalFileName))
		{
		...
		List<TFlyingAttachData>.Enumerator it;
		for (it = m_AttachDataVector.begin(); it.MoveNext();)
		{
			PrintfTabs(fp, 0, "Group AttachData\n");
			PrintfTabs(fp, 0, "{\n");
			PrintfTabs(fp, 1, "Type          %d\n", it.iType);
			PrintfTabs(fp, 1, "FlyType       %d\n", it.iFlyType);
			if (GetLocalFileName(strGlobalPathName, it.strFilename.c_str(), strLocalFileName))
			{
	...
	public _D3DVECTOR GetAcceleration()
	{
		return m_v3Accel;
	}

	public void SetBombEffect(string szEffectName)
	{
	...
	public uint AttachFlyEffect(int iType, in string strFilename, float fRoll, float fArg1, float fArg2)
	{
		TFlyingAttachData fad = new TFlyingAttachData();
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		memset(fad, 0, sizeof(TFlyingAttachData));
		...
		m_AttachDataVector.push_back(fad);
		return (uint)(m_AttachDataVector.size() - 1);
	}

	public CFlyingData.TFlyingAttachData GetAttachDataReference(int iIndex)
	{
	...
	public void DuplicateAttach(int iIndex)
	{
		Debug.Assert(0 <= iIndex && iIndex < (int)m_AttachDataVector.size());
		m_AttachDataVector.push_back(m_AttachDataVector[iIndex]);
	}

	public void RemoveAttach(int iIndex)
	{
		Debug.Assert(0 <= iIndex && iIndex < (int)m_AttachDataVector.size());
	...
	protected void __Initialize()
	{
		...
		m_fRollAngle = 0.0f;
		m_fRange = 500.0f;
		m_v3Accel = D3DXVECTOR3(0,0,0.0f);
	...
	protected bool m_bSpreading;
	protected bool m_bMaintainParallel;

	protected float m_fInitVel;
	protected float m_fConeAngle;
	protected float m_fRollAngle;

	protected _D3DVECTOR m_v3AngVel = new _D3DVECTOR();

	protected float m_fGravity;

	protected float m_fBombRange;
	protected string m_strBombEffectName = "";
	protected uint m_dwBombEffectID;

	protected bool m_bIsHoming;
	protected float m_fHomingMaxAngle;
	protected float m_fHomingStartTime;

	protected bool m_bHitOnBackground;
	protected bool m_bHitOnAnotherMonster;
	protected int m_iPierceCount;
	protected float m_fCollisionSphereRadius;

	protected float m_fRange;
	protected _D3DVECTOR m_v3Accel = new _D3DVECTOR();

	protected List<TFlyingAttachData> m_AttachDataVector = new List<TFlyingAttachData>();

	public static CFlyingData New()
	{
		return ms_kPool.Alloc();
	}
	public static void Delete(CFlyingData pData)
	{
		pData.Destroy();
		ms_kPool.Free(pData);
	}
	...
	public static CDynamicPool<CFlyingData> ms_kPool = new CDynamicPool<CFlyingData>();
}