...
public class CMapBase : CScreen
{
		...
		public abstract bool Load(float x, float y, float z);

		public abstract bool Update(float fX, float fY, float fZ);
		public abstract void UpdateAroundAmbience(float fX, float fY, float fZ);
		public abstract float GetHeight(float fx, float fy);
		public abstract void OnBeginEnvironment();

		public abstract void ApplyLight(uint dwVersion, in D3DLIGHT8 c_rkLight);

		protected abstract void OnRender();
		protected abstract void OnSetEnvironmentDataPtr();
		protected abstract void OnResetEnvironmentDataPtr();
		...
		{
			string strFileName = GetName() + "\\MapProperty.txt";

			CTokenVectorMap stTokenVectorMap = new CTokenVectorMap();

			if (!LoadMultipleTextData(strFileName, stTokenVectorMap))
			{
				TraceError("CMapBase::LoadProperty(FileName=%s) - LoadMultipleTextData ERROR ?????? ???? ???ɼ??? ???????.", strFileName);
				return false;
			}

			if (stTokenVectorMap.end() == stTokenVectorMap.find("scripttype"))
			{
				TraceError("CMapBase::LoadProperty(FileName=%s) - FIND 'scripttype' - FAILED", strFileName);
				return false;
			}

			if (stTokenVectorMap.end() == stTokenVectorMap.find("maptype"))
			{
				TraceError("CMapBase::LoadProperty(FileName=%s) - FIND 'maptype' - FAILED", strFileName);
				return false;
			}
			...
			string c_rstrMapType = stTokenVectorMap["maptype"][0];

			if (0 != string.CompareOrdinal(c_rstrType, "MapProperty"))
			{
				TraceError("CMapBase::LoadProperty(FileName=%s) - Resourse Type ERROR", strFileName);
				return false;
			}

			if (0 == string.CompareOrdinal(c_rstrMapType, "Indoor"))
			{
				SetType(MAPTYPE_INDOOR);
			}
			else if (0 == string.CompareOrdinal(c_rstrMapType, "Outdoor"))
			{
				SetType(MAPTYPE_OUTDOOR);
			}
			else if (0 == string.CompareOrdinal(c_rstrMapType, "Invalid"))
			{
				SetType(MAPTYPE_OUTDOOR);
			}
...
//Original Metin2 CPlus Line: EMAPTYPE GetType() const
		public EMAPTYPE GetType()
		{
			return m_eType;
		}
		public void SetType(EMAPTYPE eType)
		{
			m_eType = eType;
		}
...
//Original Metin2 CPlus Line: const string & GetName() const
		...
		public void SetName(in string cr_strName)
		{
			m_strName = cr_strName;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool IsCopiedMap() const
		public bool IsCopiedMap()
		{
			return m_strParentMapName.length() > 0;
		}
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const string& GetParentMapName() const
		public string GetParentMapName()
		{
			return m_strParentMapName;
		}
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const string& GetMapDataDirectory() const
		public string GetMapDataDirectory()
		{
			return IsCopiedMap() ? m_strParentMapName : m_strName;
		}

		protected EMAPTYPE m_eType = new EMAPTYPE();
		protected string m_strName = "";
		protected string m_strParentMapName = "";
		protected bool m_bReady;

		protected readonly TEnvironmentData mc_pEnvironmentData;
}