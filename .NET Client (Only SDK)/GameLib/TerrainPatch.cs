...
public class HardwareTransformPatch_SSourceVertex
{
	public _D3DVECTOR kPosition = new _D3DVECTOR();
	public _D3DVECTOR kNormal = new _D3DVECTOR();
}

public class SoftwareTransformPatch_SSourceVertex
{
	public _D3DVECTOR kPosition = new _D3DVECTOR();
	public _D3DVECTOR kNormal = new _D3DVECTOR();
...
public class CTerrainPatch
{
	public const int PATCH_TYPE_PLAIN = 0;
	public const int PATCH_TYPE_HILL = 1;
	public const int PATCH_TYPE_CLIFF = 2;

	public int TERRAIN_VERTEX_COUNT = (CTerrainImpl.PATCH_XSIZE+1) * (CTerrainImpl.PATCH_YSIZE+1);

	public static bool SOFTWARE_TRANSFORM_PATCH_ENABLE = true;

	public CTerrainPatch()
	{
		Clear();
	}
	public void Dispose()
	...
	public void SetMinZ(float fMinZ)
	{
		m_fMinZ = fMinZ;
	}
	...
	public void SetMaxZ(float fMaxZ)
	{
		m_fMaxZ = fMaxZ;
	}
	...
	public void SetWaterExist(bool bWaterExist)
	{
	...
	public void NeedUpdate(bool bNeedUpdate)
	{
	...
	public uint GetWaterFaceCount()
	{
		return m_dwWaterPriCount;
	}

	public void SoftwareTransformPatch_UpdateTerrainLighting(uint dwVersion, in D3DLIGHT8 c_rkLight, in D3DMATERIAL8 c_rkMtrl)
	{
		...
		_D3DVECTOR kLightDir = c_rkLight.Direction;


		uint dwDot;
		uint dwAmbientR = (c_rkMtrl.Ambient.r * c_rkLight.Ambient.r + c_rkMtrl.Emissive.r) * 255.0f;
		uint dwAmbientG = (c_rkMtrl.Ambient.g * c_rkLight.Ambient.g + c_rkMtrl.Emissive.g) * 255.0f;
		uint dwAmbientB = (c_rkMtrl.Ambient.b * c_rkLight.Ambient.b + c_rkMtrl.Emissive.b) * 255.0f;
		uint dwDiffuseR = (c_rkMtrl.Diffuse.r * c_rkLight.Diffuse.r) * 255.0f;
		uint dwDiffuseG = (c_rkMtrl.Diffuse.g * c_rkLight.Diffuse.g) * 255.0f;
		uint dwDiffuseB = (c_rkMtrl.Diffuse.b * c_rkLight.Diffuse.b) * 255.0f;

		if (dwDiffuseR > 255 - dwAmbientR)
		{
			dwDiffuseR = (uint)(255 - dwAmbientR);
		}

		if (dwDiffuseG + dwAmbientG > 255)
		{
			dwDiffuseG = (uint)(255 - dwAmbientG);
		}

		if (dwDiffuseB + dwAmbientB > 255)
		{
			dwDiffuseB = (uint)(255 - dwAmbientB);
		}
		...
		for (uint uIndex = 0; uIndex != CTerrainPatch.TERRAIN_VERTEX_COUNT; ++uIndex)
		{
			...
			akSrcVertex[uIndex].dwDiffuse = (0xff000000) | (((dwDiffuseR * dwDot >> S) + dwAmbientR) << 16) | (((dwDiffuseG * dwDot>> S) + dwAmbientG) << 8) | ((dwDiffuseB * dwDot>> S) + dwAmbientB);
		}

	}

	public void BuildTerrainVertexBuffer(HardwareTransformPatch_SSourceVertex akSrcVertex)
	{
	...
	public void BuildWaterVertexBuffer(SWaterVertex akSrcVertex, uint uWaterVertexCount)
	{
		...
		if (rkVB.Lock((object) akDstWaterVertex))
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
	...
	protected void __BuildHardwareTerrainVertexBuffer(HardwareTransformPatch_SSourceVertex akSrcVertex)
	{
		...
		if (rkVB.Lock((object) akDstVertex))
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
	...
	protected void __BuildSoftwareTerrainVertexBuffer(HardwareTransformPatch_SSourceVertex[] akSrcVertex)
	{
		m_kST.Create();
	...
	private bool m_bUse;
	private bool m_bWaterExist;
	private uint m_dwID;
	private uint m_dwWaterPriCount;

	private CGraphicVertexBuffer m_WaterVertexBuffer = new CGraphicVertexBuffer();
	private byte m_byType;

	private bool m_bNeedUpdate;
	private uint m_dwVersion;

	public CGraphicVertexBuffer GetWaterVertexBufferPointer()
	{
		return m_WaterVertexBuffer;
	}

	public CGraphicVertexBuffer HardwareTransformPatch_GetVertexBufferPtr()
	{
		return m_kHT.m_kVB;
	}

	protected class SHardwareTransformPatch
	{
		public CGraphicVertexBuffer m_kVB = new CGraphicVertexBuffer();
	}
	protected SHardwareTransformPatch m_kHT = new SHardwareTransformPatch();


	public SoftwareTransformPatch_SSourceVertex SoftwareTransformPatch_GetTerrainVertexDataPtr()
	{
		return m_kST.m_akTerrainVertex;
	}

	protected class SSoftwareTransformPatch
	{
		public SoftwareTransformPatch_SSourceVertex[] m_akTerrainVertex;

		public SSoftwareTransformPatch()
		{
			__Initialize();
		}
		...
		public void Create()
		{
			Debug.Assert(null == m_akTerrainVertex);
			m_akTerrainVertex = Arrays.InitializeWithDefaultInstances<SoftwareTransformPatch_SSourceVertex>(TERRAIN_VERTEX_COUNT);
		}
	...
	protected SSoftwareTransformPatch m_kST = new SSoftwareTransformPatch();

}

public class CTerrainPatchProxy
{
	public CTerrainPatchProxy()
	{
		Clear();
	}
	...
	public void SetCenterPosition(in _D3DVECTOR c_rv3Center)
	{
		m_v3Center = c_rv3Center;
	}

	public bool IsIn(in _D3DVECTOR c_rv3Target, float fRadius)
	{
	...
	public short GetPatchNum()
	{
		return m_sPatchNum;
	}
	public void SetPatchNum(short sPatchNum)
	{
		m_sPatchNum = sPatchNum;
	}

	public byte GetTerrainNum()
	{
		return m_byTerrainNum;
	}
	public void SetTerrainNum(byte byTerrainNum)
	{
		m_byTerrainNum = byTerrainNum;
	}

	public void SetTerrainPatch(CTerrainPatch pTerrainPatch)
	{
		m_pTerrainPatch = pTerrainPatch;
	}

	public bool isWaterExists()
	{
	...
	protected short m_sPatchNum;

	protected byte m_byTerrainNum;

	protected CTerrainPatch m_pTerrainPatch;
