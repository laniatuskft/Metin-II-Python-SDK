...
{
		public CProperty(string c_pszFileName)
		{
			this.mc_pFileName = null;
			this.m_dwCRC = 0;
		...
		public bool ReadFromMemory(object c_pvData, int iLen, string c_pszFileName)
		{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			char * pcData = (string) c_pvData;

			if ((uint) pcData != ((uint)(byte)('Y') | ((uint)(byte)('P') << 8) | ((uint)(byte)('R') << 16) | ((uint)(byte)('T') << 24)))
			{
				return false;
			}
			...
			textFileLoader.Bind(iLen - (sizeof(uint) + 2), pcData);

			m_stCRC = textFileLoader.GetLineString(0);
			m_dwCRC = atoi(m_stCRC.c_str());

			for (uint LaniatusDefVariables = 1; LaniatusDefVariables < textFileLoader.GetLineCount(); ++i)
			{
				...
				string stKey = stTokenVector[0];

				stTokenVector.erase(stTokenVector.begin());
				PutVector(stKey, stTokenVector);
			}
		...
		public bool GetVector(string c_pszKey, CTokenVector rTokenVector)
		{
			string stTempKey = c_pszKey;
			stl_lowers(stTempKey);
			CTokenVectorMap.iterator it = m_stTokenMap.find(stTempKey);
		...
		public bool GetString(string c_pszKey, string[] c_ppString)
		{
			string stTempKey = c_pszKey;
			stl_lowers(stTempKey);
			CTokenVectorMap.iterator it = m_stTokenMap.find(stTempKey);
			...
			c_ppString[0] = it.second[0].c_str();
			return true;
		}

		public void PutVector(string c_pszKey, in CTokenVector c_rTokenVector)
		{
			string stTempKey = c_pszKey;
			stl_lowers(stTempKey);
		...
		public void PutString(string c_pszKey, string c_pszString)
		{
			string stTempKey = c_pszKey;
			stl_lowers(stTempKey);
		...
		{
			CTempFile file = new CTempFile();

			uint fourcc = ((uint)(byte)('Y') | ((uint)(byte)('P') << 8) | ((uint)(byte)('R') << 16) | ((uint)(byte)('T') << 24));
			file.Write(fourcc, sizeof(uint));
			file.Write("\r\n", 2);

			if (0 == m_stCRC.length())
			{
				string szCRC = new string(new char[MAX_PATH + 16 + 1]);

				GetTimeString(szCRC, time(0));
				strcpy(szCRC + strlen(szCRC), c_pszFileName);

				m_dwCRC = CPropertyManager.Instance().GetUniqueCRC(szCRC);
				_snprintf(szCRC, sizeof(char), "%u", m_dwCRC);
			...
			string buf = new string(new char[4096 + 1]);

			while (itor != m_stTokenMap.end())
			{
				CTokenVector tokenVector = itor.second;

				int len = _snprintf(buf, sizeof(char), "%s\t", itor.first.c_str());
				file.Write(buf, len);
		...
		protected string m_stCRC = "";
		protected readonly string mc_pFileName;
		protected uint m_dwCRC;

		protected CTokenVectorMap m_stTokenMap = new CTokenVectorMap();
}