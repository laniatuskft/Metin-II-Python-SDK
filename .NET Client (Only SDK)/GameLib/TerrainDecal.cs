...
public class CTerrainDecal : CDecal
{
	public const int MAX_SEARCH_VERTICES = 1024;

	public CTerrainDecal(CMapOutdoor pMapOutdoor = null)
	{
		this.m_pMapOutdoor = pMapOutdoor;
	...
	public virtual void Make(_D3DVECTOR v3Center, _D3DVECTOR v3Normal, _D3DVECTOR v3Tangent, float fWidth, float fHeight, float fDepth)
	{
		Clear();
		m_v3Center = v3Center;
		m_v3Normal = v3Normal;

		_D3DVECTOR v3Binormal = new _D3DVECTOR();
		...
		float fMinY = Math.Abs(v3Center.y) - fSearchRadius;
		float fMaxY = Math.Abs(v3Center.y) + fSearchRadius;

		uint dwAffectedPrimitiveCount = 0;
		_D3DVECTOR[] v3AffectedVertex = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(MAX_SEARCH_VERTICES);
		_D3DVECTOR[] v3AffectedNormal = Arrays.InitializeWithDefaultInstances<_D3DVECTOR>(MAX_SEARCH_VERTICES);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		memset(v3AffectedVertex, 0, sizeof(_D3DVECTOR));
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		memset(v3AffectedNormal, 0, sizeof(_D3DVECTOR));
		...
		for (uint dwi = 0; dwi < m_dwVertexCount; ++dwi)
		{
			_D3DVECTOR v3 = m_Vertices[dwi].position - m_v3Center;
	...
	public virtual void Render()
	{
		...
		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
		(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'pdwAffectedPrimitiveCount', so pointers on this parameter are left unchanged:
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'pv3AffectedVertex', so pointers on this parameter are left unchanged:
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'pv3AffectedNormal', so pointers on this parameter are left unchanged:
	protected void SearchAffectedTerrainMesh(float fMinX, float fMaxX, float fMinY, float fMaxY, uint * pdwAffectedPrimitiveCount, _D3DVECTOR * pv3AffectedVertex, _D3DVECTOR * pv3AffectedNormal)
	{
		if (!m_pMapOutdoor)
		{
			return;
		...
		int iMaxY;
		{
			...
			(iMinX) = PR_ICNV > PR_FCNV ? PR_ICNV - 1 : PR_ICNV;
		};
		{
			PR_FCNV = (fMaxX);
			__asm
			...
			(iMaxX) = PR_ICNV > PR_FCNV ? PR_ICNV - 1 : PR_ICNV;
		};
		{
			PR_FCNV = (fMinY);
			__asm
			...
			(iMinY) = PR_ICNV > PR_FCNV ? PR_ICNV - 1 : PR_ICNV;
		};
		{
			PR_FCNV = (fMaxY);
			__asm
			...
			(iMaxY) = PR_ICNV > PR_FCNV ? PR_ICNV - 1 : PR_ICNV;
		};
		...
		for (int iy = iMinY; iy <= iMaxY; iy += CTerrainImpl.CELLSCALE)
		{
			...
			for (int ix = iMinX; ix <= iMaxX; ix += CTerrainImpl.CELLSCALE)
			{
				...
				ushort wTerrainNumX = ix / CTerrainImpl.TERRAIN_YSIZE;
	...
	protected CMapOutdoor m_pMapOutdoor;
}
