...
public class CArea
{
		...
		public const int ROTATION_STEP_AMOUNT = 360 / ROTATION_STEP_COUNT;
		public const int YAW_STEP_COUNT = 24;
		public const int YAW_STEP_AMOUNT = 360 / YAW_STEP_COUNT;
		public const int PITCH_STEP_COUNT = YAW_STEP_COUNT;
		public const int PITCH_STEP_AMOUNT = YAW_STEP_AMOUNT;


		public class SObjectData
		{
			public TObjectPosition Position = new TObjectPosition();
			public uint dwCRC;
			public byte[] abyPortalID = new byte[PORTAL_ID_MAX_NUM];

			public float m_fYaw;
			public float m_fPitch;
			public float m_fRoll;
			public float m_fHeightBias;

			public uint dwRange;
			public float fMaxVolumeAreaPercentage;

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			void InitializeRotation();
		}

		public class ObjectDataComp
		{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool operator ()(TObjectData & lhs, TObjectData & rhs) const
			public bool functorMethod(TObjectData lhs, TObjectData rhs)
			{
		...
		public class SAmbienceInstance : CScreen
		{
			public float fx;
			public float fy;
			public float fz;
			public uint dwRange;
			public float fMaxVolumeAreaPercentage;
			public int iPlaySoundIndex;
			public float fNextPlayTime;
			public prt.TPropertyAmbience AmbienceData = new prt.TPropertyAmbience();

			public SAmbienceInstance()
			{
				fx = 0.0f;
				fy = 0.0f;
				fz = 0.0f;
...
//			void __Update(float fxCenter, float fyCenter, float fzCenter);
			public delegate void UpdateDelegate(float fxCenter, float fyCenter, float fzCenter);
...
//			float __GetVolumeFromDistance(float fDistance);
		...
		{
			public uint dwType;
			public CAttributeInstance pAttributeInstance;

			public CSpeedTreeWrapper pTree;

			public bool isShadowFlag;
			public CGraphicThingInstance pThingInstance;

			public uint dwEffectID;
			public uint dwEffectInstanceIndex;

			public TAmbienceInstance pAmbienceInstance;

			public CDungeonBlock pDungeonBlock;
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			void SetTree(float x, float y, float z, uint dwTreeCRC, string c_szTreeName);
		...
		public class FFindIfCRC
		{

			public FFindIfCRC(uint dwCRC)
			{
				m_dwCRC = dwCRC;
			}

			public bool functorMethod(TCRCWithNumber rCRCWithNumber)
			{
		...
		public class CRCNumComp
		{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool operator ()(TCRCWithNumber & lhs, TCRCWithNumber & rhs) const
			public bool functorMethod(TCRCWithNumber lhs, TCRCWithNumber rhs)
			{
		...
		public void CollectRenderingObject(List<CGraphicThingInstance> rkVct_pkOpaqueThingInst)
		{
			TThingInstanceVector.iterator LaniatusDefVariables = new TThingInstanceVector.iterator();
			for (i = m_ThingCloneInstaceVector.begin(); LaniatusDefVariables != m_ThingCloneInstaceVector.end(); ++i)
			{
				...
				if (pkThingInst.isShow())
				{
					...
					{
						rkVct_pkOpaqueThingInst.Add(i);
					}
				}
			}
		}

		public void CollectBlendRenderingObject(List<CGraphicThingInstance> rkVct_pkBlendThingInst)
		{
			TThingInstanceVector.iterator LaniatusDefVariables = new TThingInstanceVector.iterator();
			for (i = m_ThingCloneInstaceVector.begin(); LaniatusDefVariables != m_ThingCloneInstaceVector.end(); ++i)
			{
		...
		public void Clear()
		{
			TObjectInstanceVector.iterator it = new TObjectInstanceVector.iterator();
			...
			TEffectInstanceIterator LaniatusDefVariables = new TEffectInstanceIterator();
			for (i = m_EffectInstanceMap.begin(); LaniatusDefVariables != m_EffectInstanceMap.end(); ++i)
			{
		...
		public bool Load(string c_szPathName)
		{
			Clear();

			string strObjectDataFileName = c_szPathName + "AreaData.txt";
			string strAmbienceDataFileName = c_szPathName + "AreaAmbienceData.txt";

			__Load_LoadObject(strObjectDataFileName);
			__Load_LoadAmbience(strAmbienceDataFileName);
			__Load_BuildObjectInstances();

			return true;
		}

		public uint GetObjectDataCount()
		{
			return (uint)m_ObjectDataVector.size();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool GetObjectDataPointer(uint dwIndex, const SObjectData ** ppObjectData) const;
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool GetObjectDataPointer(uint dwIndex, SObjectData[] ppObjectData);

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const uint GetObjectInstanceCount() const
		public uint GetObjectInstanceCount()
		{
			return (uint)m_ObjectInstanceVector.size();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const bool GetObjectInstancePointer(const uint & dwIndex, const SObjectInstance ** ppObjectInstance) const;
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool GetObjectInstancePointer(in uint dwIndex, SObjectInstance[] ppObjectInstance);

		public void EnablePortal(bool bFlag = true)
		{
		...
		public void AddShowingPortalID(int iNum)
		{
		...
		{
			SortedSet<TObjectInstance > kSet_ShowingObjectInstance = new SortedSet<TObjectInstance >();
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < m_ObjectDataVector.size(); ++i)
			{
				...
				for (int j = 0; j < PORTAL_ID_MAX_NUM; ++j)
				{
					...
					kSet_ShowingObjectInstance.Add(pInstance);
					break;
				}
			}
			...
			for (it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end(); ++it)
			{
				...
				{
					Debug.Assert(pObjectInstance.pDungeonBlock);
		...
		{
			TAmbienceInstanceVector.iterator LaniatusDefVariables = new TAmbienceInstanceVector.iterator();
			for (i = m_AmbienceCloneInstanceVector.begin(); LaniatusDefVariables != m_AmbienceCloneInstanceVector.end(); ++i)
			{
		...
		public void Render()
		{
			...
			m_kRenderedThingInstanceCRCWithNumberVector.clear();
			m_kRenderedGrapphicThingInstanceVector.clear();
			TGraphicThingInstanceCRCMap.iterator aGraphicThingInstanceCRCMapIterator = new TGraphicThingInstanceCRCMap.iterator();
			...
			while (i != m_ThingCloneInstaceVector.end())
			{
				...
				if (pkThingInst.Render())
				{
					...
					uint dwCRC = aGraphicThingInstanceCRCMapIterator.second;
					...
					{
						TCRCWithNumber aCRCWithNumber = new TCRCWithNumber();
...
//Original Metin2 CPlus Line: TCRCWithNumber & rCRCWithNumber = *aCRCWithNumberVectorIterator;
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private List<CEffectInstance> RenderEffect_s_kVct_pkEftInstSort = new List<CEffectInstance>();
		...
		public void RenderEffect()
		{
			...
			{
				TEffectInstanceIterator LaniatusDefVariables = new TEffectInstanceIterator();
				for (i = m_EffectInstanceMap.begin(); LaniatusDefVariables != m_EffectInstanceMap.end();)
				{
					CEffectInstance pEffectInstance = i.second;
...
//				static System.Collections.Generic.List<CEffectInstance*> s_kVct_pkEftInstSort;
				RenderEffect_s_kVct_pkEftInstSort.clear();
		...
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TEffectInstanceMap' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
				TEffectInstanceMap.iterator LaniatusDefVariables = new TEffectInstanceMap.iterator();
				for (i = rkMap_pkEftInstSrc.begin(); LaniatusDefVariables != rkMap_pkEftInstSrc.end(); ++i)
				{
					RenderEffect_s_kVct_pkEftInstSort.push_back(i.second);
				}

				std::sort(RenderEffect_s_kVct_pkEftInstSort.begin(), RenderEffect_s_kVct_pkEftInstSort.end(), CArea_LessEffectInstancePtrRenderOrder());
				std::for_each(RenderEffect_s_kVct_pkEftInstSort.begin(), RenderEffect_s_kVct_pkEftInstSort.end(), CArea_FEffectInstanceRender());
		...
		public void RenderCollision()
		{
			...
			(CStateManager.Instance()).SetRenderState(D3DRS_TEXTUREFACTOR, 0xff000000);
		...
		public void RenderAmbience()
		{
			uint dwColorArg1;
			uint dwColorOp;
			(CStateManager.Instance()).GetTextureStageState(0, D3DTSS_COLORARG1, dwColorArg1);
			(CStateManager.Instance()).GetTextureStageState(0, D3DTSS_COLOROP, dwColorOp);
			...
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLORARG1, dwColorArg1);
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_COLOROP, dwColorOp);
		}
...
//Original Metin2 CPlus Line: const float * pfPosition;
						float pfPosition;
		...
		public void SetCoordinate(in ushort usCoordX, in ushort usCoordY)
		{
			m_wX = usCoordX;
			m_wY = usCoordY;
		}
		...
		public uint DEBUG_GetRenderedCRCNum()
		{
			return (uint)m_kRenderedThingInstanceCRCWithNumberVector.size();
		}

		public List<SCRCWithNumber> DEBUG_GetRenderedCRCWithNumVector()
		{
			return m_kRenderedThingInstanceCRCWithNumberVector;
		}

		public uint DEBUG_GetRenderedGrapphicThingInstanceNum()
		{
			return (uint)m_kRenderedGrapphicThingInstanceVector.size();
		}
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool CheckObjectIndex(uint dwIndex) const
		protected bool CheckObjectIndex(uint dwIndex)
		{
		...
		protected bool __Load_LoadObject(string c_szFileName)
		{
			...
			string c_rstrCount = stTokenVectorMap["objectcount"][0];

			uint dwCount = atoi(c_rstrCount);

			string szObjectName = new string(new char[32 + 1]);

			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < dwCount; ++i)
			{
				_snprintf(szObjectName, sizeof(char), "object%03d", i);
				...
				CTokenVector rVector = stTokenVectorMap[szObjectName];

				string c_rstrxPosition = rVector[0].c_str();
				string c_rstryPosition = rVector[1].c_str();
				string c_rstrzPosition = rVector[2].c_str();
				string c_rstrCRC = rVector[3].c_str();

				TObjectData ObjectData = new TObjectData();
				ZeroMemory(ObjectData, sizeof(TObjectData));
				ObjectData.Position.x = atof(c_rstrxPosition);
				ObjectData.Position.y = atof(c_rstryPosition);
				ObjectData.Position.z = atof(c_rstrzPosition);
				ObjectData.dwCRC = atoi(c_rstrCRC);

				ObjectData.InitializeRotation();
				if (rVector.size() > 4)
				{
					int s = rVector[4].find('#');
					if (s != rVector[4].npos)
					{
				...
				if (rVector.size() > 6)
				{
					for (int portalIdx = 0; portalIdx < Math.Min(rVector.size() - 6, PORTAL_ID_MAX_NUM); ++portalIdx)
					{
		...
		protected bool __Load_LoadAmbience(string c_szFileName)
		{
			CTokenVectorMap stTokenVectorMap = new CTokenVectorMap();
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < dwCount; ++i)
			{
				...
				string c_rstrzPosition = rVector[2].c_str();
				string c_rstrCRC = rVector[3].c_str();
				string c_rstrRange = rVector[4].c_str();
				...
				ObjectData.Position.z = atof(c_rstrzPosition);
				ObjectData.dwCRC = atoi(c_rstrCRC);
				ObjectData.dwRange = atoi(c_rstrRange);
				...
				if (rVector.size() >= 6)
				{
					string c_rstrPercentage = rVector[5].c_str();
					ObjectData.fMaxVolumeAreaPercentage = atof(c_rstrPercentage);
				}
		...
		protected void __Load_BuildObjectInstances()
		{
			m_ObjectInstanceVector.clear();
		...
		protected void __UpdateAniThingList()
		{
			{
				CGraphicThingInstance pkThingInst;
		...
		protected void __UpdateEffectList()
		{
			CEffectManager rkEftMgr = CEffectManager.Instance();
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __LoadAttribute(SObjectInstance pObjectInstance, string c_szAttributeFileName);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __Clear_DestroyObjectInstance(SObjectInstance pObjectInstance);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance(SObjectInstance pObjectInstance, SObjectData c_pData);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance_SetTree(SObjectInstance pObjectInstance, SObjectData c_pData, CProperty pProperty);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance_SetBuilding(SObjectInstance pObjectInstance, SObjectData c_pData, CProperty pProperty);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance_SetEffect(SObjectInstance pObjectInstance, SObjectData c_pData, CProperty pProperty);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance_SetAmbience(SObjectInstance pObjectInstance, SObjectData c_pData, CProperty pProperty);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetObjectInstance_SetDungeonBlock(SObjectInstance pObjectInstance, SObjectData c_pData, CProperty pProperty);

		protected List<SObjectData> m_ObjectDataVector = new List<SObjectData>();
		protected List<SObjectInstance > m_ObjectInstanceVector = new List<SObjectInstance >();
		protected List<CSpeedTreeWrapper > m_TreeCloneInstaceVector = new List<CSpeedTreeWrapper >();
		protected List<CGraphicThingInstance > m_ThingCloneInstaceVector = new List<CGraphicThingInstance >();
		protected List<CDungeonBlock > m_DungeonBlockCloneInstanceVector = new List<CDungeonBlock >();
		protected List<CGraphicThingInstance > m_AniThingCloneInstanceVector = new List<CGraphicThingInstance >();
		protected List<CGraphicThingInstance > m_ShadowThingCloneInstaceVector = new List<CGraphicThingInstance >();
		protected List<SAmbienceInstance > m_AmbienceCloneInstanceVector = new List<SAmbienceInstance >();
		protected SortedDictionary<CGraphicThingInstance , uint> m_GraphicThingInstanceCRCMap = new SortedDictionary<CGraphicThingInstance , uint>();
		protected List<SCRCWithNumber> m_kRenderedThingInstanceCRCWithNumberVector = new List<SCRCWithNumber>();
		protected List<CGraphicThingInstance > m_kRenderedGrapphicThingInstanceVector = new List<CGraphicThingInstance >();
		...
		protected static CDynamicPool<SObjectInstance> ms_ObjectInstancePool = new CDynamicPool<SObjectInstance>();
		protected static CDynamicPool<CAttributeInstance> ms_AttributeInstancePool = new CDynamicPool<CAttributeInstance>();
		protected static CDynamicPool<SAmbienceInstance> ms_AmbienceInstancePool = new CDynamicPool<SAmbienceInstance>();
		protected static CDynamicPool<CDungeonBlock> ms_DungeonBlockInstancePool = new CDynamicPool<CDungeonBlock>();

		protected SortedDictionary<uint, CEffectInstance> m_EffectInstanceMap = new SortedDictionary<uint, CEffectInstance>();

		protected bool m_bPortalEnable;
		protected SortedSet<int> m_kSet_ShowingPortalID = new SortedSet<int>();
		...
		public static CArea New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CArea pkArea)
		{
			pkArea.Clear();
			ms_kPool.Free(pkArea);
		}

		public static CDynamicPool<CArea> ms_kPool = new CDynamicPool<CArea>();
}


public class CArea_LessEffectInstancePtrRenderOrder
{
	public bool functorMethod(CEffectInstance pkLeft, CEffectInstance pkRight)
	{
		return pkLeft.LessRenderOrder(pkRight);
	}
}

public class CArea_FEffectInstanceRender
{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool CArea::GetObjectDataPointer(uint dwIndex, const TObjectData ** ppObjectData) const

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const bool CArea::GetObjectInstancePointer(const uint & dwIndex, const TObjectInstance ** ppObjectInstance) const
