...
public class CFlyTrace : CScreen
{
		public CFlyTrace()
		{
			__Initialize();
		...
		public void Create(in CFlyingData.TFlyingAttachData rFlyingAttachData)
		{
		...
		public void UpdateNewPosition(in _D3DVECTOR v3Position)
		{
		...
		public void Render()
		{
			...
			List<Tuple<float, TFlyVertexSet>> VSVector = new List<Tuple<float, TFlyVertexSet>>();

			(CStateManager.Instance()).SaveRenderState(D3DRS_ZFUNC,D3DCMP_LESS);
			...
			(CStateManager.Instance()).SaveRenderState(D3DRS_BLENDOP, D3DBLENDOP_ADD);
			...
			_D3DMATRIX M = pCurrentCamera.GetViewMatrix();
			D3DXMatrixIdentity(m);
			_D3DVECTOR F = new _D3DVECTOR(pCurrentCamera.GetView());
			m._31 = F.x;
			m._32 = F.y;
			m._33 = F.z;
			...
			TTimePositionDeque.iterator it1 = new TTimePositionDeque.iterator();
			TTimePositionDeque.iterator it2 = new TTimePositionDeque.iterator();
			it2 = it1 = m_TimePositionDeque.begin();
			++it2;
			for (;it2 != m_TimePositionDeque.end();++it2,++it1)
			{
				_D3DVECTOR rkOld = it1.second;
				_D3DVECTOR rkNew = it2.second;
				_D3DVECTOR B = rkNew - rkOld;

				float radius = Math.Max(Math.Abs(B.x),Math.Max(Math.Abs(B.y),Math.Abs(B.z))) / 2;
				Vector3d c = new Vector3d(it1.second.x + B.x * 0.5f, it1.second.y + B.y * 0.5f, it1.second.z + B.z * 0.5f);
				...
				TFlyVertex[] v = {TFlyVertex(D3DXVECTOR3(0.0f,size1,0.0f), m_dwColor,D3DXVECTOR2(0.0f,0.0f)), TFlyVertex(D3DXVECTOR3(-size1,0.0f,0.0f),m_dwColor,D3DXVECTOR2(0.0f,0.5f)), TFlyVertex(D3DXVECTOR3(size1,0.0f,0.0f), m_dwColor,D3DXVECTOR2(0.5f,0.0f)), TFlyVertex(D3DXVECTOR3(-size2,0.0f,0.0f),m_dwColor,D3DXVECTOR2(0.5f,1.0f)), TFlyVertex(D3DXVECTOR3(size2,0.0f,0.0f), m_dwColor,D3DXVECTOR2(1.0f,0.5f)), TFlyVertex(D3DXVECTOR3(0.0f,-size2,0.0f),m_dwColor,D3DXVECTOR2(1.0f,1.0f))};


				_D3DVECTOR E = new _D3DVECTOR(M._41,M._42,M._43);
				E = pCurrentCamera.GetEye();
				E -= it1.second;

				_D3DVECTOR P = new _D3DVECTOR();
				D3DXVec3Cross(P, B, E);

				_D3DVECTOR U = new _D3DVECTOR();
				D3DXVec3Cross(U, F, P);
				D3DXVec3Normalize(U, U);
				_D3DVECTOR R = new _D3DVECTOR();
				D3DXVec3Cross(R, F, U);
			...
			foreach (Tuple<float, TFlyVertexSet> it in VSVector)
			{
				(CStateManager.Instance()).DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 4, it.second.v, sizeof(TVertex));
			}
			...
			(CStateManager.Instance()).RestoreRenderState(D3DRS_BLENDOP);
		...
		protected bool m_bRectShape;
		protected uint m_dwColor;
		protected float m_fSize;
		protected float m_fTailLength;

		protected LinkedList<Tuple<float, _D3DVECTOR>> m_TimePositionDeque = new LinkedList<Tuple<float, _D3DVECTOR>>();
		...
		public static CFlyTrace New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CFlyTrace pkInst)
		{
			pkInst.Destroy();
			ms_kPool.Free(pkInst);
		}

		public static CDynamicPool<CFlyTrace> ms_kPool = new CDynamicPool<CFlyTrace>();
}

public class TFlyVertex
{
	public _D3DVECTOR p = new _D3DVECTOR();
	public uint c;
	public D3DXVECTOR2 t = new D3DXVECTOR2();
	public TFlyVertex()
	{
	}
	public TFlyVertex(in _D3DVECTOR p, uint c, in D3DXVECTOR2 t)
	{
		this.p = p;
		this.c = c;
		this.t = t;
...
public class TFlyVertexSet
{
	public TFlyVertex[] v = Arrays.InitializeWithDefaultInstances<TFlyVertex>(6);
	public TFlyVertexSet(TFlyVertex pv)
	{
...
//Original Metin2 CPlus Line: bool operator < (const TFlyVertexSet&) const
	public static bool operator < (TFlyVertexSet ImpliedObject, in TFlyVertexSet UnnamedParameter)
	{
		return false;
	}
//# Laniatus Games Studio Inc. |: This 'CopyFrom' method was converted from the original copy assignment operator:
//Original Metin2 CPlus Line: TFlyVertexSet & operator = (const TFlyVertexSet& rhs)
	public TFlyVertexSet CopyFrom (in TFlyVertexSet rhs)
	{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
