...
public class CPropertyManager : CSingleton<CPropertyManager>
{
		public CPropertyManager()
		{
			this.m_isFileMode = true;
		}
...
//		void SetPack(CEterPack pPack);
		public bool BuildPack()
		{
			...
			IntPtr hFind = FindFirstFile("property\\*", fdata);
			...
			do
			{
				...
				string szSourceFileName = new string(new char[256 + 1]);
				_snprintf(szSourceFileName, sizeof(char), "property\\%s", fdata.cFileName);
		...
		public bool LoadReservedCRC(string c_pszFileName)
		{
...
//Original Metin2 CPlus Line: const char * pszLine = textFileLoader.GetLineString(i).c_str();
				char pszLine = textFileLoader.GetLineString(i).c_str();
		...
		public void ReserveCRC(uint dwCRC)
		{
			m_ReservedCRCSet.insert(dwCRC);
		}

		public uint GetUniqueCRC(string c_szSeed)
		{
			string stTmp = c_szSeed;

			while (true)
			{
				uint dwCRC = GetCRC32(stTmp, stTmp.Length);
				...
				string szAdd = new string(new char[2]);
				_snprintf(szAdd, sizeof(char), "%d", random() % 10);
				stTmp += szAdd;
			}
		}

		public bool Initialize(string c_pszPackFileName = null)
		{
			if (c_pszPackFileName != '\0')
			{
				...
				Dictionary<uint, SEterPackIndex > indexMap = m_pack.GetIndexMap();

				Dictionary<uint, SEterPackIndex >.Enumerator itor = indexMap.GetEnumerator();
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
					SEterPackIndex pIndex = itor.second;
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
					++itor;
		...
		public bool Register(string c_pszFileName, CProperty[] ppProperty = null)
		{
			CMappedFile file = new CMappedFile();
			LPCVOID c_pvData = new LPCVOID();
...
//Original Metin2 CPlus Line: *ppProperty = pProperty;
				ppProperty[0].CopyFrom(pProperty);
			}

			return true;
		}

		public bool Get(uint dwCRC, CProperty[] ppProperty)
		{
			TPropertyCRCMap.iterator itor = m_PropertyByCRCMap.find(dwCRC);
			...
			ppProperty[0] = itor.second;
			return true;
		}

		public bool Get(string c_pszFileName, CProperty[] ppProperty)
		{
			return Register(c_pszFileName, ppProperty);
		}

		public bool Put(string c_pszFileName, string c_pszSourceFileName)
		{
			...
			if (!m_isFileMode)
			{
				...
				{
					Debug.Assert(!"CPropertyManager::Put cannot write to pack file");
					return false;
				}
			}
		...
		public bool Erase(uint dwCRC)
		{
			...
			{
				string szCRC = new string(new char[64 + 1]);
				_snprintf(szCRC, sizeof(char), "%u\r\n", pProperty.GetCRC());
		...
		public bool Erase(string c_pszFileName)
		{
		...
		protected bool m_isFileMode;
		protected SortedDictionary<uint, CProperty > m_PropertyByCRCMap = new SortedDictionary<uint, CProperty >();
		protected SortedSet<uint> m_ReservedCRCSet = new SortedSet<uint>();
		protected CEterPack m_pack = new CEterPack();
