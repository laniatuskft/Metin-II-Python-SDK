...
public class CMapManager : CScreen, IPhysicsWorld
{
		public CMapManager()
		{
			this.mc_pcurEnvironmentData = null;
			m_pkMap = null;
		...
		public CMapOutdoor GetMapOutdoorRef()
		{
			Debug.Assert(null != m_pkMap);
			return m_pkMap;
		}

		public bool IsSoftwareTilingEnable()
		{
			return CTerrainPatch.SOFTWARE_TRANSFORM_PATCH_ENABLE;
		}

		public void ReserveSoftwareTilingEnable(bool isEnable)
		{
		...
		public void Create()
		{
			Debug.Assert(null == m_pkMap && "CMapManager::Create");
			if (m_pkMap)
			{
				Clear();
				return;
			...
			Debug.Assert(null != m_pkMap && "CMapManager::Create MAP is NULL");
		...
		public virtual CMapBase AllocMap()
		{
		...
		public virtual bool LoadMap(in string c_rstrMapName, float x, float y, float z)
		{
		...
		public bool UnloadMap(in string c_strMapName)
		{
			CMapOutdoor rkMap = GetMapOutdoorRef();
			if (c_strMapName != rkMap.GetName() && "" != rkMap.GetName())
			{
				LogBoxf("%s: Unload Map Failed", c_strMapName);
				return false;
			}
		...
		public bool UpdateMap(float fx, float fy, float fz)
		{
			if (!m_pkMap)
			{
				return false;
		...
		public void SetEnvironmentDataPtr(TEnvironmentData c_pEnvironmentData)
		{
			if (!m_pkMap)
			{
				return;
			...
			if (c_pEnvironmentData == null)
			{
				Debug.Assert(!"null environment data");
		...
		public void ResetEnvironmentDataPtr(TEnvironmentData c_pEnvironmentData)
		{
			if (!m_pkMap)
			{
				return;
		...
		public void SetEnvironmentData(int nEnvDataIndex)
		{
			TEnvironmentData c_pEnvironmenData;
		...
		public void BeginEnvironment()
		{
			...
			CMapOutdoor rkMap = GetMapOutdoorRef();
			 ...
			 (CStateManager.Instance()).SaveRenderState(D3DRS_LIGHTING, true);
			 (CStateManager.Instance()).SaveRenderState(D3DRS_FOGENABLE, mc_pcurEnvironmentData.bFogEnable);
			(CStateManager.Instance()).SetMaterial(mc_pcurEnvironmentData.Material);

			if (mc_pcurEnvironmentData.bDirLightsEnable[ENV_DIRLIGHT_BACKGROUND])
			{
				ms_lpd3dDevice.LightEnable(0, true);

				rkMap.ApplyLight((uint)mc_pcurEnvironmentData, mc_pcurEnvironmentData.DirLights[ENV_DIRLIGHT_BACKGROUND]);
			}
			else
			{
			...
			{
				uint dwFogColor = mc_pcurEnvironmentData.FogColor;
				...
				if (mc_pcurEnvironmentData.bDensityFog)
				{
					...
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGVERTEXMODE, D3DFOG_EXP);
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGDENSITY, (uint) & fDensity);
				}
				else
				{
					...
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGVERTEXMODE, D3DFOG_LINEAR);
					(CStateManager.Instance()).SetRenderState(D3DRS_RANGEFOGENABLE, true);
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGSTART, (uint) & fFogNear);
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGEND, (uint) & fFogFar);
				}
			}
		...
		public void BlendEnvironmentData(TEnvironmentData c_pEnvironmentData, int iTransitionTime)
		{
		}

		public void GetCurrentEnvironmentData(TEnvironmentData[] c_ppEnvironmentData)
		{
			c_ppEnvironmentData[0] = mc_pcurEnvironmentData;
		}

		public bool RegisterEnvironmentData(uint dwIndex, string c_szFileName)
		{
		...
		public bool GetEnvironmentData(uint dwIndex, TEnvironmentData[] c_ppEnvironmentData)
		{
			TEnvironmentDataMap.iterator itor = m_EnvironmentDataMap.find(dwIndex);

			if (m_EnvironmentDataMap.end() == itor)
			{
				c_ppEnvironmentData[0] = null;
				return false;
			}

			c_ppEnvironmentData[0] = itor.second;
			return true;
		}
		...
		public void AddShowingPortalID(int iID)
		{
			if (!IsMapReady())
			{
				return;
		...
		public void SetEnvironmentFog(bool flag)
		{
			if (mc_pcurEnvironmentData)
			{
		...
		public virtual bool isPhysicalCollision(in _D3DVECTOR c_rvCheckPosition)
		{
			if (!IsMapReady())
			{
				return false;
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private List<int> GetRenderedSplatNum_s_emptyVector = new List<int>();
...
//				static System.Collections.Generic.List<int> s_emptyVector;
				piPatch = null;
				piSplat = null;
				return GetRenderedSplatNum_s_emptyVector;
			}
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private CArea.TCRCWithNumberVector GetRenderedGraphicThingInstanceNum_s_emptyVector = new CArea.TCRCWithNumberVector();
...
//				static CArea::TCRCWithNumberVector s_emptyVector;
				pdwGraphicThingInstanceNum = null;
				pdwCRCNum = null;
				return GetRenderedGraphicThingInstanceNum_s_emptyVector;
			}
		...
		protected TEnvironmentData AllocEnvironmentData()
		{
		...
		protected void DeleteEnvironmentData(TEnvironmentData pEnvironmentData)
		{
			pEnvironmentData = null;
			pEnvironmentData = null;
		}

		protected bool LoadEnvironmentData(string c_szFileName, TEnvironmentData pEnvironmentData)
		{
		...
		protected CPropertyManager m_PropertyManager = new CPropertyManager();
		protected TEnvironmentDataMap m_EnvironmentDataMap = new TEnvironmentDataMap();
		protected readonly TEnvironmentData mc_pcurEnvironmentData;
		protected CMapOutdoor m_pkMap;
		protected CSpeedTreeForestDirectX8 m_Forest = new CSpeedTreeForestDirectX8();

		public void SetTerrainRenderSort(CMapOutdoor.ETerrainRenderSort eTerrainRenderSort)
		{
			if (!IsMapReady())
			{
				return;
		...
		public CMapOutdoor.ETerrainRenderSort GetTerrainRenderSort()
		{
			if (!IsMapReady())
			{
		...
		public void SetTransparentTree(bool bTransparenTree)
		{
			if (!IsMapReady())
			{
				return;
		...
		public class TMapInfo
		{
			...
			public uint m_dwBaseX;
			public uint m_dwBaseY;
			public uint m_dwSizeX;
			public uint m_dwSizeY;
			public uint m_dwEndX;
			public uint m_dwEndY;
		}

		protected List<TMapInfo> m_kVct_kMapInfo = new List<TMapInfo>();

		protected bool m_isSoftwareTilingEnableReserved;

		protected void __LoadMapInfoVector()
		{
			...
			string szMapName = new string(new char[256]);
			int x;
			int y;
			int width;
			int height;
			...
			{
				string c_rstLine = textFileLoader.GetLineString(uLineIndex);
				sscanf(c_rstLine, "%s %d %d %d %d", szMapName, x, y, width, height);
				...
				TMapInfo kMapInfo = new TMapInfo();
		...
		protected class FFindMapName
		{
			public string strNametoFind = "";
			public FFindMapName.FFindMapName(in string c_rMapName)
			{
		...
		public void SetAtlasInfoFileName(string filename)
		{
			m_stAtlasInfoFileName = filename;
		}
		private string m_stAtlasInfoFileName = "";
}
