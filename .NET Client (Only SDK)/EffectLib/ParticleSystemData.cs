...
public class CParticleSystemData : CEffectElementBase
{
		public virtual void Dispose()
		{
		}

		public CParticleSystemData()
		{
		}

		public CEmitterProperty GetEmitterPropertyPointer()
		{
			return m_EmitterProperty;
		}

		public CParticleProperty GetParticlePropertyPointer()
		{
			return m_ParticleProperty;
		}

		public void ChangeTexture(string c_szFileName)
		{
			m_ParticleProperty.SetTexture(c_szFileName);
		}

		public void BuildDecorator(CParticleInstance pInstance)
		{
			...
			if (m_ParticleProperty.m_TimeEventAirResistance.size() > 1)
			{
				...
				pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CAirResistanceValueDecorator(m_ParticleProperty.m_TimeEventAirResistance, pInstance.m_fAirResistance));
			}
			...
			if (m_ParticleProperty.m_TimeEventGravity.size() > 1)
			{
				...
				pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CGravityValueDecorator(m_ParticleProperty.m_TimeEventGravity, pInstance.m_fGravity));
			}
			...
			pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CColorAllDecorator(m_ParticleProperty.m_TimeEventColor, pInstance.m_dcColor));

			pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CScaleValueDecorator(m_ParticleProperty.m_TimeEventScaleX, pInstance.m_v2Scale.x));
			pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CScaleValueDecorator(m_ParticleProperty.m_TimeEventScaleY, pInstance.m_v2Scale.y));
			...
			if (m_ParticleProperty.GetTextureAnimationFrameCount() > 1 && m_ParticleProperty.GetTextureAnimationFrameDelay() > 1e-6)
			{
				switch (pInstance.m_byTextureAnimationType)
				{
					case CParticleProperty.TEXTURE_ANIMATION_TYPE_CW:
						pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CTextureAnimationCWDecorator(m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), pInstance.m_byFrameIndex));
						break;
					case CParticleProperty.TEXTURE_ANIMATION_TYPE_CCW:
						pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CTextureAnimationCCWDecorator(m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), pInstance.m_byFrameIndex));
						break;
					case CParticleProperty.TEXTURE_ANIMATION_TYPE_RANDOM_FRAME:
						pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CTextureAnimationRandomDecorator(m_ParticleProperty.GetTextureAnimationFrameDelay(), m_ParticleProperty.GetTextureAnimationFrameCount(), pInstance.m_byFrameIndex));
						break;
				}
			}
			...
			{
				byRotationType = (random() & 1) != 0?CParticleProperty.ROTATION_TYPE_CW:CParticleProperty.ROTATION_TYPE_CCW;
			}
			...
			switch (byRotationType)
			{
				case CParticleProperty.ROTATION_TYPE_TIME_EVENT:
					...
					pInstance.m_pDecorator = pInstance.m_pDecorator.AddChainFront(new CRotationSpeedValueDecorator(m_ParticleProperty.m_TimeEventRotation, pInstance.m_fRotationSpeed));
					break;
		...
		{
			{
			CTextFileLoader.CGotoChild GotoChild = new CTextFileLoader.CGotoChild(rTextFileLoader, "emitterproperty");
			...
			if (!GetTokenTimeEventFloat(rTextFileLoader, "timeeventemittingsize", m_EmitterProperty.m_TimeEventEmittingSize))
			{
				...
				CTimeEvent<float> TimeEventFloat = new CTimeEvent<float>();
			...
			{
			CTextFileLoader.CGotoChild GotoChild = new CTextFileLoader.CGotoChild(rTextFileLoader, "particleproperty");
			...
			{
				CTimeEvent<float> f = new CTimeEvent<float>();
				f.m_fTime = 0.0f;
				f.m_Value = fGravity;
			...
			List<CTimeEvent<float>> TimeEventR = new List<CTimeEvent<float>>();
			List<CTimeEvent<float>> TimeEventB = new List<CTimeEvent<float>>();
			List<CTimeEvent<float>> TimeEventG = new List<CTimeEvent<float>>();
			List<CTimeEvent<float>> TimeEventA = new List<CTimeEvent<float>>();
			...
			m_ParticleProperty.m_TimeEventColor.clear();
			{
				...
				for (it = times.GetEnumerator(); it.MoveNext();)
				{
					...
					CTimeEvent<DWORDCOLOR> t = new CTimeEvent<DWORDCOLOR>();
					t.m_fTime = fTime;
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < pTextureVector.Count; ++i)
			{
				...
				m_ParticleProperty.InsertTexture(strTextureFileName);
			}
			}
		...
		protected CEmitterProperty m_EmitterProperty = new CEmitterProperty();
		protected CParticleProperty m_ParticleProperty = new CParticleProperty();
		...
		public static CParticleSystemData New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CParticleSystemData pkData)
		{
			pkData.Clear();
			ms_kPool.Free(pkData);
		}

		public static CDynamicPool<CParticleSystemData> ms_kPool = new CDynamicPool<CParticleSystemData>();
}