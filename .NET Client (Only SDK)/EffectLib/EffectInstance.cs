...
public class CEffectInstance : CGraphicObjectInstance
{

		public const int ID = EFFECT_OBJECT;
...
//Original Metin2 CPlus Line: int GetType() const
		...
		public bool GetBoundingSphere(_D3DVECTOR v3Center, ref float fRadius)
		{
		...
		public static CEffectInstance New()
		{
			CEffectInstance pkEftInst = ms_kPool.Alloc();
			return pkEftInst;
		}

		public static void Delete(CEffectInstance pkEftInst)
		{
			pkEftInst.Clear();
			ms_kPool.Free(pkEftInst);
		}

		public static void ResetRenderingEffectCount()
		{
			ms_iRenderingEffectCount = 0;
		}

		public static int GetRenderingEffectCount()
		{
			return ms_iRenderingEffectCount;
		}

		public CEffectInstance()
		{
			__Initialize();
		}
		...
		{
			Debug.Assert(m_ParticleInstanceVector.empty());
			Debug.Assert(m_MeshInstanceVector.empty());
			Debug.Assert(m_LightInstanceVector.empty());
		}

		public bool LessRenderOrder(CEffectInstance pkEftInst)
		{
			return (m_pkEftData < pkEftInst.m_pkEftData);
		}

		public void SetEffectDataPointer(CEffectData pEffectData)
		{
			m_isAlive = true;
			...
			for (i = 0; LaniatusDefVariables < pEffectData.GetParticleCount(); ++i)
			{
			...
			for (i = 0; LaniatusDefVariables < pEffectData.GetMeshCount(); ++i)
			{
			...
			for (i = 0; LaniatusDefVariables < pEffectData.GetLightCount(); ++i)
			{
		...
		{
			m_EffName.assign(Name);
		}
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const string& GetEffectName() const
		...
		{
			std::for_each(m_ParticleInstanceVector.begin(), m_ParticleInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetActive));
			std::for_each(m_MeshInstanceVector.begin(), m_MeshInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetActive));
			std::for_each(m_LightInstanceVector.begin(), m_LightInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetActive));
		}

		public void SetDeactive()
		{
			std::for_each(m_ParticleInstanceVector.begin(), m_ParticleInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetDeactive));
			std::for_each(m_MeshInstanceVector.begin(), m_MeshInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetDeactive));
			std::for_each(m_LightInstanceVector.begin(), m_LightInstanceVector.end(), std::mem_fn(CEffectElementBaseInstance.SetDeactive));
		}

		public void SetGlobalMatrix(in _D3DMATRIX c_rmatGlobal)
		{
		...
		public void OnUpdate()
		{
			...
			FEffectUpdator f = new FEffectUpdator(CTimer.Instance().GetCurrentSecond() - m_fLastTime);
		...
		{
			(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_MINFILTER, D3DTEXF_NONE);
			(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_MAGFILTER, D3DTEXF_NONE);
			...
			(CStateManager.Instance()).SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TFACTOR);
		...
		public void OnBlendRender()
		{
		}
		public void OnRenderToShadowMap()
		{
		}
		public void OnRenderShadow()
		{
		}
		public void OnRenderPCBlocker()
		{
		}
		...
		protected void __SetParticleData(CParticleSystemData pData)
		{
		...
		protected void __SetMeshData(CEffectMeshScript pMesh)
		{
		...
		protected void __SetLightData(CLightData pData)
		{
		...
		protected virtual void OnUpdateCollisionData(CStaticCollisionDataVector pscdVector)
		{
		}
		...
		protected uint m_dwFrame;
		protected _D3DMATRIX m_matGlobal = new _D3DMATRIX();
		protected string m_EffName = "";

		protected CEffectData m_pkEftData;

		 protected List<CParticleSystemInstance> m_ParticleInstanceVector = new List<CParticleSystemInstance>();
		protected List<CEffectMeshInstance> m_MeshInstanceVector = new List<CEffectMeshInstance>();
		protected List<CLightInstance> m_LightInstanceVector = new List<CLightInstance>();

		protected List<SSoundInstance> m_pSoundInstanceVector;

		protected float m_fBoundingSphereRadius;
		protected _D3DVECTOR m_v3BoundingSpherePosition = new _D3DVECTOR();

		protected float m_fLastTime;

		public static CDynamicPool<CEffectInstance> ms_kPool = new CDynamicPool<CEffectInstance>();
		public static int ms_iRenderingEffectCount = 0;

		protected float m_fScaleParticle;
		protected _D3DVECTOR m_v3ScaleMesh = new _D3DVECTOR();

		public void SetParticleScale(float fScale)
		{
			m_fScaleParticle = fScale;
		}
		public void SetMeshScale(_D3DVECTOR rv3Scale)
		{
			m_v3ScaleMesh = rv3Scale;
		}
		public _D3DVECTOR GetMeshScale()
		{
			return m_v3ScaleMesh;
		}
}

public class FEffectUpdator
{
	public bool isAlive;
	public float fElapsedTime;
	public FEffectUpdator(float fElapsedTime)
	{
		this.isAlive = false;
		this.fElapsedTime = fElapsedTime;
	}
	public void functorMethod(CEffectElementBaseInstance pInstance)
	{
