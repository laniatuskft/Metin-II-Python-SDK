...
public class CParticleSystemInstance : CEffectElementBaseInstance
{
		...
		public static CParticleSystemInstance New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CParticleSystemInstance pkPSInst)
		{
			pkPSInst.Destroy();
			ms_kPool.Free(pkPSInst);
		}

		public static CDynamicPool<CParticleSystemInstance> ms_kPool = new CDynamicPool<CParticleSystemInstance>();
...
//Original Metin2 CPlus Line: template <typename T>
		public void ForEachParticleRendering<T>(T FunObj)
		{
			uint dwFrameIndex;
		...
		public CParticleSystemInstance()
		{
			Initialize();
		}
		...
		{
			Debug.Assert(m_ParticleInstanceListVector.empty());
			Debug.Assert(m_kVct_pkImgInst.empty());
		}

		public void OnSetDataPointer(CEffectElementBase pElement)
		{
		...
		public void CreateParticles(float fElapsedTime)
		{
			...
			int iMaxEmissionCount = m_pEmitterProperty.GetMaxEmissionCount();
			int iNextEmissionCount = icurEmissionCount + iCreatingCount;
			iCreatingCount -= Math.Max(0, iNextEmissionCount - iMaxEmissionCount);

			float fLifeTime = 0.0f;
			float fEmittingSize = 0.0f;
			_D3DVECTOR _v3TimePosition = new _D3DVECTOR();
			_D3DVECTOR _v3Velocity = new _D3DVECTOR();
			float fVelocity = 0.0f;
			D3DXVECTOR2 v2HalfSize = new D3DXVECTOR2();
			float fLieRotation = 0F;
			if (iCreatingCount != 0)
			{
				...
				if (BILLBOARD_TYPE_LIE == m_pParticleProperty.m_byBillboardType && mc_pmatLocal)
				{
					...
					if (fcx >= 0.00001f)
					{
						fLieRotation = ((atan2f(-mc_pmatLocal._12, mc_pmatLocal._22)) * (180.0f / ((float) 3.141592654f)));
					}
				}

			}

			CParticleInstance pFirstInstance = null;

			for (int LaniatusDefVariables = 0; LaniatusDefVariables < iCreatingCount; ++i)
			{
				...
				_D3DVECTOR v3TimePosition = new _D3DVECTOR(_v3TimePosition);
				...
				_D3DVECTOR v3Velocity = new _D3DVECTOR(_v3Velocity);
				if (mc_pmatLocal && !m_pParticleProperty.m_bAttachFlag)
				{
		...
		public bool InFrustum(CParticleInstance pInstance)
		{
			if (m_pParticleProperty.m_bAttachFlag)
			{
				return CScreen.GetFrustum().ViewVolumeTest(Vector3d(pInstance.m_v3Position.x + mc_pmatLocal._41, pInstance.m_v3Position.y + mc_pmatLocal._42, pInstance.m_v3Position.z + mc_pmatLocal._43),pInstance.GetRadiusApproximation()) != VS_OUTSIDE;
			}
			else
			{
		...
		{
			TParticleInstanceListVector.iterator LaniatusDefVariables = new TParticleInstanceListVector.iterator();
			for (i = m_ParticleInstanceListVector.begin(); LaniatusDefVariables != m_ParticleInstanceListVector.end(); ++i)
			{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to variables:
//Original Metin2 CPlus Line: TParticleInstanceList& rkLst_kParticleInst=*i;
				TParticleInstanceList rkLst_kParticleInst = *i;

				TParticleInstanceList.iterator j = new TParticleInstanceList.iterator();
		...
		protected bool OnUpdate(float fElapsedTime)
		{
			...
			if (fAngularVelocity != 0 && !m_pParticleProperty.m_bAttachFlag)
			{
		...
		protected float m_fEmissionResidue;

		protected uint m_dwCurrentEmissionCount;
		protected int m_iLoopCount;

		protected List<LinkedList<CParticleInstance>> m_ParticleInstanceListVector = new List<LinkedList<CParticleInstance>>();

		protected List<CGraphicImageInstance> m_kVct_pkImgInst = new List<CGraphicImageInstance>();

		protected CParticleSystemData m_pData;
...
{
	public class TwoSideRenderer
	{
		public readonly _D3DMATRIX pmat;
		public TwoSideRenderer(_D3DMATRIX pmat = null)
		{
			this.pmat = pmat;
		}

		public void functorMethod(CParticleInstance pInstance)
		{
			pInstance.Transform(pmat,((-30.0f) * (((float) 3.141592654f) / 180.0f)));
	...
	public class ThreeSideRenderer
	{
		public readonly _D3DMATRIX pmat;
		public ThreeSideRenderer(_D3DMATRIX pmat = null)
		{
			this.pmat = pmat;
		}
		...
		public void functorMethod(CParticleInstance pInstance)
		{
			...
			STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, pInstance.GetParticleMeshPointer(), sizeof(TPTVertex));
			pInstance.Transform(pmat,((-60.0f) * (((float) 3.141592654f) / 180.0f)));
			STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, pInstance.GetParticleMeshPointer(), sizeof(TPTVertex));
			pInstance.Transform(pmat,((+60.0f) * (((float) 3.141592654f) / 180.0f)));
			STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, pInstance.GetParticleMeshPointer(), sizeof(TPTVertex));
		}
	...
	public class AttachRenderer
	{
		public readonly _D3DMATRIX pmat;
		public AttachRenderer(_D3DMATRIX pmat)
		{
			this.pmat = pmat;
		}
