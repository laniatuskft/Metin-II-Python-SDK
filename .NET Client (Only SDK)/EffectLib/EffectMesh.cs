...
public class CEffectMesh : CResource
{
		public class SEffectFrameData
		{
			public byte byChangedFrame;
			public float fVisibility;
			public uint dwVertexCount;
			public uint dwTextureVertexCount;
			public uint dwIndexCount;
			public List<SPTVertex> PDTVertexVector = new List<SPTVertex>();
		}

		public class SEffectMeshData
		{
			public string szObjectName = new string(new char[32]);
			public string szDiffuseMapFileName = new string(new char[128]);

			public List<TEffectFrameData> EffectFrameDataVector = new List<TEffectFrameData>();
			public List<CGraphicImage> pImageVector = new List<CGraphicImage>();

			public static CEffectMesh.SEffectMeshData New()
			{
				return ms_kPool.Alloc();
			}

			public static void Delete(SEffectMeshData pkData)
			{
			...
			public static CDynamicPool<SEffectMeshData> ms_kPool = new CDynamicPool<SEffectMeshData>();
		}


//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static TType Type_s_type = StringToType("CEffectMesh");

		public static CEffectMesh.TType Type()
		{
...
//			static TType s_type = StringToType("CEffectMesh");
		...
		public CEffectMesh(string c_szFileName) : base(c_szFileName)
		{
			m_iGeomCount = 0;
			m_iFrameCount = 0;
			m_isData = false;
		...
		{
			return (uint)m_pEffectMeshDataVector.size();
		}

		public SEffectMeshData GetMeshDataPointer(uint dwMeshIndex)
		{
			Debug.Assert(dwMeshIndex < m_pEffectMeshDataVector.size());
			return m_pEffectMeshDataVector[dwMeshIndex];
		}

		public List<CGraphicImage> GetTextureVectorPointer(uint dwMeshIndex)
		{
		...
		public List<CGraphicImage> GetTextureVectorReference(uint dwMeshIndex)
		{
			return m_pEffectMeshDataVector[dwMeshIndex].pImageVector;
		}

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool GetMeshElementPointer(uint dwMeshIndex, SEffectMeshData[] ppMeshData);

		protected bool OnLoad(int iSize, object c_pvBuf)
		{
			if (!c_pvBuf)
			{
				return false;
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			byte * c_pbBuf = (byte) c_pvBuf;

			string szHeader = new string(new char[10 + 1]);
...
//Original Metin2 CPlus Line: bool OnIsEmpty() const
		protected bool OnIsEmpty()
		{
			return !m_isData;
		}

		protected bool OnIsType(TType type)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'c_pbBuf', so pointers on this parameter are left unchanged:
		protected bool __LoadData_Ver001(int iSize, in byte * c_pbBuf)
		{
			List<_D3DVECTOR> v3VertexVector = new List<_D3DVECTOR>();
			List<int> iIndexVector = new List<int>();
			List<D3DXVECTOR2> v3TextureVertexVector = new List<D3DXVECTOR2>();
			List<int> iTextureIndexVector = new List<int>();
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				...
				uint dwVertexCount;
				uint dwIndexCount;
				uint dwTextureVertexCount;

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(dwVertexCount, c_pbBuf, sizeof(uint));
				c_pbBuf += sizeof(uint);

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(dwIndexCount, c_pbBuf, sizeof(uint));
				c_pbBuf += sizeof(uint);

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				memcpy(dwTextureVertexCount, c_pbBuf, sizeof(uint));
				c_pbBuf += sizeof(uint);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(v3VertexVector[0], c_pbBuf, rFrameData.dwVertexCount * sizeof(_D3DVECTOR));
					c_pbBuf += rFrameData.dwVertexCount * sizeof(_D3DVECTOR);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					...
					for (uint j = 0; j < rFrameData.dwIndexCount; ++j)
					{
						SPTVertex rVertex = rFrameData.PDTVertexVector[j];

						uint dwIndex = (uint)iIndexVector[j];
						uint dwTextureIndex = (uint)iTextureIndexVector[j];

						Debug.Assert(dwIndex < v3VertexVector.Count);
						Debug.Assert(dwTextureIndex < v3TextureVertexVector.Count);
				...
				if (0 == string.CompareOrdinal(strExtension, "ifl"))
				{
					LPCVOID pMotionData = new LPCVOID();
					...
					if (CEterPackManager.Instance().Get(File, pMeshData.szDiffuseMapFileName, pMotionData))
					{
						...
						{
							string c_rstrFileName = textFileLoader.GetLineString(i);

							if (string.IsNullOrEmpty(c_rstrFileName))
							{
								continue;
							}
							...
							CGraphicImage pImage = (CGraphicImage)CResourceManager.Instance().GetResourcePointer(strTextureFileName);
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'c_pbBuf', so pointers on this parameter are left unchanged:
		protected bool __LoadData_Ver002(int iSize, in byte * c_pbBuf)
		{
			List<_D3DVECTOR> v3VertexVector = new List<_D3DVECTOR>();
			List<int> iIndexVector = new List<int>();
			List<D3DXVECTOR2> v3TextureVertexVector = new List<D3DXVECTOR2>();
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(rFrameData.dwVertexCount, c_pbBuf, sizeof(uint));
					c_pbBuf += sizeof(uint);

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(rFrameData.dwIndexCount, c_pbBuf, sizeof(uint));
					c_pbBuf += sizeof(uint);

//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
					memcpy(rFrameData.dwTextureVertexCount, c_pbBuf, sizeof(uint));
					c_pbBuf += sizeof(uint);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		protected int m_iGeomCount;
		protected int m_iFrameCount;
		protected List<SEffectMeshData > m_pEffectMeshDataVector = new List<SEffectMeshData >();

		protected bool m_isData;
}

public class CEffectMeshScript : CEffectElementBase
{
		public class SMeshData
		{
			public byte byBillboardType;

			public bool bBlendingEnable;
			public byte byBlendingSrcType;
			public byte byBlendingDestType;
			public bool bTextureAlphaEnable;

			public byte byColorOperationType;
			public D3DXCOLOR ColorFactor = new D3DXCOLOR();

			public bool bTextureAnimationLoopEnable;
			public float fTextureAnimationFrameDelay;

			public uint dwTextureAnimationStartFrame;

			public List<CTimeEvent<float>> TimeEventAlpha = new List<CTimeEvent<float>>();
		...
		public void ReserveMeshData(uint dwMeshCount)
		{
		...
		public bool CheckMeshIndex(uint dwMeshIndex)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool GetMeshDataPointer(uint dwMeshIndex, SMeshData[] ppMeshData);
		public int GetMeshDataCount()
		{
			return m_MeshDataVector.size();
		}

		public int GetBillboardType(uint dwMeshIndex)
		{
		...
		public bool isBlendingEnable(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false;
		...
		public byte GetBlendingSrcType(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false ? 1 : 0;
		...
		public byte GetBlendingDestType(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false ? 1 : 0;
		...
		public bool isTextureAlphaEnable(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false;
		...
		public bool GetColorOperationType(uint dwMeshIndex, ref byte pbyType)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false;
		...
		public bool GetColorFactor(uint dwMeshIndex, D3DXCOLOR pColor)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false;
		...
		public bool GetTimeTableAlphaPointer(uint dwMeshIndex, List<CTimeEvent<float>>[] pTimeEventAlpha)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return false;
			...
			pTimeEventAlpha[0] = &m_MeshDataVector[dwMeshIndex].TimeEventAlpha;

			return true;
		}

		public bool isMeshAnimationLoop()
		{
			return m_isMeshAnimationLoop;
		}

		public int GetMeshAnimationLoopCount()
		{
			return m_iMeshAnimationLoopCount;
		}

		public float GetMeshAnimationFrameDelay()
		{
			return m_fMeshAnimationFrameDelay;
		}

		public bool isTextureAnimationLoop(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return 0.0f != 0;
		...
		public float GetTextureAnimationFrameDelay(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return 0.0f;
		...
		public uint GetTextureAnimationStartFrame(uint dwMeshIndex)
		{
			if (!CheckMeshIndex(dwMeshIndex))
			{
				return 0;
		...
		protected bool OnLoadScript(CTextFileLoader rTextFileLoader)
		{
		...
		protected bool m_isMeshAnimationLoop;
		protected int m_iMeshAnimationLoopCount;
		protected float m_fMeshAnimationFrameDelay;
		protected List<SMeshData> m_MeshDataVector = new List<SMeshData>();

		protected string m_strMeshFileName = "";
		...
		public static CEffectMeshScript New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CEffectMeshScript pkData)
		{
			pkData.Clear();
			ms_kPool.Free(pkData);
		}

		public static CDynamicPool<CEffectMeshScript> ms_kPool = new CDynamicPool<CEffectMeshScript>();
}
