...
public class CEffectElementBase
{
		public CEffectElementBase()
		{
			m_fStartTime = 0.0f;
		}
		...
		public bool LoadScript(CTextFileLoader rTextFileLoader)
		{
		...
		public void GetPosition(float fTime, ref D3DXVECTOR3 rPosition)
		{
			...
			List<SEffectPosition>.Enumerator result = std::lower_bound(m_TimeEventTablePosition.begin(), m_TimeEventTablePosition.end(), fTime);

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to variables:
//Original Metin2 CPlus Line: SEffectPosition & rEffectPosition = *result;
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			SEffectPosition rEffectPosition = result;
			List<SEffectPosition>.Enumerator rPrev = result;
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to variables:
//Original Metin2 CPlus Line: SEffectPosition & rPrevEffectPosition = *rPrev;
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			SEffectPosition rPrevEffectPosition = rPrev;
			int iMovingType = rPrevEffectPosition.m_iMovingType;

			if (MOVING_TYPE_DIRECT == iMovingType)
			{
				float Head = Math.Abs(rEffectPosition.m_fTime - fTime) / Math.Abs(rEffectPosition.m_fTime - rPrevEffectPosition.m_fTime);
				float Tail = 1.0f - Math.Abs(rEffectPosition.m_fTime - fTime) / Math.Abs(rEffectPosition.m_fTime - rPrevEffectPosition.m_fTime);
			...
			else if (MOVING_TYPE_BEZIER_CURVE == iMovingType)
			{
				...
				rPosition = rPrevEffectPosition.m_vecPosition * (1.0f - ft) * (1.0f - ft) + (rPrevEffectPosition.m_vecPosition + rPrevEffectPosition.m_vecControlPoint) * (1.0f - ft) * ft * 2 + rEffectPosition.m_vecPosition * ft * ft;
			}
		}
		...
		protected abstract bool OnIsData();
		protected abstract bool OnLoadScript(CTextFileLoader rTextFileLoader);

		protected float m_fStartTime;
		protected List<SEffectPosition> m_TimeEventTablePosition = new List<SEffectPosition>();
}
