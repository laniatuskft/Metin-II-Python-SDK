...
public class CEffectData
{
		...
		public bool LoadScript(string c_szFileName)
		{
			...
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < TextFileLoader.GetChildNodeCount(); ++i)
			{
				...
				if (0 == string.CompareOrdinal(strName, "mesh"))
				{
					CEffectMeshScript pMesh = AllocMesh();
					pMesh.Clear();
					pMesh.LoadScript(TextFileLoader);
				}
				else if (0 == string.CompareOrdinal(strName, "particle"))
				{
				...
				else if (0 == string.CompareOrdinal(strName, "light"))
				{
			...
			string strPathHeader = "t:/laniaworkstate/";
			string strNoExtensionName = CFileNameHelper.NoExtension(m_strFileName);
			int iPos = strNoExtensionName.IndexOf(strPathHeader);
			if (iPos >= 0)
			{
			if (strNoExtensionName.Length > strPathHeader.Length)
			{
				string strSoundFileName = "";
				strSoundFileName = "sound/";
				strSoundFileName += strNoExtensionName[strPathHeader.Length];
				strSoundFileName += ".mss";

				LoadSoundScriptData(strSoundFileName);
			}
			}
		...
		{
			return (uint)m_ParticleVector.size();
		}

		public CParticleSystemData GetParticlePointer(uint dwPosition)
		{
		...
		{
			return (uint)m_MeshVector.size();
		}

		public CEffectMeshScript GetMeshPointer(uint dwPosition)
		{
			Debug.Assert(dwPosition < m_MeshVector.size());
			return m_MeshVector[dwPosition];
		}

		public uint GetLightCount()
		{
			return (uint)m_LightVector.size();
		}

		public CLightData GetLightPointer(uint dwPosition)
		{
			Debug.Assert(dwPosition < m_LightVector.size());
			return m_LightVector[dwPosition];
		}

		public List<SSoundInstance> GetSoundInstanceVector()
		{
		...
		public D3DXVECTOR3 GetBoundingSpherePosition()
		{
...
//Original Metin2 CPlus Line: const char * GetFileName() const
		...
		protected virtual CEffectMeshScript AllocMesh()
		{
		...
		protected virtual CLightData AllocLight()
		{
		...
		protected List<CParticleSystemData> m_ParticleVector = new List<CParticleSystemData>();
		protected List<CEffectMeshScript> m_MeshVector = new List<CEffectMeshScript>();
		protected List<CLightData> m_LightVector = new List<CLightData>();
		protected List<SSoundInstance> m_SoundInstanceVector = new List<SSoundInstance>();

		protected float m_fBoundingSphereRadius;
		protected D3DXVECTOR3 m_v3BoundingSpherePosition = new D3DXVECTOR3();
		...
		public static CEffectData New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CEffectData pkData)
		{
			pkData.Clear();
			ms_kPool.Free(pkData);
		}

		public static CDynamicPool<CEffectData> ms_kPool = new CDynamicPool<CEffectData>();
}