...
public class CEffectManager : CScreen, CSingleton<CEffectManager>
{
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private List<CEffectInstance> Render_s_kVct_pkEftInstSort = new List<CEffectInstance>();
...
//				static System.Collections.Generic.List<CEffectInstance*> s_kVct_pkEftInstSort;
				Render_s_kVct_pkEftInstSort.clear();
				...
				{
					Render_s_kVct_pkEftInstSort.push_back(i.second);
				}

				std::sort(Render_s_kVct_pkEftInstSort.begin(), Render_s_kVct_pkEftInstSort.end(), CEffectManager_LessEffectInstancePtrRenderOrder());
				std::for_each(Render_s_kVct_pkEftInstSort.begin(), Render_s_kVct_pkEftInstSort.end(), CEffectManager_FEffectInstanceRender());
			}
		}
		...
		public bool RegisterEffect(string c_szFileName, bool isExistDelete = false, bool isNeedCache = false)
		{
			string strFileName = "";
		...
		public bool RegisterEffect2(string c_szFileName, ref uint pdwRetCRC, bool isNeedCache = false)
		{
			string strFileName = "";
			StringPath(c_szFileName, strFileName);
			uint dwCRC = GetCaseCRC32(strFileName, strFileName.Length);
		...
		public int CreateEffect(uint dwID, in _D3DVECTOR c_rv3Position, in _D3DVECTOR c_rv3Rotation, float fParticleScale = 1.0f)
		{
			...
			D3DXMatrixRotationYawPitchRoll(mat, ((c_rv3Rotation.x) * (((float) 3.141592654f) / 180.0f)), ((c_rv3Rotation.y) * (((float) 3.141592654f) / 180.0f)), ((c_rv3Rotation.z) * (((float) 3.141592654f) / 180.0f)));
		...
		public int CreateEffect(string c_szFileName, in _D3DVECTOR c_rv3Position, in _D3DVECTOR c_rv3Rotation, float fParticleScale = 1.0f)
		{
			uint dwID = GetCaseCRC32(c_szFileName, strlen(c_szFileName));
			return CreateEffect(dwID, c_rv3Position, c_rv3Rotation, fParticleScale);
		}

		public void CreateEffectInstance(uint dwInstanceIndex, uint dwID, float fParticleScale = 1.0f, _D3DVECTOR c_pv3MeshScale = null)
		{
			if (dwID == 0)
			{
				return;
		...
		public bool SelectEffectInstance(uint dwInstanceIndex)
		{
		...
		public bool DestroyEffectInstance(uint dwInstanceIndex)
		{
			TEffectInstanceMap.iterator itor = m_kEftInstMap.find(dwInstanceIndex);
		...
		public void DeactiveEffectInstance(uint dwInstanceIndex)
		{
			TEffectInstanceMap.iterator itor = m_kEftInstMap.find(dwInstanceIndex);
		...
		public void HideOnSomeEffects(byte Type, bool IsVisible)
		{
			BLDisableEffects[Type] = !IsVisible;
		}

		public void SetEffectTextures(uint dwID, List<string> textures)
		{
		...
		public void SetEffectInstancePosition(in _D3DVECTOR c_rv3Position)
		{
		...
		public void SetEffectInstanceRotation(in _D3DVECTOR c_rv3Rotation)
		{
			if (!m_pSelectedEffectInstance)
			{
				return;
		...
		public void SetEffectInstanceGlobalMatrix(in _D3DMATRIX c_rmatGlobal)
		{
			if (!m_pSelectedEffectInstance)
			{
				return;
		...
		public uint GetRandomEffect()
		{
			int iIndex = random() % m_kEftDataMap.size();

			TEffectDataMap.iterator itor = m_kEftDataMap.begin();
			for (int LaniatusDefVariables = 0; LaniatusDefVariables < iIndex; ++i, ++itor)
			{
				;
			}
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private int GetEmptyIndex_iMaxIndex = 1;
...
//			static int iMaxIndex=1;
			...
			if (GetEmptyIndex_iMaxIndex > 2100000000)
			{
				GetEmptyIndex_iMaxIndex = 1;
			}

			int iNextIndex = GetEmptyIndex_iMaxIndex++;
		...
		public bool GetEffectData(uint dwID, CEffectData[] ppEffect)
		{
			...
			ppEffect[0] = itor.second;

			return true;
		}

		public bool GetEffectData(uint dwID, CEffectData[] c_ppEffect)
		{
			TEffectDataMap.iterator itor = m_kEftDataMap.find(dwID);
			...
			c_ppEffect[0] = itor.second;

			return true;
		}

		public void CreateUnsafeEffectInstance(uint dwEffectDataID, CEffectInstance[] ppEffectInstance)
		{
			CEffectData pEffect;
...
//Original Metin2 CPlus Line: *ppEffectInstance = pkEftInstNew;
			ppEffectInstance[0].CopyFrom(pkEftInstNew);
		}

		public bool DestroyUnsafeEffectInstance(CEffectInstance pEffectInstance)
		{
...
//		int GetRenderingEffectCount();
		...
		protected void __DestroyEffectInstanceMap()
		{
			for (TEffectInstanceMap.iterator LaniatusDefVariables = m_kEftInstMap.begin(); LaniatusDefVariables != m_kEftInstMap.end(); ++i)
			{
		...
		protected void __DestroyEffectCacheMap()
		{
			for (TEffectInstanceMap.iterator LaniatusDefVariables = m_kEftCacheMap.begin(); LaniatusDefVariables != m_kEftCacheMap.end(); ++i)
			{
				CEffectInstance pkEftInst = i.second;
				CEffectInstance.Delete(pkEftInst);
			}
		...
		protected void __DestroyEffectDataMap()
		{
			for (TEffectDataMap.iterator LaniatusDefVariables = m_kEftDataMap.begin(); LaniatusDefVariables != m_kEftDataMap.end(); ++i)
			{
		...
		protected bool m_isDisableSortRendering;
		protected SortedDictionary<uint, CEffectData> m_kEftDataMap = new SortedDictionary<uint, CEffectData>();
		protected SortedDictionary<uint, CEffectInstance> m_kEftInstMap = new SortedDictionary<uint, CEffectInstance>();
		protected SortedDictionary<uint, CEffectInstance> m_kEftCacheMap = new SortedDictionary<uint, CEffectInstance>();

		protected CEffectInstance m_pSelectedEffectInstance;
}


public class CEffectManager_LessEffectInstancePtrRenderOrder
{
...
public class CEffectManager_FEffectInstanceRender
{
