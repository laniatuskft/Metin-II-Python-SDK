...
public class CFileBase
{
		...
		public bool Create(string filename, EFileMode mode)
		{
			Destroy();
			...
			{
				dwMode = (uint)(GENERIC_READ | GENERIC_WRITE);
				dwShareMode = (uint)(FILE_SHARE_READ | FILE_SHARE_WRITE);
			}
			else
			{
				dwMode = GENERIC_READ;
			...
			m_hFile = CreateFile(filename, dwMode, dwShareMode, null, mode == FILEMODE_READ ? OPEN_EXISTING : OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, null);
		...
		public virtual bool Write(object src, int bytes)
		{
			uint dwUseless;
		...
		protected string m_filename = new string(new char[MAX_PATH + 1]);
		protected IntPtr m_hFile;
		protected uint m_dwSize;
}
