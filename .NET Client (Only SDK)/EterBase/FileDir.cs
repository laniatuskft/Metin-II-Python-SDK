...
public class CDir
{
		...
		public bool Create(string c_szFilter, string c_szPath = "", bool bCheckedExtension = false)
		{
			Destroy();

			string stPath = c_szPath;
			...
			m_hFind = FindFirstFile(stQuery, m_wfd);
			...
			do
			{
				...
				if (IsFolder())
				{
					if (!OnFolder(c_szFilter, stPath, m_wfd.cFileName))
					{
						return false;
					}
				}
...
//Original Metin2 CPlus Line: const char * c_szExtension = strchr(m_wfd.cFileName, '.');
					char c_szExtension = strchr(m_wfd.cFileName, '.');
					if (!c_szExtension != '\0')
					{
						continue;
					}
					...
					{
						string strFilter = c_szFilter;
						int iPos = strFilter.IndexOfAny((Convert.ToString(';')).ToCharArray(), 0);
						if (iPos > 0)
						{
							string strFirstFilter = new string(c_szFilter).substr(0, iPos);
							string strSecondFilter = new string(c_szFilter).substr(iPos + 1, strlen(c_szFilter));
							if (0 != string.CompareOrdinal(strFirstFilter, c_szExtension + 1) && 0 != string.CompareOrdinal(strSecondFilter, c_szExtension + 1))
							{
								continue;
							}
						}
					...
					if (!OnFile(stPath, m_wfd.cFileName))
					{
						return false;
					}
				}
		...
		protected abstract bool OnFolder(string c_szFilter, string c_szPath, string c_szName);
		protected abstract bool OnFile(string c_szPath, string c_szName);

		protected bool IsFolder()
		{
			if ((m_wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0)
			{
				return true;
			}
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		protected WIN32_FIND_DATA m_wfd = new WIN32_FIND_DATA();
		protected IntPtr m_hFind;
}
