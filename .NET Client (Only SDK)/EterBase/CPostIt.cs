...
public class CPostIt
{
	public CPostIt(string szAppName)
	{
	...
	public bool Flush()
	{
		...
		uDGPFormat = global::RegisterClipboardFormatA(m_szClipFormatName);
		if (global::OpenClipboard(null) == false)
		{
			return false;
		}
		if (global::SetClipboardData(uDGPFormat, m_pMemoryBlock.CreateHandle()) == null)
		{
	...
	public void Empty()
	{
		...
		IntPtr hClipboardMemory = global::GetClipboardData(uDGPFormat);
		if (hClipboardMemory)
		{
			global::SetClipboardData(uDGPFormat, null);
		}
		global::CloseClipboard();
	...
	public bool Get(string lpszKeyName, ref string lpBuffer, uint nSize)
	{
		if (m_pMemoryBlock == null)
		{
			uint uDGPFormat;
	...
	public bool Set(string lpBuffer)
	{
		if (lpBuffer == null)
		{
			return false;
	...
	public bool Set(string lpszKeyName, string lpBuffer)
	{
		if (m_pMemoryBlock == null)
		{
			m_pMemoryBlock = new _CPostItMemoryBlock();
	...
	public bool Set(string lpszKeyName, uint dwValue)
	{
		string szValue = new string(new char[12]);

		_snprintf(szValue, 12, "%d", dwValue);
		return Set(lpszKeyName, szValue);
	}

	public bool CopyTo(CPostIt pPostIt, string lpszKeyName)
	{
		if (m_pMemoryBlock == null)
		{
			return false;
...
//Original Metin2 CPlus Line: char* szText = m_pMemoryBlock->Find(lpszKeyName);
		char szText = m_pMemoryBlock.Find(lpszKeyName);
		if (szText == null)
		{
			return false;
		}
	...
	protected bool Init(string szAppName)
	{
		if (szAppName == null || !szAppName[0] != '\0')
		{
	...
	protected string m_szClipFormatName = new string(new char[_MAX_PATH]);
	protected _CPostItMemoryBlock m_pMemoryBlock;
}
...
public class _CPostItMemoryBlock
{
	public _CPostItMemoryBlock()
	{
		this.m_hHandle = null;
	}
	...
	public bool Assign(IntPtr hBlock)
	{
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		byte * lpBuffer = (byte) global::GlobalLock(hBlock);

		if (lpBuffer == null)
		{
			return false;
		}
		uint dwCount = (uint) lpBuffer;
		lpBuffer += sizeof(uint);
		for (uint dwI = 0; dwI < dwCount; dwI++)
		{
			ushort wLen = (ushort) lpBuffer;
			lpBuffer += sizeof(ushort);

			string lpszText = new string(new char[wLen]);
			global::CopyMemory(lpszText, lpBuffer, wLen);
			lpszText = StringFunctions.ChangeCharacter(lpszText, wLen, '\0');

			lpBuffer += (byte)wLen;

			Put(lpszText);
		}
		global::GlobalUnlock(hBlock);
		return true;
	}
	...
	public IntPtr CreateHandle()
	{
		...
		uint dwBlockSize = sizeof(uint);
		StrListItr itr = new StrListItr();

		for (itr = m_StrList.begin(); itr != m_StrList.end(); ++itr)
		{
			dwBlockSize += sizeof(ushort);
			dwBlockSize += (uint) strlen(*itr);
		}

		IntPtr hBlock = global::GlobalAlloc(GMEM_ZEROINIT | GMEM_MOVEABLE, dwBlockSize);
		if (hBlock == null)
		{
			return INVALID_HANDLE_VALUE;
		}
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		...
		if (lpBuffer == null)
		{
			global::GlobalFree(hBlock);
			return INVALID_HANDLE_VALUE;
		}
		(uint) lpBuffer = (uint) m_StrList.size();
		lpBuffer += sizeof(uint);
		for (itr = m_StrList.begin(); itr != m_StrList.end(); ++itr)
		{
			(ushort) lpBuffer = (ushort) strlen(*itr);
			lpBuffer += sizeof(ushort);
			global::CopyMemory(lpBuffer, *itr, strlen(*itr));
			lpBuffer += strlen(*itr);
		}
		global::GlobalUnlock(hBlock);
	...
	{
		global::GlobalFree(m_hHandle);
		m_hHandle = null;
		return true;
	}

	public string Find(string lpszKeyName)
	{
	...
	public bool Put(string lpBuffer)
	{
		string lpszText;
		...
		lpszText = new string(new char[nStrLen]);
		global::CopyMemory(lpszText, lpBuffer, nStrLen);
		lpszText = StringFunctions.ChangeCharacter(lpszText, nStrLen, '\0');

		m_StrList.push_back(lpszText);
		return true;
	}

	public bool Put(string lpszKeyName, string lpBuffer)
	{
		string lpszText;
		...
		lpszText = new string(new char[nStrLen]);
		global::CopyMemory(lpszText, lpszKeyName, strlen(lpszKeyName));
		lpszText[strlen(lpszKeyName)] = '=';
		global::CopyMemory(lpszText + strlen(lpszKeyName) + 1, lpBuffer, strlen(lpBuffer));
		lpszText[nStrLen] = '\0';
	...
	{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
		char * lpszText = Find(lpszKeyName);
		if (lpszText == null)
		{
			return false;
		}
		lpszText += (strlen(lpszKeyName) + 1);
		global::ZeroMemory(lpBuffer, nSize);
	...
	protected LinkedList<char > m_StrList = new LinkedList<char >();
}
