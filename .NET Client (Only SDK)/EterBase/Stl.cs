...
public class stl_sz_less
{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool operator ()(char * const & left, char * const & right) const
	public bool functorMethod(string left, string right)
	{
...
//Original Metin2 CPlus Line: template<typename TContainer>
public class stl_stack_pool <TData>
{
		public stl_stack_pool()
		{
			m_pos = 0;
		}

		public stl_stack_pool(int capacity)
		{
			m_pos = 0;
			initialize(capacity);
		}
		...
		public TData alloc()
		{
			Debug.Assert(!m_dataVector.empty() && "stl_stack_pool::alloc you MUST run stl_stack_pool::initialize");

			int max = m_dataVector.size();

			if (m_pos >= max)
			{
				Debug.Assert(!"stl_stack_pool::alloc OUT of memory");
				m_pos = 0;
			}
		...
		private List<TData> m_dataVector = new List<TData>();
}

//# Laniatus Games Studio Inc. | TODO TASK: C++ template specifiers containing defaults cannot be converted to C#:
//Original Metin2 CPlus Line: template<typename TData, typename THandle=int>
public class stl_circle_pool <TData>
{

		public stl_circle_pool()
		{
			initialize();
		}
		public virtual void Dispose()
		...
		public void create(int size)
		{
			...
			m_size = size;
			m_datas = Arrays.InitializeWithDefaultInstances<TData>(m_size);
		...
		public THandle alloc()
		{
			...
			Debug.Assert(!"Out of Memory");

			return 0;
		}
		public void free(THandle handle)
		{
			Debug.Assert(check(handle) && "Out of RANGE");
			m_flags[handle] = false;
		}
		public bool check(THandle handle)
		{
			if (handle >= m_size)
			{
				return false;
		...
		public TData refer(THandle handle)
		{
			Debug.Assert(check(handle) && "Out of RANGE");
		...
		protected TData[] m_datas;
		protected bool[] m_flags;

		protected THandle m_size = new THandle();

		protected THandle m_pos = new THandle();
...
{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: size_t GetHash(const string & str) const
	public size_t GetHash(in string str)
	{
...
//Original Metin2 CPlus Line: const byte * end = s + str.size();
	   byte end = (byte)(s + str.Length);
	   size_t h = 0;
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: size_t operator ()(const string & str) const
	public size_t functorMethod(in string str)
	{
		return GetHash(str);
	}
}