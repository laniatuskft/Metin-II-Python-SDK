...
public class CMemoryTextFileLoader
{
		public CMemoryTextFileLoader()
		{
		}
		...
		public void Bind(int bufSize, object c_pvBuf)
		{
			m_stLineVector.reserve(128);
			m_stLineVector.clear();

			string c_pcBuf = (string)c_pvBuf;
			string stLine = "";
			int pos = 0;
		...
		{
			return (uint)m_stLineVector.size();
		}

		public bool CheckLineIndex(uint dwLine)
		{
		...
		public bool SplitLine(uint dwLine, List<string> pstTokenVector, string c_szDelimeter = " \t")
		{
			pstTokenVector.Capacity = 10;
			pstTokenVector.Clear();

			string stToken = "";
			string c_rstLine = GetLineString(dwLine);
			...
			do
			{
				...
				{
					++beginPos;
					endPos = c_rstLine.IndexOfAny((Convert.ToString("\"")).ToCharArray(), beginPos);
				...
				{
					endPos = c_rstLine.IndexOfAny(c_szDelimeter.ToCharArray(), beginPos);
					basePos = (uint)endPos;
				}

				pstTokenVector.Add(c_rstLine.Substring(beginPos, endPos - beginPos));
		...
		public int SplitLine2(uint dwLine, List<string> pstTokenVector, string c_szDelimeter = " \t")
		{
			pstTokenVector.Capacity = 10;
			pstTokenVector.Clear();
		...
		public bool SplitLineByTab(uint dwLine, List<string> pstTokenVector)
		{
			pstTokenVector.Capacity = 10;
			pstTokenVector.Clear();

			string c_rstLine = GetLineString(dwLine);
			int c_iLineLength = c_rstLine.Length;
			...
			do
			{
				int beginPos = c_rstLine.IndexOfAny((Convert.ToString("\t")).ToCharArray(), basePos);

				pstTokenVector.Add(c_rstLine.Substring(basePos, beginPos - basePos));
		...
		public string GetLineString(uint dwLine)
		{
			Debug.Assert(CheckLineIndex(dwLine));
			return m_stLineVector[dwLine];
		}

		protected List<string> m_stLineVector = new List<string>();
}

public class CMemoryFileLoader
{
		public CMemoryFileLoader(int size, object c_pvMemoryFile)
		{
			Debug.Assert(c_pvMemoryFile != null);

			m_pos = 0;
			m_size = size;
			m_pcBase = (string) c_pvMemoryFile;
		}
		...
		public bool Read(int size, object pvDst)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		protected bool IsReadableSize(int size)
		{
		...
		protected string GetCurrentPositionPointer()
		{
			Debug.Assert(m_pcBase != null);
			return (m_pcBase + m_pos);
		}

		protected readonly string m_pcBase;
		protected int m_size;
		protected int m_pos;
}

public class CDiskFileLoader
{
		public CDiskFileLoader()
		{
			Initialize();
		}
		...
		public bool Open(string c_szFileName)
		{
			Close();
			...
			fseek(m_fp, 0, DefineConstants.SEEK_END);
			m_size = ftell(m_fp);
			fseek(m_fp, 0, DefineConstants.SEEK_SET);
			return true;
		}

		public bool Read(int size, object pvDst)
		{
			Debug.Assert(m_fp != null);
