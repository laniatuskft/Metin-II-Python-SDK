...
public class CMappedFile : CFileBase
{
	...
	{
		this.m_hFM = null;
		this.m_lpMapData = null;
		this.m_dataOffset = 0;
		this.m_mapSize = 0;
		this.m_seekPosition = 0;
		this.m_pLZObj = null;
		this.m_pbBufLinkData = null;
		this.m_dwBufLinkSize = 0;
		this.m_pbAppendResultDataBlock = null;
		this.m_dwAppendResultDataSize = 0;
	}
	...
	public void Link(uint dwBufSize, object c_pvBufData)
	{
	...
	public bool Create(string filename, object[] dest, int offset, int size)
	{
		if (!Create(filename))
		{
			return null;
	...
	public int Seek(uint offset, int iSeekType = SEEK_TYPE_BEGIN)
	{
		switch (iSeekType)
		{
			...
			case SEEK_TYPE_CURRENT:
				m_seekPosition = Math.Min(m_seekPosition + offset, Size());
				break;

			case SEEK_TYPE_END:
				m_seekPosition = Math.Max(0, Size() - offset);
				break;
		}
	...
	public int Map(object[] dest, int offset = 0, int size = 0)
	{
		m_dataOffset = offset;
		...
		uint dwSysGran = SysInfo.dwAllocationGranularity;
		uint dwFileMapStart = (m_dataOffset / dwSysGran) * dwSysGran;
		uint dwMapViewSize = (m_dataOffset % dwSysGran) + m_mapSize;
		int iViewDelta = m_dataOffset - dwFileMapStart;


		m_hFM = CreateFileMapping(m_hFile, null, PAGE_READONLY, 0, m_dataOffset + m_mapSize, null);
		...
		m_lpData = (string) m_lpMapData + iViewDelta;
		dest[0] = (string) m_lpData;
		m_seekPosition = 0;
	...
	public bool Read(object dest, int bytes)
	{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
	...
	public uint GetSeekPosition()
	{
		return m_seekPosition;
	}

	public void BindLZObject(CLZObject pLZObj)
	{
		Debug.Assert(m_pLZObj == null);
		m_pLZObj = pLZObj;

		Link(m_pLZObj.GetSize(), m_pLZObj.GetBuffer());
	}

	public void BindLZObjectWithBufferedSize(CLZObject pLZObj)
	{
		Debug.Assert(m_pLZObj == null);
		m_pLZObj = pLZObj;
...
//Original Metin2 CPlus Line: byte* AppendDataBlock(const object* pBlock, uint dwBlockSize)
	public byte[] AppendDataBlock(object pBlock, uint dwBlockSize)
	{
		if (m_pbAppendResultDataBlock)
		{
			m_pbAppendResultDataBlock = null;
...
//# Laniatus Games Studio Inc. | WARN: C# has no equivalent to methods returning pointers to value types:
//Original Metin2 CPlus Line: byte * GetCurrentSeekPoint()
	public byte GetCurrentSeekPoint()
	{
		return m_pbBufLinkData + m_seekPosition;
	}

	private void Unmap(LPCVOID data)
	{
...
//Original Metin2 CPlus Line: byte* m_pbBufLinkData;
	private byte m_pbBufLinkData;
	private uint m_dwBufLinkSize;

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte* m_pbAppendResultDataBlock;
	private byte m_pbAppendResultDataBlock;
	private uint m_dwAppendResultDataSize;

	private uint m_seekPosition;
	private IntPtr m_hFM;
	private uint m_dataOffset;
	private uint m_mapSize;
	private object m_lpMapData;
	private object m_lpData;

	private CLZObject m_pLZObj;
}
