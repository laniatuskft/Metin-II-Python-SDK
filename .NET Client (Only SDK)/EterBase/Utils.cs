...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ishprint(x) ((((x) & 0xE0) > 0x90) || isprint(x))

public class CDirRemover : CDir
{
		public CDirRemover()
		{
		}
		...
		public bool OnFolder(string c_szFilter, string c_szPathName, string c_szFileName)
		{
			string strFullPathName = "";
			strFullPathName = c_szPathName;
			strFullPathName += c_szFileName;

			CDirRemover remover = new CDirRemover();
			remover.Create(c_szFilter, strFullPathName);
		...
		public bool OnFile(string c_szPathName, string c_szFileName)
		{
			string strFullPathName = "";
			strFullPathName = c_szPathName;
			strFullPathName += c_szFileName;
			_chmod(strFullPathName, DefineConstants._S_IWRITE);
			DeleteFile(strFullPathName);
			return true;
		}

		public static void RemoveAllDirectory()
		{
			for (LinkedList<string>.Enumerator itor = ms_strDirectoryDeque.begin(); itor.MoveNext();)
			{
				string c_rstrDirectory = itor.Current;
				RemoveDirectory(c_rstrDirectory);
			}

			ms_strDirectoryDeque.clear();
		}

		protected static LinkedList<string> ms_strDirectoryDeque = new LinkedList<string>();
}