...
namespace UI
{
	...
	public class CSlotWindow : CWindow
	{
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
			private static int Type_s_Type = GetCRC32("CSlotWindow", strlen("CSlotWindow"));
...
//				static int s_Type = GetCRC32("CSlotWindow", strlen("CSlotWindow"));
			...
			public class CSlotButton : CButton
			{
					public enum ESlotButtonType
					{
						SLOT_BUTTON_TYPE_PLUS,
						SLOT_BUTTON_TYPE_COVER
					}

					public CSlotButton(ESlotButtonType dwType, uint dwSlotNumber, CSlotWindow pParent) : base(null)
					{
					...
					public bool OnMouseLeftButtonDown()
					{
						if (!IsEnable())
						{
							return true;
					...
					public bool OnMouseLeftButtonUp()
					{
						if (!IsEnable())
						{
							return true;
					...
					protected ESlotButtonType m_dwSlotButtonType = new ESlotButtonType();
					protected uint m_dwSlotNumber;
					protected CSlotWindow m_pParent;
			}
			public class CCoverButton : CSlotButton
			{
					public CCoverButton(ESlotButtonType dwType, uint dwSlotNumber, CSlotWindow pParent) : base(dwType, dwSlotNumber, pParent)
					{
					...
					public void SetLeftButtonEnable(bool bEnable)
					{
						m_bLeftButtonEnable = bEnable;
					}
					public void SetRightButtonEnable(bool bEnable)
					{
					...
					public bool OnMouseRightButtonDown()
					{
						if (!IsEnable())
						{
							return true;
					...
					protected bool m_bLeftButtonEnable;
					protected bool m_bRightButtonEnable;
			}
			public class CCoolTimeFinishEffect : CAniImageBox
			{
					public CCoolTimeFinishEffect(CSlotWindow pParent, uint dwSlotIndex) : base(null)
					{
						m_pParent = pParent;
...
//			friend class CCoverButton;
			...
			{
				public uint dwState;
				public uint dwSlotNumber;
				public uint dwCenterSlotNumber;
				public uint dwItemIndex;
				public bool isItem;

				public float fCoolTime;
				public float fStartCoolTime;

				public bool bActive;
				public D3DXCOLOR color = new D3DXCOLOR();

				public int ixPosition;
				public int iyPosition;

				public int ixCellSize;
				public int iyCellSize;

				public byte byxPlacedItemSize;
				public byte byyPlacedItemSize;

				public CGraphicImageInstance pInstance;
				public CNumberLine pNumberLine;

				public bool bRenderBaseSlotImage;
				public CCoverButton pCoverButton;
				public CSlotButton pSlotButton;
				public CImageBox pSignImage;
				public CAniImageBox pFinishCoolTimeEffect;
			}
			public class SStoreCoolDown
			{
				public float fCoolTime;
			...
			public CSlotWindow(PyObject ppyObject) : base(ppyObject)
			{
				__Initialize();
			}
...
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
			...
			public void SetSlotType(uint dwType)
			{
				m_dwSlotType = dwType;
			}

			public void SetSlotStyle(uint dwStyle)
			{
				m_dwSlotStyle = dwStyle;
			}

			public void AppendSlot(uint dwIndex, int ixPosition, int iyPosition, int ixCellSize, int iyCellSize)
			{
				TSlot Slot = new TSlot();
				Slot.pInstance = null;
			...
			public void SetCoverButton(uint dwIndex, string c_szUpImageName, string c_szOverImageName, string c_szDownImageName, string c_szDisableImageName, bool bLeftButtonEnable, bool bRightButtonEnable)
			{
				TSlot pSlot;
			...
			public void SetSlotBaseImage(string c_szFileName, float fr, float fg, float fb, float fa)
			{
				__CreateBaseImage(c_szFileName, fr, fg, fb, fa);
			}

			public void AppendSlotButton(string c_szUpImageName, string c_szOverImageName, string c_szDownImageName)
			{
				for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
				{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to variables:
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
			...
			public void AppendRequirementSignImage(string c_szImageName)
			{
				for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
				{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to variables:
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
			...
			public void EnableCoverButton(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void DisableCoverButton(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void SetAlwaysRenderCoverButton(uint dwIndex, bool bAlwaysRender = false)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
				...
				if (bAlwaysRender)
				{
					((pSlot.dwState) |= (SLOT_STATE_ALWAYS_RENDER_COVER));
				}
				else
				{
					((pSlot.dwState) &= ~(SLOT_STATE_ALWAYS_RENDER_COVER));
			...
			public void ShowSlotBaseImage(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void HideSlotBaseImage(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public bool IsDisableCoverButton(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public bool HasSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void ClearSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void SetSlot(uint dwIndex, uint dwVirtualNumber, byte byWidth, byte byHeight, CGraphicImage pImage, D3DXCOLOR diffuseColor)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
				...
				if (pSlot.dwItemIndex == dwVirtualNumber)
				{
					pSlot.dwState = 0;
					pSlot.isItem = true;
					if (pImage != null && pSlot.pInstance)
					{
				...
				if (pImage != null)
				{
					Debug.Assert(null == pSlot.pInstance);
			...
			public void SetSlotCount(uint dwIndex, uint dwCount)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
				...
				{
					string szCount = new string(new char[16 + 1]);
					_snprintf(szCount, sizeof(char), "%d", dwCount);
			...
			public void SetSlotCountNew(uint dwIndex, uint dwGrade, uint dwCount)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
				...
				else
				{
					...
					switch (dwGrade)
					{
						case 0:
							...
							break;
						case 1:
							_snprintf(szCount, sizeof(char), "m%d", dwCount);
							break;
						case 2:
							_snprintf(szCount, sizeof(char), "g%d", dwCount);
							break;
						case 3:
							_snprintf(szCount, sizeof(char), "p");
							break;
					}
			...
			public void SetSlotCoolTime(uint dwIndex, float fCoolTime, float fElapsedTime = 0.0f)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void StoreSlotCoolTime(uint dwKey, uint dwSlotIndex, float fCoolTime, float fElapsedTime = .0f)
			{
				SortedDictionary<uint, SStoreCoolDown>.Enumerator it = m_CoolDownStore[dwKey].find(dwSlotIndex);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
				...
				{
					SStoreCoolDown m_storeCoolDown = new SStoreCoolDown();
					...
					m_CoolDownStore[dwKey].insert(SortedDictionary<uint, SStoreCoolDown>.value_type(dwSlotIndex, m_storeCoolDown));
				}
			}

			public void RestoreSlotCoolTime(uint dwKey)
			{
				for (SortedDictionary<uint, SStoreCoolDown>.Enumerator it = m_CoolDownStore[dwKey].begin(); it.MoveNext();)
				{
					TSlot pSlot;
					if (!GetSlotPointer(it.Current.Key, pSlot))
					{
						return;
					}

					pSlot.fCoolTime = it.Current.Value.fCoolTime;
					pSlot.fStartCoolTime = it.Current.Value.fElapsedTime;
					if (it.Current.Value.bActive)
					{
						ActivateSlot(it.Current.Key);
					}
					else
					{
						DeactivateSlot(it.Current.Key);
					}
				}
			}

			public void ActivateSlot(uint dwIndex, float r = 1.0f, float g = 1.0f, float b = 1.0f, float a = 1.0f)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
				...
				pSlot.bActive = true;
				pSlot.color = D3DXCOLOR(r, g, b, a);

				if (!m_apSlotActiveEffect[0] || !m_apSlotActiveEffect[1] || !m_apSlotActiveEffect[2])
				{
					__CreateSlotEnableEffect();

					SortedDictionary<uint, SStoreCoolDown>.Enumerator it = m_CoolDownStore[1].find(dwIndex);
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			public void DeactivateSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			{
				return (uint)m_SlotList.size();
			}

			public void LockSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void UnlockSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
...
//			bool IsLockSlot(uint dwIndex);
			public void SetCantUseSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void SetUseSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			bool IsCantUseSlot(uint dwIndex);
			public void EnableSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
			...
			public void DisableSlot(uint dwIndex)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwIndex, pSlot))
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			bool IsEnableSlot(uint dwIndex);
			...
			public void SelectSlot(uint dwSelectingIndex)
			{
				LinkedList<uint>.Enumerator itor = m_dwSelectedSlotIndexList.begin();
				while (itor.MoveNext())
				{
					if (dwSelectingIndex == itor.Current)
					{
			...
			public bool isSelectedSlot(uint dwIndex)
			{
				LinkedList<uint>.Enumerator itor = m_dwSelectedSlotIndexList.begin();
				while (itor.MoveNext())
				{
					if (dwIndex == itor.Current)
					{
						return true;
					}
				}
			...
			public uint GetSelectedSlotCount()
			{
				return (uint)m_dwSelectedSlotIndexList.size();
			}

			public uint GetSelectedSlotNumber(uint dwIndex)
			{
				...
				return new LinkedList<uint>.Enumerator(itor);
			}

			public void ShowSlotButton(uint dwSlotNumber)
			{
				TSlot pSlot;
...
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
			...
			public void OnPressedSlotButton(uint dwType, uint dwSlotNumber, bool isLeft = true)
			{
			...
			public void ShowRequirementSign(uint dwSlotNumber)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwSlotNumber, pSlot))
				{
			...
			public void HideRequirementSign(uint dwSlotNumber)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwSlotNumber, pSlot))
				{
			...
			public bool OnOverInItem(uint dwSlotNumber)
			{
				TSlot pSlot;
				if (!GetSlotPointer(dwSlotNumber, pSlot))
				{
			...
			public void SetUseMode(bool bFlag)
			{
				m_isUseMode = bFlag;
			}

			public void SetUsableItem(bool bFlag)
			{
				m_isUsableItem = bFlag;
			}

			public void ReserveDestroyCoolTimeFinishEffect(uint dwSlotIndex)
			{
			...
			protected void __CreateToggleSlotImage()
			{
				__DestroyToggleSlotImage();

				m_pToggleSlotImage = new CImageBox(null);
				m_pToggleSlotImage.LoadImage("t:/laniaworkstate/uinterface/public/slot_toggle.sub");
				m_pToggleSlotImage.Show();
			}

			protected void __CreateSlotEnableEffect()
			{
				__DestroySlotEnableEffect();
				...
				for (int LaniatusDefVariables = 0; LaniatusDefVariables < ITEM_MAX_SIZE; ++i)
				{
					...
					for (int j = 0; j <= SLOT_ACTIVE_EFFECT_COUNT; ++j)
					{
						string buf = new string(new char[64 + 1]);
						sprintf_s(buf, "t:/laniaworkstate/uinterface/public/slotactiveeffect/slot%d/%02d.sub", (i + 1), j);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			void __CreateFinishCoolTimeEffect(SSlot pSlot);
			protected void __CreateBaseImage(string c_szFileName, float fr, float fg, float fb, float fa)
			{
				__DestroyBaseImage();
			...
			protected void __DestroyToggleSlotImage()
			{
			...
			protected void __DestroySlotEnableEffect()
			{
				for (int LaniatusDefVariables = 0; LaniatusDefVariables < ITEM_MAX_SIZE; ++i)
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			void __DestroyFinishCoolTimeEffect(SSlot pSlot);
			protected void __DestroyBaseImage()
			{
			...
			{
				for (LinkedList<uint>.Enumerator itor = m_ReserveDestroyEffectDeque.begin(); itor.MoveNext();)
				{
					uint dwSlotIndex = itor.Current;
			...
			protected void OnRender()
			{
				...
				TSlotListIterator itor = new TSlotListIterator();
...
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
					...
					if (!rSlot.isItem)
					{
						if (((rSlot.dwState) & (SLOT_STATE_ALWAYS_RENDER_COVER)) != 0)
						{
					...
					if (((rSlot.dwState) & (SLOT_STATE_DISABLE)) != 0)
					{
					...
					if (rSlot.bActive)
					{
						...
						if (iSize >= 0 && iSize < ITEM_MAX_SIZE && m_apSlotActiveEffect[iSize])
						{
							int ix = m_rect.left + rSlot.ixPosition;
							int iy = m_rect.top + rSlot.iyPosition;
							m_apSlotActiveEffect[iSize].SetPosition(ix, iy);
							m_apSlotActiveEffect[iSize].SetDiffuseColor(rSlot.color.r, rSlot.color.g, rSlot.color.b, rSlot.color.a);
			...
			protected bool OnMouseLeftButtonDown()
			{
				...
				if (pSlot.isItem && (pSlot.dwState & SLOT_STATE_LOCK) == 0)
				{
			...
			protected bool OnMouseRightButtonDown()
			{
				TSlot pSlot;
				if (!GetPickedSlotPointer(pSlot))
				{
			...
			protected void RenderSlotBaseImage()
			{
				if (!m_pBaseImageInstance)
				{
					return;
...
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
			...
			protected void RenderLockedSlot()
			{
...
//Original Metin2 CPlus Line: TSlot & rSlot = *itor;
					...
					if ((rSlot.dwState & SLOT_STATE_LOCK) != 0)
					{
						CPythonGraphic.Instance().RenderBar2d(m_rect.left + rSlot.ixPosition, m_rect.top + rSlot.iyPosition, m_rect.left + rSlot.ixPosition + rSlot.ixCellSize, m_rect.top + rSlot.iyPosition + rSlot.iyCellSize);
					}
				}
			...
			protected virtual void OnRenderPickingSlot()
			{
				if (!UI.CWindowManager.Instance().IsAttaching())
				{
					return;
				...
				CPythonGraphic.Instance().RenderBar2d(m_rect.left + pSlot.ixPosition, m_rect.top + pSlot.iyPosition, m_rect.left + pSlot.ixPosition + pSlot.ixCellSize, m_rect.top + pSlot.iyPosition + pSlot.iyCellSize);
			}

			protected virtual void OnRenderSelectedSlot()
			{
				LinkedList<uint>.Enumerator itor = m_dwSelectedSlotIndexList.begin();
				while (itor.MoveNext())
				{
					TSlot pSlot;
					if (!GetSlotPointer(itor.Current, pSlot))
					{
						continue;
					}
			...
			protected void OnSelectEmptySlot(int iSlotNumber)
			{
				PyCallClassMemberFunc(m_poHandler, "OnSelectEmptySlot", Py_BuildValue("(i)", iSlotNumber));
			}

			protected void OnSelectItemSlot(int iSlotNumber)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			bool GetSlotPointer(uint dwIndex, SSlot[] ppSlot);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			bool GetSelectedSlotPointer(SSlot[] ppSlot);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			virtual bool GetPickedSlotPointer(SSlot[] ppSlot);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			void ClearSlot(SSlot pSlot);
			protected virtual void OnRefreshSlot()
			{
			}
			...
			protected uint m_dwSlotType;
			protected uint m_dwSlotStyle;
			protected LinkedList<uint> m_dwSelectedSlotIndexList = new LinkedList<uint>();
			protected LinkedList<SSlot> m_SlotList = new LinkedList<SSlot>();
			protected uint m_dwToolTipSlotNumber;
			protected SortedDictionary<uint, SortedDictionary<uint, SStoreCoolDown>> m_CoolDownStore = new SortedDictionary<uint, SortedDictionary<uint, SStoreCoolDown>>();
			protected bool m_isUseMode;
			protected bool m_isUsableItem;

			protected CGraphicImageInstance m_pBaseImageInstance;
			protected CImageBox m_pToggleSlotImage;
			protected CAniImageBox[] m_apSlotActiveEffect = new CAniImageBox[ITEM_MAX_SIZE];
			protected LinkedList<uint> m_ReserveDestroyEffectDeque = new LinkedList<uint>();
	}
}
