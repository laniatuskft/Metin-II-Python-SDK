...
namespace UI
{
	public class CGridSlotWindow : CSlotWindow
	{
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
			private static int Type_s_Type = GetCRC32("CGridSlotWindow", strlen("CGridSlotWindow"));
...
//				static int s_Type = GetCRC32("CGridSlotWindow", strlen("CGridSlotWindow"));
			...
			public CGridSlotWindow(PyObject ppyObject) : base(ppyObject)
			{
			}
			...
			public void ArrangeGridSlot(uint dwStartIndex, uint dwxCount, uint dwyCount, int ixSlotSize, int iySlotSize, int ixTemporarySize, int iyTemporarySize)
			{
				Destroy();
				...
				for (uint x = 0; x < dwxCount; ++x)
				{
				for (uint y = 0; y < dwyCount; ++y)
				{
					uint dwIndex = dwStartIndex + x + y * dwxCount;
					int ixPosition = (int)(x * (ixSlotSize + ixTemporarySize));
					int iyPosition = (int)(y * (iySlotSize + iyTemporarySize));
				...
				int iWidth = (int)(dwxCount * (ixSlotSize + ixTemporarySize));
				int iHeight = (int)(dwyCount * (iySlotSize + iyTemporarySize));
			...
			protected bool GetPickedSlotPointer(TSlot[] ppSlot)
			{
				...
				LinkedList<TSlot> SlotList = new LinkedList<TSlot>();
				...
				for (LinkedList<TSlot>.Enumerator itor = SlotList.GetEnumerator(); itor.MoveNext();)
				{
					TSlot pSlot = itor.Current;
...
//Original Metin2 CPlus Line: *ppSlot = pCenterSlot;
					ppSlot[0].CopyFrom(pCenterSlot);

					if (UI.CWindowManager.Instance().IsAttaching())
					{
						uint dwSlotNumber = UI.CWindowManager.Instance().GetAttachingSlotNumber();

						if (dwSlotNumber == pCenterSlot.dwSlotNumber)
						{
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: *ppSlot = pMinSlot;
							ppSlot[0].CopyFrom(pMinSlot);
						}
					}
				}
			...
			protected bool GetPickedSlotList(int iWidth, int iHeight, LinkedList<TSlot> pSlotPointerList)
			{
				int lx;
				int ly;
				...
				if (GetPickedGridSlotPosition(lx, ly, ix, iy))
				{
					...
					if ((uint)ixEnd >= m_dwxCount)
					{
						int ixTemporary = (uint)ixEnd - m_dwxCount + 1;
						ixStart -= ixTemporary;
						ixEnd -= ixTemporary;
					}

					if ((uint)iyEnd >= m_dwyCount)
					{
						int iyTemporary = (uint)iyEnd - m_dwyCount + 1;
						iyStart -= iyTemporary;
						iyEnd -= iyTemporary;
					}

					for (int LaniatusDefVariables = ixStart; LaniatusDefVariables <= ixEnd; ++i)
					{
					for (int j = iyStart; j <= iyEnd; ++j)
					{
						TSlot pSlot;
						if (GetGridSlotPointer((uint)i, (uint)j, pSlot))
						{
							pSlotPointerList.AddLast(pSlot);
						}
					}
					}

					if (m_isUseMode && 1 == pSlotPointerList.Count)
					{
						TSlot pMainSlot = *pSlotPointerList.GetEnumerator();

						List<TSlot >.Enumerator itor = m_SlotVector.begin();
						while (itor.MoveNext())
						{
							TSlot pSlot = itor.Current;
							if (pSlot.dwCenterSlotNumber == pMainSlot.dwCenterSlotNumber)
							{
							if (pSlotPointerList.end() == std::find(pSlotPointerList.GetEnumerator(), pSlotPointerList.end(), pSlot))
							{
								pSlotPointerList.AddLast(pSlot);
							}
							}
			...
			protected bool GetGridSlotPointer(int ix, int iy, TSlot[] ppSlot)
			{
				uint dwSlotIndex = ix + iy * m_dwxCount;
				if (dwSlotIndex >= m_SlotVector.size())
				{
					return false;
				}

				ppSlot[0] = m_SlotVector[dwSlotIndex];

				return true;
			}

			protected bool GetPickedGridSlotPosition(int ixLocal, int iyLocal, ref int pix, ref int piy)
			{
				for (uint x = 0; x < m_dwxCount; ++x)
				{
				for (uint y = 0; y < m_dwyCount; ++y)
				{
					TSlot pSlot;
			...
			protected bool CheckMoving(uint dwSlotNumber, uint dwItemIndex, in LinkedList<TSlot> c_rSlotList)
			{
				...
				for (LinkedList<TSlot>.Enumerator itor = c_rSlotList.GetEnumerator(); itor.MoveNext();)
				{
					TSlot pSlot = itor.Current;
			...
			protected void OnRefreshSlot()
			{
				...
				for (y = 0; y < m_dwyCount; ++y)
				{
					...
					if (pSlot.isItem)
					{
						for (uint xSub = 0; xSub < pSlot.byxPlacedItemSize; ++xSub)
						{
						for (uint ySub = 0; ySub < pSlot.byyPlacedItemSize; ++ySub)
						{
							TSlot pSubSlot;
			...
			protected void OnRenderPickingSlot()
			{
				if (!UI.CWindowManager.Instance().IsAttaching())
				{
					return;
				...
				{
					uint dwSlotNumber = UI.CWindowManager.Instance().GetAttachingSlotNumber();
					uint dwItemIndex = UI.CWindowManager.Instance().GetAttachingIndex();

					if (m_isUseMode)
					{
						TSlot pSlot = *SlotList.GetEnumerator();
						TSlot pCenterSlot;
						...
						if (pCenterSlot.isItem)
						{
							...
							CPythonGraphic.Instance().RenderBar2d(m_rect.left + pCenterSlot.ixPosition, m_rect.top + pCenterSlot.iyPosition, m_rect.left + pCenterSlot.ixPosition + pCenterSlot.byxPlacedItemSize * ITEM_WIDTH, m_rect.top + pCenterSlot.iyPosition + pCenterSlot.byyPlacedItemSize * ITEM_HEIGHT);
							return;
						}
						}
					}
					...
					{
						TSlot pSlot = itor.Current;
						Rect.left = Math.Min(Rect.left, m_rect.left + pSlot.ixPosition);
						Rect.top = Math.Min(Rect.top, m_rect.top + pSlot.iyPosition);
						Rect.right = Math.Max(Rect.right, m_rect.left + pSlot.ixPosition + pSlot.byxPlacedItemSize * ITEM_WIDTH);
						Rect.bottom = Math.Max(Rect.bottom, m_rect.top + pSlot.iyPosition + pSlot.byxPlacedItemSize * ITEM_HEIGHT);
					}
			...
			protected uint m_dwxCount;
			protected uint m_dwyCount;

			protected List<TSlot > m_SlotVector = new List<TSlot >();
	}
}
