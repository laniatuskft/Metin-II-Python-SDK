...
public class CGrannyLODController : CGraphicBase
{
		public static void SetMinLODMode(bool isEnable)
		{
			ms_isMinLODModeEnable = isEnable;
		}

		public class FSetLocalTime
		{
			public float fLocalTime;
			public void functorMethod(CGrannyLODController pController)
			{
				pController.SetLocalTime(fLocalTime);
			}
		}

		public class FUpdateTime
		{
		...
		public class FUpdateLODLevel
		{
			public float fDistanceFromCenter;
			public float fDistanceFromCamera;
		...
		public class FRenderWithOneTexture
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FBlendRenderWithOneTexture
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FRenderWithTwoTexture
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FBlendRenderWithTwoTexture
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FRenderToShadowMap
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FDeform
		{
			public readonly D3DXMATRIX mc_pWorldMatrix;
		...
		public class FCreateDeviceObjects
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FDestroyDeviceObjects
		{
			public void functorMethod(CGrannyLODController pController)
			{
				if (pController.isModelInstance())
		...
		public class FBoundBox
		{
			public D3DXVECTOR3 m_vtMin;
			public D3DXVECTOR3 m_vtMax;

			public FBoundBox(D3DXVECTOR3 vtMin, D3DXVECTOR3 vtMax)
			{
				m_vtMin = vtMin;
				m_vtMax = vtMax;
			}
		...
		public class FSetMotionPointer
		{
			public readonly CGrannyMotion m_pMotion;
			public float m_speedRatio;
		...
		public class FChangeMotionPointer
		{
			public readonly CGrannyMotion m_pMotion;
			public float m_speedRatio;
			public int m_loopCount;
		...
		public class FEndStopMotionPointer
		{
			public readonly CGrannyMotion m_pMotion;
		...
		public CGrannyLODController()
		{
			this.m_pCurrentModelInstance = null;
			this.m_bLODLevel = 0;
			this.m_pAttachedParentModel = null;
			this.m_fLODDistance = 0.0f;
			this.m_dwLODAniFPS = CGrannyModelInstance.ANIFPS_MAX;
			this.m_pkSharedDeformableVertexBuffer = null;
		}
		...
		public void Clear()
		{
			...
			List<TAttachingModelData>.Enumerator itor = m_AttachedModelDataVector.begin();
...
//Original Metin2 CPlus Line: TAttachingModelData & rData = *itor;
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public void AddModel(CGraphicThing pThing, int iSrcModel, CGrannyLODController pSkelLODController = null)
		{
			if (pThing == null)
			{
				return;
			...
			if (pSkelLODController != null && pSkelLODController.m_que_pkModelInst.empty())
			{
				Debug.Assert(!"EMPTY SKELETON(CANNON LINK)");
				return;
			}

			Debug.Assert(pThing.GetReferenceCount() >= 1);
			...
			if (!m_pCurrentModelInstance)
			{
				...
				float fSize = 0.0f;
				fSize = fMAX(fSize, Math.Abs(vtMin.x - vtMax.x));
				fSize = fMAX(fSize, Math.Abs(vtMin.y - vtMax.y));
				fSize = fMAX(fSize, Math.Abs(vtMin.z - vtMax.z));
		...
		public void AttachModelInstance(CGrannyLODController pSrcLODController, string c_szBoneName)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			TAttachingModelData AttachingModelData = new TAttachingModelData();
		...
		public void DetachModelInstance(CGrannyLODController pSrcLODController)
		{
			CGrannyModelInstance pSrcInstance = pSrcLODController.GetModelInstance();
			if (pSrcInstance == null)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public void SetLODLimits(float fNearLOD, float fFarLOD)
		{
			m_fLODDistance = fFarLOD;
		}

		public void SetLODLevel(byte bLodLevel)
		{
			Debug.Assert(m_que_pkModelInst.size() > 0);
		...
		{
			LinkedList<CGrannyModelInstance >.Enumerator i;
			for (i = m_que_pkModelInst.begin(); i.MoveNext();)
			{
				CGrannyModelInstance pkModelInst = i.Current;
				pkModelInst.SetMaterialImagePointer(c_szImageName, pImage);
			}
		}

		public void SetMaterialData(string c_szImageName, in SMaterialData c_rkMaterialData)
		{
			LinkedList<CGrannyModelInstance >.Enumerator i;
			for (i = m_que_pkModelInst.begin(); i.MoveNext();)
			{
		...
		{
			Debug.Assert(m_pCurrentModelInstance != null);
		...
		public void Update(float fElapsedTime, float fDistanceFromCenter, float fDistanceFromCamera)
		{
			UpdateLODLevel(fDistanceFromCenter, fDistanceFromCamera);
			UpdateTime(fElapsedTime);
		}

		public void UpdateLODLevel(float fDistanceFromCenter, float fDistanceFromCamera)
		{
			...
			if (fDistanceFromCenter > LOD_APPLY_MIN)
			{
				...
				{
					m_dwLODAniFPS = (uint)((CGrannyModelInstance.ANIFPS_MAX - CGrannyModelInstance.ANIFPS_MIN) * fLODFactor / m_fLODDistance + CGrannyModelInstance.ANIFPS_MIN);
				}
				else
				{
				...
				if (m_fLODDistance <= 5000.0f)
				{
					...
					bLODLevel = (byte)(m_que_pkModelInst.size() - Math.Min(bLODLevel, m_que_pkModelInst.size()) - 1);
				}
		...
		public void UpdateTime(float fElapsedTime)
		{
			Debug.Assert(m_pCurrentModelInstance != null);
		...
		public void UpdateSkeleton(D3DXMATRIX c_pWorldMatrix, float fElapsedTime)
		{
			if (m_pCurrentModelInstance)
			{
		...
		public void DeformNoSkin(D3DXMATRIX c_pWorldMatrix)
		{
			if (m_pCurrentModelInstance)
			{
		...
		public void DeformAll(D3DXMATRIX c_pWorldMatrix)
		{
			LinkedList<CGrannyModelInstance >.Enumerator i;
			for (i = m_que_pkModelInst.begin(); i.MoveNext();)
			{
		...
		public bool Intersect(D3DXMATRIX c_pMatrix, ref float u, ref float v, ref float t)
		{
			if (!m_pCurrentModelInstance)
			{
				return false;
		...
		public void SetLocalTime(float fLocalTime)
		{
			if (m_pCurrentModelInstance)
			{
		...
		public void SetMotionPointer(CGrannyMotion c_pMotion, float fBlendTime, int iLoopCount, float speedRatio)
		{
			Debug.Assert(m_pCurrentModelInstance != null);
			m_pCurrentModelInstance.SetMotionPointer(c_pMotion, fBlendTime, iLoopCount, speedRatio);
		}

		public void ChangeMotionPointer(CGrannyMotion c_pMotion, int iLoopCount, float speedRatio)
		{
			Debug.Assert(m_pCurrentModelInstance != null);
		...
		public CGrannyModelInstance GetModelInstance()
		{
		...
		protected void SetCurrentModelInstance(CGrannyModelInstance pgrnModelInstance)
		{
		...
		protected void RefreshAttachedModelInstance()
		{
			if (!m_pCurrentModelInstance)
			{
				return;
		...
		protected void __ReserveSharedDeformableVertexBuffer(uint deformableVertexCount)
		{
		...
		protected float m_fLODDistance;
		protected uint m_dwLODAniFPS;

		protected class SAttachingModelData
		{
			public CGrannyLODController pkLODController;
			public string strBoneName = "";
		}

		protected List<SAttachingModelData> m_AttachedModelDataVector = new List<SAttachingModelData>();
		protected CGrannyLODController m_pAttachedParentModel;

		protected byte m_bLODLevel;
		protected CGrannyModelInstance m_pCurrentModelInstance;
		protected LinkedList<CGrannyModelInstance > m_que_pkModelInst = new LinkedList<CGrannyModelInstance >();

		protected CGraphicVertexBuffer m_pkSharedDeformableVertexBuffer;
}