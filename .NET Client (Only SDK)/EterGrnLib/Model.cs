...
public class CGrannyModel : CReferenceObject
{
		public class SMeshNode
		{
			public int iMesh;
			public readonly CGrannyMesh pMesh;
			public SMeshNode pNextMeshNode;
		}
...
//Original Metin2 CPlus Line: bool IsEmpty() const
		...
		public bool CreateFromGrannyModelPointer(granny_model pgrnModel)
		{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetRigidVertexCount() const
		public int GetRigidVertexCount()
		{
			return m_rigidVtxCount;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetDeformVertexCount() const
		public int GetDeformVertexCount()
		{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: void DeformPNTVertices(object* dstBaseVertices, D3DXMATRIX * boneMatrices, const System.Collections.Generic.List<granny_mesh_binding*>& c_rvct_pgrnMeshBinding) const
		public void DeformPNTVertices(object dstBaseVertices, D3DXMATRIX boneMatrices, in List<granny_mesh_binding> c_rvct_pgrnMeshBinding)
		{
			int meshCount = GetMeshCount();

			for (int iMesh = 0; iMesh < meshCount; ++iMesh)
			{
				Debug.Assert(iMesh < c_rvct_pgrnMeshBinding.Count);
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetMeshCount() const
		public int GetMeshCount()
		{
			return m_pgrnModel ? m_pgrnModel.MeshBindingCount : 0;
		}

		public CGrannyMesh GetMeshPointer(int iMesh)
		{
			Debug.Assert(CheckMeshIndex(iMesh));
			Debug.Assert(m_meshs != null);

			return m_meshs + iMesh;
		}

		public granny_model GetGrannyModelPointer()
		{
			return m_pgrnModel;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const CGrannyMesh* GetMeshPointer(int iMesh) const
		public CGrannyMesh GetMeshPointer(int iMesh)
		{
			Debug.Assert(CheckMeshIndex(iMesh));
			Debug.Assert(m_meshs != null);
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: LPDIRECT3DVERTEXBUFFER8 GetPNTD3DVertexBuffer() const
		public LPDIRECT3DVERTEXBUFFER8 GetPNTD3DVertexBuffer()
		{
			return m_pntVtxBuf.GetD3DVertexBuffer();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: LPDIRECT3DINDEXBUFFER8 GetD3DIndexBuffer() const
		public LPDIRECT3DINDEXBUFFER8 GetD3DIndexBuffer()
		{
			return m_idxBuf.GetD3DIndexBuffer();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const SMeshNode* GetMeshNodeList(CGrannyMesh::EType eMeshType, CGrannyMaterial::EType eMtrlType) const
		public SMeshNode GetMeshNodeList(CGrannyMesh.EType eMeshType, CGrannyMaterial.EType eMtrlType)
		{
			return m_meshNodeLists[eMeshType][eMtrlType];
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool LockVertices(object** indicies, object** vertices) const
		public bool LockVertices(object[] indicies, object[] vertices)
		{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: void UnlockVertices() const
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const CGrannyMaterialPalette& GetMaterialPalette() const
		public CGrannyMaterialPalette GetMaterialPalette()
		{
			return m_kMtrlPal;
		}

		protected bool LoadMeshs()
		{
			Debug.Assert(m_meshs == null);
			Debug.Assert(m_pgrnModel != null);
			...
			int meshCount = GetMeshCount();
			m_meshs = Arrays.InitializeWithDefaultInstances<CGrannyMesh>(meshCount);
			...
			for (int m = 0; m < meshCount; ++m)
			{
				...
				int grni32xTypeCount = GrannyGetTotalTypeSize(pgrnMesh.PrimaryVertexData.VertexType) / 32;
			...
			m_meshNodes = Arrays.InitializeWithDefaultInstances<TMeshNode>(m_meshNodeCapacity);
		...
		protected bool LoadPNTVertices()
		{
			if (m_rigidVtxCount <= 0)
			{
				return true;
		...
		protected bool LoadIndices()
		{
			...
			if (!m_idxBuf.Lock((object) & indices))
			{
				return false;
			}
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool CheckMeshIndex(int iIndex) const
		protected bool CheckMeshIndex(int iIndex)
		{
			if (iIndex < 0)
			{
				return false;
		...
		protected void AppendMeshNode(CGrannyMesh.EType eMeshType, CGrannyMaterial.EType eMtrlType, int iMesh)
		{
			Debug.Assert(m_meshNodeSize < m_meshNodeCapacity);
		...
		protected granny_model m_pgrnModel;

		protected CGrannyMesh[] m_meshs;

		protected CGraphicVertexBuffer m_pntVtxBuf = new CGraphicVertexBuffer();
		protected CGraphicIndexBuffer m_idxBuf = new CGraphicIndexBuffer();

		protected SMeshNode[] m_meshNodes;
		protected SMeshNode[][] m_meshNodeLists = RectangularArrays.RectangularSMeshNodeArray(CGrannyMesh.TYPE_MAX_NUM, CGrannyMaterial.TYPE_MAX_NUM);

		protected int m_deformVtxCount;
		protected int m_rigidVtxCount;
		protected int m_vtxCount;
		protected int m_idxCount;

		protected int m_meshNodeSize;
		protected int m_meshNodeCapacity;

		protected bool m_canDeformPNVertices;
