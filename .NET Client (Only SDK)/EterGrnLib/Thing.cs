...
public class CGraphicThing : CResource
{

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static TType Type_s_type = StringToType("CGraphicThing");

		public static CGraphicThing.TType Type()
		{
...
//			static TType s_type = StringToType("CGraphicThing");
		...
		public CGraphicThing(string c_szFileName) : base(c_szFileName)
		{
			Initialize();
		}
		...
		public virtual bool CreateDeviceObjects()
		{
			if (!m_pgrnFileInfo)
			{
				return true;
...
//Original Metin2 CPlus Line: bool CheckModelIndex(int iModel) const
		public bool CheckModelIndex(int iModel)
		{
			if (!m_pgrnFileInfo)
			{
			...
			Debug.Assert(m_pgrnFileInfo != null);
		...
		public CGrannyModel GetModelPointer(int iModel)
		{
			Debug.Assert(CheckModelIndex(iModel));
			Debug.Assert(m_models != null);
			return m_models + iModel;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetModelCount() const
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool CheckMotionIndex(int iMotion) const
		public bool CheckMotionIndex(int iMotion)
		{
			if (!m_pgrnFileInfo)
			{
				return false;
		...
		public CGrannyMotion GetMotionPointer(int iMotion)
		{
			Debug.Assert(CheckMotionIndex(iMotion));
			...
			Debug.Assert(m_motions != null);
			return (m_motions + iMotion);
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetMotionCount() const
		...
		protected bool LoadModels()
		{
			Debug.Assert(m_pgrnFile != null);
			Debug.Assert(m_models == null);
			...
			m_models = Arrays.InitializeWithDefaultInstances<CGrannyModel>(modelCount);
			...
			GrannyFreeFileSection(m_pgrnFile, granny_standard_section_index.GrannyStandardRigidVertexSection);
			GrannyFreeFileSection(m_pgrnFile, granny_standard_section_index.GrannyStandardRigidIndexSection);
			GrannyFreeFileSection(m_pgrnFile, granny_standard_section_index.GrannyStandardDeformableIndexSection);
			GrannyFreeFileSection(m_pgrnFile, granny_standard_section_index.GrannyStandardTextureSection);
			return true;
		}

		protected bool LoadMotions()
		{
			Debug.Assert(m_pgrnFile != null);
			Debug.Assert(m_motions == null);
			...
			m_motions = Arrays.InitializeWithDefaultInstances<CGrannyMotion>(motionCount);
		...
		protected bool OnLoad(int iSize, object c_pvBuf)
		{
			...
			m_pgrnFile = GrannyReadEntireFileFromMemory(iSize, (object) c_pvBuf);
...
//Original Metin2 CPlus Line: bool OnIsEmpty() const
		...
		protected granny_file m_pgrnFile;
		protected granny_file_info m_pgrnFileInfo;

		protected granny_animation m_pgrnAni;

		protected CGrannyModel[] m_models;
		protected CGrannyMotion[] m_motions;
}