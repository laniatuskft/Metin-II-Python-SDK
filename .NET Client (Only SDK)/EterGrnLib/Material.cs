...
public class CGrannyMaterial : CReferenceObject
{

		public static void CreateSphereMap(uint uMapIndex, string c_szSphereMapImageFileName)
		{
		...
		public static void DestroySphereMap()
		{
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private static float TranslateSpecularMatrix_SPECULAR_TRANSLATE_MAX = 1000000.0f;

		public static void TranslateSpecularMatrix(float fAddX, float fAddY, float fAddZ)
		{
...
//			static float SPECULAR_TRANSLATE_MAX = 1000000.0f;
			...
			if (ms_v3SpecularTrans.x >= TranslateSpecularMatrix_SPECULAR_TRANSLATE_MAX)
			{
				ms_v3SpecularTrans.x = 0.0f;
			}

			if (ms_v3SpecularTrans.y >= TranslateSpecularMatrix_SPECULAR_TRANSLATE_MAX)
			{
				ms_v3SpecularTrans.y = 0.0f;
			}

			if (ms_v3SpecularTrans.z >= TranslateSpecularMatrix_SPECULAR_TRANSLATE_MAX)
			{
		...
		private static _D3DMATRIX ms_matSpecular = new _D3DMATRIX();
		private static _D3DVECTOR ms_v3SpecularTrans = new _D3DVECTOR();

		public CGrannyMaterial()
		{
...
//		void Destroy();
		public void Copy(CGrannyMaterial rkMtrl)
		{
...
//Original Metin2 CPlus Line: bool IsEqual(granny_material* pgrnMaterial) const
		public bool IsEqual(granny_material pgrnMaterial)
		{
		...
		public bool IsIn(string c_szImageName, ref int piStage)
		{
			string strImageName = c_szImageName;
			CFileNameHelper.StringPath(strImageName);

			granny_texture pgrnDiffuseTexture = GrannyGetMaterialTextureByType(m_pgrnMaterial, granny_material_texture_type.GrannyDiffuseColorTexture);
			if (pgrnDiffuseTexture != null)
			{
				string strDiffuseFileName = pgrnDiffuseTexture.FromFileName;
			...
			granny_texture pgrnOpacityTexture = GrannyGetMaterialTextureByType(m_pgrnMaterial, granny_material_texture_type.GrannyOpacityTexture);
			if (pgrnOpacityTexture != null)
			{
				string strOpacityFileName = pgrnOpacityTexture.FromFileName;
		...
		public void SetSpecularInfo(bool bFlag, float fPower, byte uSphereMapIndex)
		{
		...
		{
			Debug.Assert(m_pfnApplyRenderState != null && "CGrannyMaterial::SaveRenderState");
			m_pfnApplyRenderState();
		}

		public void RestoreRenderState()
		{
			Debug.Assert(m_pfnRestoreRenderState != null && "CGrannyMaterial::RestoreRenderState");
		...
		public bool CreateFromGrannyMaterialPointer(granny_material pgrnMaterial)
		{
			...
			if (pgrnMaterial != null)
			{
				if (pgrnMaterial.MapCount > 1 && !_strnicmp(pgrnMaterial.Name, "Blend", 5))
				{
					pgrnDiffuseTexture = GrannyGetMaterialTextureByType(pgrnMaterial.Maps[0].Material, granny_material_texture_type.GrannyDiffuseColorTexture);
					pgrnOpacityTexture = GrannyGetMaterialTextureByType(pgrnMaterial.Maps[1].Material, granny_material_texture_type.GrannyDiffuseColorTexture);
				}
				else
				{
					pgrnDiffuseTexture = GrannyGetMaterialTextureByType(m_pgrnMaterial, granny_material_texture_type.GrannyDiffuseColorTexture);
					pgrnOpacityTexture = GrannyGetMaterialTextureByType(m_pgrnMaterial, granny_material_texture_type.GrannyOpacityTexture);
				}

				{
					...
					{
						new granny_data_type_definition(granny_member_type.GrannyInt32Member, "Two-sided"),
						new granny_data_type_definition(granny_member_type.GrannyEndMember)
					};

					granny_variant twoSideResult = new granny_variant();
		...
		public void SetImagePointer(int iStage, CGraphicImage pImage)
		{
			Debug.Assert(iStage < 2 && "CGrannyMaterial::SetImagePointer");
			m_roImage[iStage] = pImage;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: CGrannyMaterial::EType GetType() const
		public CGrannyMaterial.EType GetType()
		{
			return m_eType;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: CGraphicImage * GetImagePointer(int iStage) const
		public CGraphicImage GetImagePointer(int iStage)
		{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			CGraphicImage.TRef ratImage = m_roImage[iStage];
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const CGraphicTexture* GetDiffuseTexture() const
		public CGraphicTexture GetDiffuseTexture()
		{
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: const CGraphicTexture* GetOpacityTexture() const
		public CGraphicTexture GetOpacityTexture()
		{
			if (m_roImage[1].IsNull())
			{
				return null;
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: LPDIRECT3DTEXTURE8 GetD3DTexture(int iStage) const
		public LPDIRECT3DTEXTURE8 GetD3DTexture(int iStage)
		{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			CGraphicImage.TRef ratImage = m_roImage[iStage];
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool IsTwoSided() const
		...
		protected CGraphicImage __GetImagePointer(string fileName)
		{
			Debug.Assert(fileName[0] != '\0');
			...
			{
				string localFileName = new string(new char[256]);
				string modelLocalPath = GetModelLocalPath();

				int localFileName_len = modelLocalPath.Length + 1 + fileName_len;
				if (localFileName_len < sizeof(char) - 1)
				{
					_snprintf(localFileName, sizeof(char), "%s%s", GetModelLocalPath().c_str(), fileName);
					CResource pResource = rkResMgr.GetResourcePointer(localFileName);
					return (CGraphicImage)pResource;
				}
			}
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool __IsSpecularEnable() const
		protected bool __IsSpecularEnable()
		{
			return m_bSpecularEnable;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: float __GetSpecularPower() const
		protected float __GetSpecularPower()
		{
			return m_fSpecularPower;
		}

		protected void __ApplyDiffuseRenderState()
		{
			(CStateManager.Instance()).SetTexture(0, GetD3DTexture(0));

			if (m_bTwoSideRender)
			{
				m_dwLastCullRenderStateForTwoSideRendering = (CStateManager.Instance()).GetRenderState(D3DRS_CULLMODE);
				(CStateManager.Instance()).SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
			}
		}

		protected void __RestoreDiffuseRenderState()
		{
			if (m_bTwoSideRender)
			{
				(CStateManager.Instance()).SetRenderState(D3DRS_CULLMODE, m_dwLastCullRenderStateForTwoSideRendering);
			}
		}

		protected void __ApplySpecularRenderState()
		{
			if (true == (CStateManager.Instance()).GetRenderState(D3DRS_ALPHABLENDENABLE))
			{
			...
			if (pkTexture != null)
			{
				(CStateManager.Instance()).SetTexture(1, pkTexture.GetD3DTexture());
			}
			else
			{
			...
			(CStateManager.Instance()).SetRenderState(D3DRS_TEXTUREFACTOR, D3DXCOLOR(g_fSpecularColor.r, g_fSpecularColor.g, g_fSpecularColor.b, __GetSpecularPower()));
			(CStateManager.Instance()).SaveTextureStageState(1, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACEREFLECTIONVECTOR);
			...
			(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
			(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TFACTOR);
			(CStateManager.Instance()).SaveTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);

			(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_COLORARG1, D3DTA_CURRENT);
			(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_COLORARG2, D3DTA_TEXTURE);
			(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_COLOROP, D3DTOP_MODULATEALPHA_ADDCOLOR);
			(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_ALPHAARG1, D3DTA_CURRENT);
			(CStateManager.Instance()).SetTextureStageState(1, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1);

			(CStateManager.Instance()).SetTransform(D3DTS_TEXTURE1, ms_matSpecular);
			(CStateManager.Instance()).SaveTextureStageState(1, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);
			(CStateManager.Instance()).SaveTextureStageState(1, D3DTSS_ADDRESSU, D3DTADDRESS_WRAP);
			(CStateManager.Instance()).SaveTextureStageState(1, D3DTSS_ADDRESSV, D3DTADDRESS_WRAP);
		}

		protected void __RestoreSpecularRenderState()
		{
			if (true == (CStateManager.Instance()).GetRenderState(D3DRS_ALPHABLENDENABLE))
			{
		...
		protected granny_material m_pgrnMaterial;
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected CGraphicImage.TRef[] m_roImage = Arrays.InitializeWithDefaultInstances<TRef>(2);
		protected EType m_eType = new EType();

		protected float m_fSpecularPower;
		protected bool m_bSpecularEnable;
		protected bool m_bTwoSideRender;
		protected uint m_dwLastCullRenderStateForTwoSideRendering;
		protected byte m_bSphereMapIndex;


		protected delegate void m_pfnApplyRenderStateDelegate();
		protected m_pfnApplyRenderStateDelegate m_pfnApplyRenderState;
		protected delegate void m_pfnRestoreRenderStateDelegate();
		protected m_pfnRestoreRenderStateDelegate m_pfnRestoreRenderState;

		private const int SPHEREMAP_NUM = 10;
		private static CGraphicImageInstance[] ms_akSphereMapInstance = Arrays.InitializeWithDefaultInstances<CGraphicImageInstance>(SPHEREMAP_NUM);
}

public class CGrannyMaterialPalette
{
		public CGrannyMaterialPalette()
		{
		}
		...
		public void Copy(in CGrannyMaterialPalette rkMtrlPalSrc)
		{
			m_mtrlVector = rkMtrlPalSrc.m_mtrlVector;
		}

		public uint RegisterMaterial(granny_material pgrnMaterial)
		{
			uint size = (uint)m_mtrlVector.size();
			uint i;
		...
		public void SetMaterialImagePointer(string c_szImageName, CGraphicImage pImage)
		{
			uint size = (uint)m_mtrlVector.size();
			uint i;
			for (i = 0; LaniatusDefVariables < size; ++i)
		...
		public void SetMaterialData(string c_szMtrlName, in SMaterialData c_rkMaterialData)
		{
			if (c_szMtrlName != '\0')
			{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
				List<CGrannyMaterial.TRef>.Enumerator i;
				for (i = m_mtrlVector.begin(); i.MoveNext();)
				{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
					CGrannyMaterial.TRef roMtrl = i.Current;
		...
		public void SetSpecularInfo(string c_szMtrlName, bool bEnable, float fPower)
		{
			uint size = (uint)m_mtrlVector.size();
			uint i;
			if (c_szMtrlName != '\0')
		...
		public CGrannyMaterial GetMaterialRef(uint mtrlIndex)
		{
			Debug.Assert(mtrlIndex < m_mtrlVector.size());
			return m_mtrlVector[mtrlIndex].GetPointer();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: uint GetMaterialCount() const
		public uint GetMaterialCount()
		{
			return (uint)m_mtrlVector.size();
		}

//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TRef' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected List<CGrannyMaterial.TRef> m_mtrlVector = new List<CGrannyMaterial.TRef>();
}
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
struct _D3DVECTOR CGrannyMaterial.ms_v3SpecularTrans(0.0f, 0.0f, 0.0f);