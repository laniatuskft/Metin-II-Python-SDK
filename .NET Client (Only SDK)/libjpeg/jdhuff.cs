...
public class d_derived_tbl
{
   ...
   */
  public int[] look_nbits = new int[1 << DefineConstants.HUFF_LOOKAHEAD]; // # bits, or 0 if too long
  public short[] look_sym = new short[1 << DefineConstants.HUFF_LOOKAHEAD]; // symbol, or unused
}
...
public class bitread_perm_state
...
public class bitread_working_state
{ // Bitreading working state within an MCU
  /* Current data source location */
  /* We need a copy, rather than munging the original, in case of suspension */
  public readonly JOCTET next_input_byte; // => next byte to read from source
  public size_t bytes_in_buffer = new size_t(); // # of bytes remaining in source buffer
  ...
  public j_decompress_ptr cinfo = new j_decompress_ptr(); // back link to decompress master record
}

/* Macros to declare and load/save bitread local variables. */
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define BITREAD_STATE_VARS register bit_buf_type get_buffer; register int bits_left; bitread_working_state br_state

//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define BITREAD_LOAD_STATE(cinfop,permstate) br_state.cinfo = cinfop; br_state.next_input_byte = cinfop->src->next_input_byte; br_state.bytes_in_buffer = cinfop->src->bytes_in_buffer; get_buffer = permstate.get_buffer; bits_left = permstate.bits_left;

//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define BITREAD_SAVE_STATE(cinfop,permstate) cinfop->src->next_input_byte = br_state.next_input_byte; cinfop->src->bytes_in_buffer = br_state.bytes_in_buffer; permstate.get_buffer = get_buffer; permstate.bits_left = bits_left
...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define CHECK_BIT_BUFFER(state,nbits,action) { if (bits_left < (nbits)) { if (! jpeg_fill_bit_buffer(&(state),get_buffer,bits_left,nbits)) { action; } get_buffer = (state).get_buffer; bits_left = (state).bits_left; } }

//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GET_BITS(nbits) (((int) (get_buffer >> (bits_left -= (nbits)))) & BIT_MASK(nbits))

//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define PEEK_BITS(nbits) (((int) (get_buffer >> (bits_left - (nbits)))) & BIT_MASK(nbits))

//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define DROP_BITS(nbits) (bits_left -= (nbits))
...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define HUFF_DECODE(result,state,htbl,failaction,slowlabel) { register int nb, look; if (bits_left < HUFF_LOOKAHEAD) { if (! jpeg_fill_bit_buffer(&state,get_buffer,bits_left, 0)) {failaction;} get_buffer = state.get_buffer; bits_left = state.bits_left; if (bits_left < HUFF_LOOKAHEAD) { nb = 1; goto slowlabel; } } look = PEEK_BITS(HUFF_LOOKAHEAD); if ((nb = htbl->look_nbits[look]) != 0) { DROP_BITS(nb); result = htbl->look_sym[look]; } else { nb = HUFF_LOOKAHEAD+1; slowlabel: if ((result=jpeg_huff_decode(&state,get_buffer,bits_left,htbl,nb)) < 0) { failaction; } get_buffer = state.get_buffer; bits_left = state.bits_left; } }
...
public class savable_state
{
  public uint EOBRUN; // remaining EOBs in EOBRUN
  public int[] last_dc_val = new int[DefineConstants.MAX_COMPS_IN_SCAN]; // last DC coef for each component
}
...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ASSIGN_STATE(dest,src) ((dest) = (src))
#define ASSIGN_STATE
#else
...
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ASSIGN_STATE(dest,src) ((dest).EOBRUN = (src).EOBRUN, (dest).last_dc_val[0] = (src).last_dc_val[0], (dest).last_dc_val[1] = (src).last_dc_val[1], (dest).last_dc_val[2] = (src).last_dc_val[2], (dest).last_dc_val[3] = (src).last_dc_val[3])
#define ASSIGN_STATE
#endif
#endif


public class huff_entropy_decoder
{
  public jpeg_entropy_decoder pub = new jpeg_entropy_decoder(); // public fields

  /* These fields are loaded into local variables at start of each MCU.
   * In case of suspension, we exit WITHOUT updating them.
   */
  public bitread_perm_state bitstate = new bitread_perm_state(); // Bit buffer at start of MCU
  public savable_state saved = new savable_state(); // Other state at start of MCU

  /* These fields are NOT loaded into local working state. */
  public boolean insufficient_data = new boolean(); // set TRUE after emitting warning
  public uint restarts_to_go; // MCUs left in this restart interval

  /* Following two fields used only in progressive mode */

  /* Pointers to derived tables (these workspaces have image lifespan) */
  public d_derived_tbl[] derived_tbls = new d_derived_tbl[DefineConstants.NUM_HUFF_TBLS];
  ...
  /* Pointers to derived tables (these workspaces have image lifespan) */
  public d_derived_tbl[] dc_derived_tbls = new d_derived_tbl[DefineConstants.NUM_HUFF_TBLS];
  public d_derived_tbl[] ac_derived_tbls = new d_derived_tbl[DefineConstants.NUM_HUFF_TBLS];

  /* Precalculated info set up by start_pass for use in decode_mcu: */

  /* Pointers to derived tables to be used for each block within an MCU */
  public d_derived_tbl[] dc_cur_tbls = new d_derived_tbl[DefineConstants.D_MAX_BLOCKS_IN_MCU];
  public d_derived_tbl[] ac_cur_tbls = new d_derived_tbl[DefineConstants.D_MAX_BLOCKS_IN_MCU];
  /* Whether we care about the DC and AC coefficient values for each block */
  public int[] coef_limit = new int[DefineConstants.D_MAX_BLOCKS_IN_MCU];
}