...
public class my_main_controller
{
  public jpeg_c_main_controller pub = new jpeg_c_main_controller(); // public fields

  public uint cur_iMCU_row; // number of current iMCU row
  public uint rowgroup_ctr; // counts row groups received in iMCU row
  public boolean suspended = new boolean(); // remember if we suspended output
  public J_BUF_MODE pass_mode = new J_BUF_MODE(); // current operating mode

  /* If using just a strip buffer, this points to the entire set of buffers
   * (we allocate one for each component).  In the full-image case, this
   * points to the currently accessible strips of the virtual arrays.
   */
  public JSAMPARRAY[] buffer = Arrays.InitializeWithDefaultInstances<JSAMPARRAY>(DefineConstants.MAX_COMPONENTS);

#if FULL_MAIN_BUFFER_SUPPORTED
  /* If using full-image storage, this array holds pointers to virtual-array
   * control blocks for each component.  Unused if not full-image storage.
   */
  public jvirt_sarray_ptr[] whole_image = Arrays.InitializeWithDefaultInstances<jvirt_sarray_ptr>(DefineConstants.MAX_COMPONENTS);
#endif
}