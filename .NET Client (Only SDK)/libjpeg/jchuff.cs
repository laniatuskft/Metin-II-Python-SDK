...
public class c_derived_tbl
{
  public uint[] ehufco = new uint[256]; // code for each symbol
  public string ehufsi = new string(new char[256]); // length of code for each symbol
...
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ASSIGN_STATE(dest,src) ((dest).put_buffer = (src).put_buffer, (dest).put_bits = (src).put_bits, (dest).last_dc_val[0] = (src).last_dc_val[0], (dest).last_dc_val[1] = (src).last_dc_val[1], (dest).last_dc_val[2] = (src).last_dc_val[2], (dest).last_dc_val[3] = (src).last_dc_val[3])
#define ASSIGN_STATE
#endif
#endif


public class huff_entropy_encoder
{
  public jpeg_entropy_encoder pub = new jpeg_entropy_encoder(); // public fields

  public savable_state saved = new savable_state(); // Bit buffer & DC state at start of MCU
  ...
  public c_derived_tbl[] dc_derived_tbls = new c_derived_tbl[DefineConstants.NUM_HUFF_TBLS];
  public c_derived_tbl[] ac_derived_tbls = new c_derived_tbl[DefineConstants.NUM_HUFF_TBLS];

  /* Statistics tables for optimization */
  public int[][] dc_count_ptrs = {null};
  public int[][] ac_count_ptrs = {null};

  /* Following fields used only in progressive mode */

  /* Mode flag: TRUE for optimization, FALSE for actual data output */
  public boolean gather_statistics = new boolean();
  ...
  public j_compress_ptr cinfo = new j_compress_ptr(); // link to cinfo (needed for dump_buffer)
  ...
  public string bit_buffer; // buffer for correction bits (1 per char)
...
public class working_state
{
  public JOCTET next_output_byte; // => next byte to write in buffer
  public size_t free_in_buffer = new size_t(); // # of byte spaces remaining in buffer
  public savable_state cur = new savable_state(); // Current bit buffer & DC state
  public j_compress_ptr cinfo = new j_compress_ptr(); // dump_buffer needs access to this
}
...
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ISHIFT_TEMPS int ishift_temp;
#define ISHIFT_TEMPS
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define IRIGHT_SHIFT(x,shft) ((ishift_temp = (x)) < 0 ? (ishift_temp >> (shft)) | ((~0) << (16-(shft))) : (ishift_temp >> (shft)))
#define IRIGHT_SHIFT
#else
#define ISHIFT_TEMPS
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
