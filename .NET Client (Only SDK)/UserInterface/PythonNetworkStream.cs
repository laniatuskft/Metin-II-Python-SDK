#define ENABLE_LOAD_PLAYERSETTING
#define ENABLE_MULTI_LANGUAGE_SYSTEM
#define ENABLE_EXTENDED_WHISPER_DETAILS
...
public class CPythonNetworkStream : CNetworkStream, CSingleton<CPythonNetworkStream>
{
		public const int SERVER_COMMAND_LOG_OUT = 0;
		public const int SERVER_COMMAND_RETURN_TO_SELECT_CHARACTER = 1;
		public const int SERVER_COMMAND_QUIT = 2;
		public const int MAX_ACCOUNT_PLAYER = 3;

		public const int ERROR_NONE = 0;
		public const int ERROR_UNKNOWN = 1;
		public const int ERROR_CONNECT_MARK_SERVER = 2;
		public const int ERROR_LOAD_MARK = 3;
		public const int ERROR_MARK_WIDTH = 4;
		public const int ERROR_MARK_HEIGHT = 5;
		public const int ERROR_MARK_UPLOAD_NEED_RECONNECT = 6;
		public const int ERROR_MARK_CHECK_NEED_RECONNECT = 7;

		public const int ACCOUNT_CHARACTER_SLOT_ID = 0;
		public const int ACCOUNT_CHARACTER_SLOT_NAME = 1;
		public const int ACCOUNT_CHARACTER_SLOT_RACE = 2;
		public const int ACCOUNT_CHARACTER_SLOT_LEVEL = 3;
		public const int ACCOUNT_CHARACTER_SLOT_STR = 4;
		public const int ACCOUNT_CHARACTER_SLOT_DEX = 5;
		public const int ACCOUNT_CHARACTER_SLOT_HTH = 6;
		public const int ACCOUNT_CHARACTER_SLOT_INT = 7;
		public const int ACCOUNT_CHARACTER_SLOT_PLAYTIME = 8;
		public const int ACCOUNT_CHARACTER_SLOT_FORM = 9;
		public const int ACCOUNT_CHARACTER_SLOT_ADDR = 10;
		public const int ACCOUNT_CHARACTER_SLOT_PORT = 11;
		public const int ACCOUNT_CHARACTER_SLOT_GUILD_ID = 12;
		public const int ACCOUNT_CHARACTER_SLOT_GUILD_NAME = 13;
		public const int ACCOUNT_CHARACTER_SLOT_CHANGE_NAME_FLAG = 14;
		public const int ACCOUNT_CHARACTER_SLOT_HAIR = 15;
		public const int ACCOUNT_CHARACTER_SLOT_ACCE = 16;

		public const int PHASE_WINDOW_LOGO = 0;
		public const int PHASE_WINDOW_LOGIN = 1;
		public const int PHASE_WINDOW_SELECT = 2;
		public const int PHASE_WINDOW_CREATE = 3;
		public const int PHASE_WINDOW_LOAD = 4;
		public const int PHASE_WINDOW_GAME = 5;
		public const int PHASE_WINDOW_EMPIRE = 6;
		public const int PHASE_WINDOW_NUM = 7;

		public CPythonNetworkStream()
		{
...
//		bool SendSpecial(int nLen, object pvBuf);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void StartGame();
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void Warp(int lGlobalX, int lGlobalY);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void NotifyHack(string c_szMsg);
		public void SetWaitFlag()
		{
			m_isWaitLoginKey = true;
		}

		public void SendEmoticon(uint eEmoticon)
		{
			...
			{
				Debug.Assert(false && "SendEmoticon Error");
			}
		}
		...
		public void AbsoluteExitApplication()
		{
			PostQuitMessage(0);
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool IsChatInsultIn(string c_szMsg);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool IsInsultIn(string c_szMsg);
		...
		public uint UploadMark(string c_szImageFileName)
		{
			if (0 == m_dwGuildID)
			{
		...
		public uint UploadSymbol(string c_szImageFileName)
		{
			uint uError = ERROR_UNKNOWN;
			CGuildMarkUploader rkGuildMarkUploader = CGuildMarkUploader.Instance();
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool LoadInsultList(string c_szInsultListFileName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool LoadConvertTable(uint dwEmpireID, string c_szFileName);

		public uint GetAccountCharacterSlotDatau(uint iSlot, uint eType)
		{
			...
			SSimplePlayerInformation rkSimplePlayerInfo = m_akSimplePlayerInfo[iSlot];
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string GetAccountCharacterSlotDataz_s_szAddr = new string(new char[256]);

		public string GetAccountCharacterSlotDataz(uint iSlot, uint eType)
		{
			string sc_szEmpty = "";
...
//						static char s_szAddr[256];
						sprintf(GetAccountCharacterSlotDataz_s_szAddr, "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
						return GetAccountCharacterSlotDataz_s_szAddr;
				}
					break;
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		string GetFieldMusicFileName();
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		float GetFieldMusicVolume();

		public bool IsSelectedEmpire()
		{
			if (m_dwEmpireID)
			{
				return true;
...
//		void ToggleGameDebugInfo();
		...
		public void SetMarkServer(string c_szAddr, uint uPort)
		{
			m_kMarkAuth.m_kNetAddr.Set(c_szAddr, uPort);
		}

		public void ConnectLoginServer(string c_szAddr, uint uPort)
		{
			CNetworkStream.Connect(c_szAddr, uPort);
		}

		public void ConnectGameServer(uint iChrSlot)
		{
			...
			SSimplePlayerInformation rkSimplePlayerInfo = m_akSimplePlayerInfo[iChrSlot];
			CNetworkStream.Connect((uint)rkSimplePlayerInfo.lAddr, rkSimplePlayerInfo.wPort);
		}

		public void SetLoginInfo(string c_szID, string c_szPassword)
		{
			m_stID = c_szID;
			m_stPassword = c_szPassword;
		}

		public void SetLoginKey(uint dwLoginKey)
		{
		...
		public void SetHandler(PyObject poHandler)
		{
...
#if ENABLE_LOAD_PLAYERSETTING
#if ENABLE_LOAD_PLAYERSETTING
		public void SetSetLocaleinfoHandler(PyObject poLocaleinfoHandler)
		{
			m_poLocaleinfoHandler = poLocaleinfoHandler;
		}
#endif

#endif
		public void SetPhaseWindow(uint ePhaseWnd, PyObject poPhaseWnd)
		{
		...
		public void ClearPhaseWindow(uint ePhaseWnd, PyObject poPhaseWnd)
		{
			if (ePhaseWnd >= PHASE_WINDOW_NUM)
			{
				return;
		...
		public void SetServerCommandParserWindow(PyObject poWnd)
		{
			m_poSerCommandParserWnd = poWnd;
		}

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSyncPositionElementPacket(uint dwVictimVID, uint dwVictimX, uint dwVictimY);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendAttackPacket(uint uMotAttack, uint dwVIDVictim);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendCharacterStatePacket(in TPixelPosition c_rkPPosDst, float fDstRot, uint eFunc, uint uArg);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendUseSkillPacket(uint dwSkillIndex, uint dwTargetVID = 0);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendTargetPacket(uint dwVID);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendCharacterStartWalkingPacket(float fRotation, int lx, int ly);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendCharacterEndWalkingPacket(float fRotation, int lx, int ly);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendCharacterCheckWalkingPacket(float fRotation, int lx, int ly);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendItemDropPacket(TItemPos pos, uint elk);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendItemDropPacketNew(TItemPos pos, uint elk, uint count);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendItemDestroyPacket(TItemPos pos);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendItemMovePacket(TItemPos pos, TItemPos change_pos, ushort num);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendItemPickUpPacket(uint vid);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendShopSellPacketNew(byte bySlot, ushort wCount);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendExchangeStartPacket(uint vid);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendExchangeItemAddPacket(TItemPos ItemPos, byte byDisplayPos);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendExchangeElkAddPacket(long elk);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendScriptButtonPacket(uint iIndex);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendAnswerMakeGuildPacket(string c_szName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendQuestInputStringPacket(string c_szString);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendQuestConfirmPacket(byte byAnswer, uint dwPID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendOnClickPacket(uint vid);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendFlyTargetingPacket(uint dwTargetVID, in TPixelPosition kPPosTarget);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendAddFlyTargetingPacket(uint dwTargetVID, in TPixelPosition kPPosTarget);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendShootPacket(uint uSkill);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool ClientCommand(string c_szCommand);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void ServerCommand(ref string c_szCommand);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RegisterEmoticonString(string pcEmoticonString);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartyInvitePacket(uint dwVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartyInviteAnswerPacket(uint dwLeaderVID, byte byAccept);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartyRemovePacket(uint dwPID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartySetStatePacket(uint dwVID, byte byState, byte byFlag);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartyUseSkillPacket(byte bySkillIndex, uint dwVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendPartyParameterPacket(byte byDistributeMode);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSafeBoxMoneyPacket(byte byState, uint dwMoney);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSafeBoxItemMovePacket(byte bySourcePos, byte byTargetPos, ushort wCount);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendMallCheckoutPacket(byte byMallPos, TItemPos InventoryPos);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildAddMemberPacket(uint dwVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildRemoveMemberPacket(uint dwPID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildChangeGradeNamePacket(byte byGradeNumber, string c_szName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildChangeGradeAuthorityPacket(byte byGradeNumber, byte byAuthority);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildOfferPacket(uint dwExperience);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildPostCommentPacket(string c_szMessage);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildDeleteCommentPacket(uint dwIndex);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildRefreshCommentsPacket(uint dwHighestIndex);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildChangeMemberGradePacket(uint dwPID, byte byGrade);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildUseSkillPacket(uint dwSkillID, uint dwTargetVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildChangeMemberGeneralPacket(uint dwPID, byte byFlag);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildInvitePacket(uint dwVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildInviteAnswerPacket(uint dwGuildID, byte byAnswer);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildChargeGSPPacket(uint dwMoney);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildDepositMoneyPacket(uint dwMoney);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGuildWithdrawMoneyPacket(uint dwMoney);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendGiveItemPacket(uint dwTargetVID, TItemPos ItemPos, int iItemCount);

#if ENABLE_MULTI_LANGUAGE_SYSTEM
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendChangeLanguagePacket(byte bLanguage);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendChangeLanguage(string lang);

		public void SetLanguage(byte bLanguage)
		{
			m_bLanguage = bLanguage;
		}
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: int GetLanguage() const
...
#if ENABLE_LOAD_PLAYERSETTING
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void SendPythonData(PyObject obj, string funcname);
#endif
#if ENABLE_EXTENDED_WHISPER_DETAILS
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendWhisperDetails(string name);
#endif
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendBuildPrivateShopPacket(string c_szName, in System.Collections.Generic.List<SShopItemTable> c_rSellingItemStock);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendRefinePacket(byte byPos, byte byType);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSelectItemPacket(uint dwItemPos);
...
#endif
		public uint GetMainActorVID()
		{
			return m_dwMainActorVID;
		}

		public uint GetMainActorRace()
		{
			return m_dwMainActorRace;
		}

		public uint GetMainActorEmpire()
		{
			return m_dwMainActorEmpire;
		}

		public uint GetMainActorSkillGroup()
		{
			return m_dwMainActorSkillGroup;
		}

		public void SetEmpireID(uint dwEmpireID)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendLoginPacket(string c_szName, string c_szPassword);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendLoginPacketNew(string c_szName, string c_szPassword);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendDirectEnterPacket(string c_szName, string c_szPassword, uint uChrSlot);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendEnterGame();

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSelectEmpirePacket(uint dwEmpireID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendSelectCharacterPacket(byte account_Index);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendChangeNamePacket(byte index, string name);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendCreateCharacterPacket(byte index, string name, byte job, byte shape, byte byStat1, byte byStat2, byte byStat3, byte byStat4);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendDestroyCharacterPacket(byte index, string szPrivateCode);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendC2CPacket(uint dwSize, object pData);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendChatPacket(string c_szChat, byte byType = CHAT_TYPE_TALKING);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendWhisperPacket(string name, string c_szChat);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendMessengerAddByVIDPacket(uint vid);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendMessengerAddByNamePacket(string c_szName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool SendMessengerRemovePacket(string c_szKey, string c_szName);
...
//		void GamePhase();
		...
		protected bool __IsNotPing()
		{
			return false;
		}

		protected void __DownloadMark()
		{
		...
		protected void __DownloadSymbol(in List<uint> c_rkVec_dwGuildID)
		{
			CGuildMarkDownloader rkGuildMarkDownloader = CGuildMarkDownloader.Instance();
...
//		bool __CanActMainInstance();
		...
		protected enum REFRESH_WINDOW_TYPE
		{
			RefreshStatus = (1 << 0),
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __RefreshTargetBoardByVID(uint dwVID);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __RefreshTargetBoardByName(string c_szName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool __SendHack(string c_szMsg);
...
//		bool RecvObserverMovePacket();
		protected bool RecvErrorPacket(int header)
		{
		...
		protected bool RecvPingPacket()
		{
			Tracef("recv ping packet. (securitymode %u)\n", IsSecurityMode());

			packet_ping kPacketPing = new packet_ping();

			if (!Recv(sizeof(packet_ping), kPacketPing))
			{
				return false;
			}
			...
			packet_pong kPacketPong = new packet_pong();
			kPacketPong.bHeader = LG_HEADER_CG_PONG;

			if (!Send(sizeof(packet_pong), kPacketPong))
			{
				return false;
			}
		...
		protected bool RecvDefaultPacket(int header)
		{
			if (header == 0)
			{
				return true;
			...
			TraceError("??????? ???? ??Ŷ ??? %d, state %s\n", header, m_strPhase.c_str());
			ClearRecvBuffer();
			return true;
		}

		protected bool RecvPhasePacket()
		{
			packet_phase packet_phase = new packet_phase();

			if (!Recv(sizeof(packet_phase), packet_phase))
			{
				return false;
			}
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetFieldMusicFileName(string musicName);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetFieldMusicFileInfo(string musicName, float vol);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			bool SendTargetInfoLoadPacket(uint dwVID);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		bool ParseEmoticon(string pChatMsg, ref uint pdwEmoticon);
...
//		void OnDisconnect();
		...
		protected void SetGameOnline()
		{
			m_isGameOnline = true;
		}

		protected void SetGameOffline()
		{
			m_isGameOnline = false;
		}

		protected bool IsGameOnline()
		{
			return m_isGameOnline;
		}

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private CMainPacketHeaderMap CheckPacket_s_packetHeaderMap = new CMainPacketHeaderMap();

		protected bool CheckPacket(ref byte pRetHeader)
		{
			pRetHeader = null;
...
//			static CMainPacketHeaderMap s_packetHeaderMap;
			...
			if (0 == header)
			{
				if (!Recv(sizeof(byte), header))
				{
					return false;
				}

				while (Peek(sizeof(byte), header))
				{
					if (0 == header)
					{
						if (!Recv(sizeof(byte), header))
			...
			SPacketType PacketType = new SPacketType();

			if (!CheckPacket_s_packetHeaderMap.Get(header, PacketType))
			{
			...
			{
				packet_LG_HEADER_dynamic_size DynamicSizePacketHeader = new packet_LG_HEADER_dynamic_size();

				if (!Peek(sizeof(packet_LG_HEADER_dynamic_size), DynamicSizePacketHeader))
				{
					return false;
				}
...
//		void __InitializeGamePhase();
		protected void __InitializeMarkAuth()
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __GlobalPositionToLocalPosition(ref int rGlobalX, ref int rGlobalY);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __LocalPositionToGlobalPosition(ref int rLocalX, ref int rLocalY);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __ShowMapName(int lLocalX, int lLocalY);

		protected void __LeaveOfflinePhase()
		{
		}
		protected void __LeaveHandshakePhase()
		{
		}
		protected void __LeaveLoginPhase()
		{
		}
		protected void __LeaveSelectPhase()
		{
		}
		protected void __LeaveLoadingPhase()
		{
		}
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
...
//		void __ClearNetworkActorManager();
		...
		protected void __ClearSelectCharacterData()
		{
			NANOBEGIN memset = new NANOBEGIN(m_akSimplePlayerInfo, 0, sizeof(m_akSimplePlayerInfo));
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __ConvertEmpireText(uint dwEmpireID, ref string szText);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __FilterInsult(ref string szLine, uint uLineLen);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __SetGuildID(uint id);

#if ENABLE_LOAD_PLAYERSETTING
		public PyObject m_poLocaleinfoHandler;
#endif

		protected packet_LG_HEADER_handshake m_HandshakeData = new packet_LG_HEADER_handshake();
		protected uint m_dwChangingPhaseTime;
		protected uint m_dwBindupRetryCount;
		protected uint m_dwMainActorVID;
		protected uint m_dwMainActorRace;
		protected uint m_dwMainActorEmpire;
		protected uint m_dwMainActorSkillGroup;
		protected bool m_isGameOnline;
		protected bool m_isStartGame;

		protected uint m_dwGuildID;
		protected uint m_dwEmpireID;

		protected class SServerTimeSync
		{
			public uint m_dwChangeServerTime;
			public uint m_dwChangeClientTime;
		}
		protected SServerTimeSync m_kServerTimeSync = new SServerTimeSync();

		protected void __ServerTimeSync_Initialize()
		{
			m_kServerTimeSync.m_dwChangeClientTime = 0;
			m_kServerTimeSync.m_dwChangeServerTime = 0;
		}

		protected uint m_dwLastGamePingTime;

		protected string m_stID = "";
		protected string m_stPassword = "";
		protected string m_strLastCommand = "";
		protected string m_strPhase = "";
		protected uint m_dwLoginKey;
		protected bool m_isWaitLoginKey;

		protected string m_stMarkIP = "";

		protected CFuncObject<CPythonNetworkStream> m_phaseProcessFunc = new CFuncObject<CPythonNetworkStream>();
		protected CFuncObject<CPythonNetworkStream> m_phaseLeaveFunc = new CFuncObject<CPythonNetworkStream>();

		protected PyObject m_poHandler;
		protected PyObject[] m_apoPhaseWnd = new PyObject[PHASE_WINDOW_NUM];
		protected PyObject m_poSerCommandParserWnd;

		protected SSimplePlayerInformation[] m_akSimplePlayerInfo = Arrays.InitializeWithDefaultInstances<SSimplePlayerInformation>(PLAYER_PER_ACCOUNT4);
		protected uint[] m_adwGuildID = new uint[PLAYER_PER_ACCOUNT4];
		protected string[] m_astrGuildName = Arrays.InitializeStringArrayWithDefaultInstances(PLAYER_PER_ACCOUNT4);
		protected bool m_bSimplePlayerInfo;

		protected CRef<CNetworkActorManager> m_rokNetActorMgr = new CRef<CNetworkActorManager>();

		protected bool m_isRefreshStatus;
		protected bool m_isRefreshCharacterWnd;
		protected bool m_isRefreshEquipmentWnd;
		protected bool m_isRefreshInventoryWnd;
		protected bool m_isRefreshExchangeWnd;
		protected bool m_isRefreshSkillWnd;
		protected bool m_isRefreshSafeboxWnd;
		protected bool m_isRefreshMallWnd;
		protected bool m_isRefreshMessengerWnd;
		protected bool m_isRefreshGuildWndInfoPage;
		protected bool m_isRefreshGuildWndBoardPage;
		protected bool m_isRefreshGuildWndMemberPage;
		protected bool m_isRefreshGuildWndMemberPageGradeComboBox;
		protected bool m_isRefreshGuildWndSkillPage;
		protected bool m_isRefreshGuildWndGradePage;
		protected bool m_isRefreshAcceWnd;

		protected List<string> m_EmoticonStringVector = new List<string>();

		protected class STextConvertTable
		{
			public string acUpper = new string(new char[26]);
			public string acLower = new string(new char[26]);
			public byte[][] aacHan = RectangularArrays.RectangularByteArray(5000, 2);
		}
		protected STextConvertTable[] m_aTextConvTable = Arrays.InitializeWithDefaultInstances<STextConvertTable>(3);



		protected class SMarkAuth
		{
			public CNetworkAddress m_kNetAddr = new CNetworkAddress();
			public uint m_dwHandle;
			public uint m_dwRandomKey;
		}
		protected SMarkAuth m_kMarkAuth = new SMarkAuth();



		protected uint m_dwSelectedCharacterIndex;

		protected CInsultChecker m_kInsultChecker = new CInsultChecker();

		protected bool m_isEnableChatInsultFilter;
		protected bool m_bComboSkillFlag;

		protected LinkedList<string> m_kQue_stHack = new LinkedList<string>();

		private class SDirectEnterMode
		{
			public bool m_isSet;
			public uint m_dwChrSlotIndex;
		}
		private SDirectEnterMode m_kDirectEnterMode = new SDirectEnterMode();

		private void __DirectEnterMode_Initialize()
		{
			m_kDirectEnterMode.m_isSet = false;
			m_kDirectEnterMode.m_dwChrSlotIndex = 0;
		}

		private void __DirectEnterMode_Set(uint uChrSlotIndex)
		{
			m_kDirectEnterMode.m_isSet = true;
			m_kDirectEnterMode.m_dwChrSlotIndex = uChrSlotIndex;
		}

		private bool __DirectEnterMode_IsSet()
		{
			return m_kDirectEnterMode.m_isSet;
		}

		public uint EXPORT_GetBettingGuildWarValue(string c_szValueName)
		{
		...
		private class SBettingGuildWar
		{
			public uint m_dwBettingMoney;
			public uint m_dwObserverCount;
		}
		private SBettingGuildWar m_kBettingGuildWar = new SBettingGuildWar();

		private CInstanceBase m_pInstTarget;

		private void __BettingGuildWar_Initialize()
		{
			m_kBettingGuildWar.m_dwBettingMoney = 0;
			m_kBettingGuildWar.m_dwObserverCount = 0;
		}

		private void __BettingGuildWar_SetObserverCount(uint uObserverCount)
		{
			m_kBettingGuildWar.m_dwObserverCount = uObserverCount;
		}

		private void __BettingGuildWar_SetBettingMoney(uint uBettingMoney)
		{
			m_kBettingGuildWar.m_dwBettingMoney = uBettingMoney;
		}
}

public class CMainPacketHeaderMap : CNetworkPacketHeaderMap
{
		public const int STATIC_SIZE_PACKET = false;
		public const int DYNAMIC_SIZE_PACKET = true;

		public CMainPacketHeaderMap()
		{
			Set(LG_HEADER_GC_EMPIRE, SPacketType(sizeof(packet_empire), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_WARP, SPacketType(sizeof(packet_warp), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SKILL_COOLTIME_END, SPacketType(sizeof(packet_skill_cooltime_end), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_QUEST_INFO, SPacketType(sizeof(packet_quest_info), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_REQUEST_MAKE_GUILD, SPacketType(sizeof(packet_blank), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PVP, SPacketType(sizeof(packet_pvp), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHARACTER_ADD, SPacketType(sizeof(packet_add_char), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHAR_ADDITIONAL_INFO, SPacketType(sizeof(packet_char_additional_info), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHARACTER_ADD2, SPacketType(sizeof(packet_add_char2), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHARACTER_UPDATE, SPacketType(sizeof(packet_update_char), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHARACTER_DEL, SPacketType(sizeof(packet_del_char), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHARACTER_MOVE, SPacketType(sizeof(packet_move), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHAT, SPacketType(sizeof(packet_chatting), DYNAMIC_SIZE_PACKET));

			Set(LG_HEADER_GC_SYNC_POSITION, SPacketType(sizeof(packet_c2c), DYNAMIC_SIZE_PACKET));

			Set(LG_HEADER_GC_LOGIN_SUCCESS3, SPacketType(sizeof(packet_login_success3), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_LOGIN_SUCCESS4, SPacketType(sizeof(packet_login_success4), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_LOGIN_FAILURE, SPacketType(sizeof(packet_login_failure), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_PLAYER_CREATE_SUCCESS, SPacketType(sizeof(command_player_create_success), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PLAYER_CREATE_FAILURE, SPacketType(sizeof(command_create_failure), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PLAYER_DELETE_SUCCESS, SPacketType(sizeof(packet_blank), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PLAYER_DELETE_WRONG_SOCIAL_ID, SPacketType(sizeof(packet_blank), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_STUN, SPacketType(sizeof(packet_stun), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_DEAD, SPacketType(sizeof(packet_dead), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_MAIN_CHARACTER, SPacketType(sizeof(packet_main_character), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MAIN_CHARACTER2_EMPIRE, SPacketType(sizeof(packet_main_character2_empire), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MAIN_CHARACTER3_BGM, SPacketType(sizeof(packet_main_character3_bgm), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MAIN_CHARACTER4_BGM_VOL, SPacketType(sizeof(packet_main_character4_bgm_vol), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_PLAYER_POINTS, SPacketType(sizeof(packet_points), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PLAYER_POINT_CHANGE, SPacketType(sizeof(packet_point_change), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_ITEM_SET, SPacketType(sizeof(packet_set_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_ITEM_SET2, SPacketType(sizeof(packet_set_item2), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_ITEM_USE, SPacketType(sizeof(packet_use_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_ITEM_UPDATE, SPacketType(sizeof(packet_update_item), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_ITEM_GROUND_ADD, SPacketType(sizeof(packet_ground_add_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_ITEM_GROUND_DEL, SPacketType(sizeof(packet_ground_del_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_ITEM_OWNERSHIP, SPacketType(sizeof(packet_item_ownership), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_QUICKSLOT_ADD, SPacketType(sizeof(packet_quickslot_add), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_QUICKSLOT_DEL, SPacketType(sizeof(packet_quickslot_del), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_QUICKSLOT_SWAP, SPacketType(sizeof(packet_quickslot_swap), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_WHISPER, SPacketType(sizeof(packet_whisper), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_CHARACTER_POSITION, SPacketType(sizeof(packet_position), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MOTION, SPacketType(sizeof(packet_motion), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_SHOP, SPacketType(sizeof(packet_shop), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SHOP_SIGN, SPacketType(sizeof(SPacketGCShopSign), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_EXCHANGE, SPacketType(sizeof(packet_exchange), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_PING, SPacketType(sizeof(packet_ping), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_SCRIPT, SPacketType(sizeof(packet_script), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_QUEST_CONFIRM, SPacketType(sizeof(packet_quest_confirm), STATIC_SIZE_PACKET));
#if ENABLE_TARGET_INFO
			Set(LG_HEADER_GC_TARGET_INFO, SPacketType(sizeof(packet_target_info), STATIC_SIZE_PACKET));
#endif
			Set(LG_HEADER_GC_TARGET, SPacketType(sizeof(packet_target), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MOUNT, SPacketType(sizeof(packet_mount), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_CHANGE_SPEED, SPacketType(sizeof(packet_change_speed), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_HANDSHAKE, SPacketType(sizeof(packet_LG_HEADER_handshake), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_HANDSHAKE_OK, SPacketType(sizeof(packet_blank), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_BINDUDP, SPacketType(sizeof(packet_LG_HEADER_bindudp), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_OWNERSHIP, SPacketType(sizeof(packet_ownership), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CREATE_FLY, SPacketType(sizeof(packet_fly), STATIC_SIZE_PACKET));
#if _IMPROVED_PACKET_ENCRYPTION_
			Set(LG_HEADER_GC_KEY_AGREEMENT, SPacketType(sizeof(TPacketKeyAgreement), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_KEY_AGREEMENT_COMPLETED, SPacketType(sizeof(TPacketKeyAgreementCompleted), STATIC_SIZE_PACKET));
#endif
			Set(LG_HEADER_GC_ADD_FLY_TARGETING, SPacketType(sizeof(packet_fly_targeting), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_FLY_TARGETING, SPacketType(sizeof(packet_fly_targeting), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_PHASE, SPacketType(sizeof(packet_phase), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SKILL_LEVEL, SPacketType(sizeof(packet_skill_level), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SKILL_LEVEL_NEW, SPacketType(sizeof(packet_skill_level_new), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_MESSENGER, SPacketType(sizeof(packet_messenger), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_GUILD, SPacketType(sizeof(packet_guild), DYNAMIC_SIZE_PACKET));

			Set(LG_HEADER_GC_PARTY_INVITE, SPacketType(sizeof(packet_party_invite), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_ADD, SPacketType(sizeof(packet_party_add), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_UPDATE, SPacketType(sizeof(packet_party_update), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_REMOVE, SPacketType(sizeof(packet_party_remove), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_LINK, SPacketType(sizeof(packet_party_link), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_UNLINK, SPacketType(sizeof(packet_party_unlink), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_PARTY_PARAMETER, SPacketType(sizeof(paryt_parameter), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_SAFEBOX_SET, SPacketType(sizeof(packet_set_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SAFEBOX_DEL, SPacketType(sizeof(packet_item_del), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SAFEBOX_WRONG_PASSWORD, SPacketType(sizeof(packet_safebox_wrong_password), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SAFEBOX_SIZE, SPacketType(sizeof(packet_safebox_size), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SAFEBOX_MONEY_CHANGE, SPacketType(sizeof(packet_safebox_money_change), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_FISHING, SPacketType(sizeof(packet_fishing), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_DUNGEON, SPacketType(sizeof(packet_dungeon), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_TIME, SPacketType(sizeof(SPacketGCTime), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_WALK_MODE, SPacketType(sizeof(SPacketGCWalkMode), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHANGE_SKILL_GROUP, SPacketType(sizeof(SPacketGCChangeSkillGroup), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_REFINE_INFORMATION, SPacketType(sizeof(SPacketGCRefineInformation), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_REFINE_INFORMATION_NEW, SPacketType(sizeof(SPacketGCRefineInformationNew), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_SEPCIAL_EFFECT, SPacketType(sizeof(SPacketGCSpecialEffect), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_NPC_POSITION, SPacketType(sizeof(SPacketGCNPCPosition), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHANGE_NAME, SPacketType(sizeof(SPacketGCChangeName), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_LOGIN_KEY, SPacketType(sizeof(packet_login_key), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_AUTH_SUCCESS, SPacketType(sizeof(packet_auth_success), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_CHANNEL, SPacketType(sizeof(packet_channel), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_VIEW_EQUIP, SPacketType(sizeof(packet_view_equip), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_LAND_LIST, SPacketType(sizeof(packet_land_list), DYNAMIC_SIZE_PACKET));
			Set(LG_HEADER_GC_TARGET_UPDATE, SPacketType(sizeof(TPacketGCTargetUpdate), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_TARGET_DELETE, SPacketType(sizeof(TPacketGCTargetDelete), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_TARGET_CREATE_NEW, SPacketType(sizeof(TPacketGCTargetCreateNew), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_AFFECT_ADD, SPacketType(sizeof(TPacketGCAffectAdd), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_AFFECT_REMOVE, SPacketType(sizeof(TPacketGCAffectRemove), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_MALL_OPEN, SPacketType(sizeof(packet_mall_open), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MALL_SET, SPacketType(sizeof(packet_set_item), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_MALL_DEL, SPacketType(sizeof(packet_item_del), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_LOVER_INFO, SPacketType(sizeof(packet_lover_info), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_LOVE_POINT_UPDATE, SPacketType(sizeof(packet_love_point_update), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_DIG_MOTION, SPacketType(sizeof(packet_dig_motion), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_DAMAGE_INFO, SPacketType(sizeof(packet_damage_info), STATIC_SIZE_PACKET));

			Set(LG_HEADER_GC_SPECIFIC_EFFECT, SPacketType(sizeof(SPacketGCSpecificEffect), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_DRAGON_SOUL_REFINE, SPacketType(sizeof(SPacketGCDragonSoulRefine), STATIC_SIZE_PACKET));
			Set(LG_HEADER_GC_ACCE, SPacketType(sizeof(packet_acce), DYNAMIC_SIZE_PACKET));
#if ENABLE_MULTI_LANGUAGE_SYSTEM
			Set(LG_HEADER_GC_REQUEST_CHANGE_LANGUAGE, SPacketType(sizeof(SPacketChangeLanguage), STATIC_SIZE_PACKET));
#endif

#if ENABLE_EXTENDED_WHISPER_DETAILS
			Set(LG_HEADER_GC_WHISPER_DETAILS, SPacketType(sizeof(SPacketGCWhisperDetails), STATIC_SIZE_PACKET));
#endif
		}
}