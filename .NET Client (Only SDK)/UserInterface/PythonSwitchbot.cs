...
public class CPythonSwitchbot : CSingleton<CPythonSwitchbot>
{
//# Laniatus Games Studio Inc. | TODO TASK: There is no equivalent to most C++ 'pragma' directives in C#:
//    #pragma pack(1)
	public class TSwitchbotAttributeAlternativeTable
	{
		public TPlayerItemAttribute[] attributes = Arrays.InitializeWithDefaultInstances<TPlayerItemAttribute>(MAX_NORM_ATTR_NUM);

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool IsConfigured() const
	...
	public class TSwitchbotTable
	{
		public uint player_id;
		public bool[] active = new bool[SWITCHBOT_SLOT_COUNT];
		public bool[] finished = new bool[SWITCHBOT_SLOT_COUNT];
		public uint[] items = new uint[SWITCHBOT_SLOT_COUNT];
		public TSwitchbotAttributeAlternativeTable[][] alternatives = RectangularArrays.RectangularTSwitchbotAttributeAlternativeTableArray(SWITCHBOT_SLOT_COUNT, SWITCHBOT_ALTERNATIVE_COUNT);

		public TSwitchbotTable()
		{
			this.player_id = 0;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
	...
	public class TSwitchbottAttributeTable
	{
		public byte attribute_set;
...
//#pragma pack()
	...
	public void Update(in TSwitchbotTable table)
	{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy_s' has no equivalent in C#:
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy_s' has no equivalent in C#:
	...
	public void SetAttribute(byte slot, byte alternative, byte attrIdx, byte attrType, int attrValue)
	{
	...
	public TPlayerItemAttribute GetAttribute(byte slot, byte alternative, byte attrIdx)
	{
		TPlayerItemAttribute attr = new TPlayerItemAttribute();

		if (slot >= SWITCHBOT_SLOT_COUNT)
		{
			return new TPlayerItemAttribute(attr);
		}
	...
	public void GetAlternatives(byte slot, List<TSwitchbotAttributeAlternativeTable> vec_alternatives)
	{
		if (slot >= SWITCHBOT_SLOT_COUNT)
		{
			return;
		...
		foreach (var it in m_SwitchbotTable.alternatives[slot])
		{
			vec_alternatives.emplace_back(it);
		}
	}

	public int GetAttibuteSet(byte slot)
	{
		if (slot >= SWITCHBOT_SLOT_COUNT)
		{
			return -1;
		...
		TItemPos pos = new TItemPos(SWITCHBOT, slot);
	...
	public bool IsActive(byte slot)
	{
		if (slot >= SWITCHBOT_SLOT_COUNT)
		{
			return false;
	...
	public bool IsFinished(byte slot)
	{
		if (slot >= SWITCHBOT_SLOT_COUNT)
		{
			return false;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
	...
	public void AddAttributeToMap(in TSwitchbottAttributeTable table)
	{
		auto it = m_map_AttributesBySet.find(table.attribute_set);
		if (it == m_map_AttributesBySet.end())
		{
			SortedDictionary<byte, int> attribute_map = new SortedDictionary<byte, int>();
			attribute_map.Add(table.apply_num, table.max_value);

			m_map_AttributesBySet.insert(Tuple.Create(table.attribute_set, attribute_map));
		}
		else
		{
			...
			{
				it.second.insert(Tuple.Create(table.apply_num, table.max_value));
			}
			else
			{
	...
	public void GetAttributesBySet(int iAttributeSet, List<TPlayerItemAttribute> vec_attributes)
	{
		if (iAttributeSet == -1)
		{
			return;
	...
	public int GetAttributeMaxValue(int iAttributeSet, byte attrType)
	{
		if (iAttributeSet == -1)
		{
			return 0;
		...
		auto it2 = it.second.find(attrType);
		if (it2 == it.second.end())
		{
			return 0;
		}
	...
	protected TSwitchbotTable m_SwitchbotTable = new TSwitchbotTable();
	protected SortedDictionary<byte, SortedDictionary<byte, int>> m_map_AttributesBySet = new SortedDictionary<byte, SortedDictionary<byte, int>>();
}
#endif

//# Laniatus Games Studio Inc. |: Classes must be named in C#, so the following class has been named by the converter:
public class AnonymousClass
{
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: bool operator ()(TPlayerItemAttribute a, TPlayerItemAttribute b) const
	public bool functorMethod(TPlayerItemAttribute a, TPlayerItemAttribute b)
	{
