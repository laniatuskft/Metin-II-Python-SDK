...
public class CInsultChecker
{
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private CInsultChecker GetSingleton_s_kInsultChecker = new CInsultChecker();

		public CInsultChecker GetSingleton()
		{
...
//			static CInsultChecker s_kInsultChecker;
			return GetSingleton_s_kInsultChecker;
		}
		...
		public void AppendInsult(in string c_rstInsult)
		{
		...
		public bool IsInsultIn(string c_szLine, uint uLineLen)
		{
			for (uint uPos = 0; uPos < uLineLen;)
			{
				...
				if ((bChr & 0x80) != 0)
				{
					if (__IsInsult(c_szLine.Substring(uPos)))
					{
						return true;
					}
		...
		public void FilterInsult(ref string szLine, uint uLineLen)
		{
			...
			for (uint uPos = 0; uPos < uLineLen;)
			{
				...
				uint uInsultLen;
				if (__GetInsultLength(szLine.Substring(uPos), uInsultLen))
				{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
					memset(szLine.Substring(uPos), INSULT_FILTER_CHAR, uInsultLen);
					uPos += uInsultLen;
				}
				else
				{
		...
		private bool __GetInsultLength(string c_szWord, ref uint puInsultLen)
		{
			LinkedList<string>.Enumerator i;
			for (i = m_kList_stInsult.begin(); i.MoveNext();)
			{
				string rstInsult = i.Current;

				int ret = LocaleService_StringCompareCI(c_szWord, rstInsult, rstInsult.Length);
				if (0 == ret)
				{
		...
		private bool __IsInsult(string c_szWord)
		{
			uint uInsultLen;
			return __GetInsultLength(c_szWord, uInsultLen);
		}

		private LinkedList<string> m_kList_stInsult = new LinkedList<string>();
}
