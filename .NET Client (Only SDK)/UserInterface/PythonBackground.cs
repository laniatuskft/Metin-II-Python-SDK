...
public class CPythonBackground : CMapManager, CSingleton<CPythonBackground>
{
	public const int SHADOW_NONE = 0;
	public const int SHADOW_GROUND = 1;
	public const int SHADOW_GROUND_AND_SOLO = 2;
	public const int SHADOW_ALL = 3;
	public const int SHADOW_ALL_HIGH = 4;
	public const int SHADOW_ALL_MAX = 5;

	public const int DISTANCE0 = 0;
	public const int DISTANCE1 = 1;
	public const int DISTANCE2 = 2;
	public const int DISTANCE3 = 3;
	public const int DISTANCE4 = 4;
	public const int NUM_DISTANCE_SET = 5;

	public const int DAY_MODE_LIGHT = 0;
	public const int DAY_MODE_DARK = 1;

	public class SVIEWDISTANCESET
	{
		public float m_fFogStart;
		public float m_fFogEnd;
		public float m_fFarClip;
		public _D3DVECTOR m_v3SkyBoxScale = new _D3DVECTOR();
	}

	public CPythonBackground()
	{
	...
	public void Initialize()
	{
		...
		const string stAtlasInfoFileName = "locale/common/AtlasInfo.txt";
#else
		string stAtlasInfoFileName = LocaleService_GetLocalePath();
		stAtlasInfoFileName += "/AtlasInfo.txt";
#endif
		SetAtlasInfoFileName(stAtlasInfoFileName);
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
	private int Create_s_isCreateProperty = false;
...
//		static int s_isCreateProperty=false;
		...
		if (!Create_s_isCreateProperty)
		{
			Create_s_isCreateProperty = true;
			__CreateProperty();
		}
	...
	public void GlobalPositionToLocalPosition(ref int rGlobalX, ref int rGlobalY)
	{
		rGlobalX -= m_dwBaseX;
		rGlobalY -= m_dwBaseY;
	}

	public void LocalPositionToGlobalPosition(ref int rLocalX, ref int rLocalY)
	{
		rLocalX += m_dwBaseX;
		rLocalY += m_dwBaseY;
	}

	public void EnableTerrainOnlyForHeight()
	{
		if (!m_pkMap)
		{
			return;
	...
	public bool SetSplatLimit(int iSplatNum)
	{
		if (!m_pkMap)
		{
			return false;
	...
	public bool SetVisiblePart(int eMapOutDoorPart, bool isVisible)
	{
		if (!m_pkMap)
		{
			return false;
	...
	public bool SetShadowLevel(int eLevel)
	{
		if (!m_pkMap)
		{
			return false;
	...
	public void RefreshShadowLevel()
	{
		SetShadowLevel(CPythonSystem.Instance().GetShadowLevel());
	}

	public void SelectViewDistanceNum(int eNum)
	{
		if (!m_pkMap)
		{
			return;
	...
	public void SetViewDistanceSet(int eNum, float fFarClip)
	{
		if (!m_pkMap)
		{
			return;
	...
	public uint GetRenderShadowTime()
	{
		return m_dwRenderShadowTime;
	}

	public void GetDistanceSetInfo(ref int peNum, ref float pfStart, ref float pfEnd, ref float pfFarClip)
	{
		if (!m_pkMap)
		{
			peNum = 4;
	...
	public bool GetLightDirection(_D3DVECTOR rv3LightDirection)
	{
		if (!mc_pcurEnvironmentData)
		{
			return false;
	...
	public void Update(float fCenterX, float fCenterY, float fCenterZ)
	{
		if (!IsMapReady())
		{
			return;
		...
		if (rkMap.IsEnablePortal())
		{
			...
			FGetPortalID kGetPortalID = new FGetPortalID(fCenterX, -fCenterY);
			...
			SortedSet<int>.Enumerator itor = kGetPortalID.m_kSet_iPortalID.begin();
			if (!__IsSame(kGetPortalID.m_kSet_iPortalID, m_kSet_iShowingPortalID))
			{
				...
				while (itor.MoveNext())
				{
					AddShowingPortalIDitor.Current;
				}
				RefreshPortal();
		...
		{
			SortedDictionary<uint, uint>.Enumerator itor = m_kMap_dwTargetID_dwChrID.begin();
			while (itor.MoveNext())
			{
				uint dwTargetID = itor.Current.Key;
				uint dwChrID = itor.Current.Value;
		...
		{
			SortedDictionary<uint, SReserveTargetEffect>.Enumerator itor = m_kMap_dwID_kReserveTargetEffect.begin();
			while (itor.MoveNext())
			{
				uint dwID = itor.Current.Key;
				SReserveTargetEffect rReserveTargetEffect = itor.Current.Value;

				float ilx = rReserveTargetEffect.ilx;
				float ily = rReserveTargetEffect.ily;
	...
	public void RenderCharacterShadowToTexture()
	{
...
//		extern bool GRAPHICS_CAPS_CAN_NOT_DRAW_SHADOW;
		...
		if (m_eShadowLevel == SHADOW_ALL || m_eShadowLevel == SHADOW_ALL_HIGH || m_eShadowLevel == SHADOW_ALL_MAX || m_eShadowLevel == SHADOW_GROUND_AND_SOLO)
		{
			_D3DMATRIX matWorld = new _D3DMATRIX();
			(CStateManager.Instance()).GetTransform((D3DTRANSFORMSTATETYPE)(0 + 256), matWorld);
			...
			(CStateManager.Instance()).SetTransform((D3DTRANSFORMSTATETYPE)(0 + 256), matWorld);
		}

		uint t2 = ELTimer_GetMSec();
	...
	public bool CheckAdvancing(CInstanceBase pInstance)
	{
		if (!IsMapReady())
		{
			return true;
		...
		CollisionAdjustChecker kCollisionAdjustChecker = new CollisionAdjustChecker(pInstance);
		rkCullingMgr.ForInRange(center, radius, kCollisionAdjustChecker);
		if (kCollisionAdjustChecker.isBlocked)
		{
			CollisionChecker kCollisionChecker = new CollisionChecker(pInstance);
	...
	public void SetCharacterDirLight()
	{
		if (!IsMapReady())
		{
			return;
		...
		(CStateManager.Instance()).SetLight(0, mc_pcurEnvironmentData.DirLights[ENV_DIRLIGHT_CHARACTER]);
	}

	public void SetBackgroundDirLight()
	{
		if (!IsMapReady())
		{
			return;
		...
		(CStateManager.Instance()).SetLight(0, mc_pcurEnvironmentData.DirLights[ENV_DIRLIGHT_BACKGROUND]);
	}
	...
	public void EnableSnowEnvironment()
	{
		m_SnowEnvironment.Enable();
	}

	public void DisableSnowEnvironment()
	{
		m_SnowEnvironment.Disable();
	}

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
	private string[] SetXMaxTree_s_strTreeName = {"t:/laniaworkstate/tree/christmastree1.spt", "t:/laniaworkstate/tree/christmastree2.spt", "t:/laniaworkstate/tree/christmastree3.spt"};
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
	private string[] SetXMaxTree_s_strEffectName = {"t:/laniaworkstate/effect/etc/christmas_tree/tree_1s.mse", "t:/laniaworkstate/effect/etc/christmas_tree/tree_2s.mse", "t:/laniaworkstate/effect/etc/christmas_tree/tree_3s.mse"};

	public void SetXMaxTree(int iGrade)
	{
		if (!m_pkMap)
		{
			return;
		...
		Debug.Assert(iGrade >= 0 && iGrade <= 3);
		...
		iGrade = Math.Min(iGrade, 2);

//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//		static string s_strTreeName[3] = { "t:/laniaworkstate/tree/christmastree1.spt", "t:/laniaworkstate/tree/christmastree2.spt", "t:/laniaworkstate/tree/christmastree3.spt" };
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
//		static string s_strEffectName[3] = { "t:/laniaworkstate/effect/etc/christmas_tree/tree_1s.mse", "t:/laniaworkstate/effect/etc/christmas_tree/tree_2s.mse", "t:/laniaworkstate/effect/etc/christmas_tree/tree_3s.mse"};
		rkMap.XMasTree_Set(c_v3TreePos.x, c_v3TreePos.y, c_v3TreePos.z, SetXMaxTree_s_strTreeName[iGrade].c_str(), SetXMaxTree_s_strEffectName[iGrade].c_str());
	}
	...
	public void CreateTargetEffect(uint dwID, uint dwChrVID)
	{
		m_kMap_dwTargetID_dwChrID.insert(Tuple.Create(dwID, dwChrVID));
	}

	public void CreateTargetEffect(uint dwID, int lx, int ly)
	{
		...
		if (0.0f == fHeight)
		{
			SReserveTargetEffect ReserveTargetEffect = new SReserveTargetEffect();
			ReserveTargetEffect.ilx = ilx;
			ReserveTargetEffect.ily = ily;
			m_kMap_dwID_kReserveTargetEffect.insert(Tuple.Create(dwID, ReserveTargetEffect));
			return;
		}
	...
	public void DeleteTargetEffect(uint dwID)
	{
	...
	public void CreateSpecialEffect(uint dwID, float fx, float fy, float fz, string c_szFileName)
	{
		CMapOutdoor rkMap = GetMapOutdoorRef();
		rkMap.SpecialEffect_Create(dwID, fx, fy, fz, c_szFileName);
	}

	public void DeleteSpecialEffect(uint dwID)
	{
		CMapOutdoor rkMap = GetMapOutdoorRef();
		rkMap.SpecialEffect_Delete(dwID);
	}

	public void Warp(uint dwX, uint dwY)
	{
...
//Original Metin2 CPlus Line: TMapInfo & rMapInfo = *pkMapInfo;
		TMapInfo rMapInfo = pkMapInfo;
		Debug.Assert((dwX >= rMapInfo.m_dwBaseX) && (dwY >= rMapInfo.m_dwBaseY));
	...
	public void RegisterDungeonMapName(string c_szMapName)
	{
		m_kSet_strDungeonMapName.insert(c_szMapName);
	}

	public CPythonBackground.TMapInfo GlobalPositionToMapInfo(uint dwGlobalX, uint dwGlobalY)
	{
	...
	protected void __CreateProperty()
	{
		if (CEterPackManager.SEARCH_FILE_FIRST == CEterPackManager.Instance().GetSearchMode() && _access("property", 0) == 0)
		{
			...
			CPropertyLoader PropertyLoader = new CPropertyLoader();
	...
	protected bool __IsSame(SortedSet<int> rleft, SortedSet<int> rright)
	{
		SortedSet<int>.Enumerator itor_l;
		SortedSet<int>.Enumerator itor_r;

		for (itor_l = rleft.GetEnumerator(); itor_l.MoveNext();)
		{
			if (rright.end() == rright.finditor_l.Current)
			{
				return false;
			}
		}

		for (itor_r = rright.GetEnumerator(); itor_r.MoveNext();)
		{
			if (rleft.end() == rleft.finditor_r.Current)
			{
				return false;
			}
		}
	...
	protected string m_strMapName = "";

	private CSnowEnvironment m_SnowEnvironment = new CSnowEnvironment();

	private int m_iDayMode;
	private int m_iXMasTreeGrade;

	private int m_eShadowLevel;
	private int m_eViewDistanceNum;

	private bool m_bVisibleGuildArea;

	private uint m_dwRenderShadowTime;

	private uint m_dwBaseX;
	private uint m_dwBaseY;

	private SVIEWDISTANCESET[] m_ViewDistanceSet = Arrays.InitializeWithDefaultInstances<SVIEWDISTANCESET>(NUM_DISTANCE_SET);

	private SortedSet<int> m_kSet_iShowingPortalID = new SortedSet<int>();
	private SortedSet<string> m_kSet_strDungeonMapName = new SortedSet<string>();
	private SortedDictionary<uint, uint> m_kMap_dwTargetID_dwChrID = new SortedDictionary<uint, uint>();

	private class SReserveTargetEffect
	{
		public int ilx;
		public int ily;
	}
	private SortedDictionary<uint, SReserveTargetEffect> m_kMap_dwID_kReserveTargetEffect = new SortedDictionary<uint, SReserveTargetEffect>();

	private class FFindWarpMapName
	{
		public uint m_dwX;
		public uint m_dwY;
		public FFindWarpMapName.FFindWarpMapName(uint dwX, uint dwY)
		{
			m_dwX = dwX;
			m_dwY = dwY;
		}
		public bool functorMethod(TMapInfo rMapInfo)
		{
...
public class FGetPortalID
{
	public float m_fRequestX;
	public float m_fRequestY;
	public SortedSet<int> m_kSet_iPortalID = new SortedSet<int>();
	public FGetPortalID(float fRequestX, float fRequestY)
	{
	...
	{
		for (int LaniatusDefVariables = 0; LaniatusDefVariables < PORTAL_ID_MAX_NUM; ++i)
		{
...
public class CollisionChecker
{
	...
	public CInstanceBase pInstance;
	public CollisionChecker(CInstanceBase pInstance)
	{
		this.pInstance = new CInstanceBase(pInstance);
		this.isBlocked = false;
	}
	public void functorMethod(CGraphicObjectInstance pOpponent)
	{
		if (isBlocked)
		{
			return;
...
public class CollisionAdjustChecker
{
	public bool isBlocked;
	public CInstanceBase pInstance;
	public CollisionAdjustChecker(CInstanceBase pInstance)
	{
		this.pInstance = new CInstanceBase(pInstance);
		this.isBlocked = false;
	}
