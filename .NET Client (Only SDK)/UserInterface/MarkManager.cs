...
public class CGuildMarkManager : singleton<CGuildMarkManager>
{
		public const int MAX_IMAGE_COUNT = 5;
		public const int INVALID_MARK_ID = 0xffffffff;

		public class TGuildSymbol
		{
			public uint crc;
			public List<byte> raw = new List<byte>();
		}

		public CGuildMarkManager()
		{
		//# Laniatus Games Studio Inc. | TODO TASK: C# does not allow setting or comparing #define constants:
#if _MSC_VER >= 1200
			_mkdir("mark");
#endif
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < MAX_IMAGE_COUNT * CGuildMarkImage.MARK_TOTAL_COUNT; ++i)
			{
		...
		{
			for (SortedDictionary<uint, CGuildMarkImage>.Enumerator it = m_mapIdx_Image.begin(); it.MoveNext();)
			{
				__DeleteImage(it.Current.Value);
			}

			m_mapIdx_Image.clear();
		}

		public CGuildMarkManager.TGuildSymbol GetGuildSymbol(uint guildID)
		{
			SortedDictionary<uint, TGuildSymbol>.Enumerator it = m_mapSymbol.find(guildID);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public bool LoadSymbol(string filename)
		{
			...
			else
			{
				...
				for (uint LaniatusDefVariables = 0; LaniatusDefVariables < symbolCount; LaniatusDefVariables++)
				{
					...
					TGuildSymbol gs = new TGuildSymbol();
...
//# Laniatus Games Studio Inc. | TODO TASK: There is no equivalent to 'reinterpret_cast' in C#:
					...
					m_mapSymbol.insert(Tuple.Create(guildID, gs));
				}
			}
		...
		public void SaveSymbol(string filename)
		{
			...
			uint symbolCount = (uint)m_mapSymbol.size();
			fwrite(symbolCount, 4, 1, fp);

			for (SortedDictionary<uint, TGuildSymbol>.Enumerator it = m_mapSymbol.begin(); it.MoveNext();)
			{
				uint guildID = it.Current.Key;
				uint dwSize = (uint)it.Current.Value.raw.size();
				fwrite(guildID, 4, 1, fp);
				fwrite(dwSize, 4, 1, fp);
				fwrite(it.Current.Value.raw[0], 1, dwSize, fp);
			}

			fclose(fp);
		}

		public void UploadSymbol(uint guildID, int iSize, byte pbyData)
		{
			(0, "GuildSymbolUpload guildID %lu Size %d", guildID, iSize);

			if (m_mapSymbol.find(guildID) == m_mapSymbol.end())
			{
				m_mapSymbol.insert(Tuple.Create(guildID, TGuildSymbol()));
			}
...
//# Laniatus Games Studio Inc. | TODO TASK: There is no equivalent to 'reinterpret_cast' in C#:
		...
		public void SetMarkPathPrefix(string prefix)
		{
			m_pathPrefix = prefix;
		}

		public bool LoadMarkIndex()
		{
			string buf = new string(new char[64]);
			snprintf(buf, sizeof(char), "mark/%s_index", m_pathPrefix.c_str());
			FILE fp = fopen(buf, "r");
			...
			while (fgets(line, sizeof(char) - 1, fp))
			{
				sscanf(line, "%lu %lu", guildID, markID);
				line = StringFunctions.ChangeCharacter(line, 0, '\0');
		...
		public bool SaveMarkIndex()
		{
			...
			for (SortedDictionary<uint, uint>.Enumerator it = m_mapGID_MarkID.begin(); it.MoveNext();)
			{
				fprintf(fp, "%d %d\n", it.Current.Key, it.Current.Value);
			}

			fclose(fp);
			(0, "MarkManager::SaveMarkData: index count %d", m_mapGID_MarkID.size());
			return true;
		}

		public void LoadMarkImages()
		{
			bool[] isMarkExists = new bool[MAX_IMAGE_COUNT];
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
			memset(isMarkExists, 0, sizeof(bool));

			for (SortedDictionary<uint, uint>.Enumerator it = m_mapGID_MarkID.begin(); it.MoveNext();)
			{
				uint markID = it.Current.Value;
		...
		public void SaveMarkImage(uint imgIdx)
		{
			string path = "";

			if (GetMarkImageFilename(imgIdx, path))
			{
				if (!__GetImage(imgIdx).Save(path))
				{
...
//Original Metin2 CPlus Line: bool GetMarkImageFilename(uint imgIdx, string & path) const
		public bool GetMarkImageFilename(uint imgIdx, ref string path)
		{
			...
			string buf = new string(new char[64]);
			snprintf(buf, sizeof(char), "mark/%s_%lu.tga", m_pathPrefix.c_str(), imgIdx);
			path = buf;
			return true;
		}

		public bool AddMarkIDByGuildID(uint guildID, uint markID)
		{
			if (markID >= MAX_IMAGE_COUNT * CGuildMarkImage.MARK_TOTAL_COUNT)
			{
				return false;
			...
			m_mapGID_MarkID.insert(SortedDictionary<uint, uint>.value_type(guildID, markID));
			m_setFreeMarkID.erase(markID);
			return true;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: uint GetMarkImageCount() const
		public uint GetMarkImageCount()
		{
			return (uint)m_mapIdx_Image.size();
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: uint GetMarkCount() const
		public uint GetMarkCount()
		{
			return (uint)m_mapGID_MarkID.size();
		}

		public uint GetMarkID(uint guildID)
		{
			SortedDictionary<uint, uint>.Enumerator it = m_mapGID_MarkID.find(guildID);
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: void CopyMarkIdx(char * pcBuf) const
		public void CopyMarkIdx(ref string pcBuf)
		{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			ushort * pwBuf = (ushort) pcBuf;

			for (SortedDictionary<uint, uint>.Enumerator it = m_mapGID_MarkID.begin(); it.MoveNext();)
			{
				*(pwBuf++) = it.Current.Key;
				*(pwBuf++) = it.Current.Value;
			}
		}

		public uint SaveMark(uint guildID, ref byte pbMarkImage)
		{
			uint idMark;
			...
			if ((idMark = GetMarkID(guildID)) == INVALID_MARK_ID)
			{
				...
				{
					(0, "SaveMark: mark id alloc %u", idMark);
				}
			}
			else
			{
				(0, "SaveMark: mark id found %u", idMark);
			}

			uint imgIdx = (idMark / CGuildMarkImage.MARK_TOTAL_COUNT);
		...
		public void DeleteMark(uint guildID)
		{
			SortedDictionary<uint, uint>.Enumerator it = m_mapGID_MarkID.find(guildID);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public void GetDiffBlocks(uint imgIdx, uint crcList, in SortedDictionary<byte, SGuildMarkBlock > mapDiffBlocks)
		{
			mapDiffBlocks.Clear();
		...
		public bool SaveBlockFromCompressedData(uint imgIdx, uint posBlock, byte pbBlock, uint dwSize)
		{
			CGuildMarkImage pkImage = __GetImage(imgIdx);
		...
		public bool GetBlockCRCList(uint imgIdx, ref uint crcList)
		{
			if (m_mapIdx_Image.end() == m_mapIdx_Image.find(imgIdx))
			{
				TraceError("invalid idx %u", imgIdx);
...
#define itertype
#endif
		private CGuildMarkImage __NewImage()
		{
			return new CGuildMarkImage();
		}

		private void __DeleteImage(CGuildMarkImage pkImgDel)
		{
			pkImgDel = null;
		}

		private uint __AllocMarkID(uint guildID)
		{
			SortedSet<uint>.Enumerator it = m_setFreeMarkID.lower_bound(0);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			uint imgIdx = markID / CGuildMarkImage.MARK_TOTAL_COUNT;
			CGuildMarkImage pkImage = __GetImage(imgIdx);

			if (pkImage != null && AddMarkIDByGuildID(guildID, markID))
			{
				return markID;
			}
		...
		private CGuildMarkImage __GetImage(uint imgIdx)
		{
			SortedDictionary<uint, CGuildMarkImage >.Enumerator it = m_mapIdx_Image.find(imgIdx);
			...
			if (it == m_mapIdx_Image.end())
			{
				...
				if (GetMarkImageFilename(imgIdx, imagePath))
				{
					...
					m_mapIdx_Image.insert(SortedDictionary<uint, CGuildMarkImage>.value_type(imgIdx, pkImage));
...
//		CGuildMarkImage __GetImagePtr(uint idMark);

		private SortedDictionary<uint, CGuildMarkImage > m_mapIdx_Image = new SortedDictionary<uint, CGuildMarkImage >();
		private SortedDictionary<uint, uint> m_mapGID_MarkID = new SortedDictionary<uint, uint>();

		private SortedSet<uint> m_setFreeMarkID = new SortedSet<uint>();
		private string m_pathPrefix = "";

		private SortedDictionary<uint, TGuildSymbol> m_mapSymbol = new SortedDictionary<uint, TGuildSymbol>();
}