...
public class CPythonItem : CSingleton<CPythonItem>
{
		public const int INVALID_ID = 0xffffffff;

		public const int VNUM_MONEY = 1;

		public const int USESOUND_NONE = 0;
		public const int USESOUND_DEFAULT = 1;
		public const int USESOUND_ARMOR = 2;
		public const int USESOUND_WEAPON = 3;
		public const int USESOUND_BOW = 4;
		public const int USESOUND_ACCESSORY = 5;
		public const int USESOUND_POTION = 6;
		public const int USESOUND_PORTAL = 7;
		public const int USESOUND_NUM = 8;

		public const int DROPSOUND_DEFAULT = 0;
		public const int DROPSOUND_ARMOR = 1;
		public const int DROPSOUND_WEAPON = 2;
		public const int DROPSOUND_BOW = 3;
		public const int DROPSOUND_ACCESSORY = 4;
		public const int DROPSOUND_NUM = 5;

		public class SGroundItemInstance
		{
			public uint dwVirtualNumber;
			public _D3DVECTOR v3EndPosition = new _D3DVECTOR();

			public _D3DVECTOR v3RotationAxis = new _D3DVECTOR();
			public D3DXQUATERNION qEnd = new D3DXQUATERNION();
			public _D3DVECTOR v3Center = new _D3DVECTOR();
			public CGraphicThingInstance ThingInstance = new CGraphicThingInstance();
			public uint dwStartTime;
			public uint dwEndTime;

			public uint eDropSoundType;

			public bool bAnimEnded;
...
//			void Clear();
			...
			public string stOwnership = "";

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//			static void __PlayDropSound(uint eItemType, in _D3DVECTOR c_rv3Pos);
			public static string[] ms_astDropSoundFileName = Arrays.InitializeStringArrayWithDefaultInstances(DROPSOUND_NUM);

			public SGroundItemInstance()
			{
			}
		...
		public virtual void Dispose()
		{
			Debug.Assert(m_GroundItemInstanceMap.empty());
		}
		...
		public void Create()
		{
			CEffectManager.Instance().RegisterEffect2("t:/laniaworkstate/effect/etc/dropitem/dropitem.mse", m_dwDropItemEffectID);
		}

		public void PlayUseSound(uint dwItemID)
		{
			...
			uint eItemType = __GetUseSoundType(pkItemData);
		...
		public void PlayDropSound(uint dwItemID)
		{
			CItemData pkItemData;
			if (!CItemManager.Instance().GetItemDataPointer(dwItemID, pkItemData))
			{
			...
			uint eItemType = __GetDropSoundType(pkItemData);
		...
		public void SetUseSoundFileName(uint eItemType, in string c_rstFileName)
		{
			if (eItemType >= USESOUND_NUM)
			{
				return;
		...
		public void SetDropSoundFileName(uint eItemType, in string c_rstFileName)
		{
			if (eItemType >= DROPSOUND_NUM)
			{
				return;
			...
			Tracenf("SetDropSoundFile %d : %s", eItemType, c_rstFileName);
		...
		public void DeleteAllItems()
		{
			...
			TGroundItemInstanceMap.iterator LaniatusDefVariables = new TGroundItemInstanceMap.iterator();
			for (i = m_GroundItemInstanceMap.begin(); LaniatusDefVariables != m_GroundItemInstanceMap.end(); ++i)
			{
		...
		public void Update(in POINT c_rkPtMouse)
		{
			TGroundItemInstanceMap.iterator itor = m_GroundItemInstanceMap.begin();
			for (; itor != m_GroundItemInstanceMap.end(); ++itor)
			{
		...
		public void CreateItem(uint dwVirtualID, uint dwVirtualNumber, float x, float y, float z, bool bDrop = true)
		{
			CItemData pItemData;
			...
			if (bDrop)
			{
				...
				if (pItemData.GetType() == CItemData.ITEM_TYPE_WEAPON && (pItemData.GetWeaponType() == CItemData.WEAPON_SWORD || pItemData.GetWeaponType() == CItemData.WEAPON_ARROW || pItemData.GetWeaponType() == CItemData.WEAPON_QUIVER))
				{
					bStabGround = true;
				}
			...
			if (bDrop)
			{
				_D3DVECTOR vMin = new _D3DVECTOR();
				_D3DVECTOR vMax = new _D3DVECTOR();
				pGroundItemInstance.ThingInstance.GetBoundBox(vMin, vMax);
				pGroundItemInstance.v3Center = (vMin + vMax) * 0.5f;

				Tuple<float,int>[] f = {Tuple.Create(vMax.x - vMin.x,0), Tuple.Create(vMax.y - vMin.y,1), Tuple.Create(vMax.z - vMin.z,2)};

				std::sort(f,f + 3);

				_D3DVECTOR rEnd = new _D3DVECTOR();
				...
				else
				{
					if (f[0].Item2 == 0)
					{
						pGroundItemInstance.qEnd = RotationArc(D3DXVECTOR3(((float)(random() % 2)) * 2 - 1 + frandom(-0.1f,0.1f), 0 + frandom(-0.1f,0.1f), 0 + frandom(-0.1f,0.1f)), D3DXVECTOR3(0,0,1));
					}
					else if (f[0].Item2 == 1)
					{
						pGroundItemInstance.qEnd = RotationArc(D3DXVECTOR3(0 + frandom(-0.1f,0.1f), ((float)(random() % 2)) * 2 - 1 + frandom(-0.1f,0.1f), 0 + frandom(-0.1f,0.1f)), D3DXVECTOR3(0,0,1));
					}
					else
					{
						pGroundItemInstance.qEnd = RotationArc(D3DXVECTOR3(0 + frandom(-0.1f,0.1f), 0 + frandom(-0.1f,0.1f), ((float)(random() % 2)) * 2 - 1 + frandom(-0.1f,0.1f)), D3DXVECTOR3(0,0,1));
					}
				}

				float rot = frandom(0, 2 * 3.1415926535f);
				D3DXQUATERNION q = new D3DXQUATERNION(0,0,cosf(rot),sinf(rot));
				...
				_D3DVECTOR v3Adjust = -pGroundItemInstance.v3Center;
		...
		public void DeleteItem(uint dwVirtualID)
		{
		...
		public void SetOwnership(uint dwVID, string c_pszName)
		{
		...
		public bool GetOwnership(uint dwVID, string[] c_pszName)
		{
			TGroundItemInstanceMap.iterator itor = m_GroundItemInstanceMap.find(dwVID);
			...
			TGroundItemInstance pGroundItemInstance = itor.second;
			c_pszName[0] = pGroundItemInstance.stOwnership.c_str();

			return true;
		}

		public bool GetGroundItemPosition(uint dwVirtualID, TPixelPosition pPosition)
		{
			TGroundItemInstanceMap.iterator itor = m_GroundItemInstanceMap.find(dwVirtualID);
			if (m_GroundItemInstanceMap.end() == itor)
			{
			...
			_D3DVECTOR rkD3DVct3 = pInstance.ThingInstance.GetPosition();
		...
		public bool GetPickedItemID(ref uint pdwPickedItemID)
		{
		...
		public bool GetCloseItem(in TPixelPosition c_rPixelPosition, ref uint pdwItemID, uint dwDistance = 300)
		{
			uint dwCloseItemID = 0;
			uint dwCloseItemDistance = dwDistance * dwDistance;
			TGroundItemInstanceMap.iterator LaniatusDefVariables = new TGroundItemInstanceMap.iterator();

			for (i = m_GroundItemInstanceMap.begin(); LaniatusDefVariables != m_GroundItemInstanceMap.end(); ++i)
			{
				TGroundItemInstance pInstance = i.second;
				uint dwxDistance = DISTANCE_APPROX(c_rPixelPosition.x, pInstance.v3EndPosition.x);
				uint dwyDistance = DISTANCE_APPROX(c_rPixelPosition.y, pInstance.v3EndPosition.y);
				uint dwDistance = dwxDistance * dwxDistance + dwyDistance * dwyDistance;
		...
		public bool GetCloseMoney(in TPixelPosition c_rPixelPosition, ref uint pdwItemID, uint dwDistance = 300)
		{
			uint dwCloseItemID = 0;
			uint dwCloseItemDistance = dwDistance * dwDistance;
			TGroundItemInstanceMap.iterator LaniatusDefVariables = new TGroundItemInstanceMap.iterator();
		...
		public uint GetVirtualNumberOfGroundItem(uint dwVID)
		{
			TGroundItemInstanceMap.iterator itor = m_GroundItemInstanceMap.find(dwVID);
		...
		public void BuildNoGradeNameData(int iType)
		{
		}

		public uint GetNoGradeNameDataCount()
		{
			return (uint)m_NoGradeNameItemData.size();
		}

		public CItemData GetNoGradeNameDataPtr(uint dwIndex)
		{
		...
		protected uint __Pick(in POINT c_rkPtMouse)
		{
			float fu;
			float fv;
			float ft;
		...
		protected uint __GetUseSoundType(in CItemData c_rkItemData)
		{
		...
		protected uint __GetDropSoundType(in CItemData c_rkItemData)
		{
			switch (c_rkItemData.GetType())
			{
				case CItemData.ITEM_TYPE_WEAPON:
		...
		protected SortedDictionary<uint, SGroundItemInstance > m_GroundItemInstanceMap = new SortedDictionary<uint, SGroundItemInstance >();
		protected CDynamicPool<SGroundItemInstance> m_GroundItemInstancePool = new CDynamicPool<SGroundItemInstance>();

		protected uint m_dwDropItemEffectID;
		protected uint m_dwPickedItemID;

		protected int m_nMouseX;
		protected int m_nMouseY;

		protected string[] m_astUseSoundFileName = Arrays.InitializeStringArrayWithDefaultInstances(USESOUND_NUM);

		protected List<CItemData> m_NoGradeNameItemData = new List<CItemData>();
