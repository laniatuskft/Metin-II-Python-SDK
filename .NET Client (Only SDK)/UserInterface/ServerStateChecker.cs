...
public class CServerStateChecker : CSingleton<CServerStateChecker>
{

		//#pragma pack(pop)

		public CServerStateChecker()
		{
			Initialize();
		}
		...
		public void Create(PyObject poWnd)
		{
			m_poWnd = poWnd;
		}

		public void AddChannel(uint uServerIndex, string c_szAddr, uint uPort)
		{
		...
		public void Request()
		{
			...
			{
				for (LinkedList<TChannel>.Enumerator it = m_lstChannel.begin(); it.MoveNext();)
				{
			...
			byte bHeader = LG_HEADER_CG_STATE_CHECKER;
			if (!m_kStream.Send(sizeof(byte), bHeader))
			{
				for (LinkedList<TChannel>.Enumerator it = m_lstChannel.begin(); it.MoveNext();)
				{
					PyCallClassMemberFunc(m_poWnd, "NotifyChannelState", Py_BuildValue("(ii)", it.uServerIndex, 0));
		...
		public void Update()
		{
			...
			byte bHeader;
			if (!m_kStream.Recv(sizeof(byte), bHeader))
			{
				return;
			}
			if (LG_HEADER_GC_RESPOND_CHANNELSTATUS != bHeader)
			{
				return;
			}
			int nSize;
			if (!m_kStream.Recv(sizeof(int), nSize))
			{
				return;
			}
		...
		{
			public uint uServerIndex;
			public readonly string c_szAddr;
			public uint uPort;
		}

		private PyObject m_poWnd;

		private LinkedList<SChannel> m_lstChannel = new LinkedList<SChannel>();

		private CNetworkStream m_kStream = new CNetworkStream();
}
