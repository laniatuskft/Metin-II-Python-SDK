...
public class CPythonEventManager : CSingleton<CPythonEventManager>
{
		public class STextLine
		{
			public int ixLocal;
			public int iyLocal;
			public CGraphicTextInstance pInstance;
		}



		public const int EVENT_POSITION_START = 0;
		public const int EVENT_POSITION_END = 1;
		public const int BOX_VISIBLE_LINE_COUNT = 5;
		...
		private SortedDictionary<string, int> EventTypeMap = new SortedDictionary<string, int>();
		...
		public class SEventSet
		{
			...
			public int lLastDelayTime;

			public int iCurrentLetter;

			public D3DXCOLOR CurrentColor = new D3DXCOLOR();
			public string strCurrentLine = "";

			public CGraphicTextInstance pCurrentTextLine;
			public TScriptTextLineList ScriptTextLineList = new TScriptTextLineList();

			public bool isConfirmWait;
			public CGraphicTextInstance pConfirmTimeTextLine;
			public int iConfirmEndTime;

			public script.Group ScriptGroup = new script.Group();
			public string szFileName = new string(new char[32 + 1]);

			public int iVisibleStartLine;
			public int iVisibleLineCount;

			public int iAdjustLine;

			public D3DXCOLOR DiffuseColor = new D3DXCOLOR();
			public int lWaitingTime;
			public int iRestrictedCharacterCount;

			public int nAnswer;

			public bool isTextCenterMode;
			public bool isWaitFlag;

			public PyObject poEventHandler;
			...
			public SEventSet()
			{
				...
				this.iyLocal = 0;
				this.isLock = 0;
				this.lLastDelayTime = 0;
				this.iCurrentLetter = 0;
				this.CurrentColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
				this.strCurrentLine = "";
				this.pCurrentTextLine = null;
				this.isConfirmWait = false;
				this.pConfirmTimeTextLine = null;
				this.iConfirmEndTime = 0;
				this.iVisibleStartLine = 0;
				this.iVisibleLineCount = 0;
				this.iAdjustLine = 0;
				this.DiffuseColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
				this.lWaitingTime = 0;
				this.iRestrictedCharacterCount = 0;
				this.nAnswer = 0;
				this.isTextCenterMode = 0;
				this.isWaitFlag = 0;
				this.poEventHandler = null;
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public CPythonEventManager()
		{
			this.m_poInterface = 0;
			this.m_strLeftTimeString = "???? ?ð? : %d??";
		...
		public int RegisterEventSet(string c_szFileName)
		{
			CMappedFile File = new CMappedFile();
			LPCVOID pMap = new LPCVOID();
		...
		public int RegisterEventSetFromString(in string strScript)
		{
			TEventSet pEventSet = m_EventSetPool.Alloc();
			if (pEventSet == null)
			{
			...
			if (pEventSet.ScriptGroup.ReadCmd(ScriptCommand))
			{
				if (GetScriptEventIndex(ScriptCommand.name.c_str(), pEventPosition, iEventType))
				{
					...
					{
						string strFileName = GetArgument("value", ScriptCommand.argList);

						__ClearEventSetp(pEventSet);

						return RegisterEventSet(strFileName);
					}
				}
			}
		...
		public void ClearEventSeti(int iIndex)
		{
...
//		void __ClearEventSetp(SEventSet pEventSet);

		public void SetEventHandler(int iIndex, PyObject poEventHandler)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void SetRestrictedCount(int iIndex, int iCount)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public int GetEventSetLocalYPosition(int iIndex)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return 0;
		...
		public void AddEventSetLocalYPosition(int iIndex, int iAddValue)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void InsertText(int iIndex, string c_szText, int iX_pos = 0)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void UpdateEventSet(int iIndex, int ix, int iy)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
			...
			{
				int iLeftTime = Math.Max(0, pEventSet.iConfirmEndTime - timeGetTime() / 1000);
			...
			int lElapsedTime = CTimer.Instance().GetElapsedMilliecond();

			pEventSet.lLastDelayTime = Math.Max(0, pEventSet.lLastDelayTime - lElapsedTime);
		...
		public void RenderEventSet(int iIndex)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
...
//Original Metin2 CPlus Line: TTextLine & rkLine = *itor;
		...
		public void SetEventSetWidth(int iIndex, int iWidth)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void EndEventProcess(int iIndex)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void SelectAnswer(int iIndex, int iAnswer)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void SetVisibleStartLine(int iIndex, int iStartLine)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
			...
			if ((uint)iStartLine > pEventSet.ScriptTextLineList.size())
			{
				return;
			}
		...
		public int GetVisibleStartLine(int iIndex)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return 0;
		...
		public int GetLineCount(int iIndex)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return 0;
		...
		public void SetVisibleLineCount(int iIndex, int iLineCount)
		{
			if (!CheckEventSetIndex(iIndex))
			{
				return;
		...
		public void SetInterfaceWindow(PyObject poInterface)
		{
			m_poInterface = poInterface;
		}

		public void SetLeftTimeString(string c_szString)
		{
			m_strLeftTimeString = c_szString;
		}

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __InitEventSet(SEventSet rEventSet);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __InsertLine(SEventSet rEventSet, bool isCenter = FALSE, int iX_pos = 0);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void __AddSpace(SEventSet pEventSet, int iSpace);

		protected uint GetEmptyEventSetSlot()
		{
			...
			return (uint)(m_EventSetVector.size() - 1);
		}

		protected bool CheckEventSetIndex(int iIndex)
		{
			if (iIndex < 0)
			{
				return false;
			...
			if ((uint) iIndex >= m_EventSetVector.size())
			{
				return false;
			}
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void ProcessEventSet(SEventSet pEventSet);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void ClearLine(SEventSet pEventSet);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RefreshLinePosition(SEventSet pEventSet);
		protected bool GetScriptEventIndex(string c_szName, ref int pEventPosition, ref int pEventType)
		{
			string c_szEventName;
			...
			SortedDictionary<string,int>.Enumerator it = EventTypeMap.find(c_szEventName);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void MakeQuestion(SEventSet pEventSet, System.Collections.Generic.LinkedList<SArgumet> rArgumentList);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void MakeNextButton(SEventSet pEventSet, int iButtonType);

		protected List<SEventSet> m_EventSetVector = new List<SEventSet>();
		protected bool m_isQuestConfirmWait;

		private CDynamicPool<SEventSet> m_EventSetPool = new CDynamicPool<SEventSet>();
		private CDynamicPool<CGraphicTextInstance> m_ScriptTextLinePool = new CDynamicPool<CGraphicTextInstance>();
		private PyObject m_poInterface;
		private string m_strLeftTimeString = "";
}
