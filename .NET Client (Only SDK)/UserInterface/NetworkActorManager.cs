...
public class SNetworkActorData
{
	...
	public uint m_dwServerSrcTime;
	public uint m_dwClientSrcTime;
	public uint m_dwDuration;

	public uint m_dwArmor;
	public uint m_dwWeapon;
	public uint m_dwHair;
	public uint m_dwAcce;
	public byte m_byAcceDrainRate;

	public uint m_dwOwnerVID;
	...
	public void SetDstPosition(uint dwServerTime, int lDstX, int lDstY, uint dwDuration)
	{
		m_lSrcX = m_lCurX;
		m_lSrcY = m_lCurY;
		m_lDstX = lDstX;
	...
	public void SetPosition(int lPosX, int lPosY)
	{
	...
	{
		uint dwClientCurTime = ELTimer_GetMSec();
		uint dwElapsedTime = dwClientCurTime - m_dwClientSrcTime;

		if (dwElapsedTime < m_dwDuration)
		{
			float fRate = (float)dwElapsedTime / (float)m_dwDuration;
			m_lCurX = (int)((m_lDstX - m_lSrcX) * fRate + m_lSrcX);
			m_lCurY = (int)((m_lDstY - m_lSrcY) * fRate + m_lSrcY);
		}
		else
		{
			m_lCurX = m_lDstX;
	...
	public SNetworkActorData(in SNetworkActorData src)
	{
		__copy__(src);
	}

//# Laniatus Games Studio Inc. |: This 'CopyFrom' method was converted from the original copy assignment operator:
//Original Metin2 CPlus Line: void operator =(const SNetworkActorData& src)
	public void CopyFrom(in SNetworkActorData src)
	{
		__copy__(src);
	}

	public void __copy__(in SNetworkActorData src)
	{
		m_bType = src.m_bType;
		m_dwVID = src.m_dwVID;
		...
		m_byAcceDrainRate = src.m_byAcceDrainRate;
...
public class SNetworkMoveActorData
{
	public uint m_dwVID;
	...
	public int m_lPosY;
	public float m_fRot;
	public uint m_dwFunc;
	public uint m_dwArg;
	public uint m_dwDuration;

	public SNetworkMoveActorData()
	{
		m_dwVID = 0;
		m_dwTime = 0;
		m_fRot = 0.0f;
...
public class SNetworkUpdateActorData
{
	public uint m_dwVID;
	public uint m_dwGuildID;
	public uint m_dwArmor;
	...
	public SNetworkUpdateActorData()
	{
		m_dwGuildID = 0;
		m_dwVID = 0;
		m_dwArmor = 0;
...
public class CNetworkActorManager : CReferenceObject
{
		public CNetworkActorManager()
		{
			m_dwMainVID = 0;
		}
		...
		public void SetMainActorVID(uint dwVID)
		{
			m_dwMainVID = dwVID;
			m_lMainPosX = 0;
			m_lMainPosY = 0;
		...
		public void RemoveActor(uint dwVID)
		{
			SortedDictionary<uint, SNetworkActorData>.Enumerator f = m_kNetActorDict.find(dwVID);
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public void AppendActor(in SNetworkActorData c_rkNetActorData)
		{
		...
		public void UpdateActor(in SNetworkUpdateActorData c_rkNetUpdateActorData)
		{
			SortedDictionary<uint, SNetworkActorData>.Enumerator f = m_kNetActorDict.find(c_rkNetUpdateActorData.m_dwVID);
			if (m_kNetActorDict.end() == f)
			{
#if DEBUG
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			if (pkInstFind != null)
			{
				...
				pkInstFind.SetAcceRefineEffect(c_rkNetUpdateActorData.m_byAcceDrainRate);
			...
			rkNetActorData.m_byAcceDrainRate = c_rkNetUpdateActorData.m_byAcceDrainRate;
		...
		public void MoveActor(in SNetworkMoveActorData c_rkNetMoveActorData)
		{
			SortedDictionary<uint, SNetworkActorData>.Enumerator f = m_kNetActorDict.find(c_rkNetMoveActorData.m_dwVID);
			if (m_kNetActorDict.end() == f)
			{
#if DEBUG
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
			...
			if (pkInstFind != null)
			{
				...
				pkInstFind.PushTCPState(c_rkNetMoveActorData.m_dwTime, kPPosDst, c_rkNetMoveActorData.m_fRot, c_rkNetMoveActorData.m_dwFunc, c_rkNetMoveActorData.m_dwArg);
			}

			rkNetActorData.SetDstPosition(c_rkNetMoveActorData.m_dwTime, c_rkNetMoveActorData.m_lPosX, c_rkNetMoveActorData.m_lPosY, c_rkNetMoveActorData.m_dwDuration);
			rkNetActorData.m_fRot = c_rkNetMoveActorData.m_fRot;
		}

		public void SyncActor(uint dwVID, int lPosX, int lPosY)
		{
			SortedDictionary<uint, SNetworkActorData>.Enumerator f = m_kNetActorDict.find(dwVID);
			if (m_kNetActorDict.end() == f)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		public void SetActorOwner(uint dwOwnerVID, uint dwVictimVID)
		{
			SortedDictionary<uint, SNetworkActorData>.Enumerator f = m_kNetActorDict.find(dwVictimVID);
			if (m_kNetActorDict.end() == f)
			{
#if DEBUG
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		protected void __OLD_Update()
		{
			__UpdateMainActor();

			CPythonCharacterManager rkChrMgr = __GetCharacterManager();

			SortedDictionary<uint, SNetworkActorData>.Enumerator i;
			for (i = m_kNetActorDict.begin(); i.MoveNext();)
			{
				SNetworkActorData rkNetActorData = i.Current.Value;
		...
		protected void __UpdateMainActor()
		{
			...
			m_lMainPosX = (int)kPPosMain.x;
			m_lMainPosY = (int)kPPosMain.y;
		}

		protected bool __IsVisiblePos(int lPosX, int lPosY)
		{
			 int dx = lPosX - m_lMainPosX;
			int dy = lPosY - m_lMainPosY;
			int len = (int)Math.Sqrt((double)(dx * dx + dy * dy));
...
//			extern int CHAR_STAGE_VIEW_BOUND;
		...
		protected bool __IsVisibleActor(in SNetworkActorData c_rkNetActorData)
		{
			if (__IsMainActorVID(c_rkNetActorData.m_dwVID))
			{
				return true;
		...
		protected bool __IsMainActorVID(uint dwVID)
		{
		...
		protected void __RemoveAllGroundItems()
		{
			CPythonItem rkItemMgr = CPythonItem.Instance();
			rkItemMgr.DeleteAllItems();
		}

		protected void __RemoveAllActors()
		{
			m_kNetActorDict.clear();
		...
		protected void __RemoveDynamicActors()
		{
			List<uint> dwCharacterVIDList = new List<uint>();
			...
			{
				dwCharacterVIDList.Add(i.GetVirtualID());
			}
...
//# Laniatus Games Studio Inc. | TODO TASK: Iterators are only converted within the context of 'while' and 'for' loops:
		...
		protected void __RemoveCharacterManagerActor(SNetworkActorData rkNetActorData)
		{
			CPythonCharacterManager rkChrMgr = CPythonCharacterManager.Instance();
...
//		SNetworkActorData __FindActorData(uint dwVID);

		protected CInstanceBase __AppendCharacterManagerActor(SNetworkActorData rkNetActorData)
		{
			Lognf(0, "CNetworkActorManager::__AppendCharacterManagerActor(%d, %s)", rkNetActorData.m_dwVID, rkNetActorData.m_stName.c_str());

			uint dwVID = rkNetActorData.m_dwVID;

			CPythonCharacterManager rkChrMgr = CPythonCharacterManager.Instance();


			CInstanceBase.SCreateData kCreateData = new CInstanceBase.SCreateData();
			...
			kCreateData.m_byAcceDrainRate = rkNetActorData.m_byAcceDrainRate;
			...
			{
				TPixelPosition oldPos = new TPixelPosition();
			...
			uint dwClientCurTime = ELTimer_GetMSec();
			uint dwElapsedTime = dwClientCurTime - rkNetActorData.m_dwClientSrcTime;
		...
		protected CInstanceBase __FindActor(SNetworkActorData rkNetActorData)
		{
			CPythonCharacterManager rkChrMgr = __GetCharacterManager();
			CInstanceBase pkInstFind = rkChrMgr.GetInstancePtr(rkNetActorData.m_dwVID);
			if (pkInstFind == null)
		...
		protected CInstanceBase __FindActor(SNetworkActorData rkNetActorData, int lDstX, int lDstY)
		{
			CPythonCharacterManager rkChrMgr = __GetCharacterManager();
			CInstanceBase pkInstFind = rkChrMgr.GetInstancePtr(rkNetActorData.m_dwVID);
			if (pkInstFind == null)
		...
		protected CPythonCharacterManager __GetCharacterManager()
		{
			return CPythonCharacterManager.Instance();
		}

		protected uint m_dwMainVID;

		protected int m_lMainPosX;
		protected int m_lMainPosY;

		protected SortedDictionary<uint, SNetworkActorData> m_kNetActorDict = new SortedDictionary<uint, SNetworkActorData>();
}