...
public class CAccountConnector : CNetworkStream, CSingleton<CAccountConnector>
{
		public const int STATE_OFFLINE = 0;
		public const int STATE_HANDSHAKE = 1;
		public const int STATE_AUTH = 2;

		public CAccountConnector()
		{
			m_poHandler = null;
			m_strAddr = "";
			m_iPort = 0;
		...
		public void SetLoginInfo(string c_szName, string c_szPwd)
		{
			m_strID = c_szName;
		...
		public bool Connect(string c_szAddr, int iPort, string c_szAccountAddr, int iAccountPort)
		{
			...
			__OfflineState_Set();
			__BuildClientKey_20210328_LGMyevan();
		...
		protected void __Inialize()
		{
			m_eState = STATE_OFFLINE;
			m_isWaitKey = false;
		}

		protected bool __StateProcess()
		{
			switch (m_eState)
			{
		...
		protected void __OfflineState_Set()
		{
			__Inialize();
		}

		protected void __HandshakeState_Set()
		{
			m_eState = STATE_HANDSHAKE;
		}

		protected void __AuthState_Set()
		{
			m_eState = STATE_AUTH;
		}

		protected bool __HandshakeState_Process()
		{
			if (!__AnalyzePacket(LG_HEADER_GC_PHASE, sizeof(packet_phase), CAccountConnector.__AuthState_RecvPhase))
			{
				return false;
			}

			if (!__AnalyzePacket(LG_HEADER_GC_HANDSHAKE, sizeof(packet_LG_HEADER_handshake), CAccountConnector.__AuthState_RecvHandshake))
			{
				return false;
			}

			if (!__AnalyzePacket(LG_HEADER_GC_PING, sizeof(packet_ping), CAccountConnector.__AuthState_RecvPing))
			{
				return false;
			}

#if _IMPROVED_PACKET_ENCRYPTION_
			if (!__AnalyzePacket(LG_HEADER_GC_KEY_AGREEMENT, sizeof(TPacketKeyAgreement), CAccountConnector.__AuthState_RecvKeyAgreement))
			{
				return false;
			}

			if (!__AnalyzePacket(LG_HEADER_GC_KEY_AGREEMENT_COMPLETED, sizeof(TPacketKeyAgreementCompleted), CAccountConnector.__AuthState_RecvKeyAgreementCompleted))
			{
				return false;
			}
#endif
		...
		protected bool __AuthState_Process()
		{
			...
			if (!__AnalyzePacket(LG_HEADER_GC_AUTH_SUCCESS, sizeof(packet_auth_success), CAccountConnector.__AuthState_RecvAuthSuccess))
			{
				return true;
			}

			if (!__AnalyzePacket(LG_HEADER_GC_LOGIN_FAILURE, sizeof(packet_auth_success), CAccountConnector.__AuthState_RecvAuthFailure))
			{
				return true;
			}
		...
		protected bool __AuthState_RecvEmpty()
		{
			byte byEmpty;
		...
		protected bool __AuthState_RecvPhase()
		{
			...
			else if (kPacketPhase.phase == PHASE_AUTH)
			{
				...
				command_login3 LoginPacket = new command_login3();
				LoginPacket.header = LG_HEADER_CG_LOGIN3;
				...
				strncpy(LoginPacket.clientVersion, DefineConstants.CLIENT_VERSION, sizeof(LoginPacket.clientVersion));
				...
				if (!Send(sizeof(command_login3), LoginPacket))
				{
		...
		protected bool __AuthState_RecvHandshake()
		{
...

			{
				...
				if (!Send(sizeof(packet_LG_HEADER_handshake), kPacketHandshake))
				{
		...
		protected bool __AuthState_RecvPing()
		{
		...
		protected bool __AuthState_SendPong()
		{
		...
		protected bool __AuthState_RecvAuthSuccess()
		{
			packet_auth_success kAuthSuccessPacket = new packet_auth_success();
			if (!Recv(sizeof(packet_auth_success), kAuthSuccessPacket))
			{
				return false;
			}
		...
		protected bool __AuthState_RecvAuthFailure()
		{
...
#if _IMPROVED_PACKET_ENCRYPTION_
#if _IMPROVED_PACKET_ENCRYPTION_
		protected bool __AuthState_RecvKeyAgreement()
		{
		...
		protected bool __AuthState_RecvKeyAgreementCompleted()
		{
...
#endif
		private delegate bool pfnDispatchPacketDelegate();

		protected bool __AnalyzePacket(uint uHeader, uint uPacketSize, pfnDispatchPacketDelegate pfnDispatchPacket)
		{
			byte bHeader;
			if (!Peek(sizeof(byte), bHeader))
			{
				return true;
			}
			...
			return pfnDispatchPacket();
		}

		private delegate bool pfnDispatchPacketDelegate(int UnnamedParameter);

		protected bool __AnalyzeVarSizePacket(uint uHeader, pfnDispatchPacketDelegate pfnDispatchPacket)
		{
			byte bHeader;
			if (!Peek(sizeof(byte), bHeader))
			{
			...
			packet_LG_HEADER_dynamic_size dynamicHeader = new packet_LG_HEADER_dynamic_size();

			if (!Peek(sizeof(packet_LG_HEADER_dynamic_size), dynamicHeader))
			{
				return true;
			}
			...
			return pfnDispatchPacket(dynamicHeader.size);
		}

#if ! _IMPROVED_PACKET_ENCRYPTION_
#if ! _IMPROVED_PACKET_ENCRYPTION_
		protected void __BuildClientKey()
		{
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < 4; ++i)
			{
...
//Original Metin2 CPlus Line: const byte * c_pszKey = (const byte *) "JyTxtHljHJlVJHorRM301vf@4fvj10-v";
			byte c_pszKey = (byte) "JyTxtHljHJlVJHorRM301vf@4fvj10-v";
			tea_encrypt((uint) g_adwDecryptKey, (uint) g_adwEncryptKey, (uint) c_pszKey, 16);
		}
#endif

#endif

		protected uint m_eState;
		protected string m_strID = "";
		protected string m_strPassword = "";

		protected string m_strAddr = "";
		protected int m_iPort;
		protected bool m_isWaitKey;
...
//		void __BuildClientKey_20210328_LGMyevan();
}