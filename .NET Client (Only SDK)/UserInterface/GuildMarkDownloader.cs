...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ILAPIENTRY __stdcall
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ILAPI __stdcall
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ILAPI __declspec(dllexport)
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ILAPI __declspec(dllimport)
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ILAPI extern
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ilClearColor ilClearColour
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define ilKeyColor ilKeyColour

//# Laniatus Games Studio Inc. | TODO TASK: Multiple inheritance is not available in C#:
public class CGuildMarkDownloader : CNetworkStream, CSingleton<CGuildMarkDownloader>
{
		public CGuildMarkDownloader()
		{
		...
		public bool Connect(in CNetworkAddress c_rkNetAddr, uint dwHandle, uint dwRandomKey)
		{
			__OfflineState_Set();
		...
		public bool ConnectToRecvSymbol(in CNetworkAddress c_rkNetAddr, uint dwHandle, uint dwRandomKey, in List<uint> c_rkVec_dwGuildID)
		{
			__OfflineState_Set();
		...
		private const int STATE_OFFLINE = 0;
		private const int STATE_LOGIN = 1;
		private const int STATE_COMPLETE = 2;

		private const int TODO_RECV_NONE = 0;
		private const int TODO_RECV_MARK = 1;
		private const int TODO_RECV_SYMBOL = 2;
		...
		private uint __GetPacketSize(uint header)
		{
			switch (header)
			{
				case LG_HEADER_GC_PHASE:
					return sizeof(packet_phase);
				case LG_HEADER_GC_HANDSHAKE:
					return sizeof(packet_LG_HEADER_handshake);
				case LG_HEADER_GC_PING:
					return sizeof(packet_ping);
				case LG_HEADER_GC_MARK_IDXLIST:
					return sizeof(packet_mark_idxlist);
				case LG_HEADER_GC_MARK_BLOCK:
					return sizeof(packet_mark_block);
				case LG_HEADER_GC_GUILD_SYMBOL_DATA:
					return sizeof(packet_symbol_data);
				case LG_HEADER_GC_MARK_DIFF_DATA:
					return sizeof(byte);
...
break;
		...
		private bool __DispatchPacket(uint header)
		{
			switch (header)
			{
				case LG_HEADER_GC_PHASE:
...
break;
		...
		private void __OfflineState_Set()
		{
			__Initialize();
		}

		private void __CompleteState_Set()
		{
			m_eState = STATE_COMPLETE;
			CPythonCharacterManager.instance().RefreshAllGuildMark();
		}

		private void __LoginState_Set()
		{
			m_eState = STATE_LOGIN;
		}

		private bool __LoginState_Process()
		{
			byte header;
		...
		private bool __LoginState_RecvPhase()
		{
			packet_phase kPacketPhase = new packet_phase();

			if (!Recv(sizeof(packet_phase), kPacketPhase))
			{
				return false;
			}
			...
			if (kPacketPhase.phase == PHASE_LOGIN)
			{
				...
				switch (m_dwTodo)
				{
					case TODO_RECV_NONE:
					{
						Debug.Assert(!"CGuildMarkDownloader::__LoginState_RecvPhase - Todo type is none");
						break;
					}
					case TODO_RECV_MARK:
					{
		...
		private bool __LoginState_RecvHandshake()
		{
			packet_LG_HEADER_handshake kPacketHandshake = new packet_LG_HEADER_handshake();
			if (!Recv(sizeof(packet_LG_HEADER_handshake), kPacketHandshake))
			{
				return false;
			}

			command_mark_login kPacketMarkLogin = new command_mark_login();

			kPacketMarkLogin.header = LG_HEADER_CG_MARK_LOGIN;
			kPacketMarkLogin.handle = m_dwHandle;
			kPacketMarkLogin.random_key = m_dwRandomKey;

			if (!Send(sizeof(command_mark_login), kPacketMarkLogin))
			{
				return false;
			}
		...
		private bool __LoginState_RecvPing()
		{
		...
		private bool __LoginState_RecvMarkIndex()
		{
			packet_mark_idxlist kPacketMarkIndex = new packet_mark_idxlist();

			if (!Peek(sizeof(packet_mark_idxlist), kPacketMarkIndex))
			{
				return false;
			}
			...
			Recv(sizeof(packet_mark_idxlist));

			ushort guildID;
			ushort markID;

			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < kPacketMarkIndex.count; ++i)
			{
				Recv(sizeof(ushort), guildID);
				Recv(sizeof(ushort), markID);
		...
		private bool __LoginState_RecvMarkBlock()
		{
			packet_mark_block kPacket = new packet_mark_block();

			if (!Peek(sizeof(packet_mark_block), kPacket))
			{
				return false;
			}
			...
			Recv(sizeof(packet_mark_block));

			byte posBlock;
			uint compSize;
			string compBuf = new string(new char[SGuildMarkBlock.MAX_COMP_SIZE]);

			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < kPacket.count; ++i)
			{
				Recv(sizeof(byte), posBlock);
				Recv(sizeof(uint), compSize);
				...
				else
				{
					...
					CGuildMarkManager.Instance().SaveBlockFromCompressedData(kPacket.imgIdx, posBlock, (byte) compBuf, compSize);
				}
			}
			...
			if (kPacket.count > 0)
			{
				...
				{
					CResource pResource = CResourceManager.Instance().GetResourcePointer(imagePath);
					if (pResource.IsType(CGraphicImage.Type()))
					{
						CGraphicImage pkGrpImg = (CGraphicImage)pResource;
						pkGrpImg.Reload();
					}
				}
			}
		...
		private bool __LoginState_RecvSymbolData()
		{
			packet_blank_dynamic packet = new packet_blank_dynamic();
			if (!Peek(sizeof(packet_blank_dynamic), packet))
			{
				return true;
			}
			...
			packet_symbol_data kPacketSymbolData = new packet_symbol_data();
			if (!Recv(sizeof(packet_symbol_data), kPacketSymbolData))
			{
				return false;
			}

			ushort wDataSize = kPacketSymbolData.size - sizeof(packet_symbol_data);
			uint dwGuildID = kPacketSymbolData.guild_id;
			...
			string strFileName = GetGuildSymbolFileName(dwGuildID);
...
#if _IMPROVED_PACKET_ENCRYPTION_
#if _IMPROVED_PACKET_ENCRYPTION_
		private bool __LoginState_RecvKeyAgreement()
		{
		...
		private bool __LoginState_RecvKeyAgreementCompleted()
		{
...
#endif
		private bool __SendMarkIDXList()
		{
			command_mark_idxlist kPacketMarkIDXList = new command_mark_idxlist();
			kPacketMarkIDXList.header = LG_HEADER_CG_MARK_IDXLIST;
			if (!Send(sizeof(command_mark_idxlist), kPacketMarkIDXList))
			{
				return false;
			}
		...
		private bool __SendMarkCRCList()
		{
			command_mark_crclist kPacketMarkCRCList = new command_mark_crclist();
			...
			{
				kPacketMarkCRCList.header = LG_HEADER_CG_MARK_CRCLIST;
				kPacketMarkCRCList.imgIdx = m_currentRequestingImageIndex;
				++m_currentRequestingImageIndex;

				if (!Send(sizeof(command_mark_crclist), kPacketMarkCRCList))
				{
					return false;
				}
			}
		...
		private bool __SendSymbolCRCList()
		{
			for (uint LaniatusDefVariables = 0; LaniatusDefVariables < m_kVec_dwGuildID.size(); ++i)
			{
		...
		private uint m_dwHandle;
		private uint m_dwRandomKey;
		private uint m_dwTodo;

		private List<uint> m_kVec_dwGuildID = new List<uint>();

		private uint m_eState;

		private byte m_currentRequestingImageIndex;

		private CGuildMarkManager m_pkMarkMgr;

		private uint m_dwBlockIndex;
		private uint m_dwBlockDataSize;
		private uint m_dwBlockDataPos;
}
...
#define D3DXAssembleShaderFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXGetImageInfoFromFile
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXGetImageInfoFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXLoadSurfaceFromFile
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXLoadVolumeFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateCubeTextureFromFile
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateVolumeTextureFromFile
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateTextureFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateCubeTextureFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateVolumeTextureFromResource
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateTextureFromFileEx
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateVolumeTextureFromFileEx
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateTextureFromResourceEx
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXFilterCubeTexture
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define D3DXFilterVolumeTexture D3DXFilterTexture
#define D3DXFilterVolumeTexture
#define DXFILEFORMAT_BINARY
#define DXFILEFORMAT_TEXT
#define DXFILEFORMAT_COMPRESSED
...
#define DXFILEERR_INTERNALERROR
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define D3DXCreateEffectFromFile
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GRANNY_FORWARD_DECL_STRUCT
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GRANNY_FORWARD_DECL_OPAQUE_STRUCT(x) GRANNY_NAMESPACE_BEGIN; struct x; GRANNY_NAMESPACE_END; typedef struct GRANNY x granny_ ## x
   #define GRANNY_FORWARD_DECL_OPAQUE_STRUCT
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannyTypeSizeCheck
#define GrannyInfiniteFarClipPlane
#define GrannyLCD17PhysicalAspectRatio
#define GrannyNTSCTelevisionPhysicalAspectRatio
#define GrannyVerticalIphoneAspectRatio
#define GrannyPALTelevisionPhysicalAspectRatio
#define GrannyWidescreenMonitorPhysicalAspectRatio
#define GrannyEuropeanFilmAspectRatio
#define GrannyUSDigitalTelevisionAspectRatio
#define GrannyUSAcademyFlatPhysicalAspectRatio
#define GrannyUSPanavisionAspectRatio
#define GrannyUSAnamorphicScopePhysicalAspectRatio
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyCurrentGRNStandardTag (0x80000000 + 57)
#define GrannyCurrentGRNStandardTag
#define GrannyCurrentGRNFileVersion
#define GrannyGRNExtraTagCount
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannySeekWriterFromStart
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannySeekWriterFromEnd
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannySeekWriterFromCurrentPosition
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannyWriteBytes
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyBeginWriterCRC(Writer) (*((Writer)->BeginCRCCallback))(Writer)
#define GrannyBeginWriterCRC
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyEndWriterCRC(Writer) (*((Writer)->EndCRCCallback))(Writer)
#define GrannyEndWriterCRC
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyWriterIsCRCing(Writer) (((Writer)->CRCing))
#define GrannyWriterIsCRCing
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumSystemFileNameSize (1 << 9)
#define GrannyMaximumSystemFileNameSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumTempFileNameSize (1 << 9)
#define GrannyMaximumTempFileNameSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumMessageBufferSize (1 << 10)
#define GrannyMaximumMessageBufferSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumLogFileNameSize (1 << 9)
#define GrannyMaximumLogFileNameSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumNumberToStringBuffer (1 << 8)
#define GrannyMaximumNumberToStringBuffer
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumIKLinkCount (1 << 5)
#define GrannyMaximumIKLinkCount
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumMIPLevelCount (1 << 5)
#define GrannyMaximumMIPLevelCount
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumVertexNameLength (1 << 6)
#define GrannyMaximumVertexNameLength
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumUserData (1 << 2)
#define GrannyMaximumUserData
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumBoneNameLength (1 << 9)
#define GrannyMaximumBoneNameLength
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumChannelCount (1 << 8)
#define GrannyMaximumChannelCount
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumChannelWidth (1 << 2)
#define GrannyMaximumChannelWidth
#define GrannyMaximumBSplineDimension
#define GrannyMaximumBSplineKnotPower
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMaximumPolygonSides (1 << 7)
#define GrannyMaximumPolygonSides
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyBlockFileFixupCount (1 << 8)
#define GrannyBlockFileFixupCount
#define GrannyExpectedUsablePageSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyMinimumAllocationsPerFixedBlock (1 << 6)
#define GrannyMinimumAllocationsPerFixedBlock
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyFileCopyBufferSize (1 << 16)
#define GrannyFileCopyBufferSize
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyCRCCheckBufferSize (1 << 16)
#define GrannyCRCCheckBufferSize
#define GrannyDefaultFileDiskAlignment
#define GrannyTrackWeightEpsilon
#define GrannyPositionIdentityThreshold
#define GrannyOrientationIdentityThreshold
#define GrannyScaleShearIdentityThreshold
#define GrannyBlendEffectivelyZero
#define GrannyBlendEffectivelyOne
#define GrannyTimeEffectivelyZero
#define GrannyMaximumSectionCount
#define GrannyDefaultLocalPoseFillThreshold
#define GrannyNoSparseBone
#define GrannyDefaultAllocationAlignment
#define GrannyMatrixBufferAlignment
#define GrannyLocalPoseAlignment
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyCurrentMODStandardTag ((GrannyCurrentGRNStandardTag) | ((MOD_VERSION_TAG) << 15))
#define GrannyCurrentMODStandardTag
#define GrannyNoParentBone
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannySPUTransformTrackNoCurve (0xffffffff)
#define GrannySPUTransformTrackNoCurve
#define GrannyTopologyNullIndex
#define GrannyProductVersion
#define GrannyProductMajorVersion
#define GrannyProductMinorVersion
#define GrannyProductBuildNumber
#define GrannyProductCustomization
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyProductReleaseName release
#define GrannyProductReleaseName
#define GrannyProductCompanyName
#define GrannyProductName
#define GrannyProductCopyright
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define GrannyProductTrademarks ProductName " is a registered trademark of " ProductCompanyName
#define GrannyProductTrademarks
#define GrannyProductComments
#define GrannyProductSupportAddress
#define GrannyProductSupportPage
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define GrannyMaximumWeightCount
#define GrannyVertexPositionName
#define GrannyVertexNormalName
#define GrannyVertexTangentName
#define GrannyVertexBinormalName
#define GrannyVertexTangentBinormalCrossName
#define GrannyVertexBoneWeightsName
#define GrannyVertexBoneIndicesName
#define GrannyVertexDiffuseColorName
#define GrannyVertexSpecularColorName
#define GrannyVertexTextureCoordinatesName
#define GrannyVertexMorphCurvePrefix
#define GRANNY_H
#define MAX_MIPLEVELS
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
//Original Metin2 CPlus Line: #define COMBO_KEY_GET_COMBO_TYPE(key) ( WORD(DWORD(key) & 0xFFFF) )
#define COMBO_KEY_GET_COMBO_TYPE
#define LANIATUS_HARDWARE_LICENSE_DEMO
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
//Original Metin2 CPlus Line: #define __pascal pascal
#define __pascal
#define LZO_BROKEN_CDECL_ALT_SYNTAX
...
#define LZO_INFO_OS
#define LZO_INFO_OS
#define LZO_OS_EMBEDDED
#define LZO_INFO_OS
#define LZO_OS_CYGWIN
#define LZO_INFO_OS
#define LZO_OS_EMX
#define LZO_INFO_OS
#define LZO_OS_BEOS
#define LZO_INFO_OS
#define LZO_OS_LYNXOS
#define LZO_INFO_OS
#define LZO_OS_OS400
#define LZO_INFO_OS
#define LZO_OS_QNX
...
#define LZO_OS_POSIX
#define LZO_INFO_OS
#define LZO_OS_PALMOS
#define LZO_INFO_OS
#define LZO_OS_TOS
#define LZO_INFO_OS
#define LZO_OS_MACCLASSIC
#define LZO_INFO_OS
#define LZO_OS_VMS
#define LZO_INFO_OS
#define LZO_OS_CONSOLE
#define LZO_OS_CONSOLE_PS2
#define LZO_INFO_OS
#define LZO_INFO_OS_CONSOLE
#define LZO_OS_CONSOLE
#define LZO_OS_CONSOLE_PSP
#define LZO_INFO_OS
#define LZO_INFO_OS_CONSOLE
#define LZO_OS_POSIX
#define LZO_INFO_OS
#define LZO_OS_POSIX_AIX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_FREEBSD
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_HPUX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_INTERIX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_IRIX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_LINUX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_MACOSX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_MINIX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_NETBSD
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_OPENBSD
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_OSF
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_SOLARIS
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_SUNOS
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_ULTRIX
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_UNICOS
#define LZO_INFO_OS_POSIX
#define LZO_OS_POSIX_UNKNOWN
#define LZO_INFO_OS_POSIX
#define LZO_CC_CILLY
#define LZO_INFO_CC
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
...
#define LZO_INFO_CCVER
#define LZO_CC_UNKNOWN
#define LZO_INFO_CC
#define LZO_INFO_CCVER
#define LZO_ARCH_CRAY_MPP
#define LZO_ARCH_CRAY_PVP
#define LZO_INFO_ARCH
#define LZO_ARCH_I086
#define LZO_ARCH_IA16
#define LZO_INFO_ARCH
#define LZO_ARCH_ALPHA
#define LZO_INFO_ARCH
#define LZO_ARCH_ALPHA
#define LZO_INFO_ARCH
#define LZO_ARCH_AMD64
#define LZO_INFO_ARCH
#define LZO_ARCH_ARM
#define LZO_ARCH_ARM_THUMB
#define LZO_INFO_ARCH
#define LZO_ARCH_ARM
#define LZO_ARCH_ARM_THUMB
#define LZO_INFO_ARCH
...
#define LZO_ARCH_AVR
#define LZO_INFO_ARCH
#define LZO_ARCH_BLACKFIN
#define LZO_INFO_ARCH
#define LZO_ARCH_C166
#define LZO_INFO_ARCH
#define LZO_ARCH_CRIS
#define LZO_INFO_ARCH
#define LZO_ARCH_EZ80
#define LZO_INFO_ARCH
#define LZO_ARCH_H8300
#define LZO_INFO_ARCH
#define LZO_ARCH_HPPA
#define LZO_INFO_ARCH
#define LZO_ARCH_I386
#define LZO_ARCH_IA32
#define LZO_INFO_ARCH
#define LZO_ARCH_I386
#define LZO_ARCH_IA32
#define LZO_INFO_ARCH
...
#define LZO_INFO_ARCH
#define LZO_ARCH_M16C
#define LZO_INFO_ARCH
#define LZO_ARCH_M16C
#define LZO_INFO_ARCH
#define LZO_ARCH_M32R
#define LZO_INFO_ARCH
#define LZO_ARCH_M68K
#define LZO_INFO_ARCH
#define LZO_ARCH_MCS251
#define LZO_INFO_ARCH
#define LZO_ARCH_MCS51
#define LZO_INFO_ARCH
#define LZO_ARCH_MCS51
#define LZO_INFO_ARCH
#define LZO_ARCH_MIPS
#define LZO_INFO_ARCH
#define LZO_ARCH_MSP430
#define LZO_INFO_ARCH
#define LZO_ARCH_MSP430
#define LZO_INFO_ARCH
#define LZO_ARCH_POWERPC
#define LZO_INFO_ARCH
#define LZO_ARCH_S390
#define LZO_INFO_ARCH
#define LZO_ARCH_SH
#define LZO_INFO_ARCH
#define LZO_ARCH_SPARC
#define LZO_INFO_ARCH
#define LZO_ARCH_SPU
#define LZO_INFO_ARCH
#define LZO_ARCH_Z80
#define LZO_INFO_ARCH
#define LZO_ARCH_CRAY_SV1
#define LZO_INFO_ARCH
#define LZO_ARCH_CRAY_T90
#define LZO_INFO_ARCH
#define LZO_ARCH_CRAY_YMP
#define LZO_INFO_ARCH
#define LZO_ARCH_CRAY_XMP
#define LZO_INFO_ARCH
#define LZO_ARCH_UNKNOWN
#define LZO_INFO_ARCH
#define LZO_ARCH_I086PM
...
#define LZO_MM_HUGE
#define LZO_MM_SMALL
#define LZO_MM_MEDIUM
#define LZO_MM_COMPACT
#define LZO_MM_LARGE
#define LZO_MM_LARGE
#define LZO_MM_MEDIUM
#define LZO_MM_COMPACT
#define LZO_MM_SMALL
#define LZO_MM_LARGE
#define LZO_HAVE_MM_HUGE_PTR
#define LZO_HAVE_MM_HUGE_ARRAY
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define LZO_MM_AHSHIFT ((unsigned) _AHSHIFT)
#define LZO_MM_AHSHIFT
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define LZO_MM_AHSHIFT ((unsigned) _AHSHIFT)
#define LZO_MM_AHSHIFT
...
#define LZO_SIZEOF_PTRDIFF_T
#define LZO_ABI_BIG_ENDIAN
#define LZO_ABI_LITTLE_ENDIAN
#define LZO_ABI_BIG_ENDIAN
#define LZO_ABI_LITTLE_ENDIAN
#define LZO_ABI_BIG_ENDIAN
#define LZO_ABI_BIG_ENDIAN
...
#define LZO_ABI_BIG_ENDIAN
#define LZO_ABI_LITTLE_ENDIAN
#define LZO_INFO_ABI_ENDIAN
#define LZO_INFO_ABI_ENDIAN
#define LZO_INFO_ABI_ENDIAN
#define LZO_ABI_I8LP16
#define LZO_INFO_ABI_PM
#define LZO_ABI_ILP16
#define LZO_INFO_ABI_PM
#define LZO_ABI_ILP32
#define LZO_INFO_ABI_PM
#define LZO_ABI_LLP64
#define LZO_INFO_ABI_PM
#define LZO_ABI_LP64
#define LZO_INFO_ABI_PM
#define LZO_ABI_ILP64
#define LZO_INFO_ABI_PM
#define LZO_ABI_IP32L64
#define LZO_INFO_ABI_PM
#define LZO_INFO_LIBC
#define LZO_INFO_LIBC
#define LZO_INFO_LIBC
#define LZO_INFO_LIBC
#define LZO_INFO_LIBC
...
#define LZO_LIBC_MSL
#define LZO_INFO_LIBC
#define LZO_LIBC_ISOC90
#define LZO_INFO_LIBC
#define LZO_LIBC_DEFAULT
#define LZO_INFO_LIBC
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define __lzo_alignof
#define __lzo_HAVE_alignof
#define __lzo_constructor
#define __lzo_constructor
#define __lzo_constructor
#define __lzo_HAVE_constructor
#define __lzo_destructor
#define __lzo_destructor
#define __lzo_destructor
#define __lzo_HAVE_destructor
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
//Original Metin2 CPlus Line: #define __lzo_unlikely(e) (__builtin_expect(!!(e),0))
#define __lzo_unlikely
#define __lzo_HAVE_likely
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define __lzo_likely(e) (e)
#define __lzo_likely
#define __lzo_HAVE_unlikely
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define LZO_DEFINE_UNINITIALIZED_VAR
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define __lzo_cdecl_va
#define LZO_HAVE_WINDOWS_H
#define LZO_OPT_AVOID_UINT_INDEX
#define LZO_OPT_AVOID_SHORT
#define LZO_OPT_AVOID_USHORT
#define LZO_OPT_AVOID_INT_INDEX
#define LZO_OPT_AVOID_UINT_INDEX
#define LZO_OPT_UNALIGNED16
#define LZO_OPT_UNALIGNED32
#define LZO_OPT_UNALIGNED64
#define LZO_OPT_AVOID_SHORT
#define LZO_OPT_AVOID_USHORT
#define LZO_OPT_UNALIGNED16
#define LZO_OPT_UNALIGNED32
...
#define LZO_OPT_PREFER_POSTINC
#define LZO_OPT_PREFER_PREDEC
#define LZO_OPT_UNALIGNED16
#define LZO_OPT_UNALIGNED32
#define LZO_OPT_AVOID_UINT_INDEX
#define LZO_OPT_PREFER_PREINC
#define LZO_OPT_PREFER_PREDEC
#define LZO_OPT_UNALIGNED16
#define LZO_OPT_UNALIGNED32
#define LZO_OPT_UNALIGNED16
...
#define LZO_OPT_PREFER_POSTINC
#define LZO_OPT_PREFER_PREDEC
#define LZO_CFG_NO_INLINE_ASM
#define LZO_CFG_NO_UNALIGNED
#define LZO_ASM_SYNTAX_MSC
#define LZO_ASM_SYNTAX_GNUC
#define LZO_ASM_SYNTAX_GNUC
#define __LZO_ASM_CLOBBER
#define __LZO_ASM_CLOBBER
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define lzo_callback_p
#define LZO_E_OK
#define LZO_E_ERROR
#define LZO_E_OUT_OF_MEMORY
#define LZO_E_NOT_COMPRESSIBLE
#define LZO_E_INPUT_OVERRUN
#define LZO_E_OUTPUT_OVERRUN
#define LZO_E_LOOKBEHIND_OVERRUN
#define LZO_E_EOF_NOT_FOUND
#define LZO_E_INPUT_NOT_CONSUMED
#define LZO_E_NOT_YET_IMPLEMENTED
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
...
#define STATEMANAGER
#define SKILL_MAX_NUM

public class SMarkIndex
{
	public ushort guild_id;
	public ushort mark_id;
}
