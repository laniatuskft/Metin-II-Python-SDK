...
public enum EShopCoinType
{
	SHOP_COIN_TYPE_GOLD,
	SHOP_COIN_TYPE_SECONDARY_COIN
}

public class CPythonShop : CSingleton<CPythonShop>
{
		public CPythonShop()
		{
			Clear();
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public void SetItemData(uint dwIndex, in TShopItemData c_rShopItemData)
		{
			byte tabIdx = dwIndex / SHOP_HOST_ITEM_MAX_NUM;
			uint dwSlotPos = dwIndex % SHOP_HOST_ITEM_MAX_NUM;

			SetItemData(tabIdx, dwSlotPos, c_rShopItemData);
		}

		public bool GetItemData(uint dwIndex, TShopItemData[] c_ppItemData)
		{
			byte tabIdx = dwIndex / SHOP_HOST_ITEM_MAX_NUM;
			uint dwSlotPos = dwIndex % SHOP_HOST_ITEM_MAX_NUM;
		...
		public void SetItemData(byte tabIdx, uint dwSlotPos, in TShopItemData c_rShopItemData)
		{
		...
		public bool GetItemData(byte tabIdx, uint dwSlotPos, TShopItemData[] c_ppItemData)
		{
			if (tabIdx >= SHOP_TAB_COUNT_MAX || dwSlotPos >= SHOP_HOST_ITEM_MAX_NUM)
			{
				TraceError("Out of Index. tabIdx(%d) must be less than %d. dwSlotPos(%d) must be less than %d", tabIdx, SHOP_TAB_COUNT_MAX, dwSlotPos, SHOP_HOST_ITEM_MAX_NUM);
			...
			c_ppItemData[0] = &m_aShoptabs[tabIdx].items[dwSlotPos];

			return true;
		}

		public void SetTabCount(byte bTabCount)
		{
			m_bTabCount = bTabCount;
		}
		public byte GetTabCount()
		{
			return m_bTabCount;
		}
		...
		public void SetTabCoinType(byte tabIdx, byte coinType)
		{
		...
		public byte GetTabCoinType(byte tabIdx)
		{
			if (tabIdx >= m_bTabCount)
			{
				TraceError("Out of Index. tabIdx(%d) must be less than %d.", tabIdx, SHOP_TAB_COUNT_MAX);
		...
		public void SetTabName(byte tabIdx, string name)
		{
			if (tabIdx >= m_bTabCount)
			{
				TraceError("Out of Index. tabIdx(%d) must be less than %d.", tabIdx, SHOP_TAB_COUNT_MAX);
		...
		public string GetTabName(byte tabIdx)
		{
			if (tabIdx >= m_bTabCount)
			{
				TraceError("Out of Index. tabIdx(%d) must be less than %d.", tabIdx, SHOP_TAB_COUNT_MAX);
		...
		public void Open(bool isPrivateShop, bool isMainPrivateShop)
		{
			m_isShoping = true;
		...
		public bool IsMainPlayerPrivateShop()
		{
			return m_isMainPlayerPrivateShop;
		}

		public void ClearPrivateShopStock()
		{
			m_PrivateShopItemStock.clear();
		}

		public void AddPrivateShopItemStock(TItemPos ItemPos, byte dwDisplayPos, long lldPrice)
		{
			DelPrivateShopItemStock(ItemPos);

			SShopItemTable SellingItem = new SShopItemTable();
			SellingItem.vnum = 0;
			SellingItem.count = 0;
			SellingItem.pos = ItemPos;
			SellingItem.price = lldPrice;
			SellingItem.display_pos = dwDisplayPos;
			m_PrivateShopItemStock.insert(Tuple.Create(ItemPos, SellingItem));
		}

		public void DelPrivateShopItemStock(TItemPos ItemPos)
		{
		...
		public long GetPrivateShopItemPrice(TItemPos ItemPos)
		{
			...
			SShopItemTable rShopItemTable = itor.second;
			return rShopItemTable.price;
		}

		public void BuildPrivateShop(string c_szName)
		{
			List<SShopItemTable> ItemStock = new List<SShopItemTable>();
			ItemStock.Capacity = m_PrivateShopItemStock.size();

			TPrivateShopItemStock.iterator itor = m_PrivateShopItemStock.begin();
			for (; itor != m_PrivateShopItemStock.end(); ++itor)
			{
				ItemStock.Add(itor.second);
			}
...
//Original Metin2 CPlus Line: std::sort(ItemStock.begin(), ItemStock.end(), ItemStockSortFunc());
			ItemStock.Sort(ItemStockSortFunc());
...
//		bool CheckSlotIndex(uint dwIndex);

		protected bool m_isShoping;
		protected bool m_isPrivateShop;
		protected bool m_isMainPlayerPrivateShop;

		protected class ShopTab
		{
			public ShopTab()
			{
			...
			public string name = "";
			public TShopItemData[] items = Arrays.InitializeWithDefaultInstances<TShopItemData>(SHOP_HOST_ITEM_MAX_NUM);
		}

		protected byte m_bTabCount;
		protected ShopTab[] m_aShoptabs = Arrays.InitializeWithDefaultInstances<ShopTab>(SHOP_TAB_COUNT_MAX);

		protected SortedDictionary<TItemPos, SShopItemTable> m_PrivateShopItemStock = new SortedDictionary<TItemPos, SShopItemTable>();
}
...
public class ItemStockSortFunc
{
	public bool functorMethod(SShopItemTable rkLeft, SShopItemTable rkRight)
	{
