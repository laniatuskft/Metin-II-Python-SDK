...
public class CPythonExchange : CSingleton<CPythonExchange>
{
		public const int EXCHANGE_ITEM_MAX_NUM = 12;

		public class trade
		{
			public string name = new string(new char[DefineConstants.CHARACTER_NAME_MAX_LEN + 1]);
			public uint level;

			public uint[] item_vnum = new uint[EXCHANGE_ITEM_MAX_NUM];
			public ushort[] item_count = new ushort[EXCHANGE_ITEM_MAX_NUM];
			public uint[][] item_metin = RectangularArrays.RectangularUintArray(EXCHANGE_ITEM_MAX_NUM, ITEM_SOCKET_SLOT_MAX_NUM);
			public TPlayerItemAttribute[][] item_attr = RectangularArrays.RectangularTPlayerItemAttributeArray(EXCHANGE_ITEM_MAX_NUM, ITEM_ATTRIBUTE_SLOT_MAX_NUM);

			public byte accept;
			public long elk;
		}

		public CPythonExchange()
		{
			Clear();
			m_isTrading = false;
			m_elk_mode = false;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		{
			strncpy(m_self.name, name, DefineConstants.CHARACTER_NAME_MAX_LEN);
		}

		public void SetTargetName(string name)
		{
			strncpy(m_victim.name, name, DefineConstants.CHARACTER_NAME_MAX_LEN);
		}

		public string GetNameFromSelf()
		{
			return m_self.name;
		}

		public string GetNameFromTarget()
		{
			return m_victim.name;
		}

		public void SetTargetLevel(uint dwLevel)
		{
			m_victim.level = dwLevel;
		}

		public uint GetLevelFromTarget()
		{
			return m_victim.level;
		}

		public void SetElkToTarget(long elk)
		{
			m_victim.elk = elk;
		}

		public void SetElkToSelf(long elk)
		{
			m_self.elk = elk;
		}

		public long GetElkFromTarget()
		{
			return m_victim.elk;
		}

		public long GetElkFromSelf()
		{
			return m_self.elk;
		}

		public void SetItemToTarget(uint pos, uint vnum, ushort count)
		{
		...
		public void SetItemToSelf(uint pos, uint vnum, ushort count)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void SetItemMetinSocketToTarget(int pos, int imetinpos, uint vnum)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void SetItemMetinSocketToSelf(int pos, int imetinpos, uint vnum)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void SetItemAttributeToTarget(int pos, int iattrpos, byte byType, short sValue)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void SetItemAttributeToSelf(int pos, int iattrpos, byte byType, short sValue)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void DelItemOfTarget(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void DelItemOfSelf(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public uint GetItemVnumFromTarget(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public uint GetItemVnumFromSelf(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public ushort GetItemCountFromTarget(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public ushort GetItemCountFromSelf(byte pos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public uint GetItemMetinSocketFromTarget(byte pos, int iMetinSocketPos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public uint GetItemMetinSocketFromSelf(byte pos, int iMetinSocketPos)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return 0;
		...
		public void GetItemAttributeFromTarget(byte pos, int iAttrPos, ref byte pbyType, ref short psValue)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void GetItemAttributeFromSelf(byte pos, int iAttrPos, ref byte pbyType, ref short psValue)
		{
			if (pos >= EXCHANGE_ITEM_MAX_NUM)
			{
				return;
		...
		public void SetAcceptToTarget(byte Accept)
		{
			m_victim.accept = Accept != 0 ? true : false;
		}

		public void SetAcceptToSelf(byte Accept)
		{
			m_self.accept = Accept != 0 ? true : false;
		}

		public bool GetAcceptFromTarget()
		{
			return m_victim.accept ? true : false;
		}

		public bool GetAcceptFromSelf()
		{
			return m_self.accept ? true : false;
		}

		public bool GetElkMode()
		{
			return m_elk_mode;
		}

		public void SetElkMode(bool value)
		{
			m_elk_mode = value;
		}

		protected bool m_isTrading;

		protected bool m_elk_mode;
		protected trade m_self = new trade();
		protected trade m_victim = new trade();
}