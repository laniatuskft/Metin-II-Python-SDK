...
public class SGuildMark
{
	public const int WIDTH = 16;
	public const int HEIGHT = 12;
	public const int SIZE = WIDTH * HEIGHT;

	public uint[] m_apxBuf = new uint[SIZE];
	public void Clear()
	{
		for (uint iPixel = 0; iPixel < SIZE; ++iPixel)
		{
...
public class SGuildMarkBlock
{


	public const int MARK_PER_BLOCK_WIDTH = 4;
	public const int MARK_PER_BLOCK_HEIGHT = 4;
	public const int WIDTH = SGuildMark.WIDTH * MARK_PER_BLOCK_WIDTH;
	public const int HEIGHT = SGuildMark.HEIGHT * MARK_PER_BLOCK_HEIGHT;
	public const int SIZE = WIDTH * HEIGHT;
	public const int MAX_COMP_SIZE = (SIZE * sizeof(uint)) + ((SIZE * sizeof(uint)) >> 4) + 64 + 3;

	public uint[] m_apxBuf = new uint[SIZE];

	public byte[] m_abCompBuf = new byte[MAX_COMP_SIZE];
	public size_t m_sizeCompBuf = new size_t();
	public uint m_crc;

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: uint GetCRC() const
	public uint GetCRC()
	{
		return m_crc;
	}

	public void CopyFrom(byte pbCompBuf, uint dwCompSize, uint crc)
	{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
	...
	public void Compress(uint pxBuf)
	{
		m_sizeCompBuf = MAX_COMP_SIZE;

		if (LZO_E_OK != lzo1x_999_compress((byte) pxBuf, sizeof(uint) * SGuildMarkBlock.SIZE, m_abCompBuf, (lzo_uint) & m_sizeCompBuf, LZOManager.Instance().GetWorkMemory()))
		{
			TraceError("SGuildMarkBlock::Compress: Error! %u > %u", sizeof(uint) * SGuildMarkBlock.SIZE, m_sizeCompBuf);
			return;
		}

		m_crc = GetCRC32((string) pxBuf, sizeof(uint) * SGuildMarkBlock.SIZE);
	}
}

public class CGuildMarkImage
{

		public const int WIDTH = 512;
		public const int HEIGHT = 512;
		public const int BLOCK_ROW_COUNT = HEIGHT / SGuildMarkBlock.HEIGHT;
		public const int BLOCK_COL_COUNT = WIDTH / SGuildMarkBlock.WIDTH;
		public const int BLOCK_TOTAL_COUNT = BLOCK_ROW_COUNT * BLOCK_COL_COUNT;
		public const int MARK_ROW_COUNT = BLOCK_ROW_COUNT * SGuildMarkBlock.MARK_PER_BLOCK_HEIGHT;
		public const int MARK_COL_COUNT = BLOCK_COL_COUNT * SGuildMarkBlock.MARK_PER_BLOCK_WIDTH;
		public const int MARK_TOTAL_COUNT = MARK_ROW_COUNT * MARK_COL_COUNT;
		public const int INVALID_MARK_POSITION = 0xffffffff;

		public CGuildMarkImage()
		{
		...
		public bool Build(string c_szFileName)
		{
			Destroy();
			Create();

			ilBindImage(m_uImg);
			ilEnable(DefineConstants.IL_ORIGIN_SET);
			ilOriginFunc(DefineConstants.IL_ORIGIN_UPPER_LEFT);

//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte * data = (byte *) malloc(sizeof(uint) * WIDTH * HEIGHT);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'malloc' has no equivalent in C#:
			byte data = (byte) malloc(sizeof(uint) * WIDTH * HEIGHT);
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
			memset(data, 0, sizeof(uint) * WIDTH * HEIGHT);

			if (!ilTexImage(WIDTH, HEIGHT, 1, 4, DefineConstants.IL_BGRA, DefineConstants.IL_UNSIGNED_BYTE, data))
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'free' has no equivalent in C#:
			...
			ilEnable(DefineConstants.IL_FILE_OVERWRITE);

			if (!ilSave(DefineConstants.IL_TGA, (ILstring)c_szFileName))
			{
				return false;
			}
		...
		public bool Save(string c_szFileName)
		{
			ilEnable(DefineConstants.IL_FILE_OVERWRITE);
			ilBindImage(m_uImg);
		...
		public bool Load(string c_szFileName)
		{
			Destroy();
			Create();
			...
			if (!ilLoad(DefineConstants.IL_TYPE_UNKNOWN, (ILstring) c_szFileName))
			{
				Build(c_szFileName);
			...
			if (ilGetInteger(DefineConstants.IL_IMAGE_WIDTH) != WIDTH)
			{
				TraceError("CGuildMarkImage: %s width must be %u", c_szFileName, WIDTH);
				return false;
			}

			if (ilGetInteger(DefineConstants.IL_IMAGE_HEIGHT) != HEIGHT)
			{
				TraceError("CGuildMarkImage: %s height must be %u", c_szFileName, HEIGHT);
				return false;
			}

			ilConvertImage(DefineConstants.IL_BGRA, DefineConstants.IL_UNSIGNED_BYTE);

			BuildAllBlocks();
			return true;
		}

		public void PutData(uint x, uint y, uint width, uint height, object data)
		{
			ilBindImage(m_uImg);
			ilSetPixels(x, y, 0, width, height, 1, DefineConstants.IL_BGRA, DefineConstants.IL_UNSIGNED_BYTE, data);
		}

		public void GetData(uint x, uint y, uint width, uint height, object data)
		{
			ilBindImage(m_uImg);
			ilCopyPixels(x, y, 0, width, height, 1, DefineConstants.IL_BGRA, DefineConstants.IL_UNSIGNED_BYTE, data);
		}

		public bool SaveMark(uint posMark, ref byte pbImage)
		{
			...
			uint colMark = posMark % MARK_COL_COUNT;
			uint rowMark = posMark / MARK_COL_COUNT;
			...
			uint rowBlock = rowMark / SGuildMarkBlock.MARK_PER_BLOCK_HEIGHT;
			uint colBlock = colMark / SGuildMarkBlock.MARK_PER_BLOCK_WIDTH;

			uint[] apxBuf = new uint[SGuildMarkBlock.SIZE];
		...
		public bool DeleteMark(uint posMark)
		{
			uint[] image = new uint[SGuildMark.SIZE];
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
			memset(image, 0, sizeof(uint));
			return SaveMark(posMark, (byte) image);
		}

		public bool SaveBlockFromCompressedData(uint posBlock, byte pbComp, uint dwCompSize)
		{
			...
			uint[] apxBuf = new uint[SGuildMarkBlock.SIZE];
			size_t sizeBuf = sizeof(uint);

			if (LZO_E_OK != lzo1x_decompress_safe(pbComp, dwCompSize, (byte) apxBuf, (lzo_uint) sizeBuf, LZOManager.Instance().GetWorkMemory()))
			{
				TraceError("CGuildMarkImage::CopyBlockFromCompressedData: cannot decompress, compressed size = %u", dwCompSize);
				return false;
			}

			if (sizeBuf != sizeof(uint))
			{
				TraceError("CGuildMarkImage::CopyBlockFromCompressedData: image corrupted, decompressed size = %u", sizeBuf);
				return false;
			}

			uint rowBlock = posBlock / BLOCK_COL_COUNT;
			uint colBlock = posBlock % BLOCK_COL_COUNT;
			...
			m_aakBlock[rowBlock][colBlock].CopyFrom(pbComp, dwCompSize, GetCRC32((string) apxBuf, sizeof(uint) * SGuildMarkBlock.SIZE));
			return true;
		}

		public uint GetEmptyPosition()
		{
			SGuildMark kMark = new SGuildMark();

			for (uint row = 0; row < MARK_ROW_COUNT; ++row)
			{
				for (uint col = 0; col < MARK_COL_COUNT; ++col)
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'crcList', so pointers on this parameter are left unchanged:
		public void GetBlockCRCList(uint * crcList)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'crcList', so pointers on this parameter are left unchanged:
		public void GetDiffBlocks(in uint * crcList, in SortedDictionary<byte, SGuildMarkBlock > mapDiffBlocks)
		{
			byte posBlock = 0;
			...
			for (uint row = 0; row < BLOCK_ROW_COUNT; ++row)
			{
				for (uint col = 0; col < BLOCK_COL_COUNT; ++col)
				{
					...
					{
						mapDiffBlocks.insert(SortedDictionary<byte, const SGuildMarkBlock>.value_type(posBlock, m_aakBlock[row][col]));
					}
					++crcList;
					++posBlock;
				}
		...
		private const int INVALID_HANDLE = 0xffffffff;

		private void BuildAllBlocks()
		{
			uint[] apxBuf = new uint[SGuildMarkBlock.SIZE];
			(0, "CGuildMarkImage::BuildAllBlocks");
		...
		private SGuildMarkBlock[][] m_aakBlock = RectangularArrays.RectangularSGuildMarkBlockArray(BLOCK_ROW_COUNT, BLOCK_COL_COUNT);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		uint m_apxImage[WIDTH * HEIGHT sizeof(uint UnnamedParameter)];

		private ILuint m_uImg = new ILuint();
}



#if !_MSC_VER
//# Laniatus Games Studio Inc. | WARN: The following #include directive was ignored:
...
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define CLZO LZOManager
#define CLZO
#else
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define sys_err TraceError
#define sys_err
#define sys_log
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define thecore_memcpy memcpy
#define thecore_memcpy
//# Laniatus Games Studio Inc. | TODO TASK: #define macros defined in multiple preprocessor conditionals can only be replaced within the scope of the preprocessor conditional:
//# Laniatus Games Studio Inc. |: The following #define macro was replaced in-line:
//Original Metin2 CPlus Line: #define itertype(cont) typeof(cont.begin())
#define itertype
#endif