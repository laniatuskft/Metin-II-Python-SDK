...
public class CAffectFlagContainer
{
		public const int BIT_SIZE = 64;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not allow bit fields:
		public constexpr int BYTE_SIZE = BIT_SIZE / 8 + (1 * ((BIT_SIZE & 7) != 0 ? 1 : 0));

		public CAffectFlagContainer()
		{
			Clear();
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memset' has no equivalent in C#:
		...
		public void CopyInstance(in CAffectFlagContainer c_rkAffectContainer)
		{
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
		...
		public void Set(uint uPos, bool isSet)
		{
			if (uPos / 8 >= BYTE_SIZE)
			{
...
//Original Metin2 CPlus Line: byte& rElement=m_aElement[uPos/8];
...
//Original Metin2 CPlus Line: bool IsSet(uint uPos) const
		public bool IsSet(uint uPos)
		{
			if (uPos / 8 >= BYTE_SIZE)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to references to value types:
//Original Metin2 CPlus Line: const byte& c_rElement=m_aElement[uPos/8];
			byte c_rElement = m_aElement[uPos / 8];

			byte bMask = (byte)(1 << (uPos & 7));
			if ((c_rElement & bMask) != 0)
			{
				return true;
			}
		...
		public void CopyData(uint uPos, uint uByteSize, object c_pvData)
		{
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			byte * c_pbData = (byte)c_pvData;
			Element bMask = 0x01;

			uint uBitEnd = uPos + uByteSize * 8;
			for (uint LaniatusDefVariables = uPos; LaniatusDefVariables < uBitEnd; ++i)
			{
				Set(i, (*c_pbData & bMask) != 0 ? true : false);
				bMask <<= 1;
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: void ConvertToPosition(uint* uRetX, uint* uRetY) const
		public void ConvertToPosition(ref uint uRetX, ref uint uRetY)
		{
			uint[] pos = (uint)m_aElement;
			uRetX = pos[0];
			uRetY = pos[1];
		}


		private byte[] m_aElement = new byte[BYTE_SIZE];
}
