...
public class CPythonTextTail : CSingleton<CPythonTextTail>
{
		public class STextTail
		{
			public CGraphicTextInstance pTextInstance;
			public CGraphicTextInstance pOwnerTextInstance;

			public CGraphicMarkInstance pMarkInstance;
			public CGraphicTextInstance pGuildNameTextInstance;

			public CGraphicTextInstance pTitleTextInstance;
			public CGraphicTextInstance pLevelTextInstance;
#if ENABLE_MULTI_LANGUAGE_SYSTEM
			public CGraphicImageInstance pLanguageInstance;
#endif
			public CGraphicObjectInstance pOwner;

			public uint dwVirtualID;

			public float x;
			public float y;
			public float z;
			public float fDistanceFromPlayer;
			public D3DXCOLOR Color = new D3DXCOLOR();
			public bool bNameFlag;
		...
		public void GetInfo(string pstInfo)
		{
			...
			sprintf(szInfo, "TextTail: ChatTail %d, ChrTail (Map %d, List %d), ItemTail (Map %d, List %d), Pool %d", m_ChatTailMap.size(), m_CharacterTextTailMap.size(), m_CharacterTextTailList.size(), m_ItemTextTailMap.size(), m_ItemTextTailList.size(), m_TextTailPool.GetCapacity());
		...
		public void UpdateAllTextTail()
		{
			CInstanceBase pInstance = CPythonCharacterManager.Instance().GetMainInstancePtr();
			if (pInstance != null)
			{
				TPixelPosition pixelPos = new TPixelPosition();
				pInstance.NEW_GetPixelPosition(pixelPos);

				TTextTailMap.iterator itorMap = new TTextTailMap.iterator();
		...
		public void UpdateShowingTextTail()
		{
			TTextTailList.iterator itor = new TTextTailList.iterator();
		...
		public void ArrangeTextTail()
		{
			TTextTailList.iterator itor = new TTextTailList.iterator();
			TTextTailList.iterator itorCompare = new TTextTailList.iterator();

			uint dwTime = CTimer.Instance().GetCurrentMillisecond();
			...
			for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
			{
				...
				if (pMarkInstance != null && pGuildNameInstance != null)
				{
					int iWidth;
					int iHeight;
		...
		public void HideAllTextTail()
		{
			m_CharacterTextTailList.clear();
			m_ItemTextTailList.clear();
		}

		public void ShowAllTextTail()
		{
			TTextTailMap.iterator itor = new TTextTailMap.iterator();
		...
		public void ShowCharacterTextTail(uint VirtualID)
		{
			...
			else if (pInstance.IsMount())
			{
				m_CharacterTextTailList.push_back(pTextTail);
			}
			else if (pInstance.IsPet())
			{
				m_CharacterTextTailList.push_back(pTextTail);
			}
		}

		public void ShowItemTextTail(uint VirtualID)
		{
		...
		public void RegisterCharacterTextTail(uint dwGuildID, uint dwVirtualID, in D3DXCOLOR c_rColor, float fAddHeight = 10.0f)
		{
			...
			if (0 != dwGuildID)
			{
				...
				uint dwMarkID = CGuildMarkManager.Instance().GetMarkID(dwGuildID);
				...
				if (dwMarkID != CGuildMarkManager.INVALID_MARK_ID)
				{
					...
					{
						pTextTail.pMarkInstance.SetImageFileName(markImagePath);
			...
			if (prLanguage == null)
			{
				...
				if (pCharacterInstance.IsPC() && bLanguage != 0)
				{
					string langName = "en";
					if (bLanguage == 1)
					{
					...
					string szFileName = new string(new char[256]);
					sprintf(szFileName, "t:/laniaworkstate/uinterface/game/flag/%s.tga", langName);
		...
		public void RegisterItemTextTail(uint VirtualID, string c_szText, CGraphicObjectInstance pOwner)
		{
#if __DEBUG
		...
		public void RegisterChatTail(uint VirtualID, string c_szChat)
		{
		...
		public void RegisterInfoTail(uint VirtualID, string c_szChat)
		{
			CInstanceBase pCharacterInstance = CPythonCharacterManager.Instance().GetInstancePtr(VirtualID);
		...
		public void SetCharacterTextTailColor(uint VirtualID, in D3DXCOLOR c_rColor)
		{
		...
		public void SetItemTextTailOwner(uint dwVID, string c_szName)
		{
			var itor = m_ItemTextTailMap.find(dwVID);
			if (m_ItemTextTailMap.end() == itor)
			{
				return;
			}
		...
		public void DeleteCharacterTextTail(uint VirtualID)
		{
			TTextTailMap.iterator itorCharacter = m_CharacterTextTailMap.find(VirtualID);
		...
		public void DeleteItemTextTail(uint VirtualID)
		{
			TTextTailMap.iterator itor = m_ItemTextTailMap.find(VirtualID);
		...
		public int Pick(int ixMouse, int iyMouse)
		{
			for (TTextTailMap.iterator itor = m_ItemTextTailMap.begin(); itor != m_ItemTextTailMap.end(); ++itor)
			{
				TTextTail pTextTail = itor.second;

				if (ixMouse >= pTextTail.x + pTextTail.xStart && ixMouse <= pTextTail.x + pTextTail.xEnd && iyMouse >= pTextTail.y + pTextTail.yStart && iyMouse <= pTextTail.y + pTextTail.yEnd)
				{
		...
		public void SelectItemName(uint dwVirtualID)
		{
		...
		public bool GetTextTailPosition(uint dwVID, ref float px, ref float py, ref float pz)
		{
		...
		public bool IsChatTextTail(uint dwVID)
		{
		...
		public void EnablePKTitle(bool bFlag)
		{
			bPKTitleEnable = bFlag;
		}

		public void AttachTitle(uint dwVID, string c_szName, in D3DXCOLOR c_rColor)
		{
			if (!bPKTitleEnable)
			{
				return;
		...
		public void DetachTitle(uint dwVID)
		{
			if (!bPKTitleEnable)
			{
				return;
		...
		public void AttachLevel(uint dwVID, string c_szText, in D3DXCOLOR c_rColor)
		{
			if (!bPKTitleEnable)
			{
				return;
		...
		public void DetachLevel(uint dwVID)
		{
			if (!bPKTitleEnable)
			{
				return;
		...
		protected STextTail RegisterTextTail(uint dwVirtualID, string c_szText, CGraphicObjectInstance pOwner, float fHeight, in D3DXCOLOR c_rColor)
		{
...
//		void DeleteTextTail(STextTail pTextTail);

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void UpdateTextTail(STextTail pTextTail);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RenderTextTailBox(STextTail pTextTail);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void RenderTextTailName(STextTail pTextTail);
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void UpdateDistance(in TPixelPosition c_rCenterPosition, STextTail pTextTail);

		protected bool isIn(STextTail pSource, STextTail pTarget)
		{
		...
		protected SortedDictionary<uint, STextTail> m_CharacterTextTailMap = new SortedDictionary<uint, STextTail>();
		protected SortedDictionary<uint, STextTail> m_ItemTextTailMap = new SortedDictionary<uint, STextTail>();
		protected SortedDictionary<uint, STextTail> m_ChatTailMap = new SortedDictionary<uint, STextTail>();

		protected LinkedList<STextTail> m_CharacterTextTailList = new LinkedList<STextTail>();
		protected LinkedList<STextTail> m_ItemTextTailList = new LinkedList<STextTail>();

		private CDynamicPool<STextTail> m_TextTailPool = new CDynamicPool<STextTail>();
}
