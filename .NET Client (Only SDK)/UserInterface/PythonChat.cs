...
public class CWhisper
{
		public class SChatLine
		{
			public CGraphicTextInstance Instance = new CGraphicTextInstance();
			...
			public static CWhisper.SChatLine New()
			{
				return ms_kPool.Alloc();
			}

			public static void Delete(CWhisper.SChatLine pkChatLine)
			{
				pkChatLine.Instance.Destroy();
				ms_kPool.Free(pkChatLine);
			}

			public static CDynamicPool<SChatLine> ms_kPool = new CDynamicPool<SChatLine>();
		}
		...
		public void SetBoxSize(float fWidth, float fHeight)
		{
			m_fWidth = fWidth;
			m_fHeight = fHeight;

		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineDeque' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		...
		public void AppendChat(int iType, string c_szChat)
		{
			CGraphicText pkDefaultFont = (iType == CPythonChat.WHISPER_TYPE_GM) ? (CGraphicText)DefaultFont_GetResource() : (CGraphicText)DefaultFont_GetResource();
		...
		public void Render(float fx, float fy)
		{
			float fHeight = fy + m_fHeight;

			int iViewCount = (int)(m_fHeight / m_fLineStep) - 1;
			int iLineCount = m_ChatLineDeque.size();
			int iStartLine = -1;
			...
			RECT Rect = new RECT(fx, fy, fx + m_fWidth, fy + m_fHeight);

			for (int LaniatusDefVariables = iStartLine; LaniatusDefVariables >= 0; --i)
			{
		...
		protected float m_fLineStep;
		protected float m_fWidth;
		protected float m_fHeight;
		protected float m_fcurPosition;

		protected LinkedList<SChatLine> m_ChatLineDeque = new LinkedList<SChatLine>();
		protected LinkedList<SChatLine> m_ShowingChatLineList = new LinkedList<SChatLine>();
		...
		public static CWhisper New()
		{
			return ms_kPool.Alloc();
		}

		public static void Delete(CWhisper pkWhisper)
		{
			pkWhisper.Destroy();
			ms_kPool.Free(pkWhisper);
		}

		public static CDynamicPool<CWhisper> ms_kPool = new CDynamicPool<CWhisper>();
}

//# Laniatus Games Studio Inc. | TODO TASK: Multiple inheritance is not available in C#:
public class CPythonChat : CSingleton<CPythonChat>, IAbstractChat
{
		public enum EWhisperType
		{
			WHISPER_TYPE_CHAT = 0,
		...
		public const int CHAT_LINE_MAX_NUM = 300;
		public const int CHAT_LINE_COLOR_ARRAY_MAX_NUM = 3;

		public class SChatLine
		{
			public int iType;
			public float fAppendedTime;
			public D3DXCOLOR[] aColor = Arrays.InitializeWithDefaultInstances<D3DXCOLOR>(CHAT_LINE_COLOR_ARRAY_MAX_NUM);
			public CGraphicTextInstance Instance = new CGraphicTextInstance();
#if ENABLE_MULTI_LANGUAGE_SYSTEM
			public CGraphicImageInstance pLanguage = new CGraphicImageInstance();
#endif
			...
			public void SetColor(uint dwID, uint dwColor)
			{
				Debug.Assert(dwID < CHAT_LINE_COLOR_ARRAY_MAX_NUM);
			...
			public void SetColorAll(uint dwColor)
			{
				for (int LaniatusDefVariables = 0; LaniatusDefVariables < CHAT_LINE_COLOR_ARRAY_MAX_NUM; ++i)
				{
...
//# Laniatus Games Studio Inc. | TODO TASK: The following statement was not recognized, possibly due to an unrecognized macro:
			D3DXCOLOR GetColorRef_color(1.0f, 0.0f, 0.0f, 1.0f);

			public D3DXCOLOR GetColorRef(uint dwID)
			{
				Debug.Assert(dwID < CHAT_LINE_COLOR_ARRAY_MAX_NUM);
...
//					static D3DXCOLOR color(1.0f, 0.0f, 0.0f, 1.0f);
			...
			public static CPythonChat.SChatLine New()
			{
				return ms_kPool.Alloc();
			}

			public static void Delete(CPythonChat.SChatLine pkChatLine)
			{
				pkChatLine.Instance.Destroy();
#if ENABLE_MULTI_LANGUAGE_SYSTEM
		...
		public class SWaitChat
		{
			public int iType;
			public string strChat = "";

			public uint dwAppendingTime;
		}
...
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLine' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		...
		public class SChatSet
		{
			public int m_ix;
			public int m_iy;
			public int m_iHeight;
			public int m_iStep;
			public float m_fEndPos;

			public int m_iBoardState;
			public List<int> m_iMode = new List<int>();

//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineList' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			public TChatLineList m_ShowingChatLineList = new TChatLineList();

			public bool CheckMode(uint dwType)
			{
			...
			public static int ms_iChatModeSize = CHAT_TYPE_MAX_NUM;
		}
		...
		public virtual void Dispose()
		{
			Debug.Assert(m_ChatLineDeque.empty());
			Debug.Assert(m_ShowingChatLineList.empty());
			Debug.Assert(m_ChatSetMap.empty());
			Debug.Assert(m_WhisperMap.empty());
		}

		public void SetChatColor(uint eType, uint r, uint g, uint b)
		{
			...
			uint dwColor = (0xff000000) | (r << 16) | (g << 8) | (b);
		...
		public int CreateChatSet(uint dwID)
		{
			m_ChatSetMap.insert(Tuple.Create(dwID, TChatSet()));
			return (int)dwID;
		}

		public void Update(uint dwID)
		{
...
//Original Metin2 CPlus Line: TWaitChat & rWaitChat = *itor;
...
//		void RenderWhisper(string c_szName, float fx, float fy);

		public void SetBoardState(uint dwID, int iState)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void SetPosition(uint dwID, int ix, int iy)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void SetHeight(uint dwID, int iHeight)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void SetStep(uint dwID, int iStep)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void ToggleChatMode(uint dwID, int iMode)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void EnableChatMode(uint dwID, int iMode)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void DisableChatMode(uint dwID, int iMode)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public void SetEndPos(uint dwID, float fPos)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
			...
			fPos = Math.Max(0.0f, fPos);
			fPos = Math.Min(1.0f, fPos);
		...
		public int GetVisibleLineCount(uint dwID)
		{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineList' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			TChatLineList pLineList = GetChatLineListPtr(dwID);
			if (pLineList == null)
		...
		public int GetEditableLineCount(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public int GetLineCount(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		public int GetLineStep(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
...
#if ENABLE_MULTI_LANGUAGE_SYSTEM
			string chatStr = c_szChat;
			uint startLang = (uint)chatStr.IndexOf("|L");
			uint endLang = (uint)chatStr.IndexOf("|l");

			if (startLang != -1 && endLang != -1)
			{
				string strLanguage = chatStr.Substring(startLang + 2, endLang - 2);
				string szLangName = new string(new char[256]);
				sprintf(szLangName, "t:/laniaworkstate/uinterface/game/flag/%s.tga", strLanguage);
				if (CResourceManager.Instance().IsFileExist(szLangName))
				{
					...
					if (pLanguageImage != null)
					{
						pChatLine.pLanguage.SetImagePointer(pLanguageImage);
					}
				}

				chatStr = chatStr.Substring(endLang + 3, chatStr.Length);
			}
#endif

			pChatLine.iType = iType;
#if ENABLE_MULTI_LANGUAGE_SYSTEM
			pChatLine.Instance.SetValue(chatStr);
#else
			pChatLine.Instance.SetValue(c_szChat);
#endif
		...
		public void AppendChatWithDelay(int iType, string c_szChat, int iDelay)
		{
			TWaitChat WaitChat = new TWaitChat();
		...
		public void ArrangeShowingChat(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineDeque' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			TChatLineDeque TempChatLineDeque = new TChatLineDeque();
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineDeque' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			for (TChatLineDeque.iterator itor = m_ChatLineDeque.begin(); itor != m_ChatLineDeque.end(); ++itor)
			{
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLine' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			...
			int iVisibleLineCount = Math.Min(icurLineCount, (pChatSet.m_iHeight + pChatSet.m_iStep) / pChatSet.m_iStep);
		...
		public void IgnoreCharacter(string c_szName)
		{
		...
		public bool IsIgnoreCharacter(string c_szName)
		{
			TIgnoreCharacterSet.iterator itor = m_IgnoreCharacterSet.find(c_szName);
		...
		public CWhisper CreateWhisper(string c_szName)
		{
		...
		public void AppendWhisper(int iType, string c_szName, string c_szChat)
		{
		...
		public void ClearWhisper(string c_szName)
		{
			TWhisperMap.iterator itor = m_WhisperMap.find(c_szName);
		...
		public bool GetWhisper(string c_szName, CWhisper[] ppWhisper)
		{
			TWhisperMap.iterator itor = m_WhisperMap.find(c_szName);
			...
			ppWhisper[0] = itor.second;

			return true;
		}

		public void InitWhisper(PyObject ppyObject)
		{
			TWhisperMap.iterator itor = m_WhisperMap.begin();
			for (; itor != m_WhisperMap.end(); ++itor)
			{
				string strName = itor.first;
				PyCallClassMemberFunc(ppyObject, "MakeWhisperButton", Py_BuildValue("(s)", strName));
			}
		}
		...
		protected void __DestroyWhisperMap()
		{
			TWhisperMap.iterator itor = m_WhisperMap.begin();
			for (; itor != m_WhisperMap.end(); ++itor)
			{
...
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineList' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected CPythonChat.TChatLineList GetChatLineListPtr(uint dwID)
		{
		...
		protected SChatSet GetChatSetPtr(uint dwID)
		{
			TChatSetMap.iterator itor = m_ChatSetMap.find(dwID);
			if (m_ChatSetMap.end() == itor)
			{
		...
		protected void UpdateViewMode(uint dwID)
		{
			IAbstractApplication rApp = IAbstractApplication.GetSingleton();
		...
		//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineList' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
			TChatLineList.iterator itor = new TChatLineList.iterator();
		...
		protected void UpdateEditMode(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
			...
			int c_iAlphaLine = Math.Max(0, GetVisibleLineCount(dwID) - GetEditableLineCount(dwID) + 2);
		...
		protected void UpdateLogMode(uint dwID)
		{
			TChatSet pChatSet = GetChatSetPtr(dwID);
			if (pChatSet == null)
			{
		...
		protected uint GetChatColor(int iType)
		{
...
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineDeque' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected TChatLineDeque m_ChatLineDeque = new TChatLineDeque();
//# Laniatus Games Studio Inc. | TODO TASK: The typedef 'TChatLineList' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
		protected TChatLineList m_ShowingChatLineList = new TChatLineList();
		protected SortedDictionary<int, SChatSet> m_ChatSetMap = new SortedDictionary<int, SChatSet>();
		protected SortedDictionary<string, CWhisper> m_WhisperMap = new SortedDictionary<string, CWhisper>();
		protected SortedSet<string> m_IgnoreCharacterSet = new SortedSet<string>();
		protected LinkedList<SWaitChat> m_WaitChatList = new LinkedList<SWaitChat>();

		protected D3DXCOLOR[] m_akD3DXClrChat = Arrays.InitializeWithDefaultInstances<D3DXCOLOR>(CHAT_TYPE_MAX_NUM);
}