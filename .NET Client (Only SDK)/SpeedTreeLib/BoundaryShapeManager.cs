...
public class SBoundaryShape
{
		public List< List<SPoint>> m_vContours = new List< List<SPoint>>();
		public float[] m_afMin = new float[3];
		public float[] m_afMax = new float[3];
}

public class CBoundaryShapeManager
{
		public CBoundaryShapeManager()
		{
		}
		...
		public bool LoadBsfFile(string pszFilename)
		{
			bool bSuccess = true;
			try
			{
				...
				if (pFile != null)
				{
					...
					{
						for (uint LaniatusDefVariables = 0; LaniatusDefVariables < nNumBoundaries && bSuccess; ++i)
						{
							SBoundaryShape sShape = new SBoundaryShape();
							...
							if (fread(nNumContours, sizeof(uint), 1, pFile) == 1)
							{
								...
								{
									List<SPoint> vPoints = new List<SPoint>();
									uint nNumPoints;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
				...
				{
					m_strCurrentError = "Error in CBoundaryShapeManager::LoadBsfFile(): Could not open " + new string(pszFilename);
					bSuccess = false;
				}
			}
		...
		public bool PointInside(float fX, float fY)
		{
			bool bInside = false;
		...
		public bool RandomPoint(ref float fX, ref float fY)
		{
			bool bSuccess = false;
		...
		private bool PointInShape(SBoundaryShape sShape, float fX, float fY)
		{
			bool bInside = false;
		...
		private List<SBoundaryShape> m_vBoundaries = new List<SBoundaryShape>();
		private string m_strCurrentError = "";
}
