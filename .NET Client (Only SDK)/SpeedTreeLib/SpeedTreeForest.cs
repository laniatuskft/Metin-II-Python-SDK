...
public class CSpeedTreeForest
{

		public CSpeedTreeForest()
		{
			this.m_fWindStrength = 0.0f;
			CSpeedTreeRT.SetNumWindMatrices(c_nNumWindMatrices);

			m_afForestExtents[0] = m_afForestExtents[1] = m_afForestExtents[2] = float.MaxValue;
			m_afForestExtents[3] = m_afForestExtents[4] = m_afForestExtents[5] = -float.MaxValue;
		}
...
//		void ClearMainTree();
		...
		public bool GetMainTree(uint dwCRC, CSpeedTreeWrapper[] ppMainTree, string c_pszFileName)
		{
			...
			else
			{
				...
				if (!pTree.LoadTree(c_pszFileName, (byte) c_pvData, file.Size()))
				{
					pTree = null;
					return false;
				}

				m_pMainTreeMap.insert(SortedDictionary<uint, CSpeedTreeWrapper>.value_type(dwCRC, pTree));
...
//Original Metin2 CPlus Line: *ppMainTree = pTree;
			ppMainTree[0].CopyFrom(pTree);
			return true;
		}

		public CSpeedTreeWrapper GetMainTree(uint dwCRC)
		{
			TTreeMap.iterator itor = m_pMainTreeMap.find(dwCRC);
...
//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void DeleteMainTree(uint dwCRC);

		public CSpeedTreeWrapper CreateInstance(float x, float y, float z, uint dwTreeCRC, string c_szTreeName)
		{
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private float UpdateSystem_fLastTime = fCurrentTime;

		public void UpdateSystem(float fCurrentTime)
		{
...
//			static float fLastTime = fCurrentTime;
			float fElapsedTime = fCurrentTime - UpdateSystem_fLastTime;
		...
		public void SetLight(float[] afDirection, float[] afAmbient, float[] afDiffuse)
		{
		...
		public void SetFog(float fFogNear, float fFogFar)
		{
			float c_fFogLinearScale = (1.0f / (fFogFar - fFogNear));
...
//Original Metin2 CPlus Line: const float * GetExtents() const
...
//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: float GetWindStrength() const
...
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private float[] SetupWindMatrices_afMatrixTimes = Arrays.PadValueTypeArrayWithDefaultInstances(c_nNumWindMatrices, new float[] {0.0f});
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private float[][] SetupWindMatrices_afFrequencies =
		{
			new float[] {0.15f, 0.17f},
			new float[] {0.25f, 0.15f},
			new float[] {0.19f, 0.05f},
			new float[] {0.15f, 0.22f}
		};
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private float SetupWindMatrices_fTimeOfLastCall = 0.0f;
//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private float SetupWindMatrices_fOldStrength = m_fWindStrength;

		public void SetupWindMatrices(float fTimeInSecs)
		{
//# Laniatus Games Studio Inc. |: This static local variable declaration (not allowed in C#) has been moved just prior to the method:
...
//			static float fTimeOfLastCall = 0.0f;
			float fTimeSinceLastCall = fTimeInSecs - SetupWindMatrices_fTimeOfLastCall;
			SetupWindMatrices_fTimeOfLastCall = fTimeInSecs;
...
//			static float fOldStrength = m_fWindStrength;
			...
			for (int j = 0; j < c_nNumWindMatrices; ++j)
			{
				...
				{
					SetupWindMatrices_afMatrixTimes[j] = (SetupWindMatrices_afMatrixTimes[j] * SetupWindMatrices_fOldStrength) / m_fWindStrength;
				}

				float fBaseFreq = m_fWindStrength * 20.0f;
				float fXPercent = sinf(fBaseFreq * SetupWindMatrices_afFrequencies[j % c_nNumWindMatrices][0] * SetupWindMatrices_afMatrixTimes[j]);
				float fYPercent = cosf(fBaseFreq * SetupWindMatrices_afFrequencies[j % c_nNumWindMatrices][1] * SetupWindMatrices_afMatrixTimes[j]);
				...
				float[] afMatrix = {0.0f, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F};
				afMatrix[0] = fCosY;
				afMatrix[2] = -fSinY;
				...
				afMatrix[15] = 1.0f;
					...
					CSpeedTreeRT.SetWindMatrix((uint)j, afMatrix);
				#endif
			...
			SetupWindMatrices_fOldStrength = m_fWindStrength;
		}

//# Laniatus Games Studio Inc. | WARN: 'const' methods are not available in C#:
//Original Metin2 CPlus Line: virtual void UploadWindMatrix(uint uiLocation, const float* pMatrix) const = 0;
		public abstract void UploadWindMatrix(uint uiLocation, float pMatrix);
		public abstract void Render(uint ulRenderBitVector);

		protected SortedDictionary<uint, CSpeedTreeWrapper > m_pMainTreeMap = new SortedDictionary<uint, CSpeedTreeWrapper >();

		protected float[] m_afLighting = new float[12];
		protected float[] m_afFog = new float[4];

		private void AdjustExtents(float x, float y, float z)
		{
			m_afForestExtents[0] = Math.Min(m_afForestExtents[0], x);
			m_afForestExtents[1] = Math.Min(m_afForestExtents[1], y);
			m_afForestExtents[2] = Math.Min(m_afForestExtents[2], z);

			m_afForestExtents[3] = Math.Max(m_afForestExtents[3], x);
			m_afForestExtents[4] = Math.Max(m_afForestExtents[4], y);
			m_afForestExtents[5] = Math.Max(m_afForestExtents[5], z);
		}

		private float[] m_afForestExtents = new float[6];
		private float m_fWindStrength;

		private float m_fAccumTime;
}
