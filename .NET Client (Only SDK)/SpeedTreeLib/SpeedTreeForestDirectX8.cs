...
public class CSpeedTreeForestDirectX8 : CSpeedTreeForest, CGraphicBase, CSingleton<CSpeedTreeForestDirectX8>
{
		...
		public CSpeedTreeForestDirectX8()
		{
			this.m_dwBranchVertexShader = 0;
			this.m_dwLeafVertexShader = 0;
		}
...
//Original Metin2 CPlus Line: void UploadWindMatrix(uint uiLocation, const float* pMatrix) const
		public void UploadWindMatrix(uint uiLocation, float pMatrix)
		{
			(CStateManager.Instance()).SetVertexShaderConstant(uiLocation, pMatrix, 4);
		}

//# Laniatus Games Studio Inc. | TODO TASK: The implementation of the following method could not be found:
//		void UpdateCompundMatrix(in D3DXVECTOR3 c_rEyeVec, in D3DXMATRIX c_rmatView, in D3DXMATRIX c_rmatProj);
		public void Render(uint ulRenderBitVector = Forest_RenderAll)
		{
			...
			if ((ulRenderBitVector & Forest_RenderToShadow) == 0 && (ulRenderBitVector & Forest_RenderToMiniMap) == 0)
			{
				UpdateCompundMatrix(CCameraManager.Instance().GetCurrentCamera().GetEye(), ms_matView, ms_matProj);
			}

			uint dwLightState = (CStateManager.Instance()).GetRenderState(D3DRS_LIGHTING);
			uint dwColorVertexState = (CStateManager.Instance()).GetRenderState(D3DRS_COLORVERTEX);
			uint dwFogVertexMode = (CStateManager.Instance()).GetRenderState(D3DRS_FOGVERTEXMODE);
			...
			(CStateManager.Instance()).SetRenderState(D3DRS_COLORVERTEX, true);
#endif

			TTreeMap.iterator itor = new TTreeMap.iterator();
			uint uiCount;
			...
			(CStateManager.Instance()).SetVertexShaderConstant(c_nVertexShader_Light, m_afLighting, 3);
			(CStateManager.Instance()).SetVertexShaderConstant(c_nVertexShader_Fog, m_afFog, 1);

			if ((ulRenderBitVector & Forest_RenderToShadow) != 0)
			{
			...
			if ((CStateManager.Instance()).GetRenderState(D3DRS_FOGENABLE))
			{
				#if WRAPPER_USE_GPU_WIND
					(CStateManager.Instance()).SetRenderState(D3DRS_FOGVERTEXMODE, D3DFOG_NONE);
				#endif
			}

			(CStateManager.Instance()).SetVertexShader(m_dwBranchVertexShader);

			if ((ulRenderBitVector & Forest_RenderBranches) != 0)
			{
				itor = m_pMainTreeMap.begin();
			...
			if ((ulRenderBitVector & Forest_RenderFronds) != 0)
			{
				itor = m_pMainTreeMap.begin();
			...
			if ((ulRenderBitVector & Forest_RenderLeaves) != 0)
			{
				(CStateManager.Instance()).SetVertexShader(m_dwLeafVertexShader);

				if ((CStateManager.Instance()).GetRenderState(D3DRS_FOGENABLE))
				{
					#if WRAPPER_USE_GPU_WIND || WRAPPER_USE_GPU_LEAF_PLACEMENT
						(CStateManager.Instance()).SetRenderState(D3DRS_FOGVERTEXMODE, D3DFOG_NONE);
					#endif
				}
				...
				if ((ulRenderBitVector & Forest_RenderToShadow) != 0 || (ulRenderBitVector & Forest_RenderToMiniMap) != 0)
				{
					(CStateManager.Instance()).SetRenderState(D3DRS_ALPHAFUNC, D3DCMP_NOTEQUAL);
				...
				if ((ulRenderBitVector & Forest_RenderToShadow) != 0 || (ulRenderBitVector & Forest_RenderToMiniMap) != 0)
				{
					(CStateManager.Instance()).SetRenderState(D3DRS_ALPHAFUNC, D3DCMP_GREATER);
					(CStateManager.Instance()).RestoreRenderState(D3DRS_ALPHAREF);
				}
			}

			#if ! WRAPPER_NO_BILLBOARD_MODE
				if ((ulRenderBitVector & Forest_RenderBillboards) != 0)
				{
					(CStateManager.Instance()).SetRenderState(D3DRS_LIGHTING, false);
			...
			(CStateManager.Instance()).SetRenderState(D3DRS_LIGHTING, dwLightState);
			(CStateManager.Instance()).SetRenderState(D3DRS_COLORVERTEX, dwColorVertexState);
			(CStateManager.Instance()).SetRenderState(D3DRS_FOGVERTEXMODE, dwFogVertexMode);
		...
		public bool SetRenderingDevice(LPDIRECT3DDEVICE8 lpDevice)
		{
			m_pDx = lpDevice;
			...
			float[] c_afLightPosition = {-0.707f, -0.300f, 0.707f, 0.0f};
			float[] c_afLightAmbient = {0.5f, 0.5f, 0.5f, 1.0f};
			float[] c_afLightDiffuse = {1.0f, 1.0f, 1.0f, 1.0f};
			float[] c_afLightSpecular = {1.0f, 1.0f, 1.0f, 1.0f};

			float[] afLight1 = {c_afLightPosition[0], c_afLightPosition[1], c_afLightPosition[2], c_afLightDiffuse[0], c_afLightDiffuse[1], c_afLightDiffuse[2], c_afLightAmbient[0], c_afLightAmbient[1], c_afLightAmbient[2], c_afLightSpecular[0], c_afLightSpecular[1], c_afLightSpecular[2], c_afLightPosition[3], 1.0f, 0.0f, 0.0f};
		...
		private bool InitVertexShaders()
		{
			NANOBEGIN if (!m_dwBranchVertexShader) m_dwBranchVertexShader = LoadBranchShader(m_pDx);
			...
			NANOEND return false;
		}

		private LPDIRECT3DDEVICE8 m_pDx = new LPDIRECT3DDEVICE8();
		private uint m_dwBranchVertexShader;
		private uint m_dwLeafVertexShader;
}
