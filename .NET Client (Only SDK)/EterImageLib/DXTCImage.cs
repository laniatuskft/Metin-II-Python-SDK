...
////# Laniatus Games Studio Inc. | TODO TASK: The #define macro 'DUMMYUNIONNAMEN' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
//	}
//# Laniatus Games Studio Inc. | TODO TASK: The #define macro 'DUMMYUNIONNAMEN' was defined in multiple preprocessor conditionals and cannot be replaced in-line:
...
//Original Metin2 CPlus Line: const byte * m_pbCompBufferByLevels[DefineConstants.MAX_MIPLEVELS];
		public readonly byte[] m_pbCompBufferByLevels = {null};
		public List<byte>[] m_bCompVector = Arrays.InitializeWithDefaultInstances<List<byte>>(DefineConstants.MAX_MIPLEVELS);

		public int m_nCompSize;
		public int m_nCompLineSz;

		public string m_strFormat = new string(new char[32]);
		public EPixFormat m_CompFormat = new EPixFormat();

		public int m_lPitch;
		public uint m_dwMipMapCount;
		public bool m_bMipTexture;
		public uint m_dwFlags;

		public int m_nWidth;
		public int m_nHeight;

		public _XDDPIXELFORMAT m_xddPixelFormat = new _XDDPIXELFORMAT();

//# Laniatus Games Studio Inc. |: This was formerly a static local variable declaration (not allowed in C#):
		private string LoadFromFile_fileupper = new string(new char[MAX_PATH + 1]);
...
//			static char fileupper[MAX_PATH+1];
			...
			strncpy(LoadFromFile_fileupper, filename, MAX_PATH);
			_strupr(LoadFromFile_fileupper);

			int i;
			bool knownformat = false;

			for (i = 0; LaniatusDefVariables < next; ++i)
			{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: char * found = strstr(fileupper, exts[0]);
				char found = strstr(LoadFromFile_fileupper, exts[0]);
			...
			CMappedFile mappedFile = new CMappedFile();
			LPCVOID pvMap = new LPCVOID();
			...
			return LoadFromMemory((byte)pvMap, mappedFile.Size());
		}

//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'c_pbMap', so pointers on this parameter are left unchanged:
		public bool LoadFromMemory(in byte * c_pbMap, int iSize)
		{
			...
			if ((m_dwFlags & DDSD_PITCH) != 0)
			{
				uint dwBytesPerRow = m_nWidth * m_xddPixelFormat.dwRGBBitCount / 8;
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			{
				if ((m_dwFlags & DDSD_MIPMAPCOUNT) != 0)
				{
					uint dwLinearSize = m_lPitch;
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'c_pbMap', so pointers on this parameter are left unchanged:
		public bool LoadHeaderFromMemory(in byte * c_pbMap, int iSize)
		{
			uint dwMagic;

			if (iSize < sizeof(uint))
			{
				return false;
			}

			dwMagic = (uint) c_pbMap;
			c_pbMap += sizeof(uint);
			iSize -= sizeof(uint);
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			if (ddsd.dwMipMapCount > DefineConstants.MAX_MIPLEVELS)
			{
				ddsd.dwMipMapCount = DefineConstants.MAX_MIPLEVELS;
			}

			m_nWidth = ddsd.dwWidth;
			m_nHeight = ddsd.dwHeight;

			m_dwMipMapCount = Math.Max(1, ddsd.dwMipMapCount);
			m_dwFlags = ddsd.dwFlags;

			if ((ddsd.dwFlags & DDSD_PITCH) != 0)
			{
			...
			else
			{
				...
				{
					uint dwLinearSize = ddsd.dwLinearSize;
					uint LaniatusDefVariables = 0;
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'pbDest', so pointers on this parameter are left unchanged:
		public bool Copy(int miplevel, byte * pbDest, int lDestPitch)
		{
			if ((m_dwFlags & DDSD_MIPMAPCOUNT) == 0)
			{
				if (miplevel != 0)
...
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			...
			pbDest += (byte)lDestPitch;
			return true;
		}

		public void Decompress(int miplevel, ref uint pdwDest)
		{
			switch (m_CompFormat)
			{
				case PF_DXT1:
		...
		public void DecompressDXT1(int miplevel, ref uint pdwDest)
		{
			int xblocks;
			int yblocks;
#if DEBUG
			...
			Tracef("end check\n");
#endif
			uint nWidth = (uint)(m_nWidth >> miplevel);
			uint nHeight = (uint)(m_nHeight >> miplevel);

			xblocks = (int)(nWidth / 4);
			yblocks = (int)(nHeight / 4);

			int x;
			int y;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: uint * pBase = (uint *) pdwDest;
			uint pBase = (uint) pdwDest;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: ushort * pPos = (ushort *) &m_bCompVector[miplevel][0];
			ushort pPos = (ushort) m_bCompVector[miplevel][0];
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: uint * pImPos;
			uint pImPos;

//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			DXTColBlock * pBlock = new DXTColBlock();

			Color8888 col_0 = new Color8888();
			Color8888 col_1 = new Color8888();
			Color8888 col_2 = new Color8888();
			Color8888 col_3 = new Color8888();
			ushort wrd;
...
//Original Metin2 CPlus Line: pBlock = (DXTColBlock *)((uint) pPos + y * xblocks * 8);
				pBlock.CopyFrom((DXTColBlock)((uint) pPos + y * xblocks * 8));

				for (x = 0; x < xblocks; ++x, ++pBlock)
				{
					GetColorBlockColors(pBlock, col_0, col_1, col_2, col_3, wrd);

					pImPos = (uint)((uint) pBase + x * 16 + (y * 4) * nWidth * 4);
					DecodeColorBlock(pImPos, pBlock, nWidth, (uint) col_0, (uint) col_1, (uint) col_2, (uint) col_3);
				}
			}
		}

		public void DecompressDXT3(int miplevel, ref uint pdwDest)
		{
			int xblocks;
			int yblocks;
#if DEBUG
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
			...
			uint alphazero = (uint) col_0;

			for (y = 0; y < yblocks; ++y)
			{
//# Laniatus Games Studio Inc. | TODO TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
//Original Metin2 CPlus Line: pBlock = (DXTColBlock *)((uint)(pPos + y * xblocks * 16));
				pBlock.CopyFrom((DXTColBlock)((uint)(pPos + y * xblocks * 16)));
		...
		public void DecompressDXT5(int level, ref uint pdwDest)
		{
			int xblocks;
			int yblocks;
#if DEBUG
			...
			Tracef("end check\n");
#endif
			uint nWidth = (uint)(m_nWidth >> level);
			uint nHeight = (uint)(m_nHeight >> level);
...
//Original Metin2 CPlus Line: uint * pBase = (uint *) pdwDest;
			uint pBase = (uint) pdwDest;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: ushort * pPos = pPos = (ushort *) &m_bCompVector[level][0];
			ushort pPos = pPos = (ushort) m_bCompVector[level][0];
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: uint * pImPos;
			uint pImPos;
...
//Original Metin2 CPlus Line: pBlock = (DXTColBlock*)((uint)(pPos + y * xblocks * 16));
		...
		public void DecompressARGB(int level, ref uint pdwDest)
		{
			uint lPitch = (uint)(m_lPitch >> (level * 2));
//# Laniatus Games Studio Inc. | TODO TASK: The memory management function 'memcpy' has no equivalent in C#:
			memcpy(pdwDest, m_bCompVector[level][0], lPitch);
		}

		public void DecodePixelFormat(ref string strPixelFormat, _XDDPIXELFORMAT pxddpf)
		{
			switch (pxddpf.dwFourCC)
			{
				case 0:
				{
						ushort a = GetNumberOfBits(pxddpf.dwRGBAlphaBitMask);
						ushort r = GetNumberOfBits(pxddpf.dwRBitMask);
						ushort g = GetNumberOfBits(pxddpf.dwGBitMask);
						ushort b = GetNumberOfBits(pxddpf.dwBBitMask);

						_snprintf(strPixelFormat, 31, "ARGB-%d%d%d%d%s", a, r, g, b, (pxddpf.dwBBitMask & DDPF_ALPHAPREMULT) != 0 ? "-premul" : "");
						m_CompFormat = PF_ARGB;
				}
					break;

				case ((uint)(byte)('D') | ((uint)(byte)('X') << 8) | ((uint)(byte)('T') << 16) | ((uint)(byte)('1') << 24)):
					strncpy(strPixelFormat, "DXT1", 31);
					m_CompFormat = PF_DXT1;
					break;

				case ((uint)(byte)('D') | ((uint)(byte)('X') << 8) | ((uint)(byte)('T') << 16) | ((uint)(byte)('2') << 24)):
					strncpy(strPixelFormat, "DXT2", 31);
					m_CompFormat = PF_DXT2;
					break;

				case ((uint)(byte)('D') | ((uint)(byte)('X') << 8) | ((uint)(byte)('T') << 16) | ((uint)(byte)('3') << 24)):
					strncpy(strPixelFormat, "DXT3", 31);
					m_CompFormat = PF_DXT3;
					break;

				case ((uint)(byte)('D') | ((uint)(byte)('X') << 8) | ((uint)(byte)('T') << 16) | ((uint)(byte)('4') << 24)):
					strncpy(strPixelFormat, "DXT4", 31);
					m_CompFormat = PF_DXT4;
					break;

				case ((uint)(byte)('D') | ((uint)(byte)('X') << 8) | ((uint)(byte)('T') << 16) | ((uint)(byte)('5') << 24)):
...
//# Laniatus Games Studio Inc. | TODO TASK: Pointer arithmetic is detected on the parameter 'pbDest', so pointers on this parameter are left unchanged:
		public void Unextract(byte * pbDest, int iWidth, int iHeight, int iPitch)
		{
			...
			DXTColBlock pBlock;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not have an equivalent to pointers to value types:
//Original Metin2 CPlus Line: byte * pPos = (byte *) &m_pbCompBufferByLevels[0][0];
...
public class DXTColBlock
{
	public ushort col0;
	public ushort col1;
	public byte[] row = new byte[4];
}

public class DXTAlphaBlockExplicit
{
	public ushort[] row = new ushort[4];
}

public class DXTAlphaBlock3BitLinear
{
	public byte alpha0;
	public byte alpha1;

	public byte[] stuff = new byte[6];
}

public class Color8888
{
	public byte b;
	public byte g;
	public byte r;
...
{
//# Laniatus Games Studio Inc. | TODO TASK: C# does not allow bit fields:
	public uint nBlue : 5;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not allow bit fields:
	public uint nGreen : 6;
//# Laniatus Games Studio Inc. | TODO TASK: C# does not allow bit fields:
	public uint nRed : 5;
}